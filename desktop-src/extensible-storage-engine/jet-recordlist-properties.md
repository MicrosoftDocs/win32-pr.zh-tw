---
description: 深入瞭解： JET_RECORDLIST 屬性
title: JET_RECORDLIST 屬性
TOCTitle: JET_RECORDLIST properties
ms:assetid: Properties.T:Microsoft.Isam.Esent.Interop.JET_RECORDLIST
ms:mtpsurl: https://msdn.microsoft.com/library/microsoft.isam.esent.interop.jet_recordlist_properties(v=EXCHG.10)
ms:contentKeyID: 55103831
ms.date: 07/30/2014
ms.topic: article
ms.openlocfilehash: bd69d5357653dee59eb85880be5a87a32499871de4e416daa6fc97c2a7a52a2a
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "117893470"
---
# <a name="jet_recordlist-properties"></a>JET_RECORDLIST 屬性

包含受保護的成員  
包含繼承的成員  

[JET_RECORDLIST](./jet-recordlist-class.md)類型會公開下列成員。

## <a name="properties"></a>屬性

<table>
<thead>
<tr class="header">
<th> </th>
<th>名稱</th>
<th>描述</th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td><img src="../images/dn292128.pubproperty(exchg.10).gif" title="公用屬性" alt="Public property" /></td>
<td><a href="dn335244(v=exchg.10).md">columnidBookmark</a></td>
<td>取得臨時表中的資料行 columnid，此資料行會儲存記錄的書簽。 資料行的類型為 JET_coltyp。文本。</td>
</tr>
<tr class="even">
<td><img src="../images/dn292128.pubproperty(exchg.10).gif" title="公用屬性" alt="Public property" /></td>
<td><a href="dn335252(v=exchg.10).md">cRecords</a></td>
<td>取得臨時表中的記錄數目。</td>
</tr>
<tr class="odd">
<td><img src="../images/dn292128.pubproperty(exchg.10).gif" title="公用屬性" alt="Public property" /></td>
<td><a href="dn335255(v=exchg.10).md">tableid</a></td>
<td>取得臨時表的 tableid。 當不再需要資料表時，這應該會關閉。</td>
</tr>
</tbody>
</table>


頁首

## <a name="see-also"></a>另請參閱

#### <a name="reference"></a>參考

[JET_RECORDLIST 類別](./jet-recordlist-class.md)

[Microsoft. Esent 命名空間](./microsoft.isam.esent.interop-namespace.md)
