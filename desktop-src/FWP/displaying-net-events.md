---
title: 顯示淨活動
description: 下列範例程式碼示範伺服器應用程式如何找出可能會影響到最近的網路事件。
...
ms.locfileid: "106965364"
---
# <a name="displaying-net-events"></a><span data-ttu-id="11050-103">顯示淨活動</span><span class="sxs-lookup"><span data-stu-id="11050-103">Displaying Net Events</span></span>

<span data-ttu-id="11050-104">下列範例程式碼示範伺服器應用程式如何找出可能會受到影響的最近網路事件。</span><span class="sxs-lookup"><span data-stu-id="11050-104">The following sample code demonstrates how a server application could locate recent net events that might have affected it.</span></span>

> [!Note]  
> <span data-ttu-id="11050-105">篩選準則與下層 **IsPortAllowed** API 所支援的條件相同。</span><span class="sxs-lookup"><span data-stu-id="11050-105">The filter conditions are the same as those supported by the downlevel **IsPortAllowed** API.</span></span>
