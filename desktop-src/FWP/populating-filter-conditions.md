---
title: 填入篩選準則
description: 下列範例程式碼示範如何填入伺服器應用程式所使用的篩選準則，以尋找影響它的篩選準則和事件。
...
ms.locfileid: "104093555"
---
# <a name="populating-filter-conditions"></a><span data-ttu-id="48908-103">填入篩選準則</span><span class="sxs-lookup"><span data-stu-id="48908-103">Populating Filter Conditions</span></span>

<span data-ttu-id="48908-104">下列範例程式碼示範如何填入伺服器應用程式所使用的篩選準則，以尋找影響它的篩選準則和事件。</span><span class="sxs-lookup"><span data-stu-id="48908-104">The following sample code demonstrates how to populate filter conditions used by a server application to find filters and events that affect it.</span></span>

> [!Note]  
> <span data-ttu-id="48908-105">這些條件與下層 **IsPortAllowed** API 所支援的條件相同。</span><span class="sxs-lookup"><span data-stu-id="48908-105">These conditions are the same as those supported by the downlevel **IsPortAllowed** API.</span></span>
