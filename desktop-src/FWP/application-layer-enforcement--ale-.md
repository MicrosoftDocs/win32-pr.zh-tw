...
ALE 是一組 Windows 篩選平台 (WFP) 用於可設定狀態之篩選的核心模式層。
...
ALE 可提供原則的強制執行，例如「允許 Windows Messenger 在封鎖所有其他應用程式時存取網路」。 在此範例中，當應用程式 "Messenger" 透過網路連線時，ALE 會攔截該事件，並判斷它是由 Messenger 起始，並查詢 WFP 篩選引擎來判斷是否應該允許該通訊端繼續進行。
