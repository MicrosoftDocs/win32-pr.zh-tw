---
title: 正在查看目前的狀態
description: 下列範例程式碼示範如何尋找可能影響伺服器應用程式的所有篩選準則。
...
ms.locfileid: "106964964"
---
# <a name="viewing-current-state"></a><span data-ttu-id="7de00-103">正在查看目前的狀態</span><span class="sxs-lookup"><span data-stu-id="7de00-103">Viewing Current State</span></span>

<span data-ttu-id="7de00-104">下列範例程式碼示範如何尋找可能會影響伺服器應用程式的所有篩選準則。</span><span class="sxs-lookup"><span data-stu-id="7de00-104">The following sample code demonstrates how to find all filters that might affect a server application.</span></span>

> [!Note]  
> <span data-ttu-id="7de00-105">篩選準則與下層 **IsPortAllowed** API 所支援的條件相同。</span><span class="sxs-lookup"><span data-stu-id="7de00-105">The filter conditions are the same as those supported by the downlevel **IsPortAllowed** API.</span></span>
