...
---
# <a name="how-to-get-the-last-set-of-http-headers-received-for-each-file-in-a-bits-download-job"></a><span data-ttu-id="cf1fb-104">如何在 BITS 下載作業中取得每個檔案所收到的最後一組 HTTP 標頭</span><span class="sxs-lookup"><span data-stu-id="cf1fb-104">How to get the last set of HTTP headers received for each file in a BITS download job</span></span>

<span data-ttu-id="cf1fb-105">此範例示範如何使用新的 [**IBackgroundCopyJob5**](/windows/desktop/api/Bits5_0/nn-bits5_0-ibackgroundcopyjob5) 介面的 [**GetProperty**](/windows/desktop/api/Bits5_0/nf-bits5_0-ibackgroundcopyjob5-getproperty) 方法，取得背景智慧型傳送服務 (位) 下載作業中每個檔案所收到的最後一組 HTTP 標頭。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-105">This sample shows how to use the new [**IBackgroundCopyJob5**](/windows/desktop/api/Bits5_0/nn-bits5_0-ibackgroundcopyjob5) interface's [**GetProperty**](/windows/desktop/api/Bits5_0/nf-bits5_0-ibackgroundcopyjob5-getproperty) method to obtain the last set HTTP headers received for each file in a Background Intelligent Transfer Service (BITS) download job.</span></span> <span data-ttu-id="cf1fb-106">例如，您可以使用 HTTP 標頭中的資訊，來判斷檔案類型或上次在伺服器上變更的時間。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-106">The information in the HTTP header could be used, for example, to determine the file type or when it last changed on the server.</span></span> <span data-ttu-id="cf1fb-107">在 Windows 8 和 Windows Server 2012 之前，BITS 並未提供應用程式取得和檢查已完成下載之 HTTP 回應標頭的方法。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-107">Prior to Windows 8 and Windows Server 2012, BITS did not provide a means by which the application could retrieve and inspect the HTTP response headers of a completed download.</span></span> <span data-ttu-id="cf1fb-108">此範例示範如何使用 BITS API 來建立具有多個 Url 的 BITS 作業來下載、列出工作中的 Url，以及取得每個 URL 的 HTTP 回應標頭。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-108">This sample shows how to use the BITS API to create a BITS job with multiple URLs to download, list the URLs in a job, and retrieve the HTTP response headers for each URL.</span></span>

## <a name="what-you-need-to-know"></a><span data-ttu-id="cf1fb-109">您必須知道的事項</span><span class="sxs-lookup"><span data-stu-id="cf1fb-109">What you need to know</span></span>

### <a name="technologies"></a><span data-ttu-id="cf1fb-110">技術</span><span class="sxs-lookup"><span data-stu-id="cf1fb-110">Technologies</span></span>

### <a name="prerequisites"></a><span data-ttu-id="cf1fb-111">必要條件</span><span class="sxs-lookup"><span data-stu-id="cf1fb-111">Prerequisites</span></span>

-   <span data-ttu-id="cf1fb-112">Microsoft Visual Studio</span><span class="sxs-lookup"><span data-stu-id="cf1fb-112">Microsoft Visual Studio</span></span>

## <a name="instructions"></a><span data-ttu-id="cf1fb-113">指示</span><span class="sxs-lookup"><span data-stu-id="cf1fb-113">Instructions</span></span>

### <a name="step-1-include-the-required-bits-header-files"></a><span data-ttu-id="cf1fb-114">步驟1：包含必要的 BITS 標頭檔</span><span class="sxs-lookup"><span data-stu-id="cf1fb-114">Step 1: Include the required BITS header files</span></span>

<span data-ttu-id="cf1fb-115">將下列標頭指示詞插入原始程式檔的頂端。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-115">Insert the following header directives at the top of the source file.</span></span>
...
### <a name="step-2-initialize-com-and-instantiate-a-bits-background-copy-manager-object-interface"></a><span data-ttu-id="cf1fb-116">步驟2：初始化 COM 並具現化 BITS 背景複本管理員物件介面</span><span class="sxs-lookup"><span data-stu-id="cf1fb-116">Step 2: Initialize COM and instantiate a BITS Background Copy Manager object interface</span></span>

<span data-ttu-id="cf1fb-117">在具現化 [**IBackgroundCopyManager**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopymanager) 介面之前 (用來建立) 的 BITS 作業之前，您必須先初始化 com，並設定所需的 com 執行緒模型。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-117">Before instantiating the [**IBackgroundCopyManager**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopymanager) interface (used to create a BITS job), you must initialize COM and set the desired COM threading model.</span></span>
...
### <a name="step-3-create-the-bits-job"></a><span data-ttu-id="cf1fb-118">步驟3：建立 BITS 作業</span><span class="sxs-lookup"><span data-stu-id="cf1fb-118">Step 3: Create the BITS job</span></span>

<span data-ttu-id="cf1fb-119">只有建立作業的使用者或具有系統管理員許可權的使用者可以將檔案新增至作業，並變更作業的屬性。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-119">Only the user who creates the job or a user with administrator privileges can add files to the job and change the job's properties.</span></span>
...
### <a name="step-4-add-the-files-to-the-bits-job"></a><span data-ttu-id="cf1fb-120">步驟4：將檔案新增至 BITS 作業</span><span class="sxs-lookup"><span data-stu-id="cf1fb-120">Step 4: Add the files to the BITS job</span></span>

<span data-ttu-id="cf1fb-121">下列範例會從 Microsoft 下載中心下載可公開使用的檔。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-121">The following example downloads publicly available documents from the Microsoft Download Center.</span></span> <span data-ttu-id="cf1fb-122">您必須為特定環境變更這些值。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-122">You'll need to change these values for your specific environment.</span></span>
...
### <a name="step-5-start-the-bits-job"></a><span data-ttu-id="cf1fb-123">步驟5：啟動 BITS 工作</span><span class="sxs-lookup"><span data-stu-id="cf1fb-123">Step 5: Start the BITS job</span></span>

<span data-ttu-id="cf1fb-124">設定 BITS 作業之後，請呼叫 [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) 介面的 [**Resume**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-resume) 函式來啟動或繼續下載。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-124">After setting up the BITS job, call the [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) interface's [**Resume**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-resume) function to start or continue the download.</span></span>
...
### <a name="step-6-monitor-and-display-the-bits-jobs-progress"></a><span data-ttu-id="cf1fb-125">步驟6：監視和顯示 BITS 作業的進度</span><span class="sxs-lookup"><span data-stu-id="cf1fb-125">Step 6: Monitor and display the BITS job's progress</span></span>

<span data-ttu-id="cf1fb-126">`MonitorJobProgress`Helper 函式會採用 [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob)物件做為其唯一的參數，並每隔500毫秒輪詢作業的狀態。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-126">The `MonitorJobProgress` helper function takes an [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) object as its only parameter and polls the job for a status every 500 milliseconds.</span></span> <span data-ttu-id="cf1fb-127">在作業完成或取消之前，不會傳回此函數。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-127">This function does not return until the job has completed or been canceled.</span></span>
...
### <a name="step-7-display-the-downloaded-file-headers"></a><span data-ttu-id="cf1fb-128">步驟7：顯示已下載的檔案標頭</span><span class="sxs-lookup"><span data-stu-id="cf1fb-128">Step 7: Display the downloaded file headers</span></span>

<span data-ttu-id="cf1fb-129">`DisplayFileHeaders`Helper 函數會列舉針對 [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob)物件定義的作業。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-129">The `DisplayFileHeaders` helper function enumerates the jobs defined for an [**IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) object.</span></span>
...
## <a name="example"></a><span data-ttu-id="cf1fb-130">範例</span><span class="sxs-lookup"><span data-stu-id="cf1fb-130">Example</span></span>

<span data-ttu-id="cf1fb-131">下列程式碼範例是完整運作的主控台應用程式，示範如何使用 BITS API 來建立具有多個 Url 的 BITS 作業來下載、列出工作中的 Url，以及取得每個 URL 的 HTTP 回應標頭。</span><span class="sxs-lookup"><span data-stu-id="cf1fb-131">The following code example is a fully working console application that shows how to use the BITS API to create a BITS job with multiple URLs to download, list the URLs in a job, and retrieve the HTTP response headers for each URL.</span></span>
