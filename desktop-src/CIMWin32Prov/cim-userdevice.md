...
ms.locfileid: "118420493"
---
# <a name="cim_userdevice-class-cimwin32-wmi-providers"></a>CIM_UserDevice 類別 (CIMWin32 WMI 提供者) 

**Cim \_ UserDevice** 類別是其他類別（例如 [**CIM \_ 鍵盤**](cim-keyboard.md)或 [**cim \_ DesktopMonitor**](cim-desktopmonitor.md)）下降的父類別。 使用者裝置是邏輯裝置，可讓電腦系統的使用者輸入、查看或聽到資料。
...
**CIM \_ UserDevice** 類別具有這些方法。



| 方法                                                                | 描述                                                                                                                              |
|:----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-userdevice.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-userdevice.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
**Cim \_ UserDevice** 類別衍生自 [**cim \_ LogicalDevice**](cim-logicaldevice.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ USERDEVICE** 的 WMI 類別，請參閱 [Win32 類別](win32-provider.md)。
