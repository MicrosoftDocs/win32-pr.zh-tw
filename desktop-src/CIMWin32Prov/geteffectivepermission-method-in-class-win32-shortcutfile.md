---
description: 決定當檔案或目錄位於共用上時，使用者是否擁有快捷方式檔案所在之檔案物件、目錄和共用的許可權參數中所指定的所有必要許可權。
ms.assetid: 36f823c1-fa19-40a1-b750-41e1f73bdf01
ms.tgt_platform: multiple
title: 'Win32_ShortcutFile 類別的 GetEffectivePermission 方法 (Aclui) '
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
**GetEffectivePermission** [WMI 類別](/windows/desktop/WmiSdk/retrieving-a-class)方法會判斷使用者是否擁有快捷方式檔案所在之檔案物件、目錄和共用的 *許可權* 參數中所指定的所有必要許可權（如果檔案或目錄位於共用上）。
...
如果使用者具有指定的許可權，則傳回 **True** ; 如果使用者沒有指定的許可權，則傳回 **false** 。
