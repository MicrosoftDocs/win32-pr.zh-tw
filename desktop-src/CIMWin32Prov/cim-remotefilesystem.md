...
**CIM \_ RemoteFileSystem** 類別代表透過網路相關服務來存取的遠端檔案系統。 在此情況下，檔案存放區是由電腦主控，作為檔案伺服器。 例如，NFS 檔案系統的檔案存放區通常不在電腦系統的本機控制媒體上，也不是透過設備磁碟機直接存取。 **CIM \_ RemoteFileSystem** 的子類別包含與檔案系統存取相關的用戶端設定資訊。
...
**CIM \_ RemoteFileSystem** 類別具有下列類型的成員：

-   [屬性](#properties)

### <a name="properties"></a>屬性

**CIM \_ RemoteFileSystem** 類別具有這些屬性。
...
形成儲存範圍之區塊的大小（以位元組為單位）。 如果未知，或如果區塊概念無效， (例如，針對匯總範圍、記憶體或邏輯磁片) ，請輸入 1 (一個) 。
...
檔案系統支援的字元集或編碼。 這個屬性繼承自 [**CIM \_ 檔案系統**](cim-filesystem.md)。
...
**Cim \_ RemoteFileSystem** 類別衍生自 [**cim \_ 檔案系統**](cim-filesystem.md)。
