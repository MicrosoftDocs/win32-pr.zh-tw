...
**CIM \_ DiskDrive** 類別代表作業系統所見的實體磁片磁碟機。 磁片磁碟機功能對應到磁片磁碟機的邏輯和管理特性，在某些情況下，可能不會反映裝置的實體特性。 實體磁片磁碟機的介面是此類別的成員。 不過，根據另一個邏輯裝置的物件不是此類別的成員。
...
自由格式字串的陣列，提供 **功能** 陣列中指出之存取裝置功能的詳細說明。 這個屬性繼承自 [**CIM \_ MediaAccessDevice**](cim-mediaaccessdevice.md)。
...
建立實例時所使用)  (或子類別的名稱。 搭配類別的其他索引鍵屬性使用時，此屬性可讓類別和其子類別的所有實例都能唯一識別。
...
若 **為 TRUE**，則會清除 **LastErrorCode** 屬性中所報告的錯誤。
...
裝置所支援媒體的大小上限（以 kb 為單位）。 Kb 會被視為位元組數目乘以 1000 (而不是位元組數乘以 1024) 。
...
當媒體存取裝置支援多個個別媒體時，這個屬性會定義可支援或插入的最大數目。
...
支援 [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) 方法。
...
您可以叫用 [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) 方法，並將 *>powerstate* 參數設定為 5 ( 「電源週期」 ) 。
...
您可以叫用 [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) 方法，並將 *>powerstate* 參數設定為 5 ( 「電源週期」 ) ，並將 *時間* 參數設定為特定日期和時間（或間隔），以進行電源開啟。
...
**Cim \_ DiskDrive** 類別衍生自 [**cim \_ MediaAccessDevice**](cim-mediaaccessdevice.md)。

WMI 不會執行這個類別。 請參閱衍生自 **CIM \_ DiskDrive** 之類別的 [Win32 類別](win32-provider.md)。
