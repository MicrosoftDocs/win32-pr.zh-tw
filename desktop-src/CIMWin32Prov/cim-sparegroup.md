...
**Cim \_ SpareGroup** 類別衍生自 [**cim \_ RedundancyGroup**](cim-redundancygroup.md)類別，並指出一或多個匯總的元素可以被用。 您可以使用 [**CIM \_ ActsAsSpare**](cim-actsasspare.md) 關聯來定義備用。 備用的 Nic 範例是在電腦系統中使用多餘的 Nic，其中一個 NIC 是主要的，另一個則是備用。 主要 NIC 會是備用群組的成員，並使用 [**cim \_ RedundancyComponent**](cim-redundancycomponent.md) 類別建立關聯，而其他 nic 則會使用 **cim \_ ActsAsSpare** 關聯性建立關聯。
...
**CIM \_ SpareGroup** 類別具有下列類型的成員：

-   [屬性](#properties)

### <a name="properties"></a>屬性

**CIM \_ SpareGroup** 類別具有這些屬性。
...
這個屬性繼承自 [**CIM \_ RedundancyGroup**](cim-redundancygroup.md)。
...
<span id="Fully_Redundant"></span><span id="fully_redundant"></span><span id="FULLY_REDUNDANT"></span>**完全多餘** 的 (2) 
...
<span id="Degraded_Redundancy"></span><span id="degraded_redundancy"></span><span id="DEGRADED_REDUNDANCY"></span>**降級的冗余** (3) 


</dt> <dd>

因為某些失敗，所以可使用較少的可用冗余量。

</dd> <dt>

<span id="Redundancy_Lost"></span><span id="redundancy_lost"></span><span id="REDUNDANCY_LOST"></span>

<span id="Redundancy_Lost"></span><span id="redundancy_lost"></span><span id="REDUNDANCY_LOST"></span>**遺失** (4) 的冗余


</dt> <dd>

因為有足夠的失敗次數，所以無法使用冗余。 下一個失敗會導致整體失敗。
...
**Cim \_ SpareGroup** 類別衍生自 [**cim \_ RedundancyGroup**](cim-redundancygroup.md)。
