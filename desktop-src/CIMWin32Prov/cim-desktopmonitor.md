...
**CIM \_ DesktopMonitor** 類別代表桌面監視器 (CRT) 邏輯裝置的功能和管理。
...
**CIM \_ DesktopMonitor** 類別具有這些方法。



| 方法                                                                    | 描述                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-desktopmonitor.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-desktopmonitor.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
**Cim \_ DesktopMonitor** 類別衍生自 [**cim \_ 顯示**](cim-display.md)。

WMI 不會執行這個類別。 如需衍生自 **CIM \_ DesktopMonitor** 之類別的詳細資訊，請參閱 [Win32 類別](win32-provider.md)。
