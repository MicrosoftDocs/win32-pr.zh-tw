...
**CIM \_ DirectorySpecification** 類別會捕捉 software 元素的主要目錄結構。 此類別是用來將軟體專案的檔案組織成可在電腦系統上重新置放的可管理單位。
...
**CIM \_ DirectorySpecification** 類別具有下列類型的成員：
...
**CIM \_ DirectorySpecification** 類別具有這些方法。



| 方法                                                              | 描述                                                                     |
|:--------------------------------------------------------------------|:--------------------------------------------------------------------------------|
| [**調用**](invoke-method-in-class-cim-directoryspecification.md) | 評估特定檢查。 這個方法不是由 WMI 所執行。<br/> |
...
**CIM \_ DirectorySpecification** 類別具有這些屬性。
...
目錄名稱。 應用程式提供者所提供的值是預設或建議的路徑名稱。 您可以針對特定環境變更此值。
...
限定詞： [**Mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 位置 \| 001.2」 ) 
</dt> </dl>
...
**Product configuration directory** (3) 
...
WMI 不會執行這個類別。 針對衍生自 **CIM \_ DirectorySpecification** 的類別，請參閱 [Win32 類別](win32-provider.md)。
