...
---
# <a name="cim_serviceaccessbysap-class"></a>CIM \_ ServiceAccessBySAP 類別

**CIM \_ ServiceAccessBySAP** 關聯類別代表服務的存取點。 例如，您可以透過 NetWare、Macintosh 或 Windows 服務存取點來存取印表機 (Sap) ，這些都可能裝載在不同的系統上。
...
**CIM \_ ServiceAccessBySAP** 類別具有下列類型的成員：

-   [屬性](#properties)

### <a name="properties"></a>屬性

**CIM \_ ServiceAccessBySAP** 類別具有這些屬性。
...
描述服務存取點的 [**CIM \_ ServiceAccessPoint**](cim-serviceaccesspoint.md) 。 存取點相依于此關聯性，因為它們沒有沒有對應服務的函數。

</dd> </dl>

## <a name="remarks"></a>備註

**Cim \_ ServiceAccessBySAP** 類別衍生自 cim 相依 [**性 \_**](cim-dependency.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ SERVICEACCESSBYSAP** 的 WMI 類別，請參閱 [Win32 類別](win32-provider.md)。
