...
**CIM \_ NonVolatileStorage** 類別代表非暫時性儲存的功能和管理。 靜態記憶體原生包含 flash 和 ROM 存放裝置。 此外，如果動態記憶體是由電池所支援，則動態記憶體可能會以 volatile 儲存體為基礎。 例如，此案例會由 [**cim \_ AssociatedBattery**](cim-associatedbattery.md)關聯性的實例描述，此關聯性會以 **相依** 的形式和 **剩餘的電池** 形式來參考非暫時性的儲存體; 以及一個 [**cim \_ BasedOn**](cim-basedon.md)關聯性的實例，該關聯性會將非暫時性的儲存區視為 **相依** 的，而將非暫時性的儲存體視為 **之前的引數。**
...
**CIM \_ NonVolatileStorage** 類別具有下列類型的成員：
...
**CIM \_ NonVolatileStorage** 類別具有這些方法。



| 方法                                                                        | 描述                                                                                                                              |
|:------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-nonvolatilestorage.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-nonvolatilestorage.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
**CIM \_ NonVolatileStorage** 類別具有這些屬性。
...
列舉，指出造成最後一個錯誤的記憶體存取作業。 錯誤的類型是由 **ErrorInfo** 屬性描述。 如果 **ErrorInfo** 屬性等於 3 ( 「確定」 ) ，則此屬性沒有任何意義。
...
排序儲存在 **ErrorData** 屬性中的資料。 如果 **ErrorTransferSize** 為0，則此屬性沒有任何意義。
...
最近發生的錯誤類型。 CIM 架構中的值12到14未定義，因為 DMI 混合了錯誤類型的語義以及是否可進行更正。 在 **CorrectableError** 屬性中指出錯誤是否可更正。 這個屬性繼承自 [**CIM \_ 記憶體**](cim-memory.md)。
...
指定是否使用同位演算法、CRC 演算法、ECC 或其他機制。 也可以提供演算法的詳細資料。
...
造成最後一個錯誤的資料傳輸大小（以位為單位）。 值為 0 (零) 表示沒有錯誤。 如果 **ErrorInfo** 屬性等於 3 ( 「確定」 ) ，則此屬性應該設定為 0 (零) 。
...
連續區塊的總數，每個區塊都是包含在 **區塊** 屬性中的值大小，它會形成儲存區的大小。 您可以將 [ **區塊** ] 屬性的值乘以這個屬性的值，以計算儲存區的總大小。 如果 **區塊** 的值是 1 (一個) ，此屬性就是儲存區的總大小。
...
提供資訊的自由格式字串，如果 **ErrorType** 屬性設定為 1 ( "Other" ) 。 如果未設定為1，則此字串沒有任何意義。
...
由應用程式或作業系統參考，且由記憶體控制器針對這個記憶體物件所對應的起始位址。 起始位址的指定單位為 kb。
...
若 **為 TRUE**， **ErrorAddress** 屬性中的位址資訊就是系統層級的位址。 如果為 **FALSE**，則為實體位址。 如果 **ErrorInfo** 屬性等於 3 ( 「確定」 ) ，則此屬性沒有任何意義。
...
**Cim \_ NonVolatileStorage** 類別衍生自 [**cim \_ 記憶體**](cim-memory.md)。
