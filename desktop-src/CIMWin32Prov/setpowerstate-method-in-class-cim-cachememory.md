...
**SetPowerState** 方法會設定邏輯裝置所需的電源狀態，以及何時應將裝置置於該狀態。 在子類別中，使用方法上的 **ValueMap** 辨識符號來指定可能的傳回碼集。 您也應該在子類別中，以 **值** 陣列限定詞的形式指定要轉譯之 **ValueMap** 內容的字串。 這個方法繼承自 [**CIM \_ LogicalDevice**](cim-logicaldevice.md)。
