...
ms.locfileid: "119923778"
---
# <a name="cim_filespecification-class"></a>CIM \_ FileSpecification 類別

**CIM \_ FileSpecification** 類別代表系統上或關閉的檔案。 檔案位於 [**CIM \_ DirectorySpecificationFile**](cim-directoryspecificationfile.md) 關聯所識別的目錄中。 [**Invoke**](invoke-method-in-class-cim-filespecification.md)方法會使用此資訊來檢查檔案是否存在。 請注意，不會檢查具有 **Null** 值的屬性。
...
**CIM \_ FileSpecification** 類別具有下列類型的成員：
...
**CIM \_ FileSpecification** 類別具有這些方法。



| 方法                                                         | 描述                                                      |
|:---------------------------------------------------------------|:-----------------------------------------------------------------|
| [**調用**](invoke-method-in-class-cim-filespecification.md) | 評估特定檢查。 不是由 WMI 所執行。<br/> |
...
**CIM \_ FileSpecification** 類別具有這些屬性。
...
限定詞： [**Mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| Software Signature \| 002.4 ") 
</dt> </dl>

計算結果為檔案前32個位元組之16位加總的值。
...
限定詞： [**Mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| Software Signature \| 002.5 ") 
</dt> </dl>
...
限定詞： [**Mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| Software Signature \| 002.6 ") 
</dt> </dl>

檔案中間 512 KB 的 CRC 值（模數3）。
...
限定詞： [**maxlen**](/windows/desktop/WmiSdk/standard-qualifiers) (16) 
</dt> </dl>

針對任何檔案或物件計算128位總和檢查碼的演算法。 兩個不同檔案產生相同 MD5 總和檢查碼的可能性非常小 (大約1個在 2 ^ 64) 中，而檔案的 MD5 總和檢查碼可用來建立可能唯一識別檔案的可靠內容識別碼。 反之亦然。 如果兩個檔案的 MD5 總和檢查碼相同，則很有可能是檔案相同。 針對 MD5 屬性的 MOF 規格用途，MD5 演算法一律會產生32個字元的字串。 例如，字串 ">abcdefghijklmnopqrstuvwxyz" 會產生 "c3fcd3d76192e4007dfb496cca67e13b" 字串。 如需有關如何執行 MD5 演算法的詳細資訊，請參閱 [RFC 1321](https://www.ietf.org/rfc/rfc1321.txt)。
...
限定詞：覆 [**寫**](/windows/desktop/WmiSdk/standard-qualifiers) (名稱) 、 [**Fixed**](/windows/desktop/WmiSdk/standard-wmi-qualifiers)、 [**maxlen**](/windows/desktop/WmiSdk/standard-qualifiers) (1024) 
</dt> </dl>
...
WMI 不會執行這個類別。 針對衍生自 **CIM \_ FileSpecification** 的類別，請參閱 [Win32 類別](win32-provider.md)。
