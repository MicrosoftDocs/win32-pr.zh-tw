...
ms.locfileid: "118676574"
---
# <a name="enabledhcp-method-of-the-win32_networkadapterconfiguration-class"></a>Win32 >networkadapterconfiguration 類別的 EnableDHCP 方法 \_

**EnableDHCP** [WMI 類別](/windows/desktop/WmiSdk/retrieving-a-class)方法會啟用動態主機設定通訊協定， (DHCP) 用於此網路介面卡的服務。 DHCP 允許動態配置 IP 位址。
...
這個方法不會清除任何存在於電腦上的靜態預設閘道。

## <a name="examples"></a>範例

TechNet 資源庫上的 [啟用 dhcp 和指派 Dns 伺服器](https://Gallery.TechNet.Microsoft.Com/7b1cec46-bdb8-4afc-b240-9789eefce6de) VBScript 程式碼範例會使用 EnableDHCP 來啟用 dhcp，並將 dns 伺服器指派給電腦。

下列 VBScript 程式碼範例示範如何在 [**Win32 \_ >networkadapterconfiguration**](win32-networkadapterconfiguration.md) 的實例上啟用 DHCP 使用。 在此情況下，我們會指定索引為0的介面卡。 您應該從其他介面的 Win32 NetworkAdapter 實例中選取正確的索引 \_ 。
...
下列 Perl 程式碼範例示範如何在 [**Win32 \_ >networkadapterconfiguration**](win32-networkadapterconfiguration.md) 的實例上啟用 DHCP 使用。 在此情況下，我們會指定索引為0的介面卡。 您應該從其他介面的 Win32 NetworkAdapter 實例中選取正確的索引 \_ 。
