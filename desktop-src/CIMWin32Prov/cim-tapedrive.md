...
ms.locfileid: "119020836"
---
# <a name="cim_tapedrive-class-cimwin32-wmi-providers"></a>CIM_TapeDrive 類別 (CIMWin32 WMI 提供者) 

**CIM \_ disable-tapedrive** 類別代表系統上的磁帶機。 磁帶機主要是用來區別，因為它們只能依序存取。
...
**CIM \_ disable-tapedrive** 類別具有這些方法。



| 方法                                                               | 描述                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-tapedrive.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-tapedrive.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
提供 **功能** 陣列中所指出之存取裝置功能詳細說明的自由格式字串。 請注意，這個陣列的每個專案都與位於相同索引的 **功能** 陣列中的專案相關。
...
以位元組為單位，指定為磁帶結束區域的大小。 此區域中的存取會產生磁帶結束的警告。
...
**Cim \_ disable-tapedrive** 類別衍生自 [**cim \_ MediaAccessDevice**](cim-mediaaccessdevice.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ DISABLE-TAPEDRIVE** 的 WMI 類別，請參閱 [Win32 類別](win32-provider.md)。
