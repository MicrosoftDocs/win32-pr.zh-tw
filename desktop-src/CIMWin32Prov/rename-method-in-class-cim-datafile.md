...
**重新命名** 方法會重新命名在物件路徑中指定的邏輯檔案 (或目錄) 。 如果目的地是在另一個磁片磁碟機上，或需要覆寫現有的邏輯檔案，則不支援重新命名。 這個方法繼承自 [**CIM \_ LogicalFile**](cim-logicalfile.md)。
...
[**CIM \_ 資料檔案**](cim-datafile.md)中的 **重新命名** 方法是由 WMI 所執行。
...
[CIM \_ 資料檔案](rename-method-in-class-cim-datafile.md)
</dt> <dt>
