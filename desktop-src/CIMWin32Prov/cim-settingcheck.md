...
ms.locfileid: "122885279"
---
# <a name="cim_settingcheck-class"></a>CIM \_ SettingCheck 類別

**CIM \_ SettingCheck** 類別會指定針對包含值等於指定值的特定專案檢查特定設定檔案所需的資訊。 所有的比較都會假設為不區分大小寫。
...
**CIM \_ SettingCheck** 類別具有下列類型的成員：
...
**CIM \_ SettingCheck** 類別具有這些方法。



| 方法                                                    | 描述                                                   |
|:----------------------------------------------------------|:--------------------------------------------------------------|
| [**調用**](invoke-method-in-class-cim-settingcheck.md) | 採取特定動作。 不是由 WMI 所執行。<br/> |
...
與其他索引鍵搭配使用來唯一識別檢查的識別碼。 這個屬性繼承自 [**CIM \_ 檢查**](cim-check.md)。
...
若 **為 TRUE**，則條件應該存在於環境中 (例如，如果檔案是在系統上，則叫 [**用方法應該**](invoke-method-in-class-cim-settingcheck.md) 會傳回 **TRUE**) 。 如果 **為 FALSE**，則條件不應該存在 (例如，如果檔案不在系統上，則叫用 **方法應該** 會傳回 **FALSE**) 。
...
用來識別軟體元素的名稱。 這個屬性繼承自 [**CIM \_ 檢查**](cim-check.md)。
