...
---
# <a name="cim_pointingdevice-class-cimwin32-wmi-providers"></a>CIM_PointingDevice 類別 (CIMWin32 WMI 提供者) 

**CIM \_ PointingDevice** 類別代表指向顯示區域的裝置。 任何操作指標的裝置，或指向視覺效果顯示器上的區域，都是此類別的成員。 例如，滑鼠、手寫筆、觸控板或平板電腦。
...
**CIM \_ PointingDevice** 類別具有這些方法。



| 方法                                                                    | 描述                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-pointingdevice.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-pointingdevice.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 指標裝置 \| 001.1」 ) 
</dt> </dl>
...
限定詞： [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「每英寸計數」 ) 
</dt> </dl>
...
**Cim \_ PointingDevice** 類別衍生自 [**cim \_ UserDevice**](cim-userdevice.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ POINTINGDEVICE** 的 WMI 類別，請參閱 [Win32 類別](win32-provider.md)。
