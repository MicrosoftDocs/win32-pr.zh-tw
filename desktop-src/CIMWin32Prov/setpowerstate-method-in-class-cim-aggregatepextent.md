...
如需搭配使用此方法與 C/c + + 的詳細資訊，請參閱 [呼叫方法](/windows/desktop/WmiSdk/calling-a-method)。
...
當應該設定電源狀態時，請將其設定為一般日期時間值，或做為間隔值， (在收到方法調用時，間隔開始時間) 。 當 *>powerstate* 參數等於 5 (電源週期) 時，時間參數會指出裝置應該重新開啟電源的 *時間* 。 立即關閉電源。
