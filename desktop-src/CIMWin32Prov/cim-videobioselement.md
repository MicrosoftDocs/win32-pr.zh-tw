...
ms.locfileid: "122882025"
---
# <a name="cim_videobioselement-class"></a>CIM \_ VideoBIOSElement 類別

**CIM \_ VideoBIOSElement** 類別代表載入至非暫時性存放裝置的低層級軟體，可用來設定及存取電腦系統的視訊控制器和顯示器。
...
**CIM \_ VideoBIOSElement** 類別具有下列類型的成員：

-   [屬性](#properties)

### <a name="properties"></a>屬性

**CIM \_ VideoBIOSElement** 類別具有這些屬性。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| VIDEO BIOS \| 001.5」 ) 
</dt> </dl>

若 **為 TRUE**，則會將影片 BIOS 遮蔽。
...
Software 元素的語言版本。 應使用 ISO 639 中定義的語言代碼。 當 software 專案代表多語系或國際版的產品時，應該使用「多語系」字串。
...
限定詞： [**Maxlen**](/windows/desktop/WmiSdk/standard-qualifiers) (256) ， [**MAPPINGSTRINGS**](/windows/desktop/WmiSdk/standard-qualifiers) (」 MIF。DMTF \| 元件 \| 001.1 ") 
</dt> </dl>

Software 專案的製造商：此屬性繼承自 [**CIM \_ SoftwareElement**](cim-softwareelement.md)。
...
用來識別軟體專案的名稱。此屬性繼承自 [**CIM \_ ManagedSystemElement**](cim-managedsystemelement.md)。
...
當 **TargetOperatingSystem** 屬性的值為 1 ( "Other" ) 時，軟體專案的製造商和作業系統類型。 當 **TargetOperatingSystem** 屬性的值為1時，這個屬性必須具有非 null 值。 若為所有其他 **TargetOperatingSystem** 值，則此屬性為 null。 這個屬性繼承自 [**CIM \_ SoftwareElement**](cim-softwareelement.md)。
...
設計用來與其他金鑰搭配使用的 software 專案的識別碼，以建立 [**CIM \_ SoftwareElement**](cim-softwareelement.md) 類別的唯一標記法。
...
作業系統環境。 這個屬性的值不能確保二進位執行。 您必須使用作業系統版本檢查來指定作業系統的版本。 此外，也必須是作業系統執行所在的架構。 這些結構的組合可讓提供者清楚地識別特定軟體元素所需的作業系統層級。
...
這個屬性繼承自 [**CIM \_ SoftwareElement**](cim-softwareelement.md) 類別。

</dd> </dl>

## <a name="remarks"></a>備註

**Cim \_ VideoBIOSElement** 類別衍生自 [**cim \_ SoftwareElement**](cim-softwareelement.md)。
