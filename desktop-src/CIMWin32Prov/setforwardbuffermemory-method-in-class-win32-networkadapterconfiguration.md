...
**SetForwardBufferMemory** [WMI 類別](/windows/desktop/WmiSdk/retrieving-a-class)靜態方法是用來指定在路由器封包佇列中儲存封包資料所配置的記憶體數量。
...
*ForwardBufferMemory* \[在\]
</dt> <dd>

用來儲存封包資料的路由器封包佇列大小（以位元組為單位）。 預設值為 74240 (50 1480 位元組封包，四捨五入為 256) 的倍數。
...
填滿此緩衝區空間時，路由器會開始從其佇列中隨機捨棄封包。

封包佇列資料緩衝區的長度為256個位元組，因此 *ForwardBufferMemory* 參數的值應該是256的倍數。 多個緩衝區會連結在一起，以取得較大的封包。 封包的 IP 標頭會分開儲存。 如果未啟用 IP 路由器，則會忽略此參數，且不會配置任何緩衝區。 緩衝區大小的範圍可以從網路的最大傳輸單位 (MTU) 到小於0xFFFFFFFF 的值。

## <a name="examples"></a>範例

[修改所有網路介面卡的轉寄緩衝區記憶體](https://Gallery.TechNet.Microsoft.Com/da5712dc-f854-4099-98a9-59c0ff20a524)VBScript 範例會設定電腦上所有網路介面卡的轉寄緩衝區記憶體。
