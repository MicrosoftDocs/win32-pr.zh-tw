...
ms.locfileid: "119548228"
---
# <a name="cim_mediaaccessdevice-class-cimwin32-wmi-providers"></a>CIM_MediaAccessDevice 類別 (CIMWin32 WMI 提供者) 

**CIM \_ MediaAccessDevice** 類別代表存取一或多個媒體的能力，然後使用該媒體來儲存和取出資料。
...
| 方法                                                                       | 描述                                                                                                                              |
|:-----------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-mediaaccessdevice.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-mediaaccessdevice.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
此裝置支援的媒體大小上限（以 kb 為單位）。 Kb 會被視為位元組數目乘以 1000 (而不是位元組數乘以 1024) 。
...
**Cim \_ MediaAccessDevice** 類別衍生自 [**cim \_ LogicalDevice**](cim-logicaldevice.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ MediaAccessDevice** 的類別，請參閱 [Win32 類別](win32-provider.md)。
