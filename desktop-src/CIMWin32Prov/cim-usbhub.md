...
---
# <a name="cim_usbhub-class"></a>CIM \_ USBHub 類別

**CIM \_ USBHub** 類別代表 USB 集線器的功能和管理。
...
**CIM \_ USBHub** 類別具有這些方法。



| 方法                                                            | 描述                                                                                                                              |
|:------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**GetDescriptor**](getdescriptor-method-in-class-cim-usbhub.md) | 傳回 USB 裝置描述項。 不是由 WMI 所執行。<br/>                                                                    |
| [**重設**](reset-method-in-class-cim-usbhub.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-usbhub.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
這個屬性繼承自 [**CIM \_ USBDevice**](cim-usbdevice.md)。
...
在目前選取的設定中，每個介面的 USB 替代設定 (由 **CurrentConfigValue** 屬性) 指出。 此陣列針對設定中的每個介面都有一個專案。 如果 **CurrentConfigValue** 屬性的值為 0 (零) ，表示未設定裝置，則表示未定義該陣列。 如需有關剖析這個八位字串的詳細資訊，請參閱 USB 規格。
...
限定詞： [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ( "[**CIM \_ USBDevice**](cim-usbdevice.md).**CurrentAlternateSettings**") 
</dt> </dl>

目前為裝置選取的設定。 如果此值為 0 (零) ，則不會設定裝置。
...
若 **為 TRUE**，則電源會同時切換至中樞上的所有埠。 若為 **FALSE**，則會針對每個埠個別切換電源。
...
USB 裝置支援的最新 USB 版本。 這個屬性會以二進位編碼的十進位 (BCD) ，其中小數點是在第二個和第三位數之間隱含的。 例如，0x201 的值表示支援2.01 版。
...
**Cim \_ USBHub** 類別衍生自 [**cim \_ USBDevice**](cim-usbdevice.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ USBHUB** 的 WMI 類別，請參閱 [Win32 類別](win32-provider.md)。
