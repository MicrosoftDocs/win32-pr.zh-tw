...
---
# <a name="win32_memoryarray-class"></a>Win32 \_ MemoryArray 類別

**Win32 \_ MemoryArray** [WMI 類別](/windows/desktop/WmiSdk/retrieving-a-class)代表電腦系統記憶體陣列和對應位址的屬性。
...
**Win32 \_ MemoryArray** 類別具有下列類型的成員：
...
**Win32 \_ MemoryArray** 類別具有這些方法。



| 方法            | 描述                                                                                                                                                                                                        |
|:------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **重設**         | 未實作。 若要執行此方法，請參閱 [**Win32 \_ SMBIOSMemory**](win32-smbiosmemory.md)中的 [**Reset**](reset-method-in-class-cim-controller.md)方法以取得檔。<br/>                 |
| **SetPowerState** | 未實作。 若要執行此方法，請參閱 [**Win32 \_ SMBIOSMemory**](win32-smbiosmemory.md)中的 [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md)方法，以取得檔。<br/> |
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "SMBIOS \| Type 18 \| 32-Bit Memory Error Information Information \| 供應商症狀" ) ， [**最大**](/windows/desktop/WmiSdk/standard-qualifiers) (64) 
</dt> </dl>

其他錯誤資訊的陣列。 例如，錯誤檢查和修正 (ECC) 的症狀，或如果使用迴圈冗余檢查 (CRC) **ErrorMethodology** 值，則會傳回檢查位。 在後者的情況下，如果已辨識出單一位錯誤，而且已知 CRC 演算法，則可以判斷失敗的確切位。 這種類型的資料 (ECC 的症狀、Check Bit、同位資料或其他廠商提供的資訊) 包含在此欄位中。 只有當 **ErrorInfo** 屬性不等於3時，才會使用這個屬性。

這個屬性繼承自 [**Win32 \_ SMBIOSMemory**](win32-smbiosmemory.md)。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "SMBIOS \| Type 18 \| 32-bit Memory error Information \| error Type" ) 
</dt> </dl>

若 **為 True**，表示最新的錯誤是可修正的。 如果 **ErrorInfo** 設定為3，則不會使用這個屬性。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「SMBIOS \| 類型 19 \| 記憶體裝置對應位址 \| 結束位址」 ) 
</dt> </dl>

應用程式或作業系統所參考的結束位址。 此記憶體位址是由記憶體控制器針對這個記憶體物件所對應。

此值來自 SMBIOS 版本資訊中的 **記憶體陣列對應位址** 結構。 針對 SMBIOS 版本2.1 到2.6，此值來自 **結束位址** 成員。 針對 SMBIOS 2.7 版 +，此值來自于 **擴充的結束位址** 成員。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "SMBIOS \| Type 18 \| 32-bit Memory error Information \| error Operation" ) 
</dt> </dl>

造成最後一個錯誤的記憶體存取作業類型。 只有當 **ErrorInfo** 未設定為3時，這個屬性才有效。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "SMBIOS \| Type 18 \| 32-bit Memory error Information \| error Address" ) 
</dt> </dl>

最後一個記憶體錯誤的位址。 只有當 **ErrorInfo** 未設定為3時，才會使用這個屬性。
...
限定詞： [**ArrayType**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「已編制索引」 ) 、 [**MAPPINGSTRINGS**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「SMBIOS」 ) 、 [**最大**](/windows/desktop/WmiSdk/standard-qualifiers) (64) 
</dt> </dl>

從最後一個記憶體存取所捕捉到的資料陣列，但發生錯誤。 資料會佔用陣列的前 *n* 個八位數位，以保存 **ErrorTransferSize** 屬性所指定的位數。 如果 **ErrorTransferSize** 為 0 (零) ，則不會使用這個屬性。
...
排序儲存在 **ErrorData** 屬性中的資料。 只有當 **ErrorTransferSize** 為 0 (零) 時，才會使用這個屬性。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "SMBIOS \| Type 18 \| Error 細微性" ) 
</dt> </dl>
...
限定詞： [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「[**CIM \_ 記憶體**](cim-memory.md)」。**OtherErrorDescription**") ， [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) (" SMBIOS \| Type 18 \| 32-Bit Memory error Information \| error Type ") 
</dt> </dl>

最近發生的錯誤類型。 值12-14 （指出錯誤是否可更正）不會與這個屬性一起使用，但這項資訊可在 **CorrectableError** 屬性中找到。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "SMBIOS \| Type 16 \| 實體記憶體陣列 \| 記憶體錯誤更正" ) 
</dt> </dl>

記憶體硬體所使用的錯誤檢查類型。

這個屬性繼承自 [**CIM \_ StorageExtent**](cim-storageextent.md)。 值如下：

<dl> <dd>另外</dd> <dd>不明</dd> <dd>"None"</dd> <dd>位</dd> <dd>「單一位 ECC」</dd> <dd>「多重位 ECC」</dd> <dd>CRC</dd> </dl>
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "SMBIOS \| Type 18 \| 32-bit Memory error Information \| Error 解決" ) ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( "bytes" ) 
</dt> </dl>

實際決定造成錯誤的資料量。 當 **ErrorInfo** 屬性設定為3時，不會使用這個屬性。
...
上次發生記憶體錯誤的時間。 只有當 **ErrorInfo** 未設定為3時，這個屬性才有效。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "SMBIOS" ) ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( "bits" ) 
</dt> </dl>

資料 (的大小，其中包含上次傳送的錯誤) 。 如果沒有任何錯誤，這個屬性會設定為 0 (零) 。
...
限定詞： [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「[**CIM \_ 記憶體**](cim-memory.md)」。**ErrorInfo**") ， [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) (" SMBIOS ") 
</dt> </dl>

當 **ErrorInfo** 屬性設定為1時的詳細資訊。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「SMBIOS \| 類型 19 \| 記憶體裝置對應位址 \| 起始位址」 ) 
</dt> </dl>

應用程式或作業系統所參考的起始位址。 此記憶體位址是由記憶體控制器針對這個記憶體物件所對應。

此值來自 SMBIOS 版本資訊中的 **記憶體陣列對應位址** 結構。 針對 SMBIOS 版本2.1 到2.6，此值來自 **起始位址** 成員。 針對 SMBIOS 2.7 版 +，此值來自于 **擴充的起始位址** 成員。
...
若 **為 True**， **ErrorAddress** 屬性中的位址資訊就是系統層級的位址。 如果為 **False**，則為實體位址。 只有當 **ErrorInfo** 未設定為3時，才會使用這個屬性。
...
**Win32 \_ MemoryArray** 類別衍生自 [**win32 \_ SMBIOSMemory**](win32-smbiosmemory.md)。
