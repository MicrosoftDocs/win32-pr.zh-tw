...
ms.locfileid: "119547478"
---
# <a name="copyex-method-of-the-cim_datafile-class"></a>CIM \_ 資料檔案類別的 CopyEx 方法

**CopyEx** 方法會將物件路徑中指定的邏輯檔案 (或目錄) 複製到 *FileName* 參數所指定的位置。 如果需要覆寫現有的邏輯檔案，則不支援複本。 這個方法是 [**複製**](copy-method-in-class-cim-datafile.md) 方法的擴充版本，並且繼承自 [CIM \_ LogicalFile](cim-logicalfile.md)。
...
字串，代表方法失敗 (或目錄) 的名稱。 如果方法成功，此參數將會是 **null** 。
...
字串，表示要作為此方法起點的子檔 (或目錄) 。 一般而言， *StartFileName* 參數是 *StopFileName* 參數，可指定上一個方法呼叫發生錯誤 (或目錄) 。 如果這個參數是 **null**，則會在 [**ExecMethod**](/windows/desktop/WmiSdk/swbemservices-execmethod) 呼叫中指定的檔案 (或目錄) 上執行作業。
...
若為 TRUE，則此方法也會以遞迴方式套用至 [**CIM \_ 資料檔案**](cim-datafile.md) 實例所指定之目錄中的檔案和目錄。 若為檔案實例，則會忽略這個參數。
...
[**CIM \_ 資料檔案**](cim-datafile.md)中的 **COPYEX** 方法是由 WMI 所執行。
