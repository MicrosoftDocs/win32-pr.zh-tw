...
ms.locfileid: "118677043"
---
# <a name="cim_volumeset-class"></a>CIM \_ VolumeSet 類別

**CIM \_ VolumeSet** 類別代表針對讀取和寫入使用者資料的作業環境所呈現的連續邏輯區塊範圍。 磁片區集不應彼此重迭，而且是以一或多個實體範圍、受保護的空間範圍或匯總範圍為基礎， (所有相同類型) 。 根據需要具現化或子類別化相關關聯。
...
**CIM \_ VolumeSet** 類別具有這些方法。



| 方法                                                               | 描述                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-volumeset.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-volumeset.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
連續區塊總數，每個區塊會封鎖 **區塊** 屬性中包含的值大小，這會形成此儲存範圍。 您可以將 [ **區塊** ] 屬性的值乘以這個屬性的值，以計算儲存區的總大小。 如果 **區塊** 的值是 1 (一個) ，此屬性就是儲存區的總大小。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 磁片區組 \| 001.5 ") 
</dt> </dl>

受保護的空間範圍數，以做為集體集的 stripe。 在 SCC 中，此值會定義為要計數的 stripe 數目，然後再繼續對應到目前 stripe 以外的下一組連續範圍。

如需在腳本中使用 **uint64** 值的詳細資訊，請參閱 [WMI 中的腳本](/windows/desktop/WmiSdk/creating-a-wmi-script)。

</dd> <dt>

**PSExtentStripeLength**
</dt> <dd> <dl> <dt>
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 磁片區組 \| 001.4 ") 
</dt> </dl>

在迴圈回到目前 stripe 的第一個受保護空間範圍之前，所計算的連續受保護空間範圍數目。 這是形成使用者資料等量的範圍數目。
...
**Cim \_ VolumeSet** 類別衍生自 [**cim \_ StorageExtent**](cim-storageextent.md)。
