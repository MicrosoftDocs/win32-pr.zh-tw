...
ms.locfileid: "122885310"
---
# <a name="cim_check-class"></a>CIM \_ 檢查類別

**Cim \_ 檢查** 類別代表一種條件或特性，此條件或特性在定義的環境中應該是 true，或是由 [**cim 系統 \_ 環境**](cim-computersystem.md)類別的實例所設定的範圍。 與特定軟體專案相關聯的檢查會使用 [**CIM \_ SoftwareElementChecks**](cim-softwareelementchecks.md)關聯的 [**階段**] 屬性組織成兩個群組的其中一個。

當軟體專案在特定環境中時，預期會滿足的條件就稱為「狀態」條件。 將目前的軟體專案轉換為下一個狀態時，必須滿足的條件就稱為「下一州」條件。

[**Cim 系統 \_ 系統**](cim-computersystem.md)物件代表已安裝 [**cim \_ SoftwareElement**](cim-softwareelement.md)或將安裝 **cim \_ SoftwareElement** 的環境。 如果已安裝軟體專案，則會使用 [**cim \_ InstalledSoftwareElement**](cim-installedsoftwareelement.md) 關聯來識別代表「環境」的 cim 程式 **識別碼 \_** 物件。 當軟體專案發佈並安裝在不同的電腦上時，目標系統 **的 \_ CIM** 系統元件物件就是環境。
...
| 方法                                             | 描述                                                   |
|:---------------------------------------------------|:--------------------------------------------------------------|
| [**調用**](invoke-method-in-class-cim-check.md) | 採取特定動作。 不是由 WMI 所執行。<br/> |
...
WMI 不會執行這個類別。 如需衍生自 **CIM \_ 檢查** 之類別的詳細資訊，請參閱 [Win32 類別](win32-provider.md)。
