...
---
# <a name="cim_managedsystemelement-class-cimwin32-wmi-providers"></a>CIM_ManagedSystemElement 類別 (CIMWin32 WMI 提供者) 

**CIM \_ ManagedSystemElement** 類別是 system 元素階層的基類。 任何可辨別的系統元件都是包含在這個類別中的候選。 範例包括軟體元件，例如檔案;裝置，例如磁片磁碟機和控制器;以及實體元件，例如晶片和卡片。
...
WMI 不會執行這個類別。 針對衍生自這個類別的 WMI 類別，請參閱 [Win32 類別](win32-provider.md)。
