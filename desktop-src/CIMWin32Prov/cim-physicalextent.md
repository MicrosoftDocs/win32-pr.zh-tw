...
**CIM \_ PhysicalExtent** 類別代表 SCC RAID 的執行。 它會在單一存放裝置上定義連續可定址的區塊位址，這些位址會被視為相同 [**CIM \_ StorageRedundancyGroup**](cim-storageredundancygroup.md) 類別中的單一儲存區。 替代方式是使用自動設定時，是要具現化或擴充 [**CIM \_ AggregatePExtent**](cim-aggregatepextent.md) 類別。
...
**CIM \_ PhysicalExtent** 類別具有下列類型的成員：
...
**CIM \_ PhysicalExtent** 類別具有這些方法。



| 方法                                                                    | 描述                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-physicalextent.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-physicalextent.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
限定詞：覆 [**寫**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「區塊」 ) ， [**MAPPINGSTRINGS**](/windows/desktop/WmiSdk/standard-qualifiers) (」 MIF。DMTF \| 實體範圍 \| 001.4 ") 
</dt> </dl>

形成儲存範圍之區塊的大小（以位元組為單位）。 如果變數區塊大小，則應該指定最大區塊大小（以位元組為單位）。 如果區塊大小未知或區塊概念無效 (例如，針對匯總延伸區、記憶體或邏輯磁片) ，請輸入 1 (一個) 。
...
限定詞：覆 [**寫**](/windows/desktop/WmiSdk/standard-qualifiers) ( "NumberOfBlocks" ) ， [**MAPPINGSTRINGS**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 實體範圍 \| 001.3 ") 
</dt> </dl>
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 實體範圍 \| 001.6 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" bytes ") 
</dt> </dl>

開始檢查資料交錯之前，要略過的使用者資料位元組數目。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 實體範圍 \| 001.7 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" bytes ") 
</dt> </dl>
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 實體範圍 \| 001.8 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" bytes ") 
</dt> </dl>
...
**Cim \_ PhysicalExtent** 類別衍生自 [**cim \_ StorageExtent**](cim-storageextent.md)。
