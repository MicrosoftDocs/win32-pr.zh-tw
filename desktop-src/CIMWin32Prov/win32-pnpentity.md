...
**Win32 \_ PnPEntity** [WMI 類別](../wmisdk/retrieving-a-class.md)代表隨插即用裝置的屬性。 隨插即用的實體會顯示為位於主控台中裝置管理員的專案。
...
**Win32 \_ PnPEntity** 類別具有這些方法。



| 方法                                                             | 描述                                                                                                                                                                                    |
|:-------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**停用**](disable-win32-pnpentity.md)                         | 停用此隨插即用裝置。<br/>                                                                                                                                                 |
| [**啟用**](enable-win32-pnpentity.md)                           | 啟用此隨插即用裝置。<br/>                                                                                                                                                  |
| [**GetDeviceProperties**](getdeviceproperties-win32-pnpentity.md) | 取得此隨插即用裝置的指定屬性。<br/>                                                                                                                         |
| **重設**                                                          | 未實作。 若要執行此方法，請參閱 [**CIM \_ LogicalDevice**](cim-logicaldevice.md)中的 [**Reset**](reset-method-in-class-cim-controller.md)方法。<br/>                 |
| **SetPowerState**                                                  | 未實作。 若要執行此方法，請參閱 [**CIM \_ LogicalDevice**](cim-logicaldevice.md)中的 [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md)方法。<br/> |
...
安裝程式用來將裝置與 INF 檔案比對的廠商定義識別碼串。 通常，裝置會有相關聯的硬體識別碼清單。 例外狀況是1394匯流排驅動程式，它不會使用硬體識別碼。 清單中的第一個硬體識別碼應該是裝置識別碼。 其餘的識別碼應該依照遞減的適用性順序列出。

硬體識別碼會以下列其中一種格式出現：

-   列舉值 \\ 列舉值特定-裝置識別碼

    這是個別 PnP 裝置最常用的格式。 列舉值的範例有 BIOS 或 ISAPNP。
...
> [!WARNING]
> 雖然這個屬性是列在 MOF 檔案中，但實際上並不存在於類別中。 此屬性只會在此描述，以提供完整性，並清楚說明 MOF 檔案本身。
...
支援 **SetPowerState** 方法。 這個方法可在父 [**CIM \_ LogicalDevice**](cim-logicaldevice.md) 類別上找到，並且可以實作為。 如需詳細資訊，請參閱 [設計受控物件格式 (MOF) 類別](../wmisdk/designing-managed-object-format--mof--classes.md)。
...
支援此隨插即用裝置的服務名稱。 如需詳細資訊，請參閱 [**Win32 \_ SystemDriverPnPEntity**](win32-systemdriverpnpentity.md)。
...
**Win32 \_ PnPEntity** 類別衍生自 [**CIM \_ LogicalDevice**](cim-logicaldevice.md)。

## <a name="examples"></a>範例

TechNet 資源庫上的 [Get-BrokenHardware.ps1](https://Gallery.TechNet.Microsoft.Com/dbb678f4-b95b-45c3-bc8b-2ae2d052448e) PowerShell 範例會使用 **Win32 \_ PNPENTITY** ，以使用 WMI 抓取無法運作的硬體清單。

下列 VBScript 程式碼範例會藉由建立遠端電腦名稱稱陣列，然後在每部電腦上顯示隨插即用裝置（Win32 PnPEntity 的實例）的名稱，連接到同一網域中的一組遠端電腦 \_ 。
...
[連接至遠端電腦上的 WMI](../wmisdk/connecting-to-wmi-on-a-remote-computer.md)
</dt> <dt>
