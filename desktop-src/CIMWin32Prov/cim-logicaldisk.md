...
**CIM \_ LogicalDisk** 類別代表由檔案系統透過磁片 **DeviceID** (key) 欄位識別的連續邏輯區塊範圍。 例如，在 Windows 環境中， **DeviceID** 欄位包含磁碟機號;在 UNIX 環境中，它包含存取路徑;在 NetWare 環境中，它包含磁片區名稱。
...
**CIM \_ LogicalDisk** 類別具有這些方法。



| 方法                                                                 | 描述                                                                                                                              |
|:-----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-logicaldisk.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-logicaldisk.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
描述媒體是否為可讀取、可寫入或兩者。 這個屬性繼承自 [**CIM \_ StorageExtent**](cim-storageextent.md)。
...
形成儲存範圍之區塊的大小（以位元組為單位）。 如果變數區塊大小，則應該指定最大區塊大小（以位元組為單位）。 如果區塊大小未知，或區塊概念無效 (例如，針對匯總延伸區、記憶體或邏輯磁片) ，請輸入1。
...
連續區塊的總數，每個區塊會封鎖包含在 **區塊** 屬性中的值大小，這會形成儲存區的範圍。 您可以將 [ **區塊** ] 屬性的值乘以這個屬性的值，以計算儲存區的總大小。 如果 **區塊** 屬性的值是1，則這個屬性是儲存區的總大小。
...
**Cim \_ LogicalDisk** 類別衍生自 [**cim \_ StorageExtent**](cim-storageextent.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ LogicalDisk** 的類別，請參閱 [Win32 類別](win32-provider.md)。
