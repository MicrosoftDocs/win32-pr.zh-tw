...
---
# <a name="cim_logicaldevice-class-cimwin32-wmi-providers"></a>CIM_LogicalDevice 類別 (CIMWin32 WMI 提供者) 

**CIM \_ LogicalDevice** 類別代表可能或可能不會在實體硬體中實現的硬體實體。
...
| 方法                                                                   | 描述                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-logicaldevice.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-logicaldevice.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
支援 **SetPowerState** 方法。 這個方法可在父 **CIM \_ LogicalDevice** 類別上找到，並且可以實作為。 如需詳細資訊，請參閱 [設計受控物件格式 (MOF) 類別](/windows/desktop/WmiSdk/designing-managed-object-format--mof--classes)。
...
管理作業或設定的邏輯裝置特性包含在 **CIM \_ LogicalDevice** 物件中或與之相關聯。 例如，印表機操作屬性是支援的紙張大小或偵測到的錯誤。 例如，感應器裝置設定屬性是閾值設定。 邏輯裝置的各種設定都可以存在，而且會包含在與邏輯裝置相關聯的 [**CIM \_ 設定**](cim-setting.md) 物件中。

**Cim \_ LogicalDevice** 類別衍生自 [**cim \_ LogicalElement**](cim-logicalelement.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ LogicalDevice** 的類別，請參閱 [Win32 類別](win32-provider.md)。
