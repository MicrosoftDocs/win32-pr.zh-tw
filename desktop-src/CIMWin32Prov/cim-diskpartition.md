...
---
# <a name="cim_diskpartition-class"></a>CIM \_ DiskPartition 類別

**CIM \_ DiskPartition** 類別代表由作業系統透過分割區的類型和子類型欄位識別的連續邏輯區塊範圍。 磁碟分割應由 [**CIM \_ RealizesDiskPartition**](cim-realizesdiskpartition.md) 關聯) 或建立于儲存磁片區的實體媒體 (直接實現。
...
**CIM \_ DiskPartition** 類別具有下列類型的成員：
...
**CIM \_ DiskPartition** 類別具有這些方法。



| 方法                                                                   | 描述                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-diskpartition.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-diskpartition.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
指定媒體是否為可讀取、可寫入或兩者皆可讀取。 這個屬性繼承自 [**CIM \_ StorageExtent**](cim-storageextent.md)。
...
形成儲存範圍之區塊的大小（以位元組為單位）。 如果它是變數區塊大小，則應該指定最大區塊大小（以位元組為單位）。 如果區塊大小未知，或區塊概念無效 (例如，針對匯總延伸區、記憶體或邏輯磁片) ，請輸入值1。
...
若 **為 TRUE**，則會將磁碟分割標示為可開機。 這並不表示已在磁碟分割上載入作業系統。
...
自由格式的字串，提供 **LastErrorCode** 屬性中所記錄之錯誤的詳細資訊，以及要執行的更正動作。
...
連續區塊的數目，每個區塊會封鎖包含在 **區塊** 屬性中的值大小，以形成儲存區的範圍。 您可以將 [ **區塊** ] 屬性的值乘以這個屬性的值，以計算儲存區的總大小。 如果 **區塊** 的值是1，則這個屬性是儲存區的總大小。
...
若 **為 TRUE**，則會將磁碟分割標示為電腦系統的主要磁碟分割。
...
**Cim \_ DiskPartition** 類別衍生自 [**cim \_ StorageExtent**](cim-storageextent.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ DiskPartition** 的類別，請參閱 [Win32 類別](win32-provider.md)。
