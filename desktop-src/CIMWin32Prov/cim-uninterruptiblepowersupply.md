...
**CIM \_ UninterruptiblePowerSupply** 類別代表 (UPS) 的不斷電供應系統供應器的功能和管理。 UPS 裝置的內容會指出何時修剪或提升傳入電源，以及組成裝置的電池、產生器等的匯總資訊。 個別元件 (例如，多個電池) 也可以獨立建立模型並與 UPS 相關聯。
...
**CIM \_ UninterruptiblePowerSupply** 類別具有下列類型的成員：
...
**CIM \_ UninterruptiblePowerSupply** 類別具有這些方法。



| 方法                                                                                | 描述                                                                                                                              |
|:--------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-uninterruptiblepowersupply.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-uninterruptiblepowersupply.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
**CIM \_ UninterruptiblePowerSupply** 類別具有這些屬性。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 電源供應器 \| 002.15」 ) 
</dt> </dl>

輸入電壓範圍目前正在使用中。 如果供應目前未繪製電源，則可以指定值 6 ( 「不會」 ) 。 這項資訊在 UPS （ [**CIM \_ 電源**](cim-powersupply.md)的子類別）的情況下是必要的。

這個屬性繼承自 [**CIM \_ 電源電源**](cim-powersupply.md)。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| UPS 電池 \| 001.4」 ) ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「百分比」 ) 
</dt> </dl>

使用電池技術的 UPS 剩餘的完整費用百分比估計。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| UPS 電池 \| 001.3 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" 分鐘 ") 
</dt> </dl>

估計的時間（以分鐘為單位），在目前的負載狀況下，如果公用程式電源關閉或遺失並保持關閉，則會在目前的負載狀況下耗盡電池、產生器或其他電源。
...
限定詞： [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( "赫茲" ) 
</dt> </dl>

電源供應器輸入頻率範圍1的高階頻率（以赫茲為單位）。 值為 0 (零) 表示 DC。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 電源供應器 \| 002.17 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" 赫茲 ") 
</dt> </dl>

電源供應器輸入頻率範圍1的低端頻率（以赫茲為單位）。 值為 0 (零) 表示 DC。
...
限定詞：覆 [**寫**](/windows/desktop/WmiSdk/standard-qualifiers) ( "Range1InputVoltageHigh" ) ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( "毫伏" ) 
</dt> </dl>

如果電壓（以毫伏為單位）高於 **Range1InputVoltageHigh** 屬性所指定的值，則會藉由修剪電壓來彌補 UPS。 值為 0 (零) 表示發生修剪的電壓未知。
...
限定詞：覆 [**寫**](/windows/desktop/WmiSdk/standard-qualifiers) ( "Range1InputVoltageLow" ) ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( "毫伏" ) 
</dt> </dl>

如果電壓（以毫伏為單位）低於 **Range1InputVoltageLow** 屬性指定的值，則會使用其電源線來提高電壓來彌補 UPS。 值為0表示發生提升的電壓為未知。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 電源供應器 \| 002.20 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" 赫茲 ") 
</dt> </dl>

電源供應器輸入頻率範圍2的高階頻率（以赫茲為單位）。 值為 0 (零) 表示 DC。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 電源供應器 \| 002.19 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" 赫茲 ") 
</dt> </dl>

電源供應器輸入頻率範圍2的低端頻率（以赫茲為單位）。 0值表示 DC。
...
限定詞：覆 [**寫**](/windows/desktop/WmiSdk/standard-qualifiers) ( "Range2InputVoltageHigh" ) ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( "毫伏" ) 
</dt> </dl>

如果電壓（以毫伏為單位）高於 **Range2InputVoltageHigh** 屬性所指定的值，則會藉由修剪電壓來彌補 UPS。 值為 0 (零) 表示發生修剪的電壓未知。
...
限定詞：覆 [**寫**](/windows/desktop/WmiSdk/standard-qualifiers) ( "Range2InputVoltageLow" ) ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( "毫伏" ) 
</dt> </dl>

如果電壓（以毫伏為單位）低於 **Range2InputVoltageLow** 屬性指定的值，則會使用其電源線來提高電壓來彌補 UPS。 值為 0 (零) 表示發生提升的電壓為未知。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| UPS 電池 \| 001.1」 ) 
</dt> </dl>

指出 UPS 的電池、發電機或其他來源中剩餘的容量。
...
<span id="Normal"></span><span id="normal"></span><span id="NORMAL"></span>**一般** (2) 


</dt> <dd>

執行時間剩餘的預估分鐘數大於 UPS 定義的低電源狀態 (通常是兩分鐘的) 。

</dd> <dt>

<span id="Low"></span><span id="low"></span><span id="LOW"></span>

<span id="Low"></span><span id="low"></span><span id="LOW"></span>**低** (3) 


</dt> <dd>

執行時間的剩餘估計分鐘數小於或等於 UPS 的已定義低電源狀態。

</dd> <dt>

<span id="Depleted"></span><span id="depleted"></span><span id="DEPLETED"></span>

<span id="Depleted"></span><span id="depleted"></span><span id="DEPLETED"></span> (4) **耗盡**


</dt> <dd>

當公用程式電源遺失時，UPS 將無法承受目前的負載 (包括) 的公用程式電源目前不存在的可能性。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| UPS 電池 \| 001.2 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" 秒 ") 
</dt> </dl>

UPS 上次切換至電池電力、產生器或其他電源的經過時間（以秒為單位）。 或者，自從 UPS 上次重新開機的時間，以較少者為准。 如果 UPS 上線，則會傳回 0 (零) 。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 電源供應器 \| 002.21 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" 毫瓦 ") 
</dt> </dl>

電源供應器的總輸出能力，以毫瓦。 值為 0 (零) 表示不明。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 電源供應器 \| 002.16」 ) 
</dt> </dl>

在電源供應器中實作為輸入電壓範圍切換的類型。
...
**Cim \_ UninterruptiblePowerSupply** 類別衍生自 [**cim \_ 電源供應**](cim-powersupply.md)器。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ UNINTERRUPTIBLEPOWERSUPPLY** 的 WMI 類別，請參閱 [Win32 類別](win32-provider.md)。
