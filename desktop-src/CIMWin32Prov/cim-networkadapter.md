...
api_location:
- CIMWin32.dll
ms.openlocfilehash: fee2fcc49dc7ea6d0b5aa1be2dd81731d9585d99634d3139dd49c15dc578e3f7
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119021026"
---
# <a name="cim_networkadapter-class"></a>CIM \_ NetworkAdapter 類別

**CIM \_ NetworkAdapter** 類別是一個抽象類別，用來定義一般網路硬體概念 (例如，) 的永久位址或速度。 這項資訊是使用 [**CIM \_ DeviceSAPImplementation**](cim-devicesapimplementation.md) 關聯來傳達。

網路介面卡和相關的端點表示對等互連之間的連線可能性。 連線的可能性與主要或次級和控制器之間的差異，或 [**CIM \_ 控制器**](cim-controller.md)的受控制關係有很大的差異。 不過，有時候單一裝置可作為網路介面卡和控制器，例如，當光纖通道介面卡做為電腦系統的 SCSI 控制器運作時。 在這種情況下，裝置的各個層面都是網路導向的，有些則是控制器導向。 控制器和介面卡類別應該具現化，而且也應該建立裝置身分識別關聯性，以連結裝置的不同層面。
...
**CIM \_ NetworkAdapter** 類別具有下列類型的成員：
...
**CIM \_ NetworkAdapter** 類別具有這些方法。



| 方法                                                                    | 描述                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-networkadapter.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-networkadapter.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
若 **為 TRUE**，則網路介面卡可以自動判斷連接之網路媒體的速度或其他通訊特性。
...
將網路位址硬式編碼為介面卡。 您可以使用固件升級或軟體設定來變更硬式編碼的位址。 如果已變更，則在進行變更時應更新欄位。 如果網路介面卡沒有硬式編碼的位址，則此屬性應該保留空白。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIB。IETF \| RFC1213-MIB. ifSpeed "，" MIF。「DMTF \| 網路介面卡802埠 \| 001.5」 ) ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) ( 每秒「位數」 ) 
</dt> </dl>

估計目前的頻寬（以 BPS 為限）。 針對因頻寬而異的端點，或無法進行精確估計的端點，這個屬性應該包含名義頻寬。
...
**Cim \_ NetworkAdapter** 類別衍生自 [**cim \_ LogicalDevice**](cim-logicaldevice.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ NetworkAdapter** 的類別，請參閱 [Win32 類別](win32-provider.md)。
