...
**SetDatabasePath** [WMI 類別](/windows/desktop/WmiSdk/retrieving-a-class)靜態方法會設定標準網際網路資料庫檔案的路徑， (主機、LMHOSTS、網路和通訊協定) 。
...
如果不需要重新開機，則會傳回 0 (零) 零，1 (一個) 在需要重新開機時成功完成，而如果發生錯誤，則為不同的數位。 如需錯誤碼的詳細資訊，請參閱 [**WMI 錯誤常數**](/windows/desktop/WmiSdk/wmi-error-constants) 或 [**WbemErrorEnum**](/windows/desktop/api/wbemdisp/ne-wbemdisp-wbemerrorenum)。 如需一般 **HRESULT** 值，請參閱 [系統錯誤碼](/windows/desktop/Debug/system-error-codes)。
...
Windows 通訊端介面會使用這個方法。 預設路徑為% SystemRoot% \\ system32 \\ 驅動程式 \\ 。
