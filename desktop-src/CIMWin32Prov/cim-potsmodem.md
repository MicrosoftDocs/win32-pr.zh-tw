...
**CIM \_ POTSModem** 類別代表一種裝置，可透過連接到單純的舊電話系統 (POTS) 網路，將二進位資料轉譯成 wave modulations 進行音效型傳輸。
...
**CIM \_ POTSModem** 類別具有這些方法。



| 方法                                                               | 描述                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-potsmodem.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-potsmodem.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
目前正在進行數據機程式設計的國家或地區。 當支援多個國家或地區時，此屬性會定義目前已選取要使用哪一個。
...
如果未交換任何資料，則為電話線路自動中斷連線的時間限制 (秒) 。 值為 0 (零) 表示此功能存在但未啟用。
...
數據機本身中可定義的最大密碼數目。 如果不支援這項功能，請輸入 0 (零) 。
...
**Cim \_ POTSModem** 類別衍生自 [**cim \_ LogicalDevice**](cim-logicaldevice.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ POTSMODEM** 的 WMI 類別，請參閱 [Win32 類別](win32-provider.md)。
