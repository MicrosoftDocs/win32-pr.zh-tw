...
**CIM \_ 動作** 類別是一項作業，屬於進程的一部分，可在其下一個狀態下建立軟體專案，或刪除目前狀態中的 software 元素。
...
指出特定 **CIM \_ 動作** 物件是否為一連串動作的一部分，以將目前的軟體專案轉換為下一個狀態，例如 "Install"，或移除目前的 software 元素，例如 "Uninstall"。
...
WMI 不會執行這個類別。 請參閱衍生自 **CIM \_ 動作** 之類別的 [Win32 類別](win32-provider.md)。
