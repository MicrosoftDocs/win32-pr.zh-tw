...
**CIM \_ FileAction** 類別可讓作者找出使用者電腦上已存在的檔案，然後將這些檔案移動或複製到新的位置。
...
WMI 不會執行這個類別。 針對衍生自 **CIM \_ FileAction** 的類別，請參閱 [Win32 類別](win32-provider.md)。
