...
ms.locfileid: "118422135"
---
# <a name="cim_osversioncheck-class"></a>CIM \_ OSVersionCheck 類別

**CIM \_ OSVersionCheck** 類別會指定可支援軟體元素的作業系統版本。 您可以針對特定、最小值、最大值或作業系統版本範圍執行檢查。 若要指定特定的作業系統版本，最小和最大版本必須相等。 若要指定最小版本，必須指定最小版本。 若要指定最大版本，則只需要指定最大版本。 若要指定範圍，必須指定最小和最大版本。

作業系統的類型是在擁有的 software 專案的 **TargetOperatingSystem** 屬性中指定的。 檢查的詳細資料會與在 cim [**\_ InstalledOS**](cim-installedos.md)關聯的 cim [**\_ 作業系統**](cim-operatingsystem.md)物件中找到的對應詳細資料，與描述該環境的 cim 系統系統物件相關。 [**\_**](cim-computersystem.md) 至少有一個 **CIM \_ 作業系統** 類別必須滿足要滿足的檢查條件詳細資料。 換句話說，並非所有相關電腦系統上的作業系統都必須滿足該條件。 此外， **CIM \_ 作業系統** 類別的 **OSType** 屬性必須符合 **TargetOperatingSystem** 屬性的類型。
...
**CIM \_ OSVersionCheck** 類別具有下列類型的成員：
...
**CIM \_ OSVersionCheck** 類別具有這些方法。



| 方法                                                      | 描述                                                   |
|:------------------------------------------------------------|:--------------------------------------------------------------|
| [**調用**](invoke-method-in-class-cim-osversioncheck.md) | 採取特定動作。 不是由 WMI 所執行。<br/> |
...
限定詞： [**maxlen**](/windows/desktop/WmiSdk/standard-qualifiers) (64) ， [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ( "[**CIM \_ 作業系統**](cim-operatingsystem.md)。**版本**") 
</dt> </dl>
