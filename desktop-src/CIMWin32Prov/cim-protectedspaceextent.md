...
---
# <a name="cim_protectedspaceextent-class"></a>CIM \_ ProtectedSpaceExtent 類別

**CIM \_ ProtectedSpaceExtent** 類別代表可定址的邏輯區塊位址，這些位址會被視為單一儲存範圍，但位於單一實體範圍。 受保護的空間範圍會排除任何對應為檢查資料的邏輯區塊，並包含使用者資料 stripe 深度對應資訊。 另一個可能的情況是，如果使用自動設定，則會具現化或擴充 [**CIM \_ AggregatePSExtent**](cim-aggregatepsextent.md) 類別。
...
**CIM \_ ProtectedSpaceExtent** 類別具有下列類型的成員：
...
**CIM \_ ProtectedSpaceExtent** 類別具有這些方法。



| 方法                                                                          | 描述                                                                                                                              |
|:--------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-protectedspaceextent.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-protectedspaceextent.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
**CIM \_ ProtectedSpaceExtent** 類別具有這些屬性。
...
區塊大小（以位元組為單位），這是形成儲存區的區塊。 如果變數區塊大小，則應該指定最大區塊大小（以位元組為單位）。 如果區塊大小未知，或區塊概念無效 (例如，針對匯總範圍、記憶體或邏輯磁片) ，請輸入 1 (一個) 。
...
連續區塊的總數，每個區塊會封鎖包含在 **區塊** 屬性中的值大小，這會形成儲存區的範圍。 您可以將 [ **區塊** ] 屬性的值乘以這個屬性的值，以計算儲存區的總大小。 如果 **區塊** 的值是 1 (一個) ，則這個屬性是儲存區的總大小。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 受保護的空間範圍 \| 001.6 ") ， [**單位**](/windows/desktop/WmiSdk/standard-qualifiers) (" bytes ") 
</dt> </dl>

針對配置給 [**cim \_ VolumeSet**](cim-volumeset.md)類別的 **cim \_ ProtectedSpaceExtent** 類別，這個屬性是在移至磁片區集內下一個受保護的空間範圍之前，置於受保護空間範圍內的使用者資料位元組數目。 否則，會將這個受保護的空間範圍視為未配置，且此屬性應設定為0。
...
**Cim \_ ProtectedSpaceExtent** 類別衍生自 [**cim \_ StorageExtent**](cim-storageextent.md)。
