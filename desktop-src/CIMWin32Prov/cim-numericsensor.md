...
**CIM \_ NumericSensor** 類別代表會傳回數值讀數並選擇性支援臨界值設定的數值感應器。
...
**CIM \_ NumericSensor** 類別具有下列類型的成員：
...
**CIM \_ NumericSensor** 類別具有這些方法。



| 方法                                                                   | 描述                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-numericsensor.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-numericsensor.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
建立實例時所使用的類別或子類別。 搭配類別的其他索引鍵屬性使用時，此屬性可讓類別和其子類別的所有實例都能唯一識別。
...
**Cim \_ NumericSensor** 類別衍生自 [**cim \_ 感應器**](cim-sensor.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ NumericSensor** 的類別，請參閱 [Win32 類別](win32-provider.md)。
