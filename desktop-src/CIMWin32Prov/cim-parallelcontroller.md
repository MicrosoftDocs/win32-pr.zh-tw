...
---
# <a name="cim_parallelcontroller-class"></a>CIM \_ ParallelController 類別

**CIM \_ ParallelController** 關聯與平行埠邏輯裝置的功能和管理有關。
...
**CIM \_ ParallelController** 類別具有下列類型的成員：
...
**CIM \_ ParallelController** 類別具有這些方法。



| 方法                                                                        | 描述                                                                                                                              |
|:------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-parallelcontroller.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-parallelcontroller.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
**CIM \_ ParallelController** 類別具有這些屬性。
...
限定詞： [**ArrayType**](/windows/desktop/WmiSdk/standard-qualifiers) ( 「已編制索引」 ) ， [**MAPPINGSTRINGS**](/windows/desktop/WmiSdk/standard-qualifiers) (」 MIF。DMTF \| Parallel 埠 \| 003.8 ") ， [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("**CIM \_ ParallelController**.**CapabilityDescriptions**") 
</dt> </dl>
...
限定詞： [**ArrayType**](/windows/desktop/WmiSdk/standard-qualifiers) ( "Indexed" ) ， [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ( "**CIM \_ ParallelController**。**功能**") 
</dt> </dl>

提供 **功能** 陣列中所指出之平行控制器功能詳細說明的自由格式字串。

> [!Note]  
> 這個陣列的每個專案都與 **功能** 陣列中的專案相關，該專案位於相同的索引。
...
限定詞： [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ( "MIF。DMTF \| 平行埠 \| 003.7 ") 
</dt> </dl>

若 **為 TRUE**，則支援直接記憶體存取 (DMA) 。
...
控制器支援的直接可定址實體數目上限。 如果數位為未知或無限制，則應使用 0 (零) 的值。
...
**Cim \_ ParallelController** 類別衍生自 [**cim \_ 控制器**](cim-controller.md)。

WMI 不會執行這個類別。 針對衍生自 **CIM \_ PARALLELCONTROLLER** 的 WMI 類別，請參閱 [Win32 類別](win32-provider.md)。
