...
---
# <a name="cim_discretesensor-class"></a>CIM \_ DiscreteSensor 類別

**CIM \_ DiscreteSensor** 類別具有一組可以報告的合法字串值。 這些值會在感應器的 **PossibleValues** 屬性中列舉。 離散感應器一律會有對應至其中一個列舉值的目前讀取。

由於已將 **CurrentState** 和 **PossibleStates** 屬性新增至 [**CIM \_ 感應器**](cim-sensor.md)，因此不再需要離散的感應器子類別; 不過，它是為了回溯相容性而保留。 **CurrentReading** 和 **PossibleValues** 屬性中的資訊通常與 **CurrentState** 和 **PossibleStates** 屬性的值和語義相同，這些屬性是繼承自 **CIM \_ 感應器**。
...
**CIM \_ DiscreteSensor** 類別具有下列類型的成員：
...
**CIM \_ DiscreteSensor** 類別具有這些方法。



| 方法                                                                | 描述                                                                                                                              |
|:----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**重設**](reset-method-in-class-cim-controller.md)                 | 要求重設邏輯裝置。 不是由 WMI 所執行。<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) | 定義邏輯裝置的預期電源狀態，以及何時應將裝置置於該狀態。 不是由 WMI 所執行。<br/> |
...
**PossibleValues** 屬性中的字串被視為可接受的 (也就是說，) 不會發生錯誤。
...
這個屬性不表示電源管理功能目前已啟用，或啟用時支援哪些功能。 如需詳細資訊，請參閱 **PowerManagementCapabilities** 陣列。
...
**Cim \_ DiscreteSensor** 類別衍生自 [**cim \_ 感應器**](cim-sensor.md)。
