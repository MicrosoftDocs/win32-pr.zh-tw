...
ms.locfileid: "119061720"
---
# <a name="glevalcoord2f-function"></a>glEvalCoord2f 函式

[**GlEvalCoord2f**](glevalcoord2d.md)函數會評估啟用的二維地圖。
...
值，這個值是在上一個 [**glMap2**](glmap2.md)函式中定義之基礎函數的域座標 *u* 。
...
[**GlEvalCoord2f**](glevalcoord2d.md)函式會使用兩個定義域值（ *u* 和 *v*）來評估啟用的二維地圖。 使用 [**glMap2**](glmap2.md)定義對應。 使用 [**glEnable**](glenable.md) 和 [**glDisable**](gldisable.md)來啟用或停用它們。
...
如果啟用自動一般產生， [**glEvalCoord2f**](glevalcoord2d.md) 會使用引數 GL AUTO normal 來呼叫 [**glEnable**](glenable.md) ， \_ \_ 以產生介面法線 ANALYTICALLY，不論內容或啟用 GL \_ list.map2 \_ 一般地圖。 Let
...
下列函式會取出與 [**glEvalCoord2f**](glevalcoord2d.md) 函數相關的資訊：
