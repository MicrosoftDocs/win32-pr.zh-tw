...
---
# <a name="gldisableclientstate-function"></a>glDisableClientState 函式

[**GlEnableClientState**](glenableclientstate.md)和 **glDisableClientState** 函數會分別啟用和停用陣列。
...
[**GlEnableClientState**](glenableclientstate.md)和 **glDisableClientState** 函式會啟用和停用各種個別的陣列。 使用 [**glIsEnabled**](glisenabled.md) 或 [**glGet**](glgetbooleanv--glgetdoublev--glgetfloatv--glgetintegerv.md) 來判斷任何功能目前的設定。

在對 [**glBegin**](glbegin.md)的呼叫和 [**glEnd**](glend.md)的對應呼叫之間呼叫 [**glEnableClientState**](glenableclientstate.md)和 **glDisableClientState** 可能會造成錯誤。 如果未產生任何錯誤，則行為是未定義的。

> [!Note]  
> [**GlEnableClientState**](glenableclientstate.md)和 **glDisableClientState** 函式僅適用于 OpenGL 1.1 版或更新版本。
