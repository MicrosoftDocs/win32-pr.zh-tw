...
ms.locfileid: "119490440"
---
# <a name="gltexgend-function"></a>glTexGend 函式
...
單一值材質產生參數、GL \_ 物件 \_ 線性、gl \_ 眼 \_ 線性或 gl \_ 球體 \_ 圖的其中一個。
...
**GlTexGen** 函式會選取材質座標產生函數或提供其中一個函數的係數。 *Coord* 參數會命名其中一個 (s、t、r、q) 材質座標，而且必須是下列其中一個符號： GL \_ s、GL \_ t、gl \_ r 或 gl \_ q。 *Pname* 參數必須是三個符號常數的其中一種： gl \_ 紋理內 \_ 代 \_ 模式、gl \_ 物件 \_ 平面或 gl \_ 眼 \_ 平面。 如果 *pname* 是 gl \_ 材質 \_ 產生 \_ 模式，則 *param* 會指定一個模式，也就是 gl \_ 物件 \_ 線性、gl \_ 眼 \_ 線性或 gl \_ 球體 \_ 地圖。 如果 *pname* 是 gl \_ 物件 \_ 平面或 gl \_ 眼 \_ 平面， *param* 就會包含對應材質產生函數的係數。
...
*Param* 中的值會以眼睛座標定義參考平面。 套用至這些專案的模型矩陣，在轉換多邊形頂點時，不能是同一個效果。 此函式會建立可在移動物件上產生動態輪廓線的材質座標欄位。
...
您可以使用 [**glEnable**](glenable.md) 或 [**glDisable**](gldisable.md) 搭配其中一個符號材質座標名稱 (GL \_ 材質 \_ gen \_ 、gl \_ 材質 \_ gen \_ T、GL \_ 材質 \_ gen \_ R 或 gl \_ 材質 \_ gen \_ Q) 作為引數，以啟用或停用材質座標產生函數。 啟用此函式時，會根據與該座標相關聯的產生函數來計算指定的材質座標。 當停用函式時，後續頂點會從目前的材質座標集合取得指定的材質座標。 一開始，所有材質產生函數都會設定為 GL \_ 眼 \_ 線性，並停用。 這兩個平面方能 (1、0、0、0) ;這兩個 t 平面方能 (0、1、0、0) ;和所有 r 和 q 平面方能 (0、0、0、0) 。
