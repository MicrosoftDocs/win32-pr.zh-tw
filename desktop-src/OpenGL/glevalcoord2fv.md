...
**GlEvalCoord2fv** 函式會使用兩個定義域值（ *u* 和 *v*）來評估啟用的二維地圖。 使用 [**glMap1**](glmap1.md)定義對應。 使用 [**glEnable**](glenable.md) 和 [**glDisable**](gldisable.md)來啟用或停用它們。
...
如果啟用自動一般產生， **glEvalCoord2fv** 會使用引數 GL AUTO normal 來呼叫 [**glEnable**](glenable.md) ， \_ \_ 以產生介面法線 ANALYTICALLY，不論內容或啟用 GL \_ list.map2 \_ 一般地圖。 Let
...
下列函式會取出與 **glEvalCoord2fv** 函數相關的資訊：
