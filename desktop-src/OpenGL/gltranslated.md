...
---
# <a name="gltranslated-function"></a>glTranslated 函式

**GlTranslated** 函式會將目前的矩陣與轉譯矩陣相乘。
...
**GlTranslated** 函式會產生由 (*x*、 *y*、 *z*) 指定的平移。 轉譯向量用來計算4x4 平移矩陣：
...
如果矩陣模式是 GL \_ 模型或 gl \_ 投射，則會轉譯 **glTranslated** 之後繪製的所有物件。 使用 [**glPushMatrix**](glpushmatrix.md) 和 **glPopMatrix** 來儲存及還原未轉譯的座標系統。

下列函式會取出與 [**glTranslated**](gltranslate.md)相關的資訊：
