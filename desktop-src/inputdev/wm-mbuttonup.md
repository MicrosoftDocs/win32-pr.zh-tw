...
ms.locfileid: "119451538"
---
# <a name="wm_mbuttonup-message"></a>WM \_ MBUTTONUP 訊息

當使用者放開滑鼠左鍵，而游標位於視窗的工作區時張貼。 如果未捕捉到滑鼠，則會將訊息張貼到游標下的視窗。 否則，會將訊息張貼到已捕捉滑鼠的視窗。
...
請注意，當快捷方式功能表出現時 (顯示) ，座標是相對於畫面，而不是工作區。 由於 [**trackpopupmenu 讓**](/windows/desktop/api/winuser/nf-winuser-trackpopupmenu) 是非同步呼叫，而 **WM \_ MBUTTONUP** 通知沒有指出座標衍生的特殊旗標，因此應用程式無法分辨 *lParam* 中所包含的 x、y 座標是相對於螢幕或工作區。
