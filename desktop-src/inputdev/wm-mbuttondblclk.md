...
---
# <a name="wm_mbuttondblclk-message"></a>WM \_ MBUTTONDBLCLK 訊息

當使用者按兩下滑鼠左鍵，而游標位於視窗的工作區時張貼。 如果未捕捉到滑鼠，則會將訊息張貼到游標下的視窗。 否則，會將訊息張貼到已捕捉滑鼠的視窗。
...
只有具有 **CS \_ DBLCLKS** 樣式的視窗可以接收 **WM \_ MBUTTONDBLCLK** 訊息，系統會在使用者按下、放開，然後在系統的按兩下時間限制內再次按下滑鼠按鍵時產生此訊息。 按兩下中間的滑鼠按鍵實際上會產生四個訊息： [**wm \_ MBUTTONDOWN**](wm-mbuttondown.md)、 [**wm \_ MBUTTONUP**](wm-mbuttonup.md)、 **wm \_ MBUTTONDBLCLK** 和 **wm \_ MBUTTONUP** 。
