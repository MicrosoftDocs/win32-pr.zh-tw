...
當使用者放開滑鼠左鍵，而游標位於視窗的非工作區域內時，即會公佈。 這則訊息會張貼至包含資料指標的視窗。 如果視窗已捕捉到滑鼠，則不會張貼此訊息。
...
[**DefWindowProc**](/windows/desktop/api/winuser/nf-winuser-defwindowproca)函式會測試指定的點，以找出資料指標的位置，並執行適當的動作。 如果有的話， **DefWindowProc** 會將 [**WM \_ SYSCOMMAND**](/windows/desktop/menurc/wm-syscommand) 訊息傳送至視窗。
