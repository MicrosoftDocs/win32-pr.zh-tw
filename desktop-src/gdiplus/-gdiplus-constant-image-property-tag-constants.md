...
ms.locfileid: "117696416"
---
# <a name="image-property-tag-constants"></a>影像屬性標記常數

有數種影像檔案格式可讓您儲存中繼資料和影像。 中繼資料是關於影像的資訊，例如標題、寬度、相機型號和演出者。 Windows GDI+ 提供統一的方式，可從影像檔案以標記的影像檔案格式儲存和取出中繼資料 (TIFF) 、JPEG、便攜網狀圖形 (PNG) ，以及交換影像檔案 (EXIF) 格式。
...
下列主題列出並描述 GDI+ 所支援的屬性專案。 描述很簡單且一般，因此適用于各種不同的檔案格式。 如需有關如何在特定檔案格式中使用屬性專案的詳細說明，請參閱該檔案格式的規格。 如需多個檔案格式規格的連結，請參閱 [影像檔案格式規格](-gdiplus-constant-image-file-format-specifications.md)。 如需中繼資料的詳細資訊，請參閱 [讀取和寫入中繼資料](-gdiplus-reading-and-writing-metadata-use.md) 和 [**影像屬性標記類型常數**](-gdiplus-constant-image-property-tag-type-constants.md)。
