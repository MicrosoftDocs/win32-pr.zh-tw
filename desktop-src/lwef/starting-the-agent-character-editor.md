...
![顯示 [字元-Microsoft 代理程式字元編輯器] 之 [屬性] 頁面的螢幕擷取畫面。](images/f1chared.gif)

若要從鍵盤存取視窗中的欄位，您可以使用 TAB 鍵和 SHIFT + TAB 在控制項之間流覽，或使用 (ALT + 加底線的 *字母*) 的存取鍵移至特定的控制項。 當編輯器完成啟動之後，您就可以開始建立新的字元定義或載入現有的字元定義。

當您將指標移至命令列上方時，狀態列會顯示命令或工具列按鈕的相關資訊。 當您建立一個字元時，它也會顯示有關字元動畫資料和狀態資訊的摘要資訊。
