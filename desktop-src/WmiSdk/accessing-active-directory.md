...
Active Directory 是 Windows 的目錄服務，是 Windows 分散式網路的基礎。 您可以使用 Active Directory 來找出電腦網路網域中的物件，例如使用者、安全性原則、印表機、分散式元件及其他資源。 名稱 Active Directory 指的是目錄服務和目錄本身。
...
Windows 透過建立一組包含在 Active Directory 中的類別和物件的參考，讓 Active Directory 可透過 WMI 存取。 藉由透過 WMI 存取目錄服務提供者，您可以建立啟用 WMI 的應用程式，以存取 Active Directory 中包含的豐富資訊。
