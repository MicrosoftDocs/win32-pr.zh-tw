...
腳本可以存取硬體和軟體物件的所有 WMI 類別。 Windows Script Host (WSH) 腳本可以對檔案系統物件執行作業、操作網路印表機，或變更環境變數。 您可以找到各種系統管理員工作，以及如何在 WMI 中完成 [腳本和應用程式之 wmi](wmi-tasks-for-scripts-and-applications.md)工作的簡單指引。 如需詳細資訊和範例，請參閱 TechNet ScriptCenter [腳本存放庫](https://www.microsoft.com/technet/scriptcenter/scripts/default.mspx)。
