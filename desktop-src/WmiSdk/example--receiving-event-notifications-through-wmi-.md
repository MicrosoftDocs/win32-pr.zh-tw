...
您可以使用本主題中的程式和程式碼範例，完成執行 COM 初始化的 WMI 用戶端應用程式、連接到本機電腦上的 WMI、接收事件通知，然後清除。 當建立新的進程時，此範例會通知使用者事件。 以非同步方式接收事件。

下列程式是用來執行 WMI 應用程式。 步驟1到5包含設定和連線至 WMI 所需的所有步驟，而步驟6是接收事件通知的位置。
...
6.  使用 [**IWbemServices**](/windows/desktop/api/WbemCli/nn-wbemcli-iwbemservices) 指標對 WMI 提出要求。 這個範例會使用 [**IWbemServices：： ExecNotificationQueryAsync**](/windows/desktop/api/WbemCli/nf-wbemcli-iwbemservices-execnotificationqueryasync) 方法來接收非同步事件。 當您收到非同步事件時，您必須提供 [**IWbemObjectSink**](iwbemobjectsink.md)的執行。 這個範例會提供 EventSink 類別中的實作為。 此類別的執行程式碼和標頭檔碼會在主要範例底下提供。 每當接收到事件時， **IWbemServices：： ExecNotificationQueryAsync** 方法會呼叫 **EventSink：：指示** 方法。 在此範例中，每當建立進程時，就會呼叫 **EventSink：：表示** 方法。 若要測試此範例，請執行程式碼，並啟動 Notepad.exe 之類的進程。 這會觸發事件通知。

    如需提出 WMI 要求的詳細資訊，請參閱 [操作類別和實例資訊](manipulating-class-and-instance-information.md) 和 [呼叫方法](calling-a-method.md)。

下列範例程式碼會透過 WMI 接收事件通知。
...
下列標頭檔用於類別 EventSink。 EventSink 類別是在先前的程式碼範例中使用。
