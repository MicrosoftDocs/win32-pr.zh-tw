...
---
# <a name="swbemobjectcompareto_-method"></a>SWbemObject. CompareTo \_ 方法

[**SWbemObject**](swbemobject.md)物件的 **CompareTo \_** 方法會比較兩個 **SWbemObject** 物件。 根據 *iFlags* 參數中指定的值，這項比較會受限於某些條件約束。
...
必要。 此參數是 [**SWbemObject**](swbemobject.md) 物件。 這是用來比較第一個物件的物件。 物件必須是有效的 **SWbemObject** 實例。
...
指定比較物件與其他物件時，所要考慮的物件特性。 您可以使用 **wbemComparisonFlagIncludeAll** 來考慮所有功能 (這是預設) 或下列值的任何組合。
...
會忽略辨識符號的類別。 此旗標會將限定詞值納入考慮，但會忽略類別差異，例如傳播規則和覆寫限制。
...
以不區分大小寫的方式來比較字串值。 這同時適用于字串和限定詞值。 不論這個旗標是否指定，屬性和限定詞名稱都是以區分大小寫的方式比較。
