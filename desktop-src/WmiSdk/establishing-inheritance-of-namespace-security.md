...
請注意，預設許可權無法變更。 此外，在設定安全描述項 (SD) 時設定 **SE \_ DACL \_ 保護** 旗標，並不會用來將特製化 sd 新增至子命名空間。 若要覆寫繼承的 SD，請只設定一個新的 SD。 若要將該 SD 繼承至子命名空間，請在命名空間安全描述項中傳遞 **容器 \_ inherit \_ ACE** 旗標。 若要只繼承子系，而不是子代，請傳遞 `CONTAINER_INHERIT_ACE|NO_PROPOGATE_INHERIT_ACE`
