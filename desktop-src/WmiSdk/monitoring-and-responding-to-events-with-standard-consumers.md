...
下列範例顯示 [**NTEventLogEventConsumer**](nteventlogeventconsumer.md)實例的 MOF。 在您編譯此 MOF 之後，任何嘗試在登錄路徑中建立、刪除或修改值的 **HKEY \_ 本機 \_ 電腦 \\ 軟體 \\ Microsoft \\ Windows \\ CurrentVersion \\ Run** 會在應用程式 eventlog 的來源「WSH」下記錄一個專案。
