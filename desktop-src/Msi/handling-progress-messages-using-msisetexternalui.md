...
下列範例示範如何編碼簡單的回呼處理常式，以便在安裝期間接收 Windows Installer 進度訊息。

> [!Note]  
> 使用 [**MsiSetExternalUI**](/windows/desktop/api/Msi/nf-msi-msisetexternaluia) 搭配 INSTALLMESSAGE FILESINUSE 的訊息類型時 \_ ，傳送至外部 UI 處理常式函式的訊息不會包含使用中的檔案或 [FILESINUSE](filesinuse-dialog.md) 對話方塊所使用之視窗標題的任何資訊。 您應使用 [**MsiSetExternalUIRecord**](/windows/desktop/api/Msi/nf-msi-msisetexternaluirecord) 來取得資訊。
