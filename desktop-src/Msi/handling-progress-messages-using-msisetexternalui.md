---
description: 下列範例示範如何編碼簡單的回呼處理常式，以便在安裝期間接收 Windows Installer 進度訊息。
...
---
# <a name="handling-progress-messages-using-msisetexternalui"></a><span data-ttu-id="56bda-103">使用 MsiSetExternalUI 處理進度訊息</span><span class="sxs-lookup"><span data-stu-id="56bda-103">Handling Progress Messages Using MsiSetExternalUI</span></span>

<span data-ttu-id="56bda-104">下列範例示範如何編碼簡單的回呼處理常式，以便在安裝期間接收 Windows Installer 進度訊息。</span><span class="sxs-lookup"><span data-stu-id="56bda-104">The following sample demonstrates how to code a simple callback handler to receive Windows Installer progress messages during an installation.</span></span>

> [!Note]  
> <span data-ttu-id="56bda-105">使用 [**MsiSetExternalUI**](/windows/desktop/api/Msi/nf-msi-msisetexternaluia) 搭配 INSTALLMESSAGE FILESINUSE 的訊息類型時 \_ ，傳送至外部 UI 處理常式函式的訊息不會包含使用中的檔案或 [FILESINUSE](filesinuse-dialog.md) 對話方塊所使用之視窗標題的任何資訊。</span><span class="sxs-lookup"><span data-stu-id="56bda-105">When using [**MsiSetExternalUI**](/windows/desktop/api/Msi/nf-msi-msisetexternaluia) with a message type of INSTALLMESSAGE\_FILESINUSE, the message sent to the external UI handler function does not contain any information about files in use or window titles used by the [FilesInUse](filesinuse-dialog.md) dialog box.</span></span> <span data-ttu-id="56bda-106">您應使用 [**MsiSetExternalUIRecord**](/windows/desktop/api/Msi/nf-msi-msisetexternaluirecord) 來取得資訊。</span><span class="sxs-lookup"><span data-stu-id="56bda-106">You should use [**MsiSetExternalUIRecord**](/windows/desktop/api/Msi/nf-msi-msisetexternaluirecord) to obtain information.</span></span>
