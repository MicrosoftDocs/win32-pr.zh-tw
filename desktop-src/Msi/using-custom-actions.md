---
description: 下列各節說明如何使用自訂動作。
ms.assetid: dd2a0681-f50d-4232-bdcc-8aee6bb121a1
title: 使用自訂動作
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 5be1937adbf64b94667fc3e7c44d82843b561dfb
ms.sourcegitcommit: 831e8f3db78ab820e1710cede244553c70e50500
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 01/07/2021
ms.locfileid: "104191692"
---
# <a name="using-custom-actions"></a>使用自訂動作

下列各節說明如何使用自訂動作。

-   [叫用自訂動作](invoking-custom-actions.md)
-   [排序自訂動作](sequencing-custom-actions.md)
-   [取得順延強制自訂動作的內容資訊](obtaining-context-information-for-deferred-execution-custom-actions.md)
-   [將自訂動作新增至 ProgressBar](adding-custom-actions-to-the-progressbar.md)
-   [自訂動作的調試](debugging-custom-actions.md)
-   [從自訂動作判斷 UI 層級](determining-ui-level-from-a-custom-action.md)
-   [卸載自訂動作](uninstalling-custom-actions.md)
-   [從自訂動作傳回錯誤訊息](returning-error-messages-from-custom-actions.md)
-   [設定自訂動作的還原點](setting-a-restore-point-from-a-custom-action.md)
-   [未在自訂動作中使用的函數](functions-not-for-use-in-custom-actions.md)
-   [使用自訂動作變更系統狀態](changing-the-system-state-using-a-custom-action.md)
-   [從自訂動作內部存取目前的安裝程式會話](accessing-the-current-installer-session-from-inside-a-custom-action.md)
-   [從自訂動作內部存取資料庫或會話](accessing-a-database-or-session-from-inside-a-custom-action.md)
-   [使用自訂動作在安裝結束時啟動已安裝的檔案](using-a-custom-action-to-launch-an-installed-file-at-the-end-of-the-installation.md)
-   [使用自訂動作在本機電腦上建立使用者帳戶](using-a-custom-action-to-create-user-accounts-on-a-local-computer.md)
-   [使用64位自訂動作](using-64-bit-custom-actions.md)

如需自訂動作的總覽，請參閱 [關於自訂動作](about-custom-actions.md)。

如需在 [CustomAction 資料表](customaction-table.md)中將自訂動作編碼的詳細資訊，請參閱 [自訂動作參考](custom-action-reference.md)。

如需自訂動作的摘要，以及它們如何編碼至 CustomAction 資料表中，請參閱 [所有自訂動作類型的摘要清單](summary-list-of-all-custom-action-types.md)。

 

 



