...
安裝套件包含安裝或卸載應用程式或產品，以及執行安裝程式使用者介面時，Windows Installer 所需的所有資訊。 每個安裝套件都包含一個 .msi 檔案，其中包含安裝資料庫、摘要資訊資料流程，以及安裝的各部分的資料流程。 .Msi 檔案也可以包含一或多個轉換、內部原始程式檔，以及安裝所需的外部原始程式檔或封包檔。
