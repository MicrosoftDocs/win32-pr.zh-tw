...
此動作會參考 [IsolatedComponent 資料表](isolatedcomponent-table.md) 的每一筆記錄，並將 [元件共用] 欄位中所列元件的檔案 \_ 與 [元件應用程式] 欄位中所列的元件產生關聯 \_ 。 安裝程式會將共用的元件檔案安裝 \_ 到與元件應用程式相同的目錄中 \_ 。 安裝程式會在此目錄中產生檔案（長度為零個位元組），為元件應用程式使用金鑰檔的簡短檔案名， \_ (通常是與 .exe) 附加至本機的相同檔案名。 IsolatedComponent 動作不會影響元件應用程式的安裝 \_ 。 卸載元件 \_ 應用程式也會 \_ 從目錄中移除元件共用檔案和本機檔案。
