...
Windows Installer 3.1 適用于 Windows Server 2003、Windows XP 或 Windows 2000。 如需所有 Windows Installer 版本和可轉散發套件的清單，請參閱 [Windows Installer 的發行版本](released-versions-of-windows-installer.md)。
...
Windows Installer 服務必須在 Windows Vista 上執行，才能使用 [重新開機管理員](../rstmgr/restart-manager-portal.md)、 [*使用者帳戶控制*](u-gly.md)，以及 [ (UAC) 修補的使用者帳戶控制](user-account-control--uac--patching.md)。 如需詳細資訊，請參閱搭配 [使用 Windows Installer 與重新開機管理員](using-windows-installer-with-restart-manager.md) ，以及 [使用 Windows Installer 搭配 Uac](using-windows-installer-with-uac.md) 和 [使用者帳戶控制 (uac) 修補](user-account-control--uac--patching.md)。

Windows Installer 3.1 支援 Windows 檔案保護 (WFP) ，且不支援 Windows 資源保護 (WRP) 。 Windows Server 2008 和 Windows Vista 中的 WRP 取代了 Windows Server 2003、Windows XP 及 Windows 2000 中的 WFP。 如需 Windows Installer 和 WFP 的詳細資訊，請參閱 [使用 Windows Installer 和 Windows 資源保護](windows-resource-protection-on-windows-vista.md)。
