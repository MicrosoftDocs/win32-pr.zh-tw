...
您將需要 CScript.exe 或 WScript.exe 版本的 Windows Script Host 才能使用此範例。 若要使用 CScript.exe 執行此範例，請在命令提示字元中使用下列語法。
...
指定要建立或接收匯入之資料表的 Windows installer 資料庫路徑。 指定包含要匯入之資料表保存檔案的資料夾路徑。 列出正在匯入的封存檔案名稱。 萬用字元檔案名（例如 \* idt）可以用來匯入多個檔案。
