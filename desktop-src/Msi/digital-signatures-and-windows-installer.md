...
使用 Windows Installer，數位簽章可與 Windows Installer 套件、轉換、修補程式、合併模組和外部封包檔一起使用。 Windows Installer 已與 Microsoft Windows XP 上的軟體限制原則整合。 您可以建立原則，以根據不同的準則（包括特定的簽署者憑證或發行者）來允許或防止安裝。 Windows Installer 可以在安裝 CryptoAPI 2.0 版的所有平臺上執行外部封包檔的簽章驗證。
