---
description: 如果限制可傳送至伺服器端的公用屬性，RestrictedUserControl 屬性應該設定為1。
ms.assetid: 55b4545c-c9ab-49c6-b22f-6cd93b5fb8c1
title: RestrictedUserControl 屬性
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: 29158c485592a35d2dcc441fd048ee4ef8f321e57bdc6b8c524846bdb87a62e5
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119979278"
---
# <a name="restrictedusercontrol-property"></a>RestrictedUserControl 屬性

如果限制可傳送至伺服器端的 [公用屬性](public-properties.md)， **RestrictedUserControl** 屬性應該設定為1。 作者可以在條件陳述式中使用這個屬性，讓對話方塊或動作的執行取決於公用屬性是否有限制。

## <a name="requirements"></a>規格需求



| 需求 | 值 |
|--------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 版本<br/> | WindowsWindows Server 2012、Windows 8 Windows Server 2008 R2 或 Windows 7 上的安裝程式5.0。 WindowsWindows Server 2008 或 Windows Vista 上的安裝程式4.0 或 Windows Installer 4.5。 WindowsWindows Server 2003 或 Windows XP 上的安裝程式。 如需 Windows Installer 版本所需的最低 Windows service pack 相關資訊，請參閱[Windows Installer Run-Time 需求](windows-installer-portal.md)。<br/> |



## <a name="see-also"></a>另請參閱

<dl> <dt>

[屬性](properties.md)
</dt> <dt>

[受限制的公用屬性](restricted-public-properties.md)
</dt> </dl>

 

 




