...
---
# <a name="custom-action-type-39"></a>自訂動作類型39

自訂動作類型39用於並行安裝。 不建議將並行安裝用於安裝應用程式，以供公開發行。 如需並行安裝的詳細資訊，請參閱 [並行安裝](concurrent-installations.md)。

類型39自訂動作會安裝已公告或已安裝的應用程式。 您可以使用這個自訂動作類型，重新安裝或移除目前產品安裝套件已安裝為並行安裝的產品。 類型39自訂動作無法用來重新安裝或移除任何其他方法先前安裝的任何產品。 例如，如果在主要產品安裝期間使用類型39、類型23或類型7自訂動作安裝次要產品，則在卸載主要產品時，可能會使用類型39自訂動作來移除次要產品。

## <a name="source"></a>來源

[CustomAction 資料表](customaction-table.md)的 [來源] 欄位包含應用程式的產品代碼。
...
[CustomAction 資料表](customaction-table.md)的 [目標] 欄位包含要傳遞至並行安裝的屬性設定。 這些屬性設定可以指定功能。

## <a name="return-processing-options"></a>傳回處理選項

如果未公告或未安裝應用程式，自訂動作類型39會失敗。 若要避免這個錯誤，您必須設定 **msidbCustomActionTypeContinueflag**。
...
從並行安裝的使用者結束、失敗、暫停或成功傳回狀態的處理方式，與其他任何動作的處理方式相同。 不過請注意，Windows Installer 會在將傳回值寫入記錄檔時，轉譯所有動作的傳回值。 例如，如果動作傳回值在記錄檔中顯示為1，這表示動作傳回的錯誤 \_ 成功。 如需詳細資訊，請參閱 [動作傳回值的記錄](logging-of-action-return-values.md)。
