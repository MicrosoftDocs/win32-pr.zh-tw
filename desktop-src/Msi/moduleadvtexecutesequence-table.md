...
合併工具會評估 ModuleAdvtExecuteSequence 資料表，然後將計算的動作以正確的序號插入 [AdvtExecuteSequence 資料表](advtexecutesequence-table.md) 中。

ModuleAdvtExecuteSequence 資料表具有下列資料行。
...
如果這個資料表存在，則 [AdvtExecuteSequence 資料表](advtexecutesequence-table.md) 也必須存在於合併模組中。
