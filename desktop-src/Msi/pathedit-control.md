...
PathEdit 控制項會顯示一個編輯欄位，可讓使用者選取路徑的結尾區段。 此控制項支援在編輯欄位中輸入選取的資料夾名稱或整個路徑。 使用者也可以在沒有磁碟機號的磁片磁碟機上輸入通用命名慣例 (UNC) 路徑。 如果使用者為目前的磁片區輸入無效路徑的結束區段，PathEdit 控制項就無法將焦點轉移到下一個控制項。

PathEdit 控制項、 [DirectoryCombo](directorycombo-control.md)和 [DirectoryList](directorylist-control.md) 控制項都與相同的字串值屬性相關聯。 該屬性是使用者選取的路徑。 在 [控制資料表](control-table.md)的屬性資料行中輸入屬性的名稱。 這個屬性的初始值必須包含至少一個磁片區和一個子層次。 在 [屬性資料表](property-table.md)的 [值] 資料行中，指定屬性的初始值。

此控制項的設計目的是要用於 [流覽對話方塊](browse-dialog.md) ，以及 PathEdit 和 [DirectoryList](directorylist-control.md) 控制項。
...
PathEdit 控制項是衍生自 [編輯](edit-control.md) 控制項。

為了與螢幕閱讀程式相容，撰寫具有 PathEdit 控制項的對話方塊做為第一個使用中的控制項時，您必須將 [編輯] 欄位中的文字欄位設為 [對話方塊資料表](dialog-table.md)中的第一個使用中控制項。 由於靜態文字無法取得焦點，因此當您建立對話方塊時，[編輯] 欄位一開始會是預期的焦點;這可確保螢幕讀取器會顯示正確的資訊。
