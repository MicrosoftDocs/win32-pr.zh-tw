---
description: 本節適用于撰寫自己的安裝程式的開發人員，以及想要深入瞭解 Windows Installer 資料庫資料表的開發人員。
ms.assetid: 72493b40-04e0-4fe0-9ec7-542c63036235
title: Automation 介面
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 2817a3cc6740a9aebef3e9696fe00ea37a18dd1bdf45f8f1f4f3b329672c9d5b
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119145781"
---
# <a name="automation-interface"></a>Automation 介面

本節適用于撰寫自己的安裝程式的開發人員，以及想要深入瞭解 Windows Installer 資料庫資料表的開發人員。

如需自動化和 c + + 程式庫存取的相關資訊，請參閱 [關於 Automation 介面](about-the-automation-interface.md)。

如需使用 automation 和建立 [**安裝程式物件**](installer-object.md)的詳細資訊，請參閱 [使用 automation 介面](using-the-automation-interface.md)。

如需安裝程式物件的參考資料，請參閱 [Automation 介面參考](automation-interface-reference.md)。

如需 wmi 的詳細資訊，請參閱搭配[使用 Windows Installer 與 wmi](using-windows-installer-with-wmi.md)。

 

 



