...
使用資料表編輯器（例如 Orca 或 SQL 查詢），修改 MNPFren.msi 資料庫中任何其他可當地語系化的資料行。 若要判斷特定資料表的哪些資料行可以當地語系化成另一種語言，請參閱該資料庫資料表的參考主題。 請參閱 [資料庫資料表](database-tables.md) ，以取得所有資料庫資料表的清單。
...
[控制資料表](control-table.md) (原始 .msi 檔案的部分) 
...
您可以使用資料表編輯器來修改文字欄位，例如 SDK 所提供的 Orca 資料表編輯器或其他資料表編輯器，或使用 SQL 查詢來變更控制資料表記錄的文字欄位。 在 Windows Installer SDK 中，會以公用程式 WiRunSQL.vbs 的形式提供示範腳本驅動資料庫查詢的範例。 使用下列命令列，以 WiRunSQL.vbs 和 Windows Script Host 來修改欄位。 另請參閱 [使用 SQL 和腳本的資料庫查詢範例](examples-of-database-queries-using-sql-and-script.md)。
