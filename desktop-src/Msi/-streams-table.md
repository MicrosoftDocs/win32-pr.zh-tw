...
若要讀取內嵌于資料庫中的二進位資料串流，請使用 SQL 查詢來尋找並提取包含二進位資料的記錄。 使用 [**MsiRecordReadStream**](/windows/desktop/api/Msiquery/nf-msiquery-msirecordreadstream) 將二進位資料讀入緩衝區。
...
如果使用 SQL (ALTER TABLE 將保留放在此資料表上 <table> 保留) 或加入具有保留的資料行，必須使用免費發行資料表。 在資料表發行或認可之前，不會寫入資料流程。
