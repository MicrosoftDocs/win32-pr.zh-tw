...
Windows Installer 可以使用檔案雜湊來偵測和消除不必要的檔案複製。 儲存在 **MsiFileHash** 資料表中的檔案雜湊，可能會與使用者電腦上呼叫 [**MsiGetFileHash**](/windows/desktop/api/Msi/nf-msi-msigetfilehasha)所取得的現有檔案雜湊進行比較。 **MsiFileHash** 資料表只能搭配未建立版本檔使用。
