---
description: 封裝作者可以透過建立可執行應用程式（其中包含以記錄為基礎的回呼處理常式）來監視內部 Windows Installer 訊息，以接收訊息和功能來起始安裝。
...
---
# <a name="monitoring-an-installation-using-msisetexternaluirecord"></a><span data-ttu-id="a8d34-103">使用 MsiSetExternalUIRecord 監視安裝</span><span class="sxs-lookup"><span data-stu-id="a8d34-103">Monitoring an Installation Using MsiSetExternalUIRecord</span></span>

<span data-ttu-id="a8d34-104">封裝作者可以透過建立可執行應用程式（其中包含以記錄為基礎的回呼處理常式）來監視內部 Windows Installer 訊息，以接收訊息和功能來起始安裝。</span><span class="sxs-lookup"><span data-stu-id="a8d34-104">Package authors can monitor internal Windows Installer messages through the creation of an executable application that contains both a record-based callback handler to receive the messages and functionality to initiate an installation.</span></span>

<span data-ttu-id="a8d34-105">下列範例中以記錄為基礎的處理常式會符合 [**INSTALLUI \_ 處理常式 \_ 記錄**](/windows/win32/api/msi/nc-msi-installui_handler_record) 原型，而此回呼處理常式的指標會傳遞至 [**MsiSetExternalUIRecord**](/windows/desktop/api/Msi/nf-msi-msisetexternaluirecord) 函數。</span><span class="sxs-lookup"><span data-stu-id="a8d34-105">The record-based handler in the following example conforms to the [**INSTALLUI\_HANDLER\_RECORD**](/windows/win32/api/msi/nc-msi-installui_handler_record) prototype, and a pointer to this callback handler is passed to [**MsiSetExternalUIRecord**](/windows/desktop/api/Msi/nf-msi-msisetexternaluirecord) function.</span></span>
