...
封裝作者可以透過建立可執行應用程式（其中包含以記錄為基礎的回呼處理常式）來監視內部 Windows Installer 訊息，以接收訊息和功能來起始安裝。

下列範例中以記錄為基礎的處理常式會符合 [**INSTALLUI \_ 處理常式 \_ 記錄**](/windows/win32/api/msi/nc-msi-installui_handler_record) 原型，而此回呼處理常式的指標會傳遞至 [**MsiSetExternalUIRecord**](/windows/desktop/api/Msi/nf-msi-msisetexternaluirecord) 函數。
