...
撰寫適當的 Ices-003 進行驗證之後，開發人員必須將自訂動作一起收集到一個 .msi 資料庫（稱為 .cub 檔），其中只包含等位和其所需的資料表。 無法安裝 .cub 檔案，而且只能用來儲存和提供 ICE 自訂動作的存取權。 如需建立 .cub 檔案的詳細資訊，請參閱 [建立 ICE 資料庫](building-an-ice-database.md)。 或者，開發人員可以使用 [ICE 參考](ice-reference.md)中所述的現有內容來驗證其安裝套件。 您可以從 SDK 隨附的標準 .cub 檔案取得這些等。
