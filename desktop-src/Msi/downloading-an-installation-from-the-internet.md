---
description: Windows安裝程式會接受統一資源定位器 (URL) 作為安裝的有效來源。
...
ms.locfileid: "118637765"
---
# <a name="downloading-an-installation-from-the-internet"></a>從網際網路下載安裝

Windows安裝程式會接受統一資源定位器 (URL) 作為安裝的有效來源。 Windows安裝程式可以從 URL 位置安裝套件、修補程式和轉換。
...
Windows安裝程式2.0 版可以使用 HTTP、FILE 和 FTP 通訊協定，而且無法使用 HTTPS 和 GOPHER 通訊協定。
