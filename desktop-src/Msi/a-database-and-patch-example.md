---
description: 應用程式可以使用 MsiOpenDatabase 函式來開啟新的或現有的安裝資料庫 (.msi 檔案) 或修補套件 ( .msp 檔案。 ) 應用程式會先檢查 MsiOpenDatabase 的傳回值，再使用資料庫控制碼。
...
ms.locfileid: "119997028"
---
# <a name="a-database-and-patch-example"></a>資料庫和修補範例

應用程式可以使用 [**MsiOpenDatabase**](/windows/desktop/api/Msiquery/nf-msiquery-msiopendatabasea) 函式來開啟新的或現有的安裝資料庫 (.msi 檔案) 或修補套件 ( .msp 檔案。 ) 應用程式會先檢查 **MsiOpenDatabase** 的傳回值，再使用資料庫控制碼。
