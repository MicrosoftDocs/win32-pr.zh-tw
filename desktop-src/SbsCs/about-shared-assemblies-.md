...
在 Windows XP 之前，共用的元件是全域註冊的，並且安裝在 Windows 系統資料夾中。 在此情況下，元件的最新安裝版本可供任何系結至該元件的應用程式使用。 並存元件也可以安裝為私用元件，以用於應用程式的專屬用途。 如需詳細資訊，請參閱 [私用組件](/windows/desktop/Msi/private-assemblies)。
