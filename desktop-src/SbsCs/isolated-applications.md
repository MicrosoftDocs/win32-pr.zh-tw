...
可能無法將每個現有的應用程式更新為完全隔離的應用程式。 例如，某些[Windows 檔案保護 (WFP) ](/windows/desktop/Wfp/windows-resource-protection-portal)系統元件無法作為並存元件使用，且無法與應用程式一起安裝為私用元件。 您可以針對應用程式資訊清單中的部分應用程式元件指定並存元件相依性，以部分隔離這類應用程式。
