...
請注意，由於啟用內容是個別執行緒的，COM 會將建立執行緒的啟用內容封送處理到主執行緒，並在主執行緒上呼叫 [**LoadLibrary**](/windows/desktop/api/libloaderapi/nf-libloaderapi-loadlibrarya) 或 [**DllGetClassObject**](/windows/win32/api/combaseapi/nf-combaseapi-dllgetclassobject) 之前啟動它。 這項功能已經存在 Windows 中，用戶端程式代碼不需要執行任何動作來執行此作業。
