...
請注意，如果您 \_ 已定義啟用隔離感知的編譯 \_ ，您也不能自行進行啟用內容管理。 \_在定義隔離感知的情況 \_ 下，Windows 會忽略您的應用程式可能會嘗試在 [**ActivateActCtx**](/windows/desktop/api/Winbase/nf-winbase-activateactctx)和 [**CreateWindow**](/windows/win32/api/winuser/nf-winuser-createwindowa)呼叫之間進行的任何動態啟用內容建立。 這表示當您的應用程式使用隔離 \_ 感知功能時， \_ 您必須確定資訊清單包含元件所需之所有元件的完整清單。
