...
燒錄程式期間的電源轉換也可能會在燒錄活動成功時產生潛在問題。 在燒錄程式中避免這些複雜性，需要應用程式知道何時即將進行電源轉換。 這是藉由讓應用程式處理 [**WM \_ POWERBROADCAST**](/windows/desktop/Power/wm-powerbroadcast) 訊息來完成。 針對 Windows XP 或 Windows Server 2003 所開發的應用程式可能會傳回「**廣播 \_ 查詢 \_ 拒絕**」以回應 [**PBT \_ APMQUERYSUSPEND**](/windows/desktop/Power/pbt-apmquerysuspend)，以防止在燒錄過程中暫停。
