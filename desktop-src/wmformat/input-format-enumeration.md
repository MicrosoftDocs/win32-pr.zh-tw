...
寫入器物件會從您提供的設定檔取得串流格式資訊。 設定檔中的串流設定資訊可為寫入器提供在檔案中寫入資料所需的所有資訊。 設定檔不會提供有關您的應用程式所提供之輸入範例格式的任何資訊給寫入器。 只有使用其中一種 Windows Media 編解碼器壓縮的資料流程才會知道輸入格式：您可以根據設定檔中的資訊來預測任意資料流程類型的輸入。
