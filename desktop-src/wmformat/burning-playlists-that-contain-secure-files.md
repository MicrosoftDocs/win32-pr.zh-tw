...
keywords:
- Windows媒體格式 SDK，使用安全檔案燒錄播放清單
- Windows媒體格式 SDK，包含安全檔案的播放清單
...
ms.locfileid: "119028116"
---
# <a name="burning-playlists-that-contain-secure-files"></a>燒錄包含安全檔案的播放清單

使用 Windows 媒體版權管理員 10 SDK 的物件建立的授權可以指定將檔案複製到光碟作為播放清單一部分的許可權。 這項功能稱為播放清單燒錄，需要您先確認播放清單中所有檔案的授權，再開始複製資料。 Windows 媒體格式 SDK 會提供 [**IWMReaderPlaylistBurn**](/previous-versions/windows/desktop/api/wmsdkidl/nn-wmsdkidl-iwmreaderplaylistburn)介面，為您執行檔案驗證。
