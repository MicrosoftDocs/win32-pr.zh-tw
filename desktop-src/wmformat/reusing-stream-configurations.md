...
-   Windows Media 音訊編解碼器的數個版本使用相同的串流設定。 如果您有設定為子類型 WMMEDIASUBTYPE \_ WMAudioV2、WMMEDIASUBTYPE \_ WMAUDIOV7 或 WMMEDIASUBTYPE WMAudioV8 的串流設定物件 \_ ，則會使用最新的 Windows Media 音訊編解碼器來壓縮產生的串流。 不過，在使用現有的音訊編解碼器之前，您應該先評估您的需求。 升級至最新版的 Windows Media 音訊 Professional 編解碼器或 Windows Media 音訊的無失真編解碼器，可以改善許多類型的檔案。
