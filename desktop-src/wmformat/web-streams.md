...
建議使用 Windows 媒體格式9系列 SDK 建立的 Web 串流，或稍後為版本號碼1提供。 這個值是在 **wVersion** 成員的 [**WMT \_ WEBSTREAM \_ 格式**](/previous-versions/windows/desktop/api/Wmsdkidl/ns-wmsdkidl-wmt_webstream_format)結構中指定的。 SDK 本身不會執行任何動作來強制執行此版本。
