...
Windows 媒體格式 SDK 中會定義許多屬性，以處理最常見的中繼資料需求。 此外，您還可以建立自己的屬性。 不過，您應該在命名自訂屬性時小心，因為其他應用程式開發人員可以使用相同的名稱，而且可能會發生衝突。
...
Windows 媒體格式 SDK 提供適用于整個檔案的屬性，以及適用于個別資料流程的屬性支援。

您可以使用 Windows 媒體格式 SDK 來編輯 mp3 檔案中的中繼資料，不過，您應該一律在 mp3 檔中使用 ID3 相容的屬性，以維持與其他 MP3 操作程式的相容性。
