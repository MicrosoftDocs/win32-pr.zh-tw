...
Microsoft 數位簽署的驅動程式會將 Windows®硬體品質實驗室 (WHQL) 測試，以確保使用者使用最高品質的驅動程式。 這是標準的做法，可確保元件的真實性，因為簽章無法偽造，也不能修改程式碼，而不會終結簽章。 經認證的安全音訊路徑的驅動程式，必須保護其所處理的音訊資料免于受信任元件的存取。 

Windows Millennium Edition 和 Windows XP 隨附的驅動程式會更新為安全音訊路徑並簽署。 未簽署可用於 Windows Me 或 Windows XP 的驅動程式無法播放受保護的音樂。 驅動程式製造商可以重新發出經 WHQL 簽署的更新版驅動程式，並將其發佈到網際網路供取用者下載。
