...
ms.locfileid: "118197536"
---
# <a name="sending-asf-data-to-a-publishing-point"></a>將 ASF 資料傳送至發行端點

您可以使用 Windows 媒體格式 SDK，將 ASF 資料推送至 Windows 媒體伺服器上的發行端點。 然後伺服器會從該發行端點廣播資料。 如果您要在一部電腦上捕獲或重新編碼內容，而且想要從另一部電腦 (或數部電腦) 發佈內容，此案例就很有用。 如果您需要將內容從防火牆內的電腦移至防火牆以外的 Windows 媒體伺服器，這也很有用，因為推送發佈會使用 HTTP 通訊協定。
...
4.  藉由呼叫 [**IWMWriterPushSink：：連線**](/previous-versions/windows/desktop/api/Wmsdkidl/nf-wmsdkidl-iwmwriterpushsink-connect)連線至伺服器。
