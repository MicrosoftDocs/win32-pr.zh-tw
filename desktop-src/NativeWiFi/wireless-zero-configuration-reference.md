...
本章節包含 Windows XP 和 Windows Server 2003 上的無線零設定服務之程式設計介面的相關資訊。 主題包括：
...
若要啟用無線零設定服務，讓它自動啟動，請移至 [ **開始** ] 按鈕。 選取 [**設定**] 選項，然後選取 [**主控台**]。 如果您使用 Windows XP 視圖，請選取 [**效能及維護**] 類別，然後選取 [系統 **管理工具**]。 如果您使用的是傳統視圖，請選取 [系統 **管理工具**]。 按一下左窗格中的 [ **服務** ] 圖示。 按一下右窗格中的 [無線零設定] 圖示，並將 [dropbox] 的 [ **啟動類型** ] 變更為 [ **自動**]。 此設定會將服務設定為在開機時自動啟動。 然後按一下 [ **開始** ] 按鈕，啟動無線零無線零設定服務，然後按一下 [ **確定]** 按鈕。
