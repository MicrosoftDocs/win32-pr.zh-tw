...
ms.locfileid: "119892568"
---
# <a name="stretching-an-image"></a>延展影像

下列範例會伸展影片剪輯的影像。 它會使用 [**MCIWndPutDest**](/windows/desktop/api/Vfw/nf-vfw-mciwndputdest) 宏來增加目的矩形的維度。 播放區域的大小會保持不變，因此結果會是扭曲的影像。 這些範例會使用 **MCIWndPutDest** 函式來定位相對於播放區域的目的地矩形，以提供一種方式來查看延伸影像的不同部分。
