---
title: 從某個資料流程讀取並寫入另一個資料流程
description: 從某個資料流程讀取並寫入另一個資料流程
...
---
# <a name="reading-from-one-stream-and-writing-to-another"></a><span data-ttu-id="cbee6-103">從某個資料流程讀取並寫入另一個資料流程</span><span class="sxs-lookup"><span data-stu-id="cbee6-103">Reading from One Stream and Writing to Another</span></span>

<span data-ttu-id="cbee6-104">下列範例會從資料流程讀取資料、對資料執行某些動作，然後將壓縮的資料寫入至新檔案的資料流程。</span><span class="sxs-lookup"><span data-stu-id="cbee6-104">The following example reads data from a stream, does something with the data, and writes the compressed data into a stream of a new file.</span></span>
