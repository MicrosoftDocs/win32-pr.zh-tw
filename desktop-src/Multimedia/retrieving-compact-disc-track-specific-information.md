...
---
# <a name="retrieving-compact-disc-track-specific-information"></a>取出光碟 Track-Specific 資訊

針對 CD 音訊裝置，您可以藉由指定 MCI \_ track 旗標，並將 [**mci \_ STATUS \_ PARMS**](mci-status-parms.md)的 **dwTrack** 成員設定為所需的追蹤號碼，來取得曲目的開始位置和長度。 若要取得播放軌的開始位置，請將 **dwItem** 成員設定為 MCI \_ 狀態 \_ 位置。 若要取得曲目的長度，請將 **dwItem** 設定為 MCI \_ 狀態 \_ 長度。 例如，下列範例會抓取光碟片上的曲目總數，以及每個曲目的開始位置。然後，它會使用 [MessageBox](/windows/win32/api/winuser/nf-winuser-messagebox) 函數來報告追蹤的開始位置。
