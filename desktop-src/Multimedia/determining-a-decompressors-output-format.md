...
<span data-ttu-id="a20c2-110">下列範例會顯示應用程式如何使用 [**ICDecompressQuery**](/windows/desktop/api/Vfw/nf-vfw-icdecompressquery) 宏來判斷解壓縮程式是否可以處理輸入和輸出格式。</span><span class="sxs-lookup"><span data-stu-id="a20c2-110">The following example shows how an application can use the [**ICDecompressQuery**](/windows/desktop/api/Vfw/nf-vfw-icdecompressquery) macro to determine if a decompressor can handle the input and output formats.</span></span>
