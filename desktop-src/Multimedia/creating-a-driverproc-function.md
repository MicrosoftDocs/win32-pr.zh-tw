...
---
# <a name="creating-a-driverproc-function"></a>建立 DriverProc 函數

建立 [DriverProc](/windows/win32/api/mmiscapi/nc-mmiscapi-driverproc) 函式的方式，與建立視窗程式的方式大致相同。 函數是由 **switch** 語句所組成，而且每個案例都會處理指定的驅動程式訊息，並傳回指出成功或失敗的值。 **DriverProc** 函數具有下列形式：
