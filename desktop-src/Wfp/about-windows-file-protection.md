...
Windows 資源保護 (WRP) 可防止取代安裝為作業系統一部分的基本系統檔案、資料夾和登錄機碼。 從 Windows Server 2008 和 Windows Vista 開始，就可以開始使用。 修改受 WRP 保護之資源的完整存取權，僅限 TrustedInstaller。 WRP 保護的資源只能使用 [支援的資源取代機制](supported-file-replacement-mechanisms.md) 與 Windows 模組安裝程式服務進行變更。 保護這些資源可防止應用程式和作業系統失敗。
...
WRP 是 Windows 檔案保護 (WFP) 的新名稱。 WRP 可保護登錄機碼和資料夾，以及基本的系統檔案。 Microsoft Windows Server 2003 和 Windows XP 中有提供 WFP。 WRP 取代了 Windows Server 2008 和 Windows Vista 中的 WFP。
