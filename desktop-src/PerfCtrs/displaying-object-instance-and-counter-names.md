...
如果您想要在使用者介面中顯示物件及其計數器的清單，則必須取得效能資料。 效能資料包含不定數目的效能物件及其實例和計數器。 如需效能資料格式的詳細資訊，請參閱 [效能資料格式](performance-data-format.md)。

效能資料不包含物件和計數器名稱。 相反地，資料包含索引值，您可以用來抓取物件和計數器的名稱，這些物件和計數器會儲存在效能計數器基礎結構已知的資源中。 不過，實例名稱會儲存在效能資料中。 如需從登錄讀取物件和計數器名稱的範例，並建立稍後用來存取名稱的索引表，請參閱取得 [計數器名稱和解說文字](retrieving-counter-names-and-help-text.md)。

下列範例顯示如何從效能資料中取出物件、實例和計數器區塊。 在抓取物件名稱、實例名稱和計數器名稱之後，此範例會依名稱和每個物件來排序物件，依名稱排序其實例和計數器。 然後，此範例會列印已排序的清單。
