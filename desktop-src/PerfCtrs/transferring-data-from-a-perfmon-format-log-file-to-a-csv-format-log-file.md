...
下列範例會將效能工具所建立的計數器記錄檔中的資料，傳送至 ( .csv) 的逗點分隔格式。 此範例會傳輸從本機電腦收集的處理器時間計數器資料。 若要指定另一種計數器資料類型，請變更 szCounterPath 變數。 如果收集的計數器資料來自特定的電腦，請將電腦名稱稱新增至路徑 (例如「 \\ \\ \\ \\ <computername> \\ \\ processor (0) \\ \\ % processor Time」 ) 。
