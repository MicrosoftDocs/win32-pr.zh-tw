---
description: 大部分的計數器類型都會使用公式來計算計數器的可顯示值。
...
---
# <a name="calculating-counter-values"></a><span data-ttu-id="3c7b2-103">計算計數器值</span><span class="sxs-lookup"><span data-stu-id="3c7b2-103">Calculating Counter Values</span></span>

<span data-ttu-id="3c7b2-104">大部分的計數器類型都會使用公式來計算計數器的可顯示值。</span><span class="sxs-lookup"><span data-stu-id="3c7b2-104">Most counter types use a formula for calculating a displayable value for the counter.</span></span> <span data-ttu-id="3c7b2-105">如需計數器類型及其公式的清單，請參閱 [Windows Server 2003 部署套件](/previous-versions/windows/it-pro/windows-server-2003/cc776490(v=ws.10))的「計數器類型」一節。</span><span class="sxs-lookup"><span data-stu-id="3c7b2-105">For a list of counter types and their formulas, see the Counter Types section of the [Windows Server 2003 Deployment Kit](/previous-versions/windows/it-pro/windows-server-2003/cc776490(v=ws.10)).</span></span> <span data-ttu-id="3c7b2-106">如果計數器需要兩個範例才能計算可顯示的值，則會設定計數器類型的 `PERF_DELTA_COUNTER` 旗標。</span><span class="sxs-lookup"><span data-stu-id="3c7b2-106">If the counter requires two samples in order to calculate the displayable value, the counter type's `PERF_DELTA_COUNTER` flag is set.</span></span>

<span data-ttu-id="3c7b2-107">下列範例顯示如何使用原始資料來計算每個計數器類型的可顯示值。</span><span class="sxs-lookup"><span data-stu-id="3c7b2-107">The following example shows how to use the raw data to calculate a displayable value for each counter type.</span></span> <span data-ttu-id="3c7b2-108">這個範例是以抓取 [計數器資料](retrieving-counter-data.md)的範例為基礎。</span><span class="sxs-lookup"><span data-stu-id="3c7b2-108">This example builds on the example in [Retrieving Counter Data](retrieving-counter-data.md).</span></span>
