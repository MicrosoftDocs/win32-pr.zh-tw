...
[**OpenPerformanceData**](/previous-versions/windows/desktop/legacy/aa372200(v=vs.85))函數可讓提供者有機會初始化其效能資料結構。 當取用者第一次呼叫 [**RegQueryValueEx**](/windows/desktop/api/winreg/nf-winreg-regqueryvalueexa)，或取用者使用 [**RegOpenKey**](/windows/desktop/api/winreg/nf-winreg-regopenkeya) 或 [**RegConnectRegistry**](/windows/desktop/api/winreg/nf-winreg-regconnectregistrya) 函數來開啟 **HKEY \_ 效能 \_ 資料** 時，系統會呼叫您的 open 函式。

下列範例顯示 [**OpenPerformanceData**](/previous-versions/windows/desktop/legacy/aa372200(v=vs.85)) 函式的實作為。 包含此函式所使用之計數器定義的標頭檔會遵循此範例。 如果您使用 c + + 來執行此函式，當您宣告函式時，請務必使用 extern "C"。 此範例中使用的計數器位移常數定義了 [將計數器名稱和描述新增至](adding-counter-names-and-descriptions-to-the-registry.md)登錄中所顯示的 CounterOffsets .h 檔案。
...
下列範例顯示用來匯出開啟、收集和關閉函式的模組定義 ( .def) 檔。
...
下列範例顯示 [*ClosePerformanceData*](/windows/win32/api/winperf/nc-winperf-pm_close_proc) 函式的實作為。 當取用者呼叫 [**RegCloseKey**](/windows/desktop/api/winreg/nf-winreg-regclosekey) 來關閉 **HKEY \_ 效能 \_ 資料** 時，系統會呼叫 close 函數。 提供者會使用此呼叫來釋放已配置的任何資源。
