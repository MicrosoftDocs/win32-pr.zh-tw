...
---
# <a name="retrieving-counter-data"></a><span data-ttu-id="664e0-103">正在抓取計數器資料</span><span class="sxs-lookup"><span data-stu-id="664e0-103">Retrieving Counter Data</span></span>

<span data-ttu-id="664e0-104">效能物件可以定義一或多個計數器。</span><span class="sxs-lookup"><span data-stu-id="664e0-104">Performance objects can define one or more counters.</span></span> <span data-ttu-id="664e0-105">計數器的計數器資料位於 [**性能 \_ 計數器 \_ 區塊**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_block) 記憶體區塊中。</span><span class="sxs-lookup"><span data-stu-id="664e0-105">The counter data for the counters is located in the [**PERF\_COUNTER\_BLOCK**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_block) memory block.</span></span> <span data-ttu-id="664e0-106">物件區塊內的計數器區塊位置取決於物件是否包含單一實例計數器或多個實例計數器。</span><span class="sxs-lookup"><span data-stu-id="664e0-106">The location of the counter block within the object block depends on whether the object contains single instance counters or multiple instance counters.</span></span> <span data-ttu-id="664e0-107">如需詳細資訊，請參閱 [效能資料格式](performance-data-format.md)。</span><span class="sxs-lookup"><span data-stu-id="664e0-107">For details, see [Performance Data Format](performance-data-format.md).</span></span>

<span data-ttu-id="664e0-108">您可以使用 [**性能 \_ 計數器 \_ 定義**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition)的 **CounterOffset** 和 **CounterSize** 成員，存取計數器區塊內的計數器資料。</span><span class="sxs-lookup"><span data-stu-id="664e0-108">You use the **CounterOffset** and **CounterSize** members of [**PERF\_COUNTER\_DEFINITION**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) to access the counter's data within the counter block.</span></span> <span data-ttu-id="664e0-109">目前，計數器資料僅限於 **DWORD** 和 **ULONGLONG** 資料類型 (這些是效能工具支援) 的唯一類型。</span><span class="sxs-lookup"><span data-stu-id="664e0-109">Currently, counter data is limited to **DWORD** and **ULONGLONG** data types (these are the only types that the Performance tool supports).</span></span>

<span data-ttu-id="664e0-110">性能 [**\_ 計數器 \_ 定義**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition)的 **CounterType** 成員會告訴您，效能物件需要哪些其他資訊才能使用計數器資料。</span><span class="sxs-lookup"><span data-stu-id="664e0-110">The **CounterType** member of [**PERF\_COUNTER\_DEFINITION**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) tells you what other information you need from the performance object in order to use the counter data.</span></span> <span data-ttu-id="664e0-111">針對某些計數器，您可以直接使用計數器資料，但對於其他計數器，您可能需要時間基底資訊或來自另一個計數器的資料，才能計算可顯示的值。</span><span class="sxs-lookup"><span data-stu-id="664e0-111">For some counters, you can use the counter data directly, but for others, you may need time base information or data from another counter in order to calculate a displayable value.</span></span>

<span data-ttu-id="664e0-112">下列範例示範如何使用計數器型別來判斷您需要從計數器的效能資料中取出的資訊，以便計算可顯示的計數器值。</span><span class="sxs-lookup"><span data-stu-id="664e0-112">The following example shows how to use the counter type to determine the information you need to retrieve from the performance data for a counter in order to calculate a displayable counter value.</span></span> <span data-ttu-id="664e0-113">如需根據計數器類型計算可顯示值的範例，請參閱 [計算計數器值](calculating-counter-values.md)。</span><span class="sxs-lookup"><span data-stu-id="664e0-113">For an example that calculates a displayable value based on the counter type, see [Calculating Counter Values](calculating-counter-values.md).</span></span>
