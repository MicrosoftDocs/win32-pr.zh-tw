...
效能物件可以定義一或多個計數器。 計數器的計數器資料位於 [**性能 \_ 計數器 \_ 區塊**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_block) 記憶體區塊中。 物件區塊內的計數器區塊位置取決於物件是否包含單一實例計數器或多個實例計數器。 如需詳細資訊，請參閱 [效能資料格式](performance-data-format.md)。

您可以使用 [**性能 \_ 計數器 \_ 定義**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition)的 **CounterOffset** 和 **CounterSize** 成員，存取計數器區塊內的計數器資料。 目前，計數器資料僅限於 **DWORD** 和 **ULONGLONG** 資料類型 (這些是效能工具支援) 的唯一類型。

性能 [**\_ 計數器 \_ 定義**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition)的 **CounterType** 成員會告訴您，效能物件需要哪些其他資訊才能使用計數器資料。 針對某些計數器，您可以直接使用計數器資料，但對於其他計數器，您可能需要時間基底資訊或來自另一個計數器的資料，才能計算可顯示的值。

下列範例示範如何使用計數器型別來判斷您需要從計數器的效能資料中取出的資訊，以便計算可顯示的計數器值。 如需根據計數器類型計算可顯示值的範例，請參閱 [計算計數器值](calculating-counter-values.md)。
