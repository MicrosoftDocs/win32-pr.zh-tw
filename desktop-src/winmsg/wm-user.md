...
用來定義私用訊息以供私用視窗類別使用，通常是以 **WM \_ 使用者** x 形式來使用， + 其中 *x* 是整數值。
...
| 範圍                                                        | 意義                                                        |
|--------------------------------------------------------------|----------------------------------------------------------------|
| 0到 **WM \_ 使用者** -1<br/>                         | 保留供系統使用的訊息。<br/>            |
| **WM \_使用者** 透過0x7FFF<br/>                       | 私用視窗類別使用的整數訊息。<br/> |
| [**WM \_應用程式**](wm-app.md) (0x8000) 至0xBFFF<br/> | 應用程式可使用的訊息。<br/>         |
...
第一個範圍中的訊息編號 (0 到 **WM \_ 使用者** -1) 是由系統所定義。 此範圍中未明確定義的值是由系統所保留。

第二個範圍中的訊息編號 (**WM \_ 使用者** 透過 0x7FFF) 可以由應用程式定義和使用，以在私用視窗類別內傳送訊息。 這些值不能用來定義整個應用程式中有意義的訊息，因為某些預先定義的視窗類別已經在這個範圍中定義值。 例如，[ **按鈕**]、[ **編輯**]、[ **LISTBOX**] 和 [ **COMBOBOX** ] 等預先定義的控制項類別，都可能會使用這些值。 除非應用程式已設計為交換訊息，並將相同意義附加至訊息編號，否則不應將此範圍中的訊息傳送至其他應用程式。
