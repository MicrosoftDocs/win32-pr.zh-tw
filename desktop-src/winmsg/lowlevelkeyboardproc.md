...
**注意：**  當呼叫此回呼函式以回應索引鍵的狀態變更時，會先呼叫回呼函式，然後再更新索引鍵的非同步狀態。
因此，無法從回呼函式中呼叫 [GetAsyncKeyState](/windows/desktop/api/winuser/nf-winuser-getasynckeystate) 來判斷索引鍵的非同步狀態。

**HOOKPROC** 型別定義此回呼函數的指標。
**LowLevelKeyboardProc** 是應用程式定義或程式庫定義函數名稱的預留位置。
...
| 值 | 意義 |
|-------|---------|
| **HC_ACTION** 0 | *WParam* 和 *lParam* 參數包含鍵盤訊息的相關資訊。 |
...
鍵盤訊息的識別碼。
這個參數可以是下列其中一個訊息： [WM_KEYDOWN](/windows/desktop/inputdev/wm-keydown)、 [WM_KEYUP](/windows/desktop/inputdev/wm-keyup)、 [WM_SYSKEYDOWN](/windows/desktop/inputdev/wm-syskeydown)或 [WM_SYSKEYUP](/windows/desktop/inputdev/wm-syskeyup)。

### <a name="lparam-in"></a>lParam [in]

類型： **LPARAM**

[KBDLLHOOKSTRUCT](/windows/desktop/api/winuser/ns-winuser-kbdllhookstruct)結構的指標。
...
如果 *nCode* 大於或等於零，且攔截程式未處理訊息，強烈建議您呼叫 **CallNextHookEx** 並傳回傳回的值;否則，已安裝 [WH_KEYBOARD_LL](about-hooks.md) 勾點的其他應用程式將不會收到攔截通知，而且可能會導致不正確的行為。
如果攔截程式處理訊息，它可能會傳回非零值，以防止系統將訊息傳遞至其餘的攔截鏈或目標視窗程式。

## <a name="remarks"></a>備註

應用程式會在 **SetWindowsHookEx** 函式的呼叫中指定 **WH_KEYBOARD_LL** 攔截類型和攔截程式指標，以安裝攔截程式。

此攔截器會在其安裝所在的執行緒內容中呼叫。
呼叫是藉由將訊息傳送至已安裝攔截器的執行緒進行。
因此，安裝攔截的執行緒必須有訊息迴圈。

鍵盤輸入可以來自本機鍵盤驅動程式，或來自 [keybd_event](/windows/desktop/api/winuser/nf-winuser-keybd_event) 函式的呼叫。
如果輸入來自 **keybd_event** 的呼叫，則輸入為「已插入」。
不過，WH_KEYBOARD_LL 的勾點不會插入另一個進程。
