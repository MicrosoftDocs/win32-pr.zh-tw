---
title: Windows 遠端管理
description: Windows遠端系統管理 (Windows 遠端管理) 是 WS-Management 通訊協定的 Microsoft 執行，這是一種標準的 SOAP 型、可供防火牆使用的通訊協定，可讓不同廠商的硬體與作業系統交互操作。
ms.assetid: 6429e748-e0bf-431a-8989-db5b211665d5
ms.tgt_platform: multiple
keywords:
- Windows遠端系統管理 (WinRM) ，起始頁
- WS-Management
ms.topic: article
ms.date: 05/31/2018
topic_type:
...
WS-Management 通訊協定規格提供了一種常見的方式，讓系統在整個 IT 基礎結構之間存取及交換管理資訊。 [*(IPMI) 的 WinRM 和智慧型平臺管理介面*](windows-remote-management-glossary.md)，以及 [事件收集器](/previous-versions/windows/it-pro/windows-server-2003/cc785056(v=ws.10))是 [Windows 硬體管理](/previous-versions/windows/it-pro/windows-server-2003/cc785056(v=ws.10))功能的元件。

## <a name="where-applicable"></a>適用時

您可以使用 winrm 腳本物件、winrm 命令列工具或 Windows 遠端 Shell 命令列工具 WinRS，從可能有基礎板 [*管理控制器 (bmc)*](windows-remote-management-glossary.md)的本機和遠端電腦取得管理資料。 如果電腦執行包含 WinRM 的 Windows 為基礎的作業系統版本，則管理資料是由[Windows Management Instrumentation (WMI) ](/windows/desktop/WmiSdk/wmi-start-page)所提供。
...
開發人員 Pro 負責撰寫腳本，以自動化伺服器的管理或 ISV 開發人員為管理應用程式取得資料。
