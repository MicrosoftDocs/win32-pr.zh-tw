...
---
# <a name="asynchronous-callback-handling"></a><span data-ttu-id="02f77-103">非同步回呼處理</span><span class="sxs-lookup"><span data-stu-id="02f77-103">Asynchronous Callback Handling</span></span>

<span data-ttu-id="02f77-104">當用戶端與提供者虛擬化根底下的檔案和目錄互動時，這些互動通常會導致叫用提供者的回呼。</span><span class="sxs-lookup"><span data-stu-id="02f77-104">When a client interacts with the files and directories beneath the provider's virtualization root, these interactions typically result in the provider's callbacks being invoked.</span></span>  <span data-ttu-id="02f77-105">ProjFS 會叫用提供者回呼，方法是將訊息從核心模式傳送至 ProjFS 使用者模式程式庫，其中背景工作執行緒會接收訊息並呼叫適當的回呼。</span><span class="sxs-lookup"><span data-stu-id="02f77-105">ProjFS invokes provider callbacks by sending a message from kernel mode to the ProjFS user-mode library, where a worker thread receives the message and calls the appropriate callback.</span></span>  <span data-ttu-id="02f77-106">回呼傳回後，背景工作執行緒會等待另一則訊息抵達核心模式。</span><span class="sxs-lookup"><span data-stu-id="02f77-106">Once the callback has returned, the worker thread waits for another message to arrive from kernel mode.</span></span>  <span data-ttu-id="02f77-107">如果所有背景工作執行緒都忙於執行提供者回呼程式碼，則會觸發回呼區塊的任何進一步用戶端 i/o，直到背景工作執行緒可供接收訊息並叫用相關的回呼為止。</span><span class="sxs-lookup"><span data-stu-id="02f77-107">If all the worker threads are busy executing provider callback code, any further client I/O that triggers a callback blocks until a worker thread becomes available to receive the message and invoke the relevant callback.</span></span>  <span data-ttu-id="02f77-108">當提供者啟動時，可以透過 **[PrjStartVirtualizing](/windows/desktop/api/projectedfslib/nf-projectedfslib-prjstartvirtualizing)** 的 _options_ 參數，指定其想要 ProjFS 建立給服務回呼的背景工作執行緒數目。</span><span class="sxs-lookup"><span data-stu-id="02f77-108">When a provider starts up, it can specify the number of worker threads it wants ProjFS to create to service callbacks via the _options_ parameter of **[PrjStartVirtualizing](/windows/desktop/api/projectedfslib/nf-projectedfslib-prjstartvirtualizing)**.</span></span>  <span data-ttu-id="02f77-109">提供者可以非同步服務回呼，以改善訊息接收工作者執行緒的效率。</span><span class="sxs-lookup"><span data-stu-id="02f77-109">A provider can improve the efficiency of the message-receiving worker threads by servicing its callbacks asynchronously.</span></span>

> <span data-ttu-id="02f77-110">如果提供者未指定要 **PrjStartVirtualizing** 的 _options_ 參數，或針對 _options_ 參數的 ConcurrentThreadCount 成員指定0，ProjFS 將會使用系統中的邏輯處理器數目來作為 ConcurrentThreadCount 的值。</span><span class="sxs-lookup"><span data-stu-id="02f77-110">If the provider does not specify the _options_ parameter to **PrjStartVirtualizing**, or if it specifies 0 for the ConcurrentThreadCount member of the _options_ parameter, ProjFS will use the number of logical processors in the system for the value of ConcurrentThreadCount.</span></span>
>
> <span data-ttu-id="02f77-111">如果提供者未指定要 **PrjStartVirtualizing** 的 _options_ 參數，或針對 _options_ 參數的 PoolThreadCount 成員指定0，ProjFS 將會使用 ConcurrentThreadCount 值的兩倍作為 PoolThreadCount 的值。</span><span class="sxs-lookup"><span data-stu-id="02f77-111">If the provider does not specify the _options_ parameter to **PrjStartVirtualizing**, or if it specifies 0 for the PoolThreadCount member of the _options_ parameter, ProjFS will use twice the value of ConcurrentThreadCount for the value of PoolThreadCount.</span></span>

<span data-ttu-id="02f77-112">提供者會透過從回呼傳回 HRESULT_FROM_WIN32 (ERROR_IO_PENDING) ，並在稍後使用 **[PrjCompleteCommand](/windows/desktop/api/projectedfslib/nf-projectedfslib-prjcompletecommand)** 完成這些回呼，以非同步方式回呼。</span><span class="sxs-lookup"><span data-stu-id="02f77-112">A provider services callbacks asynchronously by returning HRESULT_FROM_WIN32(ERROR_IO_PENDING) from its callbacks and later completing them using **[PrjCompleteCommand](/windows/desktop/api/projectedfslib/nf-projectedfslib-prjcompletecommand)**.</span></span>  <span data-ttu-id="02f77-113">以非同步方式處理回呼的提供者，也應藉由實 **[PRJ_CANCEL_COMMAND_CB](/windows/desktop/api/projectedfslib/nc-projectedfslib-prj_cancel_command_cb)** 回呼來支援回呼取消。</span><span class="sxs-lookup"><span data-stu-id="02f77-113">A provider that processes callbacks asynchronously should also support callback cancellation by implementing the **[PRJ_CANCEL_COMMAND_CB](/windows/desktop/api/projectedfslib/nc-projectedfslib-prj_cancel_command_cb)** callback.</span></span>

<span data-ttu-id="02f77-114">當 ProjFS 呼叫提供者的回呼時，它會使用回呼的 _callbackData_ 參數的 CommandId 成員來識別回呼的特定調用。</span><span class="sxs-lookup"><span data-stu-id="02f77-114">When ProjFS calls a provider's callback, it identifies the specific invocation of the callback using the CommandId member of the callback's _callbackData_ parameter.</span></span>  <span data-ttu-id="02f77-115">如果提供者決定以非同步方式處理該回呼，它必須儲存 CommandId 成員的值，並從回呼傳回 HRESULT_FROM_WIN32 (ERROR_IO_PENDING) 。</span><span class="sxs-lookup"><span data-stu-id="02f77-115">If the provider decides to process that callback asynchronously, it must store the value of the CommandId member and return HRESULT_FROM_WIN32(ERROR_IO_PENDING) from the callback.</span></span>  <span data-ttu-id="02f77-116">一旦提供者完成回呼的處理，它就會呼叫 **PrjCompleteCommand**，並在 _commandId_ 參數中傳遞預存的識別碼。</span><span class="sxs-lookup"><span data-stu-id="02f77-116">Once the provider has finished processing the callback, it calls **PrjCompleteCommand**, passing the stored identifier in the _commandId_ parameter.</span></span>  <span data-ttu-id="02f77-117">這會告訴 ProjFS 已完成的回呼調用。</span><span class="sxs-lookup"><span data-stu-id="02f77-117">This tells ProjFS which callback invocation has been completed.</span></span>

<span data-ttu-id="02f77-118">實 **PRJ_CANCEL_COMMAND_CB** 回呼的提供者必須追蹤尚未完成的回呼。</span><span class="sxs-lookup"><span data-stu-id="02f77-118">A provider that implements the **PRJ_CANCEL_COMMAND_CB** callback must keep track of which callbacks it has not yet completed.</span></span>  <span data-ttu-id="02f77-119">如果提供者收到此回呼，則表示會明確地取消導致叫用其中一個回呼的 i/o，或是因為它在終止時所發出的執行緒而被取消。</span><span class="sxs-lookup"><span data-stu-id="02f77-119">If the provider receives this callback, it indicates that the I/O that caused one of those callbacks to be invoked was canceled, either explicitly or because the thread it was issued on terminated.</span></span> <span data-ttu-id="02f77-120">提供者應該盡可能地取消處理 CommandId 所識別的回呼調用。</span><span class="sxs-lookup"><span data-stu-id="02f77-120">The provider should cancel processing the callback invocation identified by CommandId as soon as possible.</span></span>

> <span data-ttu-id="02f77-121">雖然 ProjFS 只會在叫用取消回呼之後叫用指定 CommandId 的 **PRJ_CANCEL_COMMAND_CB** ，但是取消和原始調用可能會在多執行緒提供者中並存執行。</span><span class="sxs-lookup"><span data-stu-id="02f77-121">Although ProjFS will invoke **PRJ_CANCEL_COMMAND_CB** for a given CommandId only after the callback to be canceled is invoked, the cancellation and original invocation may run concurrently in a multithreaded provider.</span></span>  <span data-ttu-id="02f77-122">提供者必須能夠正常地處理這種情況。</span><span class="sxs-lookup"><span data-stu-id="02f77-122">The provider must be able to gracefully handle this situation.</span></span>

<span data-ttu-id="02f77-123">下列範例是針對 [列舉檔案和目錄](enumerating-files-and-directories.md) 主題提供的範例版本，修改為說明非同步回呼處理。</span><span class="sxs-lookup"><span data-stu-id="02f77-123">The following sample is a version of the sample given for the [Enumerating Files and Directories](enumerating-files-and-directories.md) topic, modified to illustrate asynchronous callback handling.</span></span>
...
<span data-ttu-id="02f77-124">以下是使用上述範例程式碼的簡短範例 PRJ_CANCEL_COMMAND_CB。</span><span class="sxs-lookup"><span data-stu-id="02f77-124">And here is a brief example PRJ_CANCEL_COMMAND_CB that works with the preceding sample code.</span></span>
