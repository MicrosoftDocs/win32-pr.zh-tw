...
Microsoft 已發行64位版本的 Windows 作業系統。 64位 Windows 的設計是以相容性為考慮。 開發人員可以藉由遷移應用程式，確保其現有的32位應用程式在64位 Windows 下正常執行，或利用64位 Windows 的優點。
...
-   應用程式可以輕鬆且更可靠地處理大量資料。 移動圖片工作的影片組合需要64位的 Windows，因為這個原因。 科學和財務應用程式的模型化，可從不可能在32位 Windows 上的記憶體常駐資料結構獲益。

企業也有很重要的優點：

-   提高生產力。 知識工作者可以花時間思考和產生，而不是等待軟體完成其工作。
-   降低所有權的成本。 每部伺服器可以支援更大量的使用者和應用程式，因此您的企業需要較少的伺服器。 這會直接轉譯為較少的管理額外負荷，也就是任何運算環境中最高的成本之一。
-   新的應用程式機會。 您可以設計新的應用程式，而不會有32位 Windows 所加諸的阻礙。 新的圖形應用程式可讓工作變得更簡單且更愉快。 目前無法使用的資料密集工作，可透過64位 Windows 來完成。

## <a name="in-this-section"></a>本章節內容

-   [準備開始64位 Windows](getting-ready-for-64-bit-windows.md)
