...
---
# <a name="ipv4-only-client-code"></a><span data-ttu-id="f407b-103">僅限 IPv4 的用戶端程式代碼</span><span class="sxs-lookup"><span data-stu-id="f407b-103">IPv4-only Client Code</span></span>

<span data-ttu-id="f407b-104">下列程式碼是 Simplec .c 檔案，其為僅限 IPv4 的 Windows 通訊端用戶端 (Simplec 的 IPv6 啟用版本可在附錄 B) 中找到。</span><span class="sxs-lookup"><span data-stu-id="f407b-104">The following code is the Simplec.c file, which is an IPv4-only Windows Sockets client (an IPv6 enabled version of the Simplec.c file can be found in Appendix B).</span></span> <span data-ttu-id="f407b-105">這段程式碼僅供比較之用，請使用附錄 B 來瞭解如何撰寫具備 IPv6 功能的用戶端。</span><span class="sxs-lookup"><span data-stu-id="f407b-105">This code is provided for comparison purposes only — use Appendix B for an example of how to write an IPv6-enabled client.</span></span>
