---
description: 呼叫 Winsock 函式 (應用程式或 dll) 的所有進程都必須先初始化使用 Windows 通訊端 DLL，然後再進行其他 Winsock 函式呼叫。 這也可確保系統支援 Winsock。
...
ms.locfileid: "119051516"
---
# <a name="initializing-winsock"></a>初始化 Winsock

呼叫 Winsock 函式 (應用程式或 dll) 的所有進程都必須先初始化使用 Windows 通訊端 DLL，然後再進行其他 Winsock 函式呼叫。 這也可確保系統支援 Winsock。
...
[**WSADATA**](/windows/desktop/api/winsock/ns-winsock-wsadata)結構包含 Windows 通訊端執行的相關資訊。 [**WSAStartup**](/windows/desktop/api/winsock/nf-winsock-wsastartup)的 MAKEWORD (2，2) 參數會在系統上提出2.2 版 Winsock 的要求，並將傳遞的版本設定為呼叫端可使用之 Windows 通訊端支援的最高版本。
