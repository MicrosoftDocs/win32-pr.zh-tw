...
Windows 通訊端2應用程式開發人員需要隔離錯誤：

-   應用程式。
-   *Ws2 \_32.dll* 或其中一個相容性填充碼 dll。
-   服務提供者。

Windows 通訊端2透過數個元件和功能來解決這項需求：

-   在 Windows Vista 和更新版本上對 Winsock 追蹤的整合支援。
-   在 Windows Vista 上， *Ws2 \_32.dll* 特別設計的 debug 版。
-   用於 Windows Server 2003 和 Windows XP 的個別基本 debug 和 trace 設備。

## <a name="winsock-tracing-using-event-tracing-for-windows"></a>使用 Windows 事件追蹤的 Winsock 追蹤

Windows Vista 及更新版本中包含了使用 Windows 事件追蹤 (ETW) 的對 Winsock 追蹤的整合支援。 這是在 Windows Vista 和更新版本上追蹤 Winsock 呼叫的慣用方法。 使用 ETW 的 Winsock 追蹤很輕量，且適用于 Windows 的零售版本。 不需要額外的軟體或元件。 這項功能只需要在 Windows Vista 和更新版本上啟用。 如需詳細資訊，請參閱 [Winsock 追蹤](winsock-tracing.md) 主題。

## <a name="using-a-debug-version-of-ws2_32dll"></a>使用 Ws232.dll 的偵錯工具版本 \_

在 Windows Vista 和 Winsock 追蹤上， *Ws2 \_32.dll* 的偵錯工具組合，可讓您監視 windows 通訊端 2 API 或 SPI 之間的所有程序呼叫，並進行控制。

如果已將適用于 Windows Vista 的 Microsoft Windows 軟體開發套件 (SDK) 版本安裝到預設位置，則適用于各種架構的 *Ws2 \_32.dll* debug 版本位於下列資料夾底下：

**C： \\ Program Files \\ Microsoft sdk \\ Windows \\ v 6.0 \\ NoRedist**

*Ws2 \_32.dll* 的已檢查版本，應使用符合您要測試的 Windows 版本和 Service Pack。 請注意，可能已套用安全性修補程式，以更新您測試系統上的 *Ws2 \_32.dll* 。 Windows Vista 和舊版平臺軟體發展工具組的 Windows SDK (SDK) DVD/CD 訂用帳戶包含適用于各種 Windows 版本的已檢查組建。 您應使用相同的 *Ws2 \_32.dll* 檢查版本，做為要測試之系統上所使用的零售版本。 另外也請注意，在已檢查的組建下執行的行為，不會與使用零售組建執行的行為相同。
...
舊版 Windows Windows 8 之前的版本和 Windows Server 2012 支援個別的基本偵錯工具和追蹤功能，此功能包含在 Windows SDK 和舊版平臺 SDK 的範例中。 只有在不支援 Winsock 追蹤的 Windows Server 2003 和 Windows XP 上，才應該使用 debug/trace 設備。

如果將 Windows 7 的 Windows SDK 安裝到預設位置，則會在下列資料夾中安裝這個基本的 Winsock 追蹤功能：
