...
這項機制是設計來適用于 Windows 的單一執行緒和先占式多執行緒版本。 不過請注意，在指定進程中的執行緒之間可共用該通訊端，而不使用 [**WSPDuplicateSocket**](/previous-versions/windows/hardware/network/ff566282(v=vs.85)) 函式，因為通訊端描述項在所有進程的執行緒中都是有效的。
