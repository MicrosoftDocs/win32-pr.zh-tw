---
description: 請注意，名稱解析的所有 Windows 通訊端1.1 函式都是 IPv4 tcp/ip 網路專用。
ms.assetid: 5a2a37f3-85c5-4b27-9ce3-f5b707b1564a
title: Windows 通訊端 1.1 API 中的 tcp/ip 相容名稱解析
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "118322298"
---
# <a name="compatible-name-resolution-for-tcpip-in-the-windows-sockets-11-api"></a>Windows 通訊端 1.1 API 中的 tcp/ip 相容名稱解析

> [!Note]  
> 所有用於名稱解析的 Windows 通訊端1.1 函式都是 IPv4 tcp/ip 網路專用的。 應用程式開發人員強烈建議您繼續使用這些僅支援 IPv4 的傳輸特定功能。
...
Windows通訊端1.1 定義了數個用來搭配 TCP/IP (IP 第4版) 網路進行名稱解析的常式。 這些有時稱為 **getXbyY** 函式，並包含下列各項：
...
為了維持與 Windows 通訊端1.1 的嚴格回溯相容性，只要至少有一個命名空間提供者支援 af INET 位址系列，就會繼續支援所有較舊的 IPv4 函式 \_ (這些函式與 IP 版本6（以 AF INET6) 表示）無關 \_ 。
...
本節說明 Windows 通訊端 1.1 API 中的 tcp/ip 相容名稱解析。 下列清單說明本節中的主題：
