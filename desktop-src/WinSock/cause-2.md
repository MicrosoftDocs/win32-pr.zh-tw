...
除了先前所述的所有資訊元素之外，在呼叫 [**WSAConnect**](/windows/desktop/api/Winsock2/nf-winsock2-wsaconnect)時，可以在 ATM 專屬的 [**QoS**](/windows/win32/api/winsock2/ns-winsock2-qos)結構中指定，這會導致 IE 只能在呼叫發行期間使用。 中斷連線時，Windows 通訊端2應用程式可以選擇性地指定此 IE 作為 [**WSASendDisconnect**](/windows/desktop/api/Winsock2/nf-winsock2-wsasenddisconnect)中的中斷連線資料。 遠端方可以在收到 FD 關閉通知之後，透過 [**WSARecvDisconnect**](/windows/desktop/api/Winsock2/nf-winsock2-wsarecvdisconnect) 取得此 IE \_ 。
