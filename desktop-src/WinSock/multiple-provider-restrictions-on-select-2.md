...
Windows通訊端2可讓應用程式使用多個服務提供者，但 [**select**](/windows/desktop/api/Winsock2/nf-winsock2-select)函式僅限於一組與單一服務提供者相關聯的通訊端。 這不會以任何方式限制應用程式必須透過多個提供者來開啟多個通訊端。
...
-   當採用非封鎖作業，而且應用程式已經使用 Windows 訊息抽取時，使用 [**WSAAsyncSelect**](/windows/desktop/api/winsock/nf-winsock-wsaasyncselect)函數。
