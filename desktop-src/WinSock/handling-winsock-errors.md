...
ms.locfileid: "120097898"
---
# <a name="handling-winsock-errors"></a>處理 Winsock 錯誤

當函式傳回時，大部分的 Windows 通訊端2函式不會傳回錯誤的特定原因。 如果成功，部分 Winsock 函數會傳回零的值。 否則，會傳回值通訊端 \_ 錯誤 (-1) ，而且可以藉由呼叫 [**WSAGetLastError**](/windows/desktop/api/winsock/nf-winsock-wsagetlasterror) 函式來取出特定的錯誤號碼。 針對傳回控制碼的 Winsock 函式，無效通訊端的傳回值 \_ (0xffff) 表示錯誤，而且可以藉由呼叫 **WSAGetLastError** 來抓取特定的錯誤號碼。 針對傳回指標的 Winsock 函式，傳回值為 **Null** 表示錯誤，而且可以藉由呼叫 **WSAGetLastError** 函數來抓取特定的錯誤號碼。
