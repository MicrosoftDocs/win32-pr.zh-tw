...
儲存體介面卡會管理範本資料庫。 介面卡開發人員必須執行下列函數。 Windows 生物識別服務會呼叫它們。
...
| 主題                                                                                         | 描述                                                                                                                             |
|-----------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------|
| [**StorageAdapterActivate**](/windows/desktop/api/Winbio_adapter/nc-winbio_adapter-pibio_storage_activate_fn)<br/>                           | 讓存放裝置介面卡有機會執行任何需要的工作，以使儲存體元件退出閒置狀態。<br/>         |
...
| [**StorageAdapterDeactivate**](/windows/desktop/api/Winbio_adapter/nc-winbio_adapter-pibio_storage_deactivate_fn)<br/>                       | 讓存放裝置介面卡有機會執行任何需要的工作，以將儲存元件置於閒置狀態。<br/>             |
...
| [**StorageAdapterPipelineCleanup**](/windows/desktop/api/Winbio_adapter/nc-winbio_adapter-pibio_storage_pipeline_cleanup_fn)<br/>             | 讓存放裝置介面卡有機會執行任何清理，以準備關閉範本資料庫。<br/>                |
| [**StorageAdapterPipelineInit**](/windows/desktop/api/Winbio_adapter/nc-winbio_adapter-pibio_storage_pipeline_init_fn)<br/>                   | 讓存放裝置介面卡有機會執行任何未完成的初始化。<br/>                                  |
