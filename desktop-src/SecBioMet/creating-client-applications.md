...
下列主題討論如何使用 Windows 生物特徵辨識架構 API 來建立使用私用感應器集區的用戶端應用程式。

## <a name="enroll-biometric-information"></a>註冊生物特徵辨識資訊

下列程式碼範例示範如何在系統集區中註冊生物特徵辨識範本。

### <a name="synchronous-enrollment"></a>同步註冊

下列程式碼範例：

-   呼叫 [**WinBioOpenSession**](/windows/desktop/api/Winbio/nf-winbio-winbioopensession) 以開啟生物識別會話，並連接到系統集區。
-   呼叫 [**WinBioLocateSensor**](/windows/desktop/api/Winbio/nf-winbio-winbiolocatesensor) 以找出生物特徵辨識單位。
-   呼叫 [**WinBioEnrollBegin**](/windows/desktop/api/Winbio/nf-winbio-winbioenrollbegin) 以開始註冊順序。
-   呼叫 [**WinBioEnrollCapture**](/windows/desktop/api/Winbio/nf-winbio-winbioenrollcapture) 來處理感應器上的多個手指撥動。
-   呼叫 [**WinBioEnrollCommit**](/windows/desktop/api/Winbio/nf-winbio-winbioenrollcommit) 以將範本儲存在存放區中。

若要編譯此範例，請連結至 Winbio .lib 靜態程式庫，並包含下列標頭檔：
...
### <a name="asynchronous-enrollment"></a>非同步註冊
...
-   呼叫 [**WinBioLocateSensor**](/windows/desktop/api/Winbio/nf-winbio-winbiolocatesensor) 以找出生物特徵辨識單位。
-   呼叫 [**WinBioEnrollBegin**](/windows/desktop/api/Winbio/nf-winbio-winbioenrollbegin) 以開始註冊順序。
-   呼叫 [**WinBioEnrollCaptureWithCallback**](/windows/desktop/api/Winbio/nf-winbio-winbioenrollcapturewithcallback) 來處理多個手指撥動。 這個函式是非同步，而且會使用自訂回呼函式，在個別的執行緒上繼續處理。 範例回呼函數如下所示。
-   呼叫 [**WinBioWait**](/windows/desktop/api/Winbio/nf-winbio-winbiowait) 以等候非同步註冊程式完成或取消。
-   呼叫 [**WinBioEnrollCommit**](/windows/desktop/api/Winbio/nf-winbio-winbioenrollcommit) 以儲存範本。

連結至 Winbio .lib 靜態程式庫以編譯此範例。
...
## <a name="locate-biometric-units"></a>找出生物特徵辨識單位

下列程式碼範例示範如何找出已安裝的生物識別單位。

### <a name="locate-biometric-units-synchronously"></a>同步找出生物識別單位
...
### <a name="locate-biometric-units-asynchronously"></a>以非同步方式找出生物特徵辨識單位

下列程式碼範例：

-   呼叫 [**WinBioOpenSession**](/windows/desktop/api/Winbio/nf-winbio-winbioopensession) 以開啟生物識別會話，並連接到系統集區。
-   呼叫 [**WinBioLocateSensorWithCallback**](/windows/desktop/api/Winbio/nf-winbio-winbiolocatesensorwithcallback) 以找出生物特徵辨識感應器。 這是非同步函式，可將生物識別子系統設定為在另一個執行緒上找出感應器。 生物識別子系統的輸出會傳送至自訂回呼函式，此處稱為 LocateSensorCallback。
-   呼叫 [**WinBioWait**](/windows/desktop/api/Winbio/nf-winbio-winbiowait) 以等候非同步處理常式完成或取消。
...
## <a name="verify-user-identity"></a>驗證使用者身分識別

### <a name="synchronous-verification"></a>同步驗證

下列程式碼範例：

-   呼叫 [**WinBioOpenSession**](/windows/desktop/api/Winbio/nf-winbio-winbioopensession) 以開啟生物識別會話，並連接到系統集區。
-   呼叫 [**WinBioVerify**](/windows/desktop/api/Winbio/nf-winbio-winbioverify) 來判斷生物特徵辨識範例是否符合目前使用者的登入身分識別。
...
### <a name="asynchronous-verification"></a>非同步驗證

下列程式碼範例：

-   呼叫 [**WinBioOpenSession**](/windows/desktop/api/Winbio/nf-winbio-winbioopensession) 以開啟生物識別會話，並連接到系統集區。
-   呼叫 [**WinBioVerifyWithCallback**](/windows/desktop/api/Winbio/nf-winbio-winbioverifywithcallback) 來判斷生物特徵辨識範例是否符合目前使用者的登入身分識別。 這是非同步函式，它會設定生物特徵辨識子系統，以驗證另一個執行緒上的使用者。 生物識別子系統的輸出會傳送至自訂回呼函式，此處稱為 VerifyCallback。
-   呼叫 [**WinBioWait**](/windows/desktop/api/Winbio/nf-winbio-winbiowait) 以等候非同步處理常式完成或取消。
...
-   無法針對來賓或內建的系統管理員帳戶或任何非互動式帳戶（例如 LocalSystem、LocalService 或 NetworkService）儲存、查詢或刪除認證。
-   所有函式都會傳回 **HRESULT** 錯誤碼，此錯誤碼可能是常見的錯誤碼，例如 **電子 \_ ACCESSDENIED** 或認證管理員特定的錯誤，例如 **WINBIO \_ E 未知的 \_ \_ 識別碼**。
-   如果有的話， \_ 會在更明確的錯誤碼之前傳回 E ACCESSDENIED，例如 SEC \_ E \_ LOGON \_ 拒絕或 \_ 傳回 WINBIO E \_ 未知 \_ 的識別碼。
-   許可權未提高許可權的使用者，只能針對自己的帳戶設定、查詢或移除認證。 提高許可權的呼叫端可以查詢認證狀態，並移除其他使用者的認證。
-   所有函式都將失敗，並傳回 WINBIO \_ E 認證 \_ \_ >prov \_ DISABLED：
    -   所有使用者的認證提供者未啟用全系統時。
    -   當提供者未啟用網域使用時，為網域使用者。
-   新增或移除認證時，就會產生事件通知。

### <a name="set-a-credential"></a>設定認證

下列程式碼範例：

-   呼叫 GetCurrentUserIdentity 以取得目前使用者的 [**WINBIO \_ 識別**](winbio-identity.md) 物件。 GetCurrentUserIdentity 是 helper 函式，不是 Windows 生物特徵辨識架構的一部分。
-   呼叫 GetCredentials helper 函式，以取得包含驗證資訊的位元組陣列。 GetCredentials 會顯示一個對話方塊，提示使用者輸入認證。
-   呼叫 [**WinBioSetCredential**](/windows/desktop/api/Winbio/nf-winbio-winbiosetcredential) 以將認證儲存在存放區中。

若要編譯此函式連結至 Winbio .lib 靜態程式庫，並包含下列標頭檔：
...
### <a name="remove-a-credential"></a>移除認證

下列程式碼範例：

-   呼叫 GetCurrentUserIdentity 以取得目前使用者的 [**WINBIO \_ 識別**](winbio-identity.md) 物件。 GetCurrentUserIdentity 是 helper 函式，不是 Windows 生物特徵辨識架構的一部分。
-   呼叫 [**WinBioRemoveCredential**](/windows/desktop/api/Winbio/nf-winbio-winbioremovecredential) 來刪除存放區中的認證。
