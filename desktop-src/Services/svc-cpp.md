---
description: 以下是完整的服務範例。 將此程式碼作為範本使用時，只需將程式碼加入至前面要進行的區段 \_ 。
...
以下是完整的服務範例。 將此程式碼作為範本使用時，只需將程式碼加入開頭為的區段 `TO_DO` 。

建立範例時，請務必連結 Kernel32.dll .lib 和 Advapi32.dll。 建立僅限資源的 DLL 時，會產生 file 範例 .h，Sample.dll。 如需詳細資訊，請參閱 [Sample.mc](sample-mc.md)。
