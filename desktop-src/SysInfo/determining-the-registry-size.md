...
在 Windows 2000 上，安裝公用程式通常會檢查登錄的目前大小和大小上限，以判斷是否有足夠的可用空間來容納即將新增的新資料。 這個範例示範如何使用系統物件內的「% Registry 配額使用中」效能計數器，以程式設計方式執行此作業。

下列範例會使用效能資料協助程式 (PDH) 來取得計數器值;它必須與 Pdh 連結。 PDH 是一組高層級的 Api，用來取得效能資料。

> [!Note]  
> 不需要在 Windows Server 2003 或 Windows XP 上執行此登錄大小檢查，因為它們沒有登錄配額限制。
