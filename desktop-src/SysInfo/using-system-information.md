---
description: 使用系統資訊的流覽頁面。
ms.assetid: a20977f8-6c02-40c8-8335-d3400b1bcdb8
title: 使用系統資訊
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 40cf5ca456ecfdecc17cb0d6290853d8f865617d721c7c6e9187582a16b0b9ea
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "117957595"
---
# <a name="using-system-information"></a>使用系統資訊

下列主題描述如何使用系統資訊：

-   [取得系統資訊](getting-system-information.md)
-   [取得系統版本](getting-the-system-version.md)
-   [驗證系統版本](verifying-the-system-version.md)
-   [偵測產品套件](detecting-a-product-suite.md)
-   [取得硬體資訊](getting-hardware-information.md)
-   [從通用 Windows 應用程式存取 SMBIOS 資訊](access-smbios-information-from-a-universal-windows-app.md)
-   [從通用 Windows 應用程式存取 UEFI 固件變數](access-uefi-firmware-variables-from-a-universal-windows-app.md)

 

 



