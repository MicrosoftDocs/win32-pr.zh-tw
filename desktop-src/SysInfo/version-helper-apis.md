---
description: 您可以使用下列函式來判斷目前的作業系統版本，或識別它是 Windows 或 Windows 伺服器版本。
...
ms.locfileid: "118884169"
---
# <a name="version-helper-functions"></a>版本 Helper 函數

您可以使用下列函式來判斷目前的作業系統版本，或識別它是 Windows 或 Windows 伺服器版本。 這些函式會提供簡單的測試，以使用 [VerifyVersionInfo](/windows/win32/api/Winbase/nf-winbase-verifyversioninfoa) 函式和建議的大於或等於比較，以確定作業系統版本的健全方法。

> [!Note]  
> 這些 api 是由 Windows 8.1 軟體發展工具組 (SDK) 中包含的 **versionhelpers** 所定義。 這個檔案可以與其他 Microsoft Visual Studio 版本搭配使用，以針對 Windows 8.1 之前的 Windows 版本執行相同的功能。
...
<td>指出目前的作業系統版本是否符合或大於 Windows Vista Service Pack 1 (SP1) 版本。<br/></td>
</tr>
<tr class="odd">
...
<td>指出目前的作業系統是否為 Windows 的伺服器版本。 需要區分伺服器和用戶端版本 Windows 的應用程式應該呼叫這個函數。<br/></td>
</tr>
<tr class="even">
...
<br/>指出目前的作業系統版本是否符合或大於提供的版本資訊。 這項功能有助於確認未與用戶端版本共用版本號碼的 Windows Server 版本。
</td>
</tr>
</tbody>
</table>
...
**VersionHelpers .h** 標頭檔中定義的內嵌函式可讓您在測試版本的 Windows 時，藉由傳回 **布林** 值來確認作業系統版本。
