...
ms.locfileid: "118604430"
---
# <a name="change-method-of-the-win32_service-class-mbnapih---terminalservice"></a>Win32_Service 類別的 Change 方法 (Mbnapi) -TerminalService

**變更** [WMI 類別](/windows/desktop/WmiSdk/retrieving-a-class)方法會修改 [**Win32 \_ TerminalService**](win32-terminalservice.md)。
...
**正常。** 啟動會繼續。 在使用者登入之前，使用者會收到通知：「在啟動期間，至少有一個服務或裝置失敗。」
...
**嚴重。** 電腦嘗試以最後一個已知的正確設定重新開機。 如果服務再次失敗，則會繼續啟動，並將通知提供給使用者。
...
**關鍵。** 電腦嘗試以最後一個已知的正確設定重新開機。 如果服務再次失敗，啟動就會停止。
...
服務控制管理員會在系統啟動期間自動啟動服務。 自動啟動服務會在使用者登入電腦之前啟動，即使沒有任何使用者登入電腦，也會執行。
...
當進程呼叫 [**StartService**](win32-terminalservice-startservice.md) 方法時，服務控制管理員要啟動的服務。 雖然手動服務必須由使用者 (或腳本) 來明確啟動，但是即使使用者登出，還是會繼續執行。 手動服務通常稱為「隨選服務」。
...
無法再啟動的服務。 若要啟動已停用的服務，您必須先將 [啟動] 選項變更為 [自動] 或 [手動]。
...
有時，您可能需要變更指定服務執行時所使用的帳戶。 例如，您可以在系統管理帳戶下執行服務。 因為這可能會產生安全性弱點，您可能會將服務切換為具有較少許可權的帳戶。 或者，您可能在即將刪除的帳戶下執行服務，或者您可能想要確保在所有伺服器上，某些服務會在特定帳戶下執行。 您可以使用 [**Win32 \_ TerminalService**](win32-terminalservice.md)類別的 **Change** 方法，將服務設定為在指定的使用者帳戶下執行。 選取帳戶時，請記住下列事項：
...
[**Win32 \_ LoadOrderGroup**](/windows/desktop/CIMWin32Prov/win32-loadordergroup)參數代表定義執行相依性的一組系統服務。 服務必須依「載入順序」群組所指定的順序起始，因為服務會彼此相依。 這些相依服務需要有前面的服務才能正確運作。
