...
從報告 (RDS 每個使用者的 Cal) 取得遠端桌面服務的每個使用者用戶端存取授權摘要。
...
要從報表物件取出的報表專案數目。 值為零表示要抓取從 *StartIndex* 開始的所有報表專案。 傳回時，包含 *ReportSummaryEntries* 陣列中的專案數。

</dd> <dt>

*ReportSummaryEntries* \[擴展\]
</dt> <dd>

傳回 [**Win32 \_ TSLicenseReportSummaryEntry**](win32-tslicensereportsummaryentry.md) 類別的陣列。
