...
不支援這個屬性。 對 **ConnectToServerConsole** 的呼叫一律會傳回 **\_ FALSE**。

使用 [**ConnectToAdministerServer**](imsrdpclientadvancedsettings6-connecttoadministerserver.md) 屬性連接到用於管理用途的會話。
...
將此參數設定為 **VARIANT \_ FALSE**。 **變異 \_** 不支援 TRUE。
