...
---
# <a name="wsdl-interface-for-custom-vdi-solutions"></a><span data-ttu-id="e1854-104">自訂 VDI 解決方案的 WSDL 介面</span><span class="sxs-lookup"><span data-stu-id="e1854-104">WSDL Interface for Custom VDI Solutions</span></span>

<span data-ttu-id="e1854-105">開發人員可以建立自訂 web 服務，以管理 (VDI) 解決方案的虛擬桌面基礎結構。</span><span class="sxs-lookup"><span data-stu-id="e1854-105">Developers can create custom web services that manage virtual desktop infrastructure (VDI) solutions.</span></span>

<span data-ttu-id="e1854-106">從 Windows Server 2008 R2 開始，用戶端電腦可以使用 VMMWebServerClient.dll 篩選外掛程式，與執行虛擬機器管理的 web 服務進行通訊。</span><span class="sxs-lookup"><span data-stu-id="e1854-106">Beginning with Windows Server 2008 R2, client computers can communicate with web services that perform virtual machine management by using the VMMWebServerClient.dll filter plug-in.</span></span> <span data-ttu-id="e1854-107">您可以執行使用此篩選外掛程式的自訂 web 服務。</span><span class="sxs-lookup"><span data-stu-id="e1854-107">You can implement a custom web service that works with this filter plug-in.</span></span> <span data-ttu-id="e1854-108">若要與篩選外掛程式進行通訊，您的 web 服務必須執行下列 WSDL (Web 服務定義語言) 檔中定義的方法。</span><span class="sxs-lookup"><span data-stu-id="e1854-108">To communicate with the filter plug-in, your web service must implement the methods defined in the following WSDL (Web Services Definition Language) file.</span></span>
...
<span data-ttu-id="e1854-109">此 WSDL 檔案會定義下列方法：</span><span class="sxs-lookup"><span data-stu-id="e1854-109">This WSDL file defines the following methods:</span></span>

-   [<span data-ttu-id="e1854-110">GetVM</span><span class="sxs-lookup"><span data-stu-id="e1854-110">GetVM</span></span>](#getvm)
-   [<span data-ttu-id="e1854-111">JobUpdated</span><span class="sxs-lookup"><span data-stu-id="e1854-111">JobUpdated</span></span>](#jobupdated)
-   [<span data-ttu-id="e1854-112">PlaceVM</span><span class="sxs-lookup"><span data-stu-id="e1854-112">PlaceVM</span></span>](#placevm)
-   [<span data-ttu-id="e1854-113">SetLocale</span><span class="sxs-lookup"><span data-stu-id="e1854-113">SetLocale</span></span>](#setlocale)
-   [<span data-ttu-id="e1854-114">StartVM</span><span class="sxs-lookup"><span data-stu-id="e1854-114">StartVM</span></span>](#startvm)

## <a name="getvm"></a><span data-ttu-id="e1854-115">GetVM</span><span class="sxs-lookup"><span data-stu-id="e1854-115">GetVM</span></span>

<span data-ttu-id="e1854-116">篩選外掛程式會呼叫這個方法，以取得指定之虛擬機器的相關資訊。</span><span class="sxs-lookup"><span data-stu-id="e1854-116">The filter plug-in calls this method to obtain information about a specified virtual machine.</span></span> <span data-ttu-id="e1854-117">它應該會傳回虛擬機器的狀態、主機名稱、主機類型和 GUID。</span><span class="sxs-lookup"><span data-stu-id="e1854-117">It should return the virtual machine's state, host name, host type and GUID.</span></span>

<span data-ttu-id="e1854-118">篩選外掛程式會將下列元素傳遞給方法。</span><span class="sxs-lookup"><span data-stu-id="e1854-118">The filter plug-in passes the following element to the method.</span></span>
...
<span data-ttu-id="e1854-119">Web 服務必須將下列元素傳回至篩選外掛程式。</span><span class="sxs-lookup"><span data-stu-id="e1854-119">The web service must return the following element to the filter plug-in.</span></span>
...
## <a name="jobupdated"></a><span data-ttu-id="e1854-120">JobUpdated</span><span class="sxs-lookup"><span data-stu-id="e1854-120">JobUpdated</span></span>

<span data-ttu-id="e1854-121">Web 服務會呼叫這個方法，以通知篩選外掛程式對現有作業進行了變更。</span><span class="sxs-lookup"><span data-stu-id="e1854-121">The web service calls this method to notify the filter plug-in that changes were made to an existing job.</span></span> <span data-ttu-id="e1854-122">當您執行此方法時，如果函式成功，則傳回 **S \_ OK** 。</span><span class="sxs-lookup"><span data-stu-id="e1854-122">When you are implementing this method, return **S\_OK** if the function succeeds.</span></span> <span data-ttu-id="e1854-123">如果失敗，則傳回表示錯誤的 **HRESULT** 值。</span><span class="sxs-lookup"><span data-stu-id="e1854-123">If it fails, return an **HRESULT** value that indicates the error.</span></span> <span data-ttu-id="e1854-124">如需常見錯誤碼的清單，請參閱[常見的 HRESULT 值](/windows/desktop/SecCrypto/common-hresult-values)。</span><span class="sxs-lookup"><span data-stu-id="e1854-124">For a list of common error codes, see [Common HRESULT Values](/windows/desktop/SecCrypto/common-hresult-values)</span></span>

<span data-ttu-id="e1854-125">Web 服務會將下列元素傳遞至篩選外掛程式。</span><span class="sxs-lookup"><span data-stu-id="e1854-125">The web service passes the following element to the filter plug-in.</span></span>
...
## <a name="placevm"></a><span data-ttu-id="e1854-126">PlaceVM</span><span class="sxs-lookup"><span data-stu-id="e1854-126">PlaceVM</span></span>

<span data-ttu-id="e1854-127">篩選外掛程式會呼叫這個方法，將虛擬機器從程式庫遷移至主機電腦。</span><span class="sxs-lookup"><span data-stu-id="e1854-127">The filter plug-in calls this method to migrate a virtual machine from a library to a host computer.</span></span> <span data-ttu-id="e1854-128">方法應該產生作業，並將唯一的工作識別碼傳回至篩選外掛程式，以供追蹤之用。</span><span class="sxs-lookup"><span data-stu-id="e1854-128">The method should spawn a job and return a unique job identifier to the filter plug-in for tracking purposes.</span></span> <span data-ttu-id="e1854-129">當處理完成時，web 服務應該呼叫 JobUpdated。</span><span class="sxs-lookup"><span data-stu-id="e1854-129">When processing is complete, the web service should call JobUpdated.</span></span>

<span data-ttu-id="e1854-130">篩選外掛程式會將下列元素傳遞給方法。</span><span class="sxs-lookup"><span data-stu-id="e1854-130">The filter plug-in passes the following element to the method.</span></span>
...
<span data-ttu-id="e1854-131">Web 服務必須將下列元素傳回至篩選外掛程式。</span><span class="sxs-lookup"><span data-stu-id="e1854-131">The web service must return the following element to the filter plug-in.</span></span>
...
## <a name="setlocale"></a><span data-ttu-id="e1854-132">SetLocale</span><span class="sxs-lookup"><span data-stu-id="e1854-132">SetLocale</span></span>

<span data-ttu-id="e1854-133">篩選外掛程式會呼叫這個方法，以指定要用於錯誤字串的地區設定。</span><span class="sxs-lookup"><span data-stu-id="e1854-133">The filter plug-in calls this method to specify the locale to be used for error strings.</span></span>

<span data-ttu-id="e1854-134">篩選外掛程式會將下列元素傳遞給方法。</span><span class="sxs-lookup"><span data-stu-id="e1854-134">The filter plug-in passes the following element to the method.</span></span>
...
<span data-ttu-id="e1854-135">Web 服務必須將下列元素傳回至篩選外掛程式。</span><span class="sxs-lookup"><span data-stu-id="e1854-135">The web service must return the following element to the filter plug-in.</span></span>
...
## <a name="startvm"></a><span data-ttu-id="e1854-136">StartVM</span><span class="sxs-lookup"><span data-stu-id="e1854-136">StartVM</span></span>

<span data-ttu-id="e1854-137">篩選外掛程式會呼叫這個方法，以啟動目前主機電腦上的虛擬機器。</span><span class="sxs-lookup"><span data-stu-id="e1854-137">The filter plug-in calls this method to start the virtual machine on the current host computer.</span></span> <span data-ttu-id="e1854-138">如果無法在目前的主機電腦上啟動虛擬機器，這個方法應該將虛擬機器遷移至最可能的主機，然後啟動它。</span><span class="sxs-lookup"><span data-stu-id="e1854-138">If the virtual machine cannot be started on the current host computer, this method should migrate the virtual machine to the best possible host and then start it.</span></span> <span data-ttu-id="e1854-139">方法應該產生作業，並將唯一的工作識別碼傳回至篩選外掛程式，以供追蹤之用。</span><span class="sxs-lookup"><span data-stu-id="e1854-139">The method should spawn a job and return a unique job identifier to the filter plug-in for tracking purposes.</span></span> <span data-ttu-id="e1854-140">當處理完成時，web 服務應該呼叫 JobUpdated。</span><span class="sxs-lookup"><span data-stu-id="e1854-140">When processing is complete, the web service should call JobUpdated.</span></span>

<span data-ttu-id="e1854-141">篩選外掛程式會將下列元素傳遞給方法。</span><span class="sxs-lookup"><span data-stu-id="e1854-141">The filter plug-in passes the following element to the method.</span></span>
...
<span data-ttu-id="e1854-142">Web 服務必須將下列元素傳回至篩選外掛程式。</span><span class="sxs-lookup"><span data-stu-id="e1854-142">The web service must return the following element to the filter plug-in.</span></span>
