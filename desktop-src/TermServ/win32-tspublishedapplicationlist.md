...
---
# <a name="win32_tspublishedapplicationlist-class"></a>Win32 \_ TSPublishedApplicationList 類別

代表遠端桌面工作階段主機 (RD 工作階段主機) 伺服器上 [RemoteApp 程式] 清單中的程式清單。
...
**Win32 \_ TSPublishedApplicationList** 類別具有下列類型的成員：

-   [屬性](#properties)

### <a name="properties"></a>屬性

**Win32 \_ TSPublishedApplicationList** 類別具有這些屬性。
...
指出 RD 工作階段主機伺服器是否會在初始連線至 [RemoteApp 程式] 清單中的程式時，限制使用者可以啟動的程式。
...
您可以透過 RemoteApp 管理員或 RemoteApp WMI 提供者，在伺服器上設定此設定。
...
[ **已停用** ] 屬性不會防止使用者在使用 RemoteApp 程式連線到 RD 工作階段主機伺服器之後，從遠端啟動未列出的程式。

只有在下列登錄專案遺失時， **已停用** 的屬性才會設定為2的值：

**HKEY \_LOCAL \_ MACHINE** \\ **SOFTWARE** \\ **Microsoft** \\ **WindowsNT** \\ **CurrentVersion** \\ **TerminalServer** \\ **TSAppAllowList** \\ **fDisabledAllowList**
