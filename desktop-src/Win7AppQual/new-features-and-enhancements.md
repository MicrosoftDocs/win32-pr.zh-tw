---
description: Windows 7 和 Windows Server 2008 R2 包含許多增強功能，可改善使用者和開發人員的體驗。
ms.assetid: 6f168d39-5c1e-4bbd-9e99-59318eb58074
title: 新功能-Windows 7 相容性
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 2beb97a4cfc834e927a974c330a22e81a74af2ce
ms.sourcegitcommit: 831e8f3db78ab820e1710cede244553c70e50500
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 01/08/2021
ms.locfileid: "103851244"
---
# <a name="new-features-and-enhancements-in-windows-7"></a>Windows 7 的新功能和增強功能

Windows 7 和 Windows Server 2008 R2 包含許多增強功能，可改善使用者和開發人員的體驗。 不過，其中有幾個會影響開發人員必須留意現有和新產品的相關資訊。 以下列出新功能和增強功能：

-   [檔案庫取代檔資料夾](file-library-replaces-document-folder.md)
-   [使用者介面 - 高 DPI 感知](user-interface---high-dpi-awareness.md)
-   [容錯堆積](fault-tolerant-heap.md)
-   [Internet Explorer 8-資料執行防止/NX](internet-explorer-8---data-execution-protection-nx.md)
-   [動態記憶體](dynamic-memory.md)
-   [消費者介面-使用者帳戶控制對話方塊更新](user-interface---user-account-control-dialog-updates.md)
-   [ChooseFont () Win32 通用對話方塊](choosefont-win32-common-dialog.md)
-   [應用程式資訊清單](compatibility---application-manifest.md)
-   [伺服器 Hyper-v](server-hyper-v.md)
-   [啟用 Intel AVX 的 Windows 7 支援](enable-windows-7-support-for-intel-avx.md)
-   [Microsoft Message Queuing (MSMQ) 改善的佇列處理](microsoft-message-queuing--msmq----improved-queue-handling.md)
-   [遠端桌面服務](terminal-services.md)

 

 
