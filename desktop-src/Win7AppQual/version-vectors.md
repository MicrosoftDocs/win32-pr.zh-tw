...
在此情況下，如果 Windows Internet Explorer 瀏覽器版本至少為5.5，則對應的段落會顯示在網頁中。 雖然此範例中的第一個條件說明條件式批註的函式，但這些批註通常不會用來顯示第一個條件之類的標記。 相反地，上述範例中的其餘條件式批註更為常見。 在這些其餘的批註中，條件式批註會針對不同版本的瀏覽器使用不同的樣式表單。
