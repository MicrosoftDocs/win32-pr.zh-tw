...
ms.locfileid: "119795708"
---
# <a name="dns-wmi-provider-samplesmanaging-a-dns-server"></a>DNS WMI 提供者範例—管理 DNS 伺服器

> [!NOTE]
> 本文包含「從屬」一詞的參考，Microsoft 已不再使用該字詞。 從軟體中移除該字詞時，我們也會將其從本文中移除。

本節將示範與管理 DNS 伺服器相關聯的腳本工作。 下列連結會跳到腳本檔案中的副程式：

-   [列出 DNS 伺服器屬性](#list-dns-server-properties)
-   [停止、啟動或重新開機 DNS 伺服器](#stop-start-or-restart-a-dns-server)
-   [修改 DNS 伺服器屬性](#modify-dns-server-properties)
-   [新增 IP 位址](#add-an-ip-address)
-   [刪除 IP 位址](#delete-an-ip-address)
-   [列出區域](#list-zones)
...
## <a name="list-dns-server-properties"></a>列出 DNS 伺服器屬性
...
## <a name="stop-start-or-restart-a-dns-server"></a>停止、啟動或重新開機 DNS 伺服器
...
## <a name="modify-dns-server-properties"></a>修改 DNS 伺服器屬性
...
## <a name="add-an-ip-address"></a>新增 IP 位址
...
## <a name="delete-an-ip-address"></a>刪除 IP 位址
