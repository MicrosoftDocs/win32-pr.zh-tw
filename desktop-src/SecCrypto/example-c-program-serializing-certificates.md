...
ms.locfileid: "119007516"
---
# <a name="example-c-program-serializing-certificates"></a>範例 C 程式：序列化憑證

下列範例示範將 [*憑證內容*](../secgloss/c-gly.md) 和其內容序列化成可儲存在檔案中的表單、以電子郵件訊息傳送，或是傳送給其他使用者。 此範例也會顯示如何將序列化的憑證變更回憑證，並將其新增至憑證存放區。 相同的程式也適用于使用 [**CertSerializeCRLStoreElement**](/windows/desktop/api/Wincrypt/nf-wincrypt-certserializecrlstoreelement)和 [**CertSerializeCTLStoreElement**](/windows/desktop/api/Wincrypt/nf-wincrypt-certserializectlstoreelement)的 [*crl*](../secgloss/c-gly.md)和 [*ctl*](../secgloss/c-gly.md) 。
...
-   使用 [**CertEnumCertificatesInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore)從存放區中取出憑證。
-   使用 [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa)取得憑證主體的名稱。
-   使用 [**CertSerializeCertificateStoreElement**](/windows/desktop/api/Wincrypt/nf-wincrypt-certserializecertificatestoreelement)建立已序列化形式的 [*憑證內容*](../secgloss/c-gly.md)及其屬性。
-   使用 [**CertAddSerializedElementToStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certaddserializedelementtostore)從序列化字串建立新的憑證，並將它新增至憑證存放區。
-   使用 [**CertAddEncodedCertificateToStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certaddencodedcertificatetostore) ，從現有憑證的編碼部分建立新的憑證。
