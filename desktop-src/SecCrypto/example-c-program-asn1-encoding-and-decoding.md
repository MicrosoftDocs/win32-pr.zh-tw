...
下列範例示範如何使用 [**CryptEncodeObjectEx**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencodeobjectex) 和 [**CryptDecodeObjectEx**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecodeobjectex)。 您可以輕鬆地修改此範例，以使用 [**CryptEncodeObject**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencodeobject) 和 [**CryptDecodeObject**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecodeobject)。

此範例也會使用修改過的函式版本 [**ByteToStr**](bytetostr.md) 來列印 [*抽象語法標記法 (一)*](../secgloss/a-gly.md) (asn.1) 編碼的八位數列。 它也會使用 [**MyHandleError**](myhandleerror.md)。 範例中包含這些函式的程式碼。
