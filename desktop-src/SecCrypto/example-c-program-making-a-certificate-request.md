...
ms.locfileid: "119140891"
---
# <a name="example-c-program-making-a-certificate-request"></a>範例 C 程式：提出憑證要求

下列範例示範上一節中所述的程式。 這個範例會建立一個簡單的 [*憑證要求*](../secgloss/c-gly.md) ，其中包含一個簽署者、單一 [*相對辨別名稱*](../secgloss/r-gly.md) (RDN) [*屬性*](../secgloss/a-gly.md)，以及沒有一般屬性。

此範例說明下列 [*CryptoAPI*](../secgloss/c-gly.md) 函數：
...
此範例也會使用 [**ByteToStr**](bytetostr.md) 和 [**MyHandleError**](myhandleerror.md)函數。 範例中包含這些函式的程式碼。 [一般目的函數](general-purpose-functions.md) 會列出這些和其他輔助函式的程式碼。
