...
ms.locfileid: "119007716"
---
# <a name="example-c-program-encoding-and-decoding-a-hashed-message"></a>範例 C 程式：編碼和解碼雜湊訊息

下列範例會對文字訊息進行 [*雜湊*](../secgloss/h-gly.md) 和編碼，然後再解碼並驗證訊息。

雖然為了簡單起見，這兩個不同的函式在此範例中是合併的，但在更實際的設定中，這兩個部分會分開使用。

此範例說明下列工作和 CryptoAPI 函數：

-   呼叫 [**CryptAcquireCoNtext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta) 以取得 CSP 提供者。
-   使用 [**CryptMsgCalculateEncodedLength**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcalculateencodedlength) 來計算編碼訊息的長度。
-   為緩衝區配置記憶體以保存編碼的資料。
-   使用 [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode)開啟要編碼的訊息。
-   使用 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)將內容新增至訊息以進行編碼。
-   使用 [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam) 將編碼的訊息複製到配置的緩衝區。
-   使用 [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode)開啟要解碼的訊息。
-   將編碼的訊息新增至要使用 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)解碼的訊息。
-   使用 [**CryptMsgDuplicate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgduplicate)建立訊息的重複指標。
-   使用 [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam)檢查訊息類型。
-   使用 [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam) 來解碼訊息。
-   使用 [**CryptMsgControl**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcontrol)驗證雜湊。
-   使用 [**CryptMsgClose**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgclose) 釋放訊息控制碼。
