---
description: 您可以使用 CryptSetProvParam 函數修改 PP 索引 \_ 鍵集的 \_ SEC 描述參數，以程式設計方式取得 LocalService 或 NetworkService 帳戶中的金鑰存取權 \_ 。
...
---
# <a name="modifying-key-container-access"></a><span data-ttu-id="e3642-103">修改金鑰容器存取</span><span class="sxs-lookup"><span data-stu-id="e3642-103">Modifying Key Container Access</span></span>

<span data-ttu-id="e3642-104">CryptoAPI 所建立的預設金鑰容器不允許存取 LocalService 或 NetworkService 帳戶中的金鑰。</span><span class="sxs-lookup"><span data-stu-id="e3642-104">The default key container that is created by CryptoAPI does not allow access to the keys from the LocalService or NetworkService accounts.</span></span> <span data-ttu-id="e3642-105">您可以使用 [**CryptSetProvParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetprovparam) 函式來修改 **PP 索引 \_ 鍵集的 \_ SEC \_ 描述** 參數，以程式設計方式更正這項功能。</span><span class="sxs-lookup"><span data-stu-id="e3642-105">This can be corrected programmatically by using the [**CryptSetProvParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetprovparam) function to modify the **PP\_KEYSET\_SEC\_DESCR** parameter.</span></span>

<span data-ttu-id="e3642-106">下列範例示範如何使用 [**CryptSetProvParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetprovparam) 函數修改 **PP 索引 \_ 鍵集的 \_ SEC \_ 描述** ，以允許存取 LocalService 或 NetworkService 帳戶的金鑰容器。</span><span class="sxs-lookup"><span data-stu-id="e3642-106">The following example shows how to use the [**CryptSetProvParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetprovparam) function to modify the **PP\_KEYSET\_SEC\_DESCR** to allow access to a key container to the LocalService or NetworkService accounts.</span></span>

> [!Note]  
> <span data-ttu-id="e3642-107">下列程式碼是以工具的形式提供，只有在絕對必要時才使用。</span><span class="sxs-lookup"><span data-stu-id="e3642-107">The following code is given as a tool and should only be used if absolutely necessary.</span></span> <span data-ttu-id="e3642-108">您只需在每部電腦上執行此程式碼一次，即可允許存取金鑰。</span><span class="sxs-lookup"><span data-stu-id="e3642-108">You should only have to run this code once on each computer to allow access to the keys.</span></span>
