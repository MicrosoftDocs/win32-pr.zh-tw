...
CryptoAPI 所建立的預設金鑰容器不允許存取 LocalService 或 NetworkService 帳戶中的金鑰。 您可以使用 [**CryptSetProvParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetprovparam) 函式來修改 **PP 索引 \_ 鍵集的 \_ SEC \_ 描述** 參數，以程式設計方式更正這項功能。

下列範例示範如何使用 [**CryptSetProvParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetprovparam) 函數修改 **PP 索引 \_ 鍵集的 \_ SEC \_ 描述** ，以允許存取 LocalService 或 NetworkService 帳戶的金鑰容器。

> [!Note]  
> 下列程式碼是以工具的形式提供，只有在絕對必要時才使用。 您只需在每部電腦上執行此程式碼一次，即可允許存取金鑰。
