...
---
# <a name="example-c-program-working-with-key-identifiers"></a>範例 C 程式：使用金鑰識別碼

下列範例示範使用金鑰識別碼的方式。 此範例說明下列工作和 CryptoAPI 函數：

-   使用 [**CryptCreateKeyIdentifierFromCSP**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptcreatekeyidentifierfromcsp)建立金鑰識別碼。
-   使用 [**CryptSetKeyIdentifierProperty**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetkeyidentifierproperty)設定金鑰識別碼的屬性。
-   使用 [**CryptGetKeyIdentifierProperty**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetkeyidentifierproperty)抓取金鑰識別碼屬性的內容。
-   使用 [**CryptEnumKeyIdentifierProperties**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumkeyidentifierproperties)列出金鑰識別碼的屬性。
-   宣告、定義和使用回呼函數。
