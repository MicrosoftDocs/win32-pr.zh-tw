...
下列範例會將資料檔案加密。 此範例會以互動方式要求包含要加密之 [*純文字*](../secgloss/p-gly.md) 的檔案名，以及要寫入加密資料的檔案名稱。

此範例會提示使用者輸入檔案和輸出檔案的名稱。 它也會提示使用者是否要使用密碼來建立加密工作階段金鑰。 如果要在資料加密中使用密碼，則必須在解密檔案的程式中使用相同的密碼。 如需詳細資訊，請參閱 [範例 C 程式：解密](example-c-program-decrypting-a-file.md)檔案。
