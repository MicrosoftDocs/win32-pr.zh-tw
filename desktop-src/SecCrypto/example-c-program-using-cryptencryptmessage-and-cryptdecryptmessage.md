...
ms.locfileid: "104560920"
---
# <a name="example-c-program-using-cryptencryptmessage-and-cryptdecryptmessage"></a><span data-ttu-id="b6684-103">範例 C 程式：使用 CryptEncryptMessage 和 CryptDecryptMessage</span><span class="sxs-lookup"><span data-stu-id="b6684-103">Example C Program: Using CryptEncryptMessage and CryptDecryptMessage</span></span>

<span data-ttu-id="b6684-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) 是完成 [加密訊息](../secauthn/encrypting-a-message.md)中列出的所有工作所需的唯一函式呼叫。</span><span class="sxs-lookup"><span data-stu-id="b6684-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) is the only function call necessary to accomplish all of the tasks listed in [Encrypting a Message](../secauthn/encrypting-a-message.md).</span></span> <span data-ttu-id="b6684-105">需要初始化資料結構。</span><span class="sxs-lookup"><span data-stu-id="b6684-105">Initialization of data structures is necessary.</span></span> <span data-ttu-id="b6684-106">下圖顯示這些函式參數之間的關聯性，這些參數會指向結構或陣列與其初始化的資料。</span><span class="sxs-lookup"><span data-stu-id="b6684-106">The following illustration shows the relationship between those function parameters that point to structures or arrays and their initialized data.</span></span> <span data-ttu-id="b6684-107">此範例也會使用 [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage)來解密訊息。</span><span class="sxs-lookup"><span data-stu-id="b6684-107">This example also decrypts the message using [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span></span>

![呼叫 cryptencryptmessage 的初始化對應](images/crypenc.png)

<span data-ttu-id="b6684-109">**加密資料**</span><span class="sxs-lookup"><span data-stu-id="b6684-109">**To encrypt data**</span></span>

1.  <span data-ttu-id="b6684-110">取得要加密之內容的指標。</span><span class="sxs-lookup"><span data-stu-id="b6684-110">Get a pointer to the content to be encrypted.</span></span>
2.  <span data-ttu-id="b6684-111">判斷要加密的內容大小。</span><span class="sxs-lookup"><span data-stu-id="b6684-111">Determine the size of the content to be encrypted.</span></span>
3.  <span data-ttu-id="b6684-112">取得密碼編譯提供者的控制碼。</span><span class="sxs-lookup"><span data-stu-id="b6684-112">Acquire a handle to a cryptographic provider.</span></span>
4.  <span data-ttu-id="b6684-113">開啟 [*憑證存放區*](../secgloss/c-gly.md)。</span><span class="sxs-lookup"><span data-stu-id="b6684-113">Open a [*certificate store*](../secgloss/c-gly.md).</span></span>
5.  <span data-ttu-id="b6684-114">取得收件者憑證。</span><span class="sxs-lookup"><span data-stu-id="b6684-114">Get the recipient certificate.</span></span>
6.  <span data-ttu-id="b6684-115">建立收件者憑證陣列。</span><span class="sxs-lookup"><span data-stu-id="b6684-115">Create the recipient certificate array.</span></span>
7.  <span data-ttu-id="b6684-116">將 [**CRYPT \_ 演算法 \_ 識別碼**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) 結構初始化。</span><span class="sxs-lookup"><span data-stu-id="b6684-116">Initialize the [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) structure.</span></span>
8.  <span data-ttu-id="b6684-117">將 [**CRYPT \_ 加密的 \_ 訊息 \_ 段落**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) 結構初始化。</span><span class="sxs-lookup"><span data-stu-id="b6684-117">Initialize the [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) structure.</span></span>
9.  <span data-ttu-id="b6684-118">呼叫 [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) 來加密內容，並建立數位的封包訊息。</span><span class="sxs-lookup"><span data-stu-id="b6684-118">Call [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) to encrypt the content and create a digitally enveloped message.</span></span>

<span data-ttu-id="b6684-119">下列範例會執行此程式。</span><span class="sxs-lookup"><span data-stu-id="b6684-119">The following example implements this procedure.</span></span>

<span data-ttu-id="b6684-120">批註會將程式碼片段與程式中的每個步驟產生關聯。</span><span class="sxs-lookup"><span data-stu-id="b6684-120">Comments relate code fragments to each step in the procedure.</span></span> <span data-ttu-id="b6684-121">如需有關函數的詳細資訊，請參閱 [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage)。</span><span class="sxs-lookup"><span data-stu-id="b6684-121">For more information about the function, see [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span></span> <span data-ttu-id="b6684-122">如需有關資料結構的詳細資訊，請參閱 [**CRYPT \_ 演算法 \_ 識別碼**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) 和 [**CRYPT \_ 加密 \_ 訊息 \_ 段落**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para)。</span><span class="sxs-lookup"><span data-stu-id="b6684-122">For more information about the data structures, see [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) and [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span></span>

<span data-ttu-id="b6684-123">此範例會使用 [**MyHandleError**](myhandleerror.md) 和 [**ByteToStr**](bytetostr.md)函數。</span><span class="sxs-lookup"><span data-stu-id="b6684-123">This example uses the functions [**MyHandleError**](myhandleerror.md) and [**ByteToStr**](bytetostr.md).</span></span> <span data-ttu-id="b6684-124">這個函式的程式碼會包含在範例中。</span><span class="sxs-lookup"><span data-stu-id="b6684-124">The code for this function is included with the sample.</span></span> <span data-ttu-id="b6684-125">這項功能和其他輔助函式的程式碼也會列在 [一般用途](general-purpose-functions.md)的函式底下。</span><span class="sxs-lookup"><span data-stu-id="b6684-125">Code for this and other auxiliary functions is also listed under [General Purpose Functions](general-purpose-functions.md).</span></span>
