...
---
# <a name="example-c-program-using-cryptencryptmessage-and-cryptdecryptmessage"></a>範例 C 程式：使用 CryptEncryptMessage 和 CryptDecryptMessage

[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) 是完成 [加密訊息](../secauthn/encrypting-a-message.md)中列出的所有工作所需的唯一函式呼叫。 需要初始化資料結構。 下圖顯示這些函式參數之間的關聯性，這些參數會指向結構或陣列與其初始化的資料。 此範例也會使用 [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage)來解密訊息。
...
2.  判斷要加密的內容大小。
3.  取得密碼編譯提供者的控制碼。
4.  開啟 [*憑證存放區*](../secgloss/c-gly.md)。
5.  取得收件者憑證。
6.  建立收件者憑證陣列。
7.  將 [**CRYPT \_ 演算法 \_ 識別碼**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) 結構初始化。
8.  將 [**CRYPT \_ 加密的 \_ 訊息 \_ 段落**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) 結構初始化。
9.  呼叫 [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) 來加密內容，並建立數位的封包訊息。

下列範例會執行此程式。

批註會將程式碼片段與程式中的每個步驟產生關聯。 如需有關函數的詳細資訊，請參閱 [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage)。 如需有關資料結構的詳細資訊，請參閱 [**CRYPT \_ 演算法 \_ 識別碼**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) 和 [**CRYPT \_ 加密 \_ 訊息 \_ 段落**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para)。

此範例會使用 [**MyHandleError**](myhandleerror.md) 和 [**ByteToStr**](bytetostr.md)函數。 這個函式的程式碼會包含在範例中。 這項功能和其他輔助函式的程式碼也會列在 [一般用途](general-purpose-functions.md)的函式底下。
