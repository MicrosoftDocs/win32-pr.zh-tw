...
---
# <a name="example-c-program-signing-encoding-decoding-and-verifying-a-message"></a><span data-ttu-id="e3090-103">範例 C 程式：簽署、編碼、解碼和驗證訊息</span><span class="sxs-lookup"><span data-stu-id="e3090-103">Example C Program: Signing, Encoding, Decoding, and Verifying a Message</span></span>

<span data-ttu-id="e3090-104">下列範例結合了簽署和編碼訊息，以及解碼已簽署的訊息和驗證簽章。</span><span class="sxs-lookup"><span data-stu-id="e3090-104">The following example combines signing and encoding a message, and decoding a signed message and verifying the signature.</span></span> <span data-ttu-id="e3090-105">這兩項作業通常會在個別的程式中。</span><span class="sxs-lookup"><span data-stu-id="e3090-105">The two operations would usually be in separate programs.</span></span> <span data-ttu-id="e3090-106">編碼範例會建立編碼的訊息、將它儲存到磁片檔案，或以其他方式將它傳送給另一位使用者。</span><span class="sxs-lookup"><span data-stu-id="e3090-106">The encoding example would create the encoded message, save it to a disk file or in some other way send it to another user.</span></span> <span data-ttu-id="e3090-107">解碼範例會接收編碼的訊息、將其解碼，以及驗證簽章。</span><span class="sxs-lookup"><span data-stu-id="e3090-107">The decoding example would receive the encoded message, decode it, and verify the signature.</span></span> <span data-ttu-id="e3090-108">這兩個程式已結合在這裡，以顯示這兩個程式的運作。</span><span class="sxs-lookup"><span data-stu-id="e3090-108">The two processes have been combined here to show both procedures working.</span></span>

<span data-ttu-id="e3090-109">簽署及編碼訊息並不會確保該訊息的隱私權。</span><span class="sxs-lookup"><span data-stu-id="e3090-109">Signing and encoding a message does not ensure privacy for that message.</span></span> <span data-ttu-id="e3090-110">相反地，它可確保訊息的真實性。</span><span class="sxs-lookup"><span data-stu-id="e3090-110">Rather it ensures the authenticity of the message.</span></span> <span data-ttu-id="e3090-111">由於訊息是以寄件者的私密金鑰簽署，因此當訊息的接收者使用傳送者的 [*公開金鑰*](../secgloss/p-gly.md) 解密簽章時 (可從隨訊息) 傳送的憑證中取得，因此接收者可以確定訊息是由與憑證相關聯的人員或實體所傳送，而且訊息在簽署後並未變更。</span><span class="sxs-lookup"><span data-stu-id="e3090-111">Because the message is signed with the sender's private key, when the receiver of the message decrypts the signature with the sender's [*public key*](../secgloss/p-gly.md) (available from the certificate that is sent along with the message), the receiver can be sure that the message was sent by the person or entity associated with the certificate and that the message was not changed after it was signed.</span></span>

<span data-ttu-id="e3090-112">此範例說明下列用於編碼訊息的工作和 CryptoAPI 函式：</span><span class="sxs-lookup"><span data-stu-id="e3090-112">This example illustrates the following tasks and CryptoAPI functions for encoding a message:</span></span>

-   <span data-ttu-id="e3090-113">使用 [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore)開啟憑證存放區。</span><span class="sxs-lookup"><span data-stu-id="e3090-113">Opening a certificate store using [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore).</span></span>
-   <span data-ttu-id="e3090-114">使用 [**CertFindCertificateInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore)來抓取具有特定主體名稱的憑證。</span><span class="sxs-lookup"><span data-stu-id="e3090-114">Retrieving a certificate with a specific subject name using [**CertFindCertificateInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore).</span></span>
-   <span data-ttu-id="e3090-115">使用 [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa)取得和列印憑證的主體名稱。</span><span class="sxs-lookup"><span data-stu-id="e3090-115">Getting and printing a certificate's subject name using [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa).</span></span>
-   <span data-ttu-id="e3090-116">初始化要在呼叫 [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage)時使用的 [**CRYPT \_ 符號 \_ 訊息 \_ 段落**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_sign_message_para)結構。</span><span class="sxs-lookup"><span data-stu-id="e3090-116">Initializing a [**CRYPT\_SIGN\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_sign_message_para) structure to be used in a call to [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage).</span></span>
-   <span data-ttu-id="e3090-117">使用 [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage)簽署及編碼訊息。</span><span class="sxs-lookup"><span data-stu-id="e3090-117">Signing and encoding a message with [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage).</span></span>

<span data-ttu-id="e3090-118">此範例說明用於解碼訊息和驗證簽章的下列工作和 CryptoAPI 函式：</span><span class="sxs-lookup"><span data-stu-id="e3090-118">This example illustrates the following tasks and CryptoAPI functions for decoding a message and verifying the signature:</span></span>

-   <span data-ttu-id="e3090-119">開啟使用 [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode)解碼的訊息。</span><span class="sxs-lookup"><span data-stu-id="e3090-119">Opening a message for decoding with [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode).</span></span>
-   <span data-ttu-id="e3090-120">將編碼的 [*BLOB*](../secgloss/b-gly.md) 新增至要使用 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)解碼的訊息。</span><span class="sxs-lookup"><span data-stu-id="e3090-120">Adding the encoded [*BLOB*](../secgloss/b-gly.md) to the message to be decoded by using [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate).</span></span>
-   <span data-ttu-id="e3090-121">使用 [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam)解碼訊息。</span><span class="sxs-lookup"><span data-stu-id="e3090-121">Decoding the message using [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam).</span></span>
-   <span data-ttu-id="e3090-122">使用已接收和解碼的訊息，以 [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore) 開啟記憶體中的憑證存放區。</span><span class="sxs-lookup"><span data-stu-id="e3090-122">Opening a certificate store in memory with [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore) using the message received and decoded.</span></span>
-   <span data-ttu-id="e3090-123">使用 [**CertGetSubjectCertificateFromStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetsubjectcertificatefromstore) 取得訊息簽署者的憑證。</span><span class="sxs-lookup"><span data-stu-id="e3090-123">Using [**CertGetSubjectCertificateFromStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetsubjectcertificatefromstore) to get the certificate of the message's signer.</span></span>
-   <span data-ttu-id="e3090-124">使用 [**CryptMsgControl**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcontrol)驗證訊息的簽章。</span><span class="sxs-lookup"><span data-stu-id="e3090-124">Verifying a message's signature using [**CryptMsgControl**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcontrol).</span></span>
-   <span data-ttu-id="e3090-125">釋放記憶體、關閉 [*憑證存放區*](../secgloss/c-gly.md)，以及釋放 [*憑證內容*](../secgloss/c-gly.md)。</span><span class="sxs-lookup"><span data-stu-id="e3090-125">Freeing memory, closing [*certificate stores*](../secgloss/c-gly.md), and freeing [*certificate context*](../secgloss/c-gly.md).</span></span>

<span data-ttu-id="e3090-126">如需如何使用資料流程回呼執行這些類似作業的範例，請參閱 [C 程式範例：使用資料流程編碼和解碼訊息](example-c-program--encoding-and-decoding-a-message-using-a-stream.md)。</span><span class="sxs-lookup"><span data-stu-id="e3090-126">For an example of how to perform these similar operations using a stream callback, see [Example C Program: Encoding and Decoding a Message Using a Stream](example-c-program--encoding-and-decoding-a-message-using-a-stream.md).</span></span>

<span data-ttu-id="e3090-127">這個範例會使用 [**MyHandleError**](myhandleerror.md)函數。</span><span class="sxs-lookup"><span data-stu-id="e3090-127">This example uses the function [**MyHandleError**](myhandleerror.md).</span></span> <span data-ttu-id="e3090-128">此範例包含此函式的程式碼。</span><span class="sxs-lookup"><span data-stu-id="e3090-128">Code for this function is included with the sample.</span></span> <span data-ttu-id="e3090-129">這項功能和其他輔助函式的程式碼也會列在 [一般 \_ 用途 \_ ](general-purpose-functions.md)的函式底下。</span><span class="sxs-lookup"><span data-stu-id="e3090-129">Code for this and other auxiliary functions is also listed under [General\_Purpose\_Functions](general-purpose-functions.md).</span></span>
