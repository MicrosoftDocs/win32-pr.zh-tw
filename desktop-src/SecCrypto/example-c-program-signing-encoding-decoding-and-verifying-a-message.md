...
ms.locfileid: "119007466"
---
# <a name="example-c-program-signing-encoding-decoding-and-verifying-a-message"></a>範例 C 程式：簽署、編碼、解碼和驗證訊息

下列範例結合了簽署和編碼訊息，以及解碼已簽署的訊息和驗證簽章。 這兩項作業通常會在個別的程式中。 編碼範例會建立編碼的訊息、將它儲存到磁片檔案，或以其他方式將它傳送給另一位使用者。 解碼範例會接收編碼的訊息、將其解碼，以及驗證簽章。 這兩個程式已結合在這裡，以顯示這兩個程式的運作。
...
此範例說明下列用於編碼訊息的工作和 CryptoAPI 函式：
...
-   初始化要在呼叫 [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage)時使用的 [**CRYPT \_ 符號 \_ 訊息 \_ 段落**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_sign_message_para)結構。
-   使用 [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage)簽署及編碼訊息。

此範例說明用於解碼訊息和驗證簽章的下列工作和 CryptoAPI 函式：

-   開啟使用 [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode)解碼的訊息。
-   將編碼的 [*BLOB*](../secgloss/b-gly.md) 新增至要使用 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)解碼的訊息。
-   使用 [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam)解碼訊息。
-   使用已接收和解碼的訊息，以 [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore) 開啟記憶體中的憑證存放區。
-   使用 [**CertGetSubjectCertificateFromStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetsubjectcertificatefromstore) 取得訊息簽署者的憑證。
-   使用 [**CryptMsgControl**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcontrol)驗證訊息的簽章。
-   釋放記憶體、關閉 [*憑證存放區*](../secgloss/c-gly.md)，以及釋放 [*憑證內容*](../secgloss/c-gly.md)。

如需如何使用資料流程回呼執行這些類似作業的範例，請參閱 [C 程式範例：使用資料流程編碼和解碼訊息](example-c-program--encoding-and-decoding-a-message-using-a-stream.md)。
