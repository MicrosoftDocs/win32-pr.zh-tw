...
---
# <a name="rsa-server-code-for-creating-the-master-key"></a><span data-ttu-id="5b2ee-103">用來建立主要金鑰的 RSA 伺服器程式碼</span><span class="sxs-lookup"><span data-stu-id="5b2ee-103">RSA Server Code for Creating the Master Key</span></span>

<span data-ttu-id="5b2ee-104">下列範例顯示用來建立 [*主要金鑰*](../secgloss/m-gly.md)的一般 RSA/Schannel 伺服器端程式碼。</span><span class="sxs-lookup"><span data-stu-id="5b2ee-104">The following example shows typical RSA/Schannel server-side code for creating a [*master key*](../secgloss/m-gly.md).</span></span>
