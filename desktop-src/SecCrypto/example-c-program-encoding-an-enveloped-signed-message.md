...
下列範例會建立、簽署和建立訊息的信封，並說明下列工作和 [*CryptoAPI*](../secgloss/c-gly.md) 函數：

-   使用 [**CryptAcquireCoNtext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta)取得 CSP 的控制碼。
-   使用 [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore)開啟系統存放區。
-   使用 [**CertFindCertificateInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore)尋找簽署者和收件者憑證。
-   初始化適當的資料結構以簽署封裝的訊息。
-   使用 [**CryptMsgCalculateEncodedLength**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcalculateencodedlength)尋找封包訊息的長度。
-   使用 [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode)、 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)和 [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam)建立和簽署訊息。
-   使用 [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode)、 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)和 [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam)，封套接收者的已簽署和編碼訊息。

如果沒有可用的私密金鑰存在於預設 [*金鑰容器*](../secgloss/k-gly.md)中，此範例將會失敗。 如果所需的私密金鑰無法使用，則使用 [**CryptAcquireCertificatePrivateKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecertificateprivatekey)的程式碼（如程式碼範例 [C 程式：傳送和接收已簽署和已加密的訊息](example-c-program-sending-and-receiving-a-signed-and-encrypted-message.md)）中所示範，可以使用。
