...
---
# <a name="example-c-program-encoding-an-enveloped-signed-message"></a><span data-ttu-id="9e930-103">範例 C 程式：編碼封包、簽署的訊息</span><span class="sxs-lookup"><span data-stu-id="9e930-103">Example C Program: Encoding an Enveloped, Signed Message</span></span>

<span data-ttu-id="9e930-104">下列範例會建立、簽署和建立訊息的信封，並說明下列工作和 [*CryptoAPI*](../secgloss/c-gly.md) 函數：</span><span class="sxs-lookup"><span data-stu-id="9e930-104">The following example creates, signs, and envelopes a message, and it illustrates the following tasks and [*CryptoAPI*](../secgloss/c-gly.md) functions:</span></span>

-   <span data-ttu-id="9e930-105">使用 [**CryptAcquireCoNtext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta)取得 CSP 的控制碼。</span><span class="sxs-lookup"><span data-stu-id="9e930-105">Acquiring the handle of a CSP using [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta).</span></span>
-   <span data-ttu-id="9e930-106">使用 [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore)開啟系統存放區。</span><span class="sxs-lookup"><span data-stu-id="9e930-106">Opening a system store using [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore).</span></span>
-   <span data-ttu-id="9e930-107">使用 [**CertFindCertificateInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore)尋找簽署者和收件者憑證。</span><span class="sxs-lookup"><span data-stu-id="9e930-107">Finding a signer and recipient certificates using [**CertFindCertificateInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore).</span></span>
-   <span data-ttu-id="9e930-108">初始化適當的資料結構以簽署封裝的訊息。</span><span class="sxs-lookup"><span data-stu-id="9e930-108">Initializing appropriate data structures for signing an enveloped message.</span></span>
-   <span data-ttu-id="9e930-109">使用 [**CryptMsgCalculateEncodedLength**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcalculateencodedlength)尋找封包訊息的長度。</span><span class="sxs-lookup"><span data-stu-id="9e930-109">Finding the length of the enveloped message using [**CryptMsgCalculateEncodedLength**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgcalculateencodedlength).</span></span>
-   <span data-ttu-id="9e930-110">使用 [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode)、 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)和 [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam)建立和簽署訊息。</span><span class="sxs-lookup"><span data-stu-id="9e930-110">Creating and signs the message using [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode), [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate), and [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam).</span></span>
-   <span data-ttu-id="9e930-111">使用 [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode)、 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)和 [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam)，封套接收者的已簽署和編碼訊息。</span><span class="sxs-lookup"><span data-stu-id="9e930-111">Enveloping the signed and encoded message for a receiver using [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode), [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate), and [**CryptMsgGetParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsggetparam).</span></span>

<span data-ttu-id="9e930-112">如果沒有可用的私密金鑰存在於預設 [*金鑰容器*](../secgloss/k-gly.md)中，此範例將會失敗。</span><span class="sxs-lookup"><span data-stu-id="9e930-112">This example will fail if a usable private key does not exist in the default [*key container*](../secgloss/k-gly.md).</span></span> <span data-ttu-id="9e930-113">如果所需的私密金鑰無法使用，則使用 [**CryptAcquireCertificatePrivateKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecertificateprivatekey)的程式碼（如程式碼範例 [C 程式：傳送和接收已簽署和已加密的訊息](example-c-program-sending-and-receiving-a-signed-and-encrypted-message.md)）中所示範，可以使用。</span><span class="sxs-lookup"><span data-stu-id="9e930-113">If the needed private key is not available, code using [**CryptAcquireCertificatePrivateKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecertificateprivatekey), as demonstrated in the code sample [Example C Program: Sending and Receiving a Signed and Encrypted Message](example-c-program-sending-and-receiving-a-signed-and-encrypted-message.md), can be used.</span></span>

<span data-ttu-id="9e930-114">這個範例會使用 [**MyHandleError**](myhandleerror.md)函數。</span><span class="sxs-lookup"><span data-stu-id="9e930-114">This example uses the function [**MyHandleError**](myhandleerror.md).</span></span> <span data-ttu-id="9e930-115">這個函式的程式碼會包含在範例中。</span><span class="sxs-lookup"><span data-stu-id="9e930-115">The code for this function is included with the sample.</span></span> <span data-ttu-id="9e930-116">這項功能和其他輔助函式的程式碼也會列在 [一般用途](general-purpose-functions.md)的函式底下。</span><span class="sxs-lookup"><span data-stu-id="9e930-116">Code for this and other auxiliary functions is also listed under [General Purpose Functions](general-purpose-functions.md).</span></span>
