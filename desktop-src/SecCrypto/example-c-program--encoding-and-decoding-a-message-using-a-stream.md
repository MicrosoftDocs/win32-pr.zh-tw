...
ms.locfileid: "117765650"
---
# <a name="example-c-program-encoding-and-decoding-a-message-using-a-stream"></a>範例 C 程式：使用資料流程編碼和解碼訊息

下列範例示範如何搭配使用 [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode)、 [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode)和 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate) 函式和 [**CMSG \_ 串流 \_ 資訊**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_stream_info) 結構，使用這些函式的資料流程功能來編碼和解碼訊息。

簽署及編碼訊息並不會確保該訊息的隱私權。 相反地，它可確保訊息的真實性。 由於訊息是以寄件者的私密金鑰簽署，因此當訊息的接收者使用傳送者的 [*公開金鑰*](../secgloss/p-gly.md) 解密簽章時 (可從隨訊息) 傳送的憑證中取得，因此接收者可以確定訊息是由與憑證相關聯的人員或實體所傳送，而且訊息在簽署後並未變更。

此範例的這份編碼簽署部分說明下列工作和 CryptoAPI 函數：

-   使用 [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore)開啟憑證存放區。
-   使用 [**CertFindCertificateInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore)來抓取具有特定主體名稱的憑證。
-   使用 [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa)取得和列印憑證的主體名稱。
-   取得密碼編譯提供者的控制碼，該提供者可以使用 [**CryptAcquireCertificatePrivateKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecertificateprivatekey) 函式提供私密金鑰。
-   將 [**CMSG \_ 簽署的 \_ 編碼 \_ 資訊**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_signed_encode_info) 和 [**CMSG \_ 資料流程 \_ 資訊**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_stream_info) 結構初始化，以用於 [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode)的呼叫中。
-   使用 [**CryptMsgOpenToEncode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentoencode) 和 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)簽署和編碼訊息。
-   執行資料流程回呼函式，此函式可以使用任何持續格式來儲存編碼和簽署的訊息，例如將它寫入檔案。

此範例的解碼部分說明下列工作和 CryptoAPI 函數：

-   初始化要用於呼叫 [**CryptMsgOpenToDecode**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgopentodecode)的 [**CMSG \_ 資料流程 \_ 資訊**](/windows/desktop/api/Wincrypt/ns-wincrypt-cmsg_stream_info)結構。
-   執行資料流程回呼函式，此函式可將已解碼的訊息儲存為任何持續格式，例如將它列印到螢幕。
-   從檔案讀取編碼的訊息，並使用 [**CryptMsgUpdate**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptmsgupdate)解碼訊息。

如需如何在不使用資料流程回呼的情況下執行這些相同作業的範例，請參閱 [範例 C 程式：簽署、編碼、解碼和驗證訊息](example-c-program-signing-encoding-decoding-and-verifying-a-message.md)。

這個範例會使用 [**MyHandleError**](myhandleerror.md)函數。 此範例包含此函式的程式碼。 這項功能和其他輔助函式的程式碼也會列在 [一般 \_ 用途 \_ ](general-purpose-functions.md)的函式底下。
