---
description: 下列範例會建立並安裝非預設的憑證鏈引擎。 引擎用來建立憑證存放區中每個憑證的憑證鏈。
...
---
# <a name="example-c-program-creating-a-certificate-chain"></a><span data-ttu-id="bed60-104">範例 C 程式：建立憑證鏈</span><span class="sxs-lookup"><span data-stu-id="bed60-104">Example C Program: Creating a Certificate Chain</span></span>

<span data-ttu-id="bed60-105">下列範例會建立並安裝非預設的憑證鏈引擎。</span><span class="sxs-lookup"><span data-stu-id="bed60-105">The following example creates and installs a nondefault certificate chain engine.</span></span> <span data-ttu-id="bed60-106">引擎用來建立 [*憑證存放區*](../secgloss/c-gly.md)中每個憑證的憑證鏈。</span><span class="sxs-lookup"><span data-stu-id="bed60-106">The engine is used to build certificate chains for each of the certificates in a [*certificate store*](../secgloss/c-gly.md).</span></span>

<span data-ttu-id="bed60-107">此範例說明下列工作和 [*CryptoAPI*](../secgloss/c-gly.md) 函數：</span><span class="sxs-lookup"><span data-stu-id="bed60-107">This example illustrates the following tasks and [*CryptoAPI*](../secgloss/c-gly.md) functions:</span></span>

-   <span data-ttu-id="bed60-108">藉由宣告和初始化 [**證書 \_ 鏈 \_ 引擎 \_**](/windows/desktop/api/Wincrypt/ns-wincrypt-cert_chain_engine_config) 設定資料結構，來準備建立非預設的憑證鏈引擎。</span><span class="sxs-lookup"><span data-stu-id="bed60-108">Preparing to create a nondefault certificate chain engine by declaring and initializing a [**CERT\_CHAIN\_ENGINE\_CONFIG**](/windows/desktop/api/Wincrypt/ns-wincrypt-cert_chain_engine_config) data structure.</span></span>
-   <span data-ttu-id="bed60-109">使用 [**CertCreateCertificateChainEngine**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcreatecertificatechainengine)建立搜尋引擎。</span><span class="sxs-lookup"><span data-stu-id="bed60-109">Creating the search engine using [**CertCreateCertificateChainEngine**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcreatecertificatechainengine).</span></span>
-   <span data-ttu-id="bed60-110">使用 [**CertOpenSystemStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopensystemstorea) 開啟 [我的系統] 存放區。</span><span class="sxs-lookup"><span data-stu-id="bed60-110">Using [**CertOpenSystemStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopensystemstorea) to open the My system store.</span></span>
-   <span data-ttu-id="bed60-111">在迴圈中使用 [**CertEnumCertificatesInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore) ，從開啟的存放區中取出所有憑證。</span><span class="sxs-lookup"><span data-stu-id="bed60-111">Retrieving all of the certificates from the open store using [**CertEnumCertificatesInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore) in a loop.</span></span>
-   <span data-ttu-id="bed60-112">針對開啟存放區中的每個憑證，使用 [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa)從憑證中取出主體名稱。</span><span class="sxs-lookup"><span data-stu-id="bed60-112">For each certificate in the open store, retrieving the subject name from the certificate using [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa).</span></span>
-   <span data-ttu-id="bed60-113">使用 [**CertGetCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetcertificatechain)建立每個憑證的憑證鏈。</span><span class="sxs-lookup"><span data-stu-id="bed60-113">Building a certificate chain for each certificate using [**CertGetCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetcertificatechain).</span></span>
-   <span data-ttu-id="bed60-114">使用 [**CertDuplicateCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certduplicatecertificatechain)建立憑證鏈的複本。</span><span class="sxs-lookup"><span data-stu-id="bed60-114">Creating a duplicate of the certificate chain using [**CertDuplicateCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certduplicatecertificatechain).</span></span>
-   <span data-ttu-id="bed60-115">使用 [**CertFreeCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfreecertificatechain) 在建立下一個鏈之前釋放每個鏈。</span><span class="sxs-lookup"><span data-stu-id="bed60-115">Using [**CertFreeCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfreecertificatechain) to release each chain before the next chain is built.</span></span>
