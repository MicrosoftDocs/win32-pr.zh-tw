...
下列範例會建立並安裝非預設的憑證鏈引擎。 引擎用來建立 [*憑證存放區*](../secgloss/c-gly.md)中每個憑證的憑證鏈。

此範例說明下列工作和 [*CryptoAPI*](../secgloss/c-gly.md) 函數：

-   藉由宣告和初始化 [**證書 \_ 鏈 \_ 引擎 \_**](/windows/desktop/api/Wincrypt/ns-wincrypt-cert_chain_engine_config) 設定資料結構，來準備建立非預設的憑證鏈引擎。
-   使用 [**CertCreateCertificateChainEngine**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcreatecertificatechainengine)建立搜尋引擎。
-   使用 [**CertOpenSystemStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopensystemstorea) 開啟 [我的系統] 存放區。
-   在迴圈中使用 [**CertEnumCertificatesInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore) ，從開啟的存放區中取出所有憑證。
-   針對開啟存放區中的每個憑證，使用 [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa)從憑證中取出主體名稱。
-   使用 [**CertGetCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetcertificatechain)建立每個憑證的憑證鏈。
-   使用 [**CertDuplicateCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certduplicatecertificatechain)建立憑證鏈的複本。
-   使用 [**CertFreeCertificateChain**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfreecertificatechain) 在建立下一個鏈之前釋放每個鏈。
