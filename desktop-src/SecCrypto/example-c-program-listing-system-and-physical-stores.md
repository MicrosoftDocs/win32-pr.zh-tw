...
下列範例會列舉系統 [*憑證存放區*](../secgloss/c-gly.md) 位置、系統憑證存放區，以及與每個系統存放區相關聯的實體存放區。 這個範例示範如何建立回呼函式，以及呼叫其他回呼函數的回呼函數。
...
此範例也會使用函數 [**MyHandleError**](myhandleerror.md)。 此範例包含此函式的程式碼。 這項功能和其他輔助函式的程式碼也會列在 [一般用途](general-purpose-functions.md)的函式底下。
