...
---
# <a name="example-c-program-using-certoidtoalgid-and-certcompareintegerblob"></a><span data-ttu-id="edb44-103">範例 C 程式：使用 CertOIDToAlgId 和 CertCompareIntegerBlob</span><span class="sxs-lookup"><span data-stu-id="edb44-103">Example C Program: Using CertOIDToAlgId and CertCompareIntegerBlob</span></span>

<span data-ttu-id="edb44-104">下列範例將示範如何使用 [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid) 和 [**CertCompareIntegerBlob**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcompareintegerblob) 函數。</span><span class="sxs-lookup"><span data-stu-id="edb44-104">The following example demonstrates using the [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid) and [**CertCompareIntegerBlob**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcompareintegerblob) functions.</span></span>

<span data-ttu-id="edb44-105">首先，所有可用的 Oid 都會使用 [**CryptEnumOIDInfo**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumoidinfo)來列舉。</span><span class="sxs-lookup"><span data-stu-id="edb44-105">First, all available OIDs are enumerated using [**CryptEnumOIDInfo**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumoidinfo).</span></span> <span data-ttu-id="edb44-106">搭配此函式使用的程式碼也會示範回呼函數的用法。</span><span class="sxs-lookup"><span data-stu-id="edb44-106">Code used with this function also demonstrates the use of a callback function.</span></span> <span data-ttu-id="edb44-107">回呼函式會示範中斷邏輯，以在每個 OID 群組之間，以及在提供了設定的 Oid 數目的資訊之後暫停。</span><span class="sxs-lookup"><span data-stu-id="edb44-107">The callback function demonstrates break logic to pause between each OID group and after presenting information on a set number of OIDs.</span></span>

<span data-ttu-id="edb44-108">其次， (OID 的三個 [*物件識別碼*](../secgloss/o-gly.md)) 字串會使用 [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid)轉換成 **DWORD** 演算法識別碼整數。</span><span class="sxs-lookup"><span data-stu-id="edb44-108">Second, three [*object identifier*](../secgloss/o-gly.md) (OID) strings are converted into **DWORD** algorithm identifier integers using [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid).</span></span> <span data-ttu-id="edb44-109">程式碼也會示範所有 OID 字串都沒有相關的演算法識別碼。</span><span class="sxs-lookup"><span data-stu-id="edb44-109">The code also demonstrates that all OID strings do not have related algorithm identifiers.</span></span>

<span data-ttu-id="edb44-110">最後，此範例會示範如何比較整數 Blob。</span><span class="sxs-lookup"><span data-stu-id="edb44-110">Finally, the example demonstrates comparing integer BLOBs.</span></span> <span data-ttu-id="edb44-111">這個範例會示範從正數和前導0xFF's 的正負號截斷前置0x00's。</span><span class="sxs-lookup"><span data-stu-id="edb44-111">This example demonstrates the truncation of leading 0x00's from positive numbers and leading 0xFF's from negative numbers.</span></span>

<span data-ttu-id="edb44-112">此外，它也會顯示整數的比較方式，就像是以 [*位元組*](../secgloss/l-gly.md) 由小到大形式儲存的整數，以及右邊最有效的數位一樣。</span><span class="sxs-lookup"><span data-stu-id="edb44-112">It also shows that integers are compared as though they are stored in [*little-endian*](../secgloss/l-gly.md) form with the most significant digits on the right.</span></span>
