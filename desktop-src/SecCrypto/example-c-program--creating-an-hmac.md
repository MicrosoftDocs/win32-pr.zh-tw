...
ms.locfileid: "117765886"
---
# <a name="example-c-program-creating-an-hmac"></a>範例 C 程式：建立 HMAC

雜湊的訊息驗證總和檢查碼 (HMAC) 通常用來驗證訊息在傳輸期間未經過變更。 訊息的雙方都必須有共用秘密金鑰。 傳送者會將金鑰和訊息合併為字串、使用 SHA-1 或 MD5 等演算法建立字串的摘要，並傳輸訊息和摘要。 接收者會將共用金鑰與訊息合併，套用適當的演算法，並比較摘要，因此會以傳送者所傳輸的摘要來取得。 如果摘要完全相同，訊息就不會遭到篡改。
...
-   藉由呼叫 [**CryptCreateHash**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptcreatehash)、 [**CryptHashData**](/windows/desktop/api/Wincrypt/nf-wincrypt-crypthashdata)和 [**CryptDeriveKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptderivekey)，從位元組字串衍生對稱金鑰。
-   藉由呼叫 [**CryptCreateHash**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptcreatehash) 和 [**CryptSetHashParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsethashparam)，使用對稱金鑰來建立 HMAC 雜湊物件。
-   藉由呼叫 [**CryptHashData**](/windows/desktop/api/Wincrypt/nf-wincrypt-crypthashdata)來雜湊訊息。
-   藉由呼叫 [**CryptGetHashParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgethashparam)來取出雜湊。
