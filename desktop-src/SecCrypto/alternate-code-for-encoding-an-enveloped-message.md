...
ms.locfileid: "117773692"
---
# <a name="alternate-code-for-encoding-an-enveloped-message"></a>編碼封包訊息的替代程式碼

下列範例示範使用已簽署的訊息做為封包訊息的 [*內部內容*](../secgloss/i-gly.md) ，來編碼已簽署訊息的替代程式。 為了進行解碼的準備，內部內容會經過測試，以判斷其內部內容類型。
...
此範例也會使用 [**MyHandleError**](myhandleerror.md) 和 [**GetSignerCert**](getsignercert.md)函數。 範例中包含這些函式的 C 程式碼。 如需示範這些功能和其他輔助函式的程式碼，請參閱 [一般目的](general-purpose-functions.md)函式。
