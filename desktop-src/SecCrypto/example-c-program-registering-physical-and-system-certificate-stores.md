...
ms.locfileid: "119140871"
---
# <a name="example-c-program-registering-physical-and-system-certificate-stores"></a>範例 C 程式：註冊實體和系統憑證存放區

實體存放區可能會成為系統存放區的更多或更少永久成員。 當實體存放區是系統存放區的成員時，系統存放區上的作業（例如尋找憑證）將會查看註冊為系統存放區成員的所有實體存放區。 您可以使用取消註冊的函式，從系統存放區的成員資格中移除實體存放區。

此範例會顯示下列工作和 CryptoAPI 函數：

-   使用 [**CertRegisterSystemStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certregistersystemstore)註冊 (建立) 新的系統存放區。
-   使用 [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore)開啟新建立的系統存放區。
-   使用 [**CertRegisterPhysicalStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certregisterphysicalstore)將實體存放區註冊為系統存放區的成員。
-   使用 [**CertUnregisterSystemStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certunregistersystemstore)取消註冊 (刪除) 系統存放區。
