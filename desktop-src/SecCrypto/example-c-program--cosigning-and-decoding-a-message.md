---
description: 您可以使用 CryptSignMessage 函數來 cosign 訊息。
...
---
# <a name="example-c-program-cosigning-and-decoding-a-message"></a><span data-ttu-id="8e935-103">範例 C 程式： Cosigning 和解碼訊息</span><span class="sxs-lookup"><span data-stu-id="8e935-103">Example C Program: Cosigning and Decoding a Message</span></span>

<span data-ttu-id="8e935-104">您可以使用 [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) 函數來 cosign 訊息。</span><span class="sxs-lookup"><span data-stu-id="8e935-104">You can use the [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) function to cosign a message.</span></span> <span data-ttu-id="8e935-105">這是藉由呼叫 [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) 一次來簽署原始訊息來完成，然後再次呼叫 **CryptSignMessage** 以 cosign 簽署的訊息。</span><span class="sxs-lookup"><span data-stu-id="8e935-105">This is accomplished by calling [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) once to sign the original message, and then call **CryptSignMessage** again to cosign the signed message.</span></span>

<span data-ttu-id="8e935-106">當您驗證 cosigned 訊息的簽章時，您會使用 [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) 函數來取得訊息的簽署者數目，然後呼叫每個簽章的 [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) 。</span><span class="sxs-lookup"><span data-stu-id="8e935-106">When you verify the signature of a cosigned message, you use the [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) function to get the number of signers of the message and then call the [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) for each signature.</span></span> <span data-ttu-id="8e935-107">如果所有簽章都經過驗證，則您知道 cosigned 訊息是有效的。</span><span class="sxs-lookup"><span data-stu-id="8e935-107">If all of the signatures are verified, then you know the cosigned message is valid.</span></span>

<span data-ttu-id="8e935-108">下列範例將示範如何使用多人來簽署訊息 (cosign 訊息) 、確認所有簽章，以及解碼訊息。</span><span class="sxs-lookup"><span data-stu-id="8e935-108">The following example shows how to sign a message by more than one person (cosign the message), verify all signatures, and decode the message.</span></span>
