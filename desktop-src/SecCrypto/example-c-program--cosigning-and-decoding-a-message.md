...
您可以使用 [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) 函數來 cosign 訊息。 這是藉由呼叫 [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) 一次來簽署原始訊息來完成，然後再次呼叫 **CryptSignMessage** 以 cosign 簽署的訊息。

當您驗證 cosigned 訊息的簽章時，您會使用 [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) 函數來取得訊息的簽署者數目，然後呼叫每個簽章的 [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) 。 如果所有簽章都經過驗證，則您知道 cosigned 訊息是有效的。

下列範例將示範如何使用多人來簽署訊息 (cosign 訊息) 、確認所有簽章，以及解碼訊息。
