...
---
# <a name="example-c-program-sending-and-receiving-a-signed-and-encrypted-message"></a><span data-ttu-id="f8075-103">範例 C 程式：傳送和接收已簽署和已加密的訊息</span><span class="sxs-lookup"><span data-stu-id="f8075-103">Example C Program: Sending and Receiving a Signed and Encrypted Message</span></span>

<span data-ttu-id="f8075-104">下列範例會使用傳送者的 [*私密金鑰*](../secgloss/p-gly.md) 來簽署訊息，並使用接收者的 [*公開金鑰*](../secgloss/p-gly.md)來加密已簽署的訊息。</span><span class="sxs-lookup"><span data-stu-id="f8075-104">The following example signs a message using a sender's [*private key*](../secgloss/p-gly.md) and encrypts the signed message using a receiver's [*public key*](../secgloss/p-gly.md).</span></span> <span data-ttu-id="f8075-105">然後，此範例會使用接收者的私密金鑰來解密訊息，並使用傳送者的公開金鑰來驗證簽章。</span><span class="sxs-lookup"><span data-stu-id="f8075-105">The example then decrypts the message using the receiver's private key and verifies the signature using the sender's public key.</span></span> <span data-ttu-id="f8075-106">包含所需公開金鑰的寄件者憑證包含在加密的訊息中。</span><span class="sxs-lookup"><span data-stu-id="f8075-106">The sender's certificate containing the needed public key is included in the encrypted message.</span></span> <span data-ttu-id="f8075-107">這個範例也會將已簽署和加密的訊息寫入檔案。</span><span class="sxs-lookup"><span data-stu-id="f8075-107">This example also writes the signed and encrypted message to a file.</span></span> <span data-ttu-id="f8075-108">如需詳細資訊，請參閱 [C 程式範例：接收已簽署和加密的訊息](example-c-program-receiving-a-signed-and-encrypted-message.md)。</span><span class="sxs-lookup"><span data-stu-id="f8075-108">For more information, see [Example C Program: Receiving a Signed and Encrypted Message](example-c-program-receiving-a-signed-and-encrypted-message.md).</span></span>

<span data-ttu-id="f8075-109">若要簽署訊息，簽署人的私密金鑰和簽署者的憑證必須可供使用。</span><span class="sxs-lookup"><span data-stu-id="f8075-109">To sign the message, the signer's private key and the signer's certificate must be available.</span></span> <span data-ttu-id="f8075-110">若要加密已簽署的訊息，必須要有接收者的憑證，包括接收者的公開金鑰。</span><span class="sxs-lookup"><span data-stu-id="f8075-110">To encrypt the signed message, a receiver's certificate including the receiver's public key must be available.</span></span>

<span data-ttu-id="f8075-111">若要解密訊息，接收者的私密金鑰必須可用。</span><span class="sxs-lookup"><span data-stu-id="f8075-111">To decrypt the message, the receiver's private key must be available.</span></span> <span data-ttu-id="f8075-112">解密訊息之後，就會使用加密訊息中包含的憑證公開金鑰來驗證簽章。</span><span class="sxs-lookup"><span data-stu-id="f8075-112">After the message is decrypted, the signature is verified using the public key from the certificate included in the encrypted message.</span></span>

> [!Note]  
> <span data-ttu-id="f8075-113">並非 [*憑證存放區*](../secgloss/c-gly.md) 中的所有憑證都會提供與該憑證相關聯之 [*私密金鑰*](../secgloss/p-gly.md) 的存取權。</span><span class="sxs-lookup"><span data-stu-id="f8075-113">Not all of the certificates in a [*certificate store*](../secgloss/c-gly.md) provide access to the [*private key*](../secgloss/p-gly.md) associated with that certificate.</span></span> <span data-ttu-id="f8075-114">當訊息經過簽署和加密時，必須使用屬於簽署者之私密金鑰的憑證，才能存取該簽署者的私密金鑰。</span><span class="sxs-lookup"><span data-stu-id="f8075-114">When the message is signed and encrypted, a certificate belonging to the signer with access to the private key of that signer must be used.</span></span> <span data-ttu-id="f8075-115">此外，訊息接收者必須能夠存取與用來加密訊息的公開金鑰相關聯的私密金鑰。</span><span class="sxs-lookup"><span data-stu-id="f8075-115">In addition, the receiver of the message must have access to the private key associated with the public key used to encrypt the message.</span></span>

 

<span data-ttu-id="f8075-116">此範例說明下列工作：</span><span class="sxs-lookup"><span data-stu-id="f8075-116">This example illustrates the following tasks:</span></span>

-   <span data-ttu-id="f8075-117">開啟和關閉系統憑證存放區。</span><span class="sxs-lookup"><span data-stu-id="f8075-117">Opening and closing system certificate stores.</span></span>
-   <span data-ttu-id="f8075-118">在開啟的憑證存放區中尋找訊息寄件者和訊息接收者的憑證。</span><span class="sxs-lookup"><span data-stu-id="f8075-118">Finding certificates for a message sender and message receiver in the open certificate stores.</span></span>
-   <span data-ttu-id="f8075-119">尋找和列印憑證的主體名稱。</span><span class="sxs-lookup"><span data-stu-id="f8075-119">Finding and printing the subject name from certificates.</span></span>
-   <span data-ttu-id="f8075-120">初始化簽署、加密、解密和驗證訊息所需的資料結構。</span><span class="sxs-lookup"><span data-stu-id="f8075-120">Initializing data structures needed to sign, encrypt, decrypt, and verify a message.</span></span>
-   <span data-ttu-id="f8075-121">呼叫 CryptoAPI 函式以找出所需的緩衝區大小，配置所需大小的緩衝區，然後再次呼叫 CryptoAPI 函式來填滿緩衝區。</span><span class="sxs-lookup"><span data-stu-id="f8075-121">Calling a CryptoAPI function to find the required size of a buffer, allocating the buffer of the required size, and calling the CryptoAPI function again to fill the buffer.</span></span> <span data-ttu-id="f8075-122">如需詳細資訊，請參閱 [取出未知長度的資料](retrieving-data-of-unknown-length.md)。</span><span class="sxs-lookup"><span data-stu-id="f8075-122">For more information, see [Retrieving Data of Unknown Length](retrieving-data-of-unknown-length.md).</span></span>
-   <span data-ttu-id="f8075-123">顯示某些已加密的緩衝區內容。</span><span class="sxs-lookup"><span data-stu-id="f8075-123">Displaying some of the encrypted contents of a buffer.</span></span> <span data-ttu-id="f8075-124">包含的區域函式 **ShowBytes** 會以 ' 0 ' 和 ' z ' 之間的值，顯示緩衝區中的字元。</span><span class="sxs-lookup"><span data-stu-id="f8075-124">The included local function, **ShowBytes**, displays characters in the buffer with values between '0' and 'z'.</span></span> <span data-ttu-id="f8075-125">所有其他字元會顯示為 '-' 字元。</span><span class="sxs-lookup"><span data-stu-id="f8075-125">All other characters are displayed as the '-' character.</span></span>

<span data-ttu-id="f8075-126">此範例使用下列 CryptoAPI 函數：</span><span class="sxs-lookup"><span data-stu-id="f8075-126">This example uses the following CryptoAPI functions:</span></span>

-   [<span data-ttu-id="f8075-127">**CertOpenStore**</span><span class="sxs-lookup"><span data-stu-id="f8075-127">**CertOpenStore**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore)
-   [<span data-ttu-id="f8075-128">**CertFindCertificateInStore**</span><span class="sxs-lookup"><span data-stu-id="f8075-128">**CertFindCertificateInStore**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore)
-   [<span data-ttu-id="f8075-129">**CertGetNameString**</span><span class="sxs-lookup"><span data-stu-id="f8075-129">**CertGetNameString**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa)
-   [<span data-ttu-id="f8075-130">**CryptAcquireCertificatePrivateKey**</span><span class="sxs-lookup"><span data-stu-id="f8075-130">**CryptAcquireCertificatePrivateKey**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecertificateprivatekey)
-   [<span data-ttu-id="f8075-131">**CryptSignAndEncryptMessage**</span><span class="sxs-lookup"><span data-stu-id="f8075-131">**CryptSignAndEncryptMessage**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignandencryptmessage)
-   [<span data-ttu-id="f8075-132">**CryptDecryptAndVerifyMessageSignature**</span><span class="sxs-lookup"><span data-stu-id="f8075-132">**CryptDecryptAndVerifyMessageSignature**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptandverifymessagesignature)
-   [<span data-ttu-id="f8075-133">**CertFreeCertificateCoNtext**</span><span class="sxs-lookup"><span data-stu-id="f8075-133">**CertFreeCertificateContext**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certfreecertificatecontext)
-   [<span data-ttu-id="f8075-134">**CertCloseStore**</span><span class="sxs-lookup"><span data-stu-id="f8075-134">**CertCloseStore**</span></span>](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore)

<span data-ttu-id="f8075-135">此範例會使用不同的函式來顯示簽署/加密程式和解密/簽章驗證程式。</span><span class="sxs-lookup"><span data-stu-id="f8075-135">This example uses separate functions to show the signing/encryption process and the decryption/signature-verification process.</span></span> <span data-ttu-id="f8075-136">它也會使用 [**MyHandleError**](myhandleerror.md) ，以在發生任何失敗時正常結束程式。</span><span class="sxs-lookup"><span data-stu-id="f8075-136">It also uses [**MyHandleError**](myhandleerror.md) to exit the program gracefully in case of any failure.</span></span> <span data-ttu-id="f8075-137">程式碼 **MyHandleError** 包含在範例中，也可以在 [一般目的](general-purpose-functions.md)函式下與其他輔助函式一起找到。</span><span class="sxs-lookup"><span data-stu-id="f8075-137">The code **MyHandleError** is included with the example and can also be found along with other auxiliary functions under [General Purpose Functions](general-purpose-functions.md).</span></span>
