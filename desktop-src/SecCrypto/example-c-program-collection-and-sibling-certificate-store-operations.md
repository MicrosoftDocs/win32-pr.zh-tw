...
ms.locfileid: "119873868"
---
# <a name="example-c-program-collection-and-sibling-certificate-store-operations"></a>範例 C 程式：收集和同級憑證存放區作業

下列範例示範收集存放區的概念，這是一個實際包含數個憑證存放區內容的暫時 [*憑證存放區*](../secgloss/c-gly.md) 。 您可以將一或多個存放區加入至集合，以便使用單一函式呼叫來存取集合中任何存放區的內容。

此範例說明下列工作和 [*CryptoAPI*](../secgloss/c-gly.md) 函數：

-   使用 [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore) 和 [**CertCloseStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore)來開啟和關閉收集存放區、記憶體存放區和系統存放區。
-   使用 [**CertAddStoreToCollection**](/windows/desktop/api/Wincrypt/nf-wincrypt-certaddstoretocollection)將同級存放區加入至集合存放區。
-   使用 [**CertFindCertificateInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certfindcertificateinstore)尋找符合某些準則之存放區中憑證和憑證的連結。
...
-   將存放區儲存到磁片上的檔案中。
-   開啟和關閉以檔案為基礎的憑證存放區。
-   使用 [**CertRemoveStoreFromCollection**](/windows/desktop/api/Wincrypt/nf-wincrypt-certremovestorefromcollection)從集合中移除同級存放區。
...
下列範例會開啟一個收集存放區、在記憶體中建立新的憑證存放區，並將新的存放區做為做為做為做為的新存放區加入至集合存放區。 程式接著會開啟系統存放區，並抓取憑證。 該憑證會加入至記憶體存放區。 系統會從系統存放區中取出第二個憑證，並將該憑證的連結新增至記憶體存放區。 然後，會從集合存放區中取出憑證和連結，顯示可以從集合存放區中取出位於同級存放區的憑證和連結。 記憶體會儲存至磁片。 然後，會從集合中移除記憶體存放區。 您仍然可以在記憶體存放區中找到新增至記憶體存放區的連結，但無法再于收集存放區中找到該連結。 所有存放區和檔案都會關閉，然後重新開啟檔案存放區，並針對憑證連結進行搜尋。 此計畫的成功與否取決於我的商店是否可供使用。 該存放區必須包含具有「插入憑證主體 name1」主體的憑證 \_ \_ \_ ，以及具有「插入憑證主體 name2」主體的第二個憑證 \_ \_ \_ 。 主體的名稱應該變更為「我的存放區」中已知的憑證主體名稱。
