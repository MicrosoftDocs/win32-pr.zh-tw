---
description: 下列範例會提供程式碼來開啟各種一般憑證存放區。 這些範例是一系列的程式碼片段，並不是獨立的程式。
ms.assetid: 975a56d1-aa45-470e-b385-753baa1911ff
title: 用於開啟憑證存放區的範例 C 程式碼
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "117766059"
---
# <a name="example-c-code-for-opening-certificate-stores"></a>用於開啟憑證存放區的範例 C 程式碼

下列範例會提供程式碼來開啟各種一般憑證存放區。 這些範例是一系列的程式碼片段，並不是獨立的程式。
...
下列範例會從磁片開啟存放區。 此範例會使用在 [建立 DACL](../secbp/creating-a-dacl.md)主題中定義的 **CreateMyDACL** 函式，以確保使用正確的 dacl 建立開啟的檔案。
...
下列範例會使用 CERT \_ store >prov FILENAME 來開啟以檔案為基礎的存放區 \_ \_ 。
