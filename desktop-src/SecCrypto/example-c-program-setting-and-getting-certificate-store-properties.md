...
---
# <a name="example-c-program-setting-and-getting-certificate-store-properties"></a><span data-ttu-id="6ec08-104">範例 C 程式：設定和取得憑證存放區屬性</span><span class="sxs-lookup"><span data-stu-id="6ec08-104">Example C Program: Setting and Getting Certificate Store Properties</span></span>

<span data-ttu-id="6ec08-105">下列範例會設定並取得憑證存放區屬性，也就是當地語系化的存放區名稱。</span><span class="sxs-lookup"><span data-stu-id="6ec08-105">The following example sets and gets a certificate store property, the localized store name.</span></span> <span data-ttu-id="6ec08-106">當存放區關閉時，不會保存此屬性。</span><span class="sxs-lookup"><span data-stu-id="6ec08-106">This property is not persisted when the store is closed.</span></span>

<span data-ttu-id="6ec08-107">此範例說明下列工作和 [*CryptoAPI*](../secgloss/c-gly.md) 函數：</span><span class="sxs-lookup"><span data-stu-id="6ec08-107">This example illustrates the following tasks and [*CryptoAPI*](../secgloss/c-gly.md) functions:</span></span>

-   <span data-ttu-id="6ec08-108">使用 [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore)開啟憑證存放區。</span><span class="sxs-lookup"><span data-stu-id="6ec08-108">Opening a certificate store using [**CertOpenStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certopenstore).</span></span>
-   <span data-ttu-id="6ec08-109">使用 [**CertSetStoreProperty**](/windows/desktop/api/Wincrypt/nf-wincrypt-certsetstoreproperty)設定存放區的當地語系化名稱。</span><span class="sxs-lookup"><span data-stu-id="6ec08-109">Setting the localized name of the store using [**CertSetStoreProperty**](/windows/desktop/api/Wincrypt/nf-wincrypt-certsetstoreproperty).</span></span>
-   <span data-ttu-id="6ec08-110">使用 [**CertGetStoreProperty**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetstoreproperty)抓取存放區的當地語系化名稱。</span><span class="sxs-lookup"><span data-stu-id="6ec08-110">Retrieving the localized name of the store using [**CertGetStoreProperty**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetstoreproperty).</span></span>
-   <span data-ttu-id="6ec08-111">使用 [**CryptFindLocalizedName**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptfindlocalizedname)來取出預先定義的當地語系化存放區名稱。</span><span class="sxs-lookup"><span data-stu-id="6ec08-111">Retrieving the predefined localized store name using [**CryptFindLocalizedName**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptfindlocalizedname).</span></span>
-   <span data-ttu-id="6ec08-112">使用 [**CertSaveStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certsavestore)將憑證存放區以 [*PKCS \# 7*](../secgloss/p-gly.md)訊息儲存至檔案。</span><span class="sxs-lookup"><span data-stu-id="6ec08-112">Save the certificate store as a [*PKCS \#7*](../secgloss/p-gly.md) message to a file using [**CertSaveStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certsavestore).</span></span>
-   <span data-ttu-id="6ec08-113">使用 [**CertSaveStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certsavestore)將憑證存放區儲存至記憶體 [*BLOB*](../secgloss/b-gly.md) 。</span><span class="sxs-lookup"><span data-stu-id="6ec08-113">Save the certificate store to a memory [*BLOB*](../secgloss/b-gly.md) using [**CertSaveStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certsavestore).</span></span>
-   <span data-ttu-id="6ec08-114">使用 CryptGetMessageSignercount 來判斷 PKCS 7 訊息的簽署者數目 \# 。 [](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount)</span><span class="sxs-lookup"><span data-stu-id="6ec08-114">Determine the number of signers of the PKCS \#7 message using [**CryptGetMessageSignercount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount).</span></span>
-   <span data-ttu-id="6ec08-115">\#使用 [**CryptGetMessageCertificates**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagecertificates)，從記憶體中的 PKCS 7 訊息開啟憑證存放區。</span><span class="sxs-lookup"><span data-stu-id="6ec08-115">Open a certificate store from a PKCS \#7 message in memory using [**CryptGetMessageCertificates**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagecertificates).</span></span>
-   <span data-ttu-id="6ec08-116">初始化 [*雜湊*](../secgloss/h-gly.md)訊息所需的 [**CRYPT \_ 演算法 \_ 識別碼**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier)和 [**CRYPT \_ 雜湊 \_ 訊息 \_ 段落**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_hash_message_para)資料結構</span><span class="sxs-lookup"><span data-stu-id="6ec08-116">Initialize the [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) and [**CRYPT\_HASH\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_hash_message_para) data structures needed to [*hash*](../secgloss/h-gly.md) the message</span></span>
-   <span data-ttu-id="6ec08-117">使用 [**CryptHashMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-crypthashmessage)來雜湊及編碼訊息。</span><span class="sxs-lookup"><span data-stu-id="6ec08-117">Hash and encode the message using [**CryptHashMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-crypthashmessage).</span></span>
-   <span data-ttu-id="6ec08-118">判斷是否已對開啟的憑證存放區進行變更，並在需要時使用 [**CertControlStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcontrolstore)同步處理存放區。</span><span class="sxs-lookup"><span data-stu-id="6ec08-118">Determine whether changes have been made to an open certificate store and synchronizing the store if needed using [**CertControlStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcontrolstore).</span></span>
-   <span data-ttu-id="6ec08-119">使用 [**CertCloseStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore) 搭配 CERT \_ CLOSE \_ store \_ FORCE 旗標來關閉憑證存放區 \_ 。</span><span class="sxs-lookup"><span data-stu-id="6ec08-119">Closing a certificate store using [**CertCloseStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore) with the CERT\_CLOSE\_STORE\_FORCE\_FLAG.</span></span>
