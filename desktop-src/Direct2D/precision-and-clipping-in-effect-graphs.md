...
Direct2D 通常會將效果圖形分成各區段，並在個別的步驟中轉譯每個區段。 某些步驟的輸出可能會儲存在中繼 Direct3D 紋理中，依預設會有有限的數值範圍和精確度。 Direct2D 不會保證是否使用這些中繼紋理。 此行為可能會根據 GPU 功能以及 Windows 版本而有所不同。
...
可能會發出這些超出範圍色彩的某些效果會提供 "ClampOutput" 屬性。 這些包括：
...
其他內建效果也可能會產生超出0的輸出色彩 \[ ， \] unpremultiplied 空間中的1個範圍，即使在該範圍內有任何) ，也可能會產生色彩圖元 (和 "Color" 屬性。 這些包括：
