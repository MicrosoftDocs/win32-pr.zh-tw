...
---
# <a name="migratevirtualsystemtohost-method-of-the-cim_virtualsystemmigrationservice-class"></a>CIM VirtualSystemMigrationService 類別的 MigrateVirtualSystemToHost 方法 \_

將虛擬系統移動、遷移或重新放置到網路名稱或 IP 位址所指定之目標主機的方法。

> [!Note]  
> 這種方法的目的是要在叢集支援模型可用之前，才使用過渡解決方案。
...
| <dl> <dt>**不正確參數**</dt> <dt>4</dt> </dl>                          | 一或多個參數正式無效。 例如，您可以使用不支援的格式來指定 *DestinationHost* 參數的值。<br/>                                                                                                                                    |
| <dl> <dt>**狀態**</dt> <dt>5</dt>無效 </dl>                              | 來源虛擬系統、來源主機系統或目標主機系統處於允許起始要求的虛擬系統移轉的狀態;這可能是暫時性的狀況。<br/>                                                                                           |
| <dl> <dt>**不相容的參數**</dt> <dt>6</dt> </dl>                    | 一或多個輸入參數與一組或目標主機不相容。 例如， *MigrationNewSettingData* 參數的值包含由 *DestinationHost* 參數的值所識別的目標主機系統不支援的屬性。<br/> |
