...
ms.locfileid: "118949887"
---
# <a name="msvm_syntheticdisplaycontroller-class"></a>Msvm \_ SyntheticDisplayController 類別

代表存在於每個虛擬機器設定中的綜合顯示控制器的狀態。 在虛擬機器中，每次只能有一個顯示控制器可供使用，而且只有在客體作業系統已載入必要的影片加速服務時，才可啟用綜合控制器。
...
**Msvm \_ SyntheticDisplayController** 類別具有下列類型的成員：
...
**Msvm \_ SyntheticDisplayController** 類別具有這些方法。
...
| [**RequestStateChange**](msvm-syntheticdisplaycontroller-requeststatechange.md) | 要求狀態變更。<br/>      |
| [**重設**](msvm-syntheticdisplaycontroller-reset.md)                           | 重設虛擬裝置。<br/>    |
...
**Msvm \_ SyntheticDisplayController** 類別具有這些屬性。
...
顯示控制器的圖形和3D 功能。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))，而且一律設定為 2 (圖形加速器) 。
...
這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，而且一律設定為 6 (不適用) 。
...
自由格式字串的陣列，可針對 **AcceleratorCapabilities** 屬性陣列中所指定的任何影片加速器功能提供更詳細的說明。 這個陣列的每個專案都與位於相同索引的 **AcceleratorCapabilities** 屬性陣列中的專案相關。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))，而且一律設定為「圖形加速器」。
...
物件的簡短描述。 這個屬性繼承自 [**CIM \_ ManagedElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)，一律設為「顯示控制器」。
...
建立實例時所使用之類別或子類別的名稱。 這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，而且一律設定為 "Msvm \_ SyntheticDisplayController"。
...
對物件的描述。 這個屬性繼承自 [**CIM \_ ManagedElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)，一律設為「Microsoft 綜合顯示控制器」。
...
這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，而且一律設定為 "Microsoft：*GUID*"。
...
物件的顯示名稱。 這個屬性繼承自 [**CIM \_ ManagedElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)，預設一律設定為「顯示控制器」。
...
專案的已啟用和停用狀態。 也可以指出這些要求狀態之間的轉換。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))，而且一律設定為 2 (啟用) 或 3 (停用) 。
...
專案目前的健康情況。 這個屬性工作表示這個專案的健康情況，但不一定是它的子項目。 可能的值為0到30，其中5表示元素完全狀況良好，而30表示元素完全無法正常運作。 這個屬性繼承自 [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)，而且一律設定為 5 (確定) 。
...
這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，而且一律設定為 **Null**。
...
支援的最大記憶體數量（以位元組為單位）。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))，而且一律設定為 4194304 (0x400000) 。
...
此控制器支援的直接可定址實體數目上限。 如果數位為未知或無限制，則應使用0值。 控制器用來存取受控制裝置的通訊協定。 這個屬性繼承自 [**CIM \_ 控制器**](/windows/desktop/CIMWin32Prov/cim-controller)，而且一律設定為1。
...
提供目前的解析度和可用記憶體所支援的影片頁面數目。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))，一律設為1024。
...
描述影片架構類型的字串，當 **VideoArchitecture** 屬性是 1 ( "Other" ) 時。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))。
...
當實例的 **VideoMemoryType** 屬性為1時 (其他) 的視訊記憶體類型。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))，而且一律設定為 **Null**。
...
提供與控制器所支援的通訊協定相關之詳細資訊的字串。 這個屬性繼承自 [**CIM \_ 控制器**](/windows/desktop/CIMWin32Prov/cim-controller)，而且一律設定為「影片」。
...
控制器用來存取受控制裝置的通訊協定。 這個屬性繼承自 [**CIM \_ 控制器**](/windows/desktop/CIMWin32Prov/cim-controller)，而且一律設定為 1 (其他) 。
...
上次要求或預期的專案狀態。 元素的實際狀態是由 **EnabledState** 表示。 這個屬性是用來比較最後一個要求和目前啟用或停用的狀態。 特定的 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85)) 實例可能不支援 **RequestStateChange**。 如果發生這種情況，則會使用值 12 (不適用) 。 這個屬性繼承自 **CIM \_ EnabledLogicalElement**，它設定為 2 (啟用) 、3 (停用) 或 12 (不適用) 。
...
指定用來產生視訊訊號的顯示器控制器影片架構。 通常，專用的視頻處理器會根據指定的架構產生影片信號。 它是顯示控制器最大解析度功能的指標。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))。
...
<span id="Linear_Frame_Buffer"></span><span id="linear_frame_buffer"></span><span id="LINEAR_FRAME_BUFFER"></span>**線性框架緩衝區** (11) 
</dt> <dt>
...
視訊記憶體的類型。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))，而且一律設定為 2 (VRAM) 。
...
描述視頻處理器/控制器的字串。 這個屬性會繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))，而且一律會設定為「綜合視頻處理器」。

</dd> </dl>

## <a name="remarks"></a>備註

存取 **Msvm \_ SyntheticDisplayController** 類別可能受 UAC 篩選所限制。 如需詳細資訊，請參閱 [使用者帳戶控制和 WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi)。
