...
---
# <a name="getting-the-virtual-machine-dns-name"></a>取得虛擬機器 DNS 名稱

下列 c # 和 Visual Basic Scripting Edition (VBScript) 範例會取得虛擬機器的 DNS 名稱。

若要執行此範例程式碼，您必須安裝用戶端 integration services，而且用戶端作業系統必須正在執行。
...
[虛擬化範例 (V2) 的常見公用程式 ](common-utilities-for-the-virtualization-samples-v2.md)
