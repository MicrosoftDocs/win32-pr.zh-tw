...
ms.locfileid: "120046448"
---
# <a name="msvm_ps2mouse-class"></a>Msvm \_ Ps2Mouse 類別

表示 PS2 滑鼠裝置。 此類別的實例是一律存在於虛擬機器中的邏輯裝置，因此不會透過資源集區配置。 一個實例一律存在於虛擬機器中。
...
**Msvm \_ Ps2Mouse** 類別具有這些方法。



| 方法                                                           | 描述                                                                                           |
|:-----------------------------------------------------------------|:------------------------------------------------------------------------------------------------------|
| [**ClickButton**](clickbutton-msvm-ps2mouse.md)                 | 在指定的裝置按鈕上，模擬按鈕點擊、向下序列。<br/>              |
| **EnableDevice**                                                 | 不支援這個方法。<br/>                                                              |
| [**GetButtonState**](getbuttonstate-msvm-ps2mouse.md)           | 抓取指定裝置按鈕的目前狀態。<br/>                                |
| **OnlineDevice**                                                 | 不支援這個方法。<br/>                                                              |
| **QuiesceDevice**                                                | 不支援這個方法。<br/>                                                              |
| [**RequestStateChange**](msvm-ps2mouse-requeststatechange.md)   | 要求狀態變更。<br/>                                                                   |
| [**重設**](msvm-ps2mouse-reset.md)                             | 重設裝置。<br/>                                                                         |
| **RestoreProperties**                                            | 不支援這個方法。<br/>                                                              |
| **SaveProperties**                                               | 不支援這個方法。<br/>                                                              |
| [**SetButtonState**](setbuttonstate-msvm-ps2mouse.md)           | 設定指定裝置按鈕的目前狀態。<br/>                                     |
| **SetPowerState**                                                | 不支援這個方法。<br/>                                                              |
| [**SetRelativePosition**](setrelativeposition-msvm-ps2mouse.md) | 以指定的水準和垂直差異來位移滑鼠指標的位置。<br/> |
| [**SetScrollPosition**](setscrollposition-msvm-ps2mouse.md)     | 調整指標裝置之滾輪控制項的 z 座標。<br/>                      |
...
指出裝置是否在絕對座標上運作。 如果未設定，則裝置的座標是相對的。
...
當 [**EnabledState**](msvm-keyboard.md) 屬性設定為 1 (其他) 時，元素的啟用或停用狀態。 當 **EnabledState** 是1以外的任何值時，這個屬性必須設定為 **Null** 。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))。
...
指標裝置的型別。 這個屬性繼承自 [**CIM \_ PointingDevice**](/windows/desktop/CIMWin32Prov/cim-pointingdevice)。
...
描述各種 [**OperationalStatus**](msvm-bioselement.md) 陣列值的字串。 這個屬性繼承自 [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement) ，且一律設定為 "OK"。
...
專案的已啟用狀態上次變更的日期或時間。 如果未變更元素的狀態，且已填入此屬性，則必須將它設定為0間隔值。 如果已要求狀態變更，但遭到拒絕或尚未處理，則必須更新屬性。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))。
...
存取 **Msvm \_ Ps2Mouse** 類別可能受 UAC 篩選所限制。 如需詳細資訊，請參閱 [使用者帳戶控制和 WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi)。
