...
抓取由於作業失敗所造成的錯誤。 當作業正在執行或已終止但沒有錯誤時，此方法不會傳回任何 [**CIM \_ 錯誤**](cim-error.md) 實例。 但是，如果工作因為某些內部問題而失敗，或由於用戶端已終止作業，則會傳回 **CIM \_ 錯誤** 實例。
...
如果作業的 **OperationalStatus** 不是「確定」，則傳回 [**CIM \_ 錯誤**](cim-error.md)實例，否則會傳回 **null**。
