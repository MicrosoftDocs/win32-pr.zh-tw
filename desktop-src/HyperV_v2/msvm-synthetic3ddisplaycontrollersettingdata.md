...
**Msvm \_ Synthetic3DDisplayControllerSettingData** 類別具有下列類型的成員：

-   [屬性](#properties)

### <a name="properties"></a>屬性

**Msvm \_ Synthetic3DDisplayControllerSettingData** 類別具有這些屬性。
...
3D 顯示控制器可用的監視器數目上限。 監視器的最小數目是1，而最大值則取決於螢幕解析度上限。 下表定義允許不同解析度的監視器數目上限。
...
> [!Note]  
> 在 Windows 10 和 Windows Server 2016 中新增。 msvm \_ synte
...
保留給虛擬機器使用的 CPU 資源數量。 這些資源保證可供虛擬機器取用。 這個屬性繼承自 [**CIM \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata)。
...
虛擬機器的核心總數。 這個屬性繼承自 [**CIM \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata)。
...
為每個虛擬機器處理器定義權數的整數。 符合所有保留之後，裝載平臺的剩餘實體處理器容量將會根據其相對權數配置給虛擬機器。 這個屬性繼承自 [**CIM \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata)。
