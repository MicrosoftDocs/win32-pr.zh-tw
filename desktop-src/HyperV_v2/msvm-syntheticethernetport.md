...
---
# <a name="msvm_syntheticethernetport-class"></a>Msvm \_ SyntheticEthernetPort 類別

代表綜合乙太網路介面卡。 此介面卡是慣用的網路介面卡，因為它的效能很高，因為它的變更可能會立即生效，但使用 (熱可配置) 。
...
**Msvm \_ SyntheticEthernetPort** 類別具有下列類型的成員：
...
**Msvm \_ SyntheticEthernetPort** 類別具有這些方法。
...
| [**RequestStateChange**](msvm-syntheticethernetport-requeststatechange.md) | 要求狀態變更。<br/>      |
| [**重設**](msvm-syntheticethernetport-reset.md)                           | 重設裝置。<br/>            |
...
**Msvm \_ SyntheticEthernetPort** 類別具有這些屬性。
...
裝置的其他可用性和狀態，超出 **可用性** 屬性中指定的範圍。 這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，而且一律設定為 6 (不適用) 。
...
Ethernet 埠的功能。 例如，裝置可能支援 AlertOnLan、WakeOnLan、負載平衡或容錯移轉。 如果列出容錯移轉或負載平衡功能，則應該也要定義 SpareGroup (容錯移轉) 或 ExtraCapacityGroup (負載平衡) ，以完整描述該功能。 這個屬性繼承自 [**CIM \_ EthernetPort**](/previous-versions/windows/desktop/iscsitarg/cim-ethernetport) ，而且不會使用。
...
自由格式字串的陣列，可針對 **功能** 屬性陣列中所指出的任何 EthernetPort 功能提供更詳細的說明。 請注意，這個陣列的每個專案都與位於相同索引的 [ **功能** ] 屬性陣列中的專案相關。 這個屬性繼承自 [**CIM \_ EthernetPort**](/previous-versions/windows/desktop/iscsitarg/cim-ethernetport) ，而且不會使用。
...
從所有支援的專案清單中啟用的功能，這些功能是在 [ **功能** ] 屬性陣列中定義。 這個屬性繼承自 [**CIM \_ EthernetPort**](/previous-versions/windows/desktop/iscsitarg/cim-ethernetport)，但不會使用它。
...
此元素的已啟用和停用狀態。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))。
...
自由格式字串的陣列，提供 **OtherIdentifyingInfo** 陣列中專案背後的說明和詳細資料。 這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) ，而且不會使用。
...
將乙太網路埠新增到虛擬機器的日期。 這個屬性繼承自 [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)。
...
連結類型的列舉。 當設定為1時 (其他) ，相關的屬性 **OtherLinkTechnology** 會包含連結類型的字串描述。 這個屬性繼承自 [**CIM \_ NetworkPort**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)。
...
埠的最大頻寬（以位/秒為單位）。 這個屬性繼承自 [**CIM \_ LogicalPort**](/previous-versions//cc136869(v=vs.85))。
...
物件的顯示名稱。 這個屬性繼承自 [**CIM \_ ManagedSystemElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)。
...
Ethernet/802.3 MAC 位址格式化為12個十六進位數位 (例如 "010203040506" ) ，每一組代表以標準位 (順序表示之 MAC 位址的六個八位八位組中的其中一個（以字串) 的第一個字元的低序位位表示）。 這個屬性繼承自 [**CIM \_ EthernetPort**](/previous-versions/windows/desktop/iscsitarg/cim-ethernetport)，而且不會使用它。
...
當 **EnabledState** 屬性設定為 1 (其他) 時，元素的啟用或停用狀態。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))。
...
除了可用來識別邏輯裝置的裝置識別碼資訊以外的任何資料。 這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) ，而且不會使用。
...
當 **PortType** 設定為1時 (其他) 的模組類型。 這個屬性繼承自 [**CIM \_ LogicalPort**](/previous-versions//cc136869(v=vs.85))。
...
硬式編碼在埠中的網路位址。 您可以使用固件升級或軟體設定來變更這個硬式編碼的位址。 進行這項變更時，應同時更新欄位。 如果網路介面卡沒有任何硬式編碼位址存在， **PermanentAddress** 屬性應該保留空白。 這個屬性繼承自 [**CIM \_ NetworkPort**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)。
...
埠目前的頻寬（以每秒位數為單位）。 對於頻寬不同的埠，或無法精確估計的埠，這個屬性應該包含名義的頻寬。 這個屬性繼承自 [**CIM \_ NetworkPort**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)。
...
主控電腦系統的 NetBIOS 名稱。 這個屬性會包含 **GUID** 形式的虛擬機器識別碼。 這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)。
...
在某些情況下，邏輯埠可能會識別為前端或後端埠。 如果埠的使用沒有限制，則值應該設定為 4 (不受限制的) 。 這個屬性繼承自 [**CIM \_ LogicalPort**](/previous-versions//cc136869(v=vs.85))。

</dd> </dl>

## <a name="remarks"></a>備註

存取 **Msvm \_ SyntheticEthernetPort** 類別可能受 UAC 篩選所限制。 如需詳細資訊，請參閱 [使用者帳戶控制和 WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi)。
