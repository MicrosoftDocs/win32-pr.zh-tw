...
| [**RequestStateChange**](msvm-serialport-requeststatechange.md) | 要求狀態變更。<br/>      |
| [**重設**](msvm-serialport-reset.md)                           | 重設虛擬裝置。<br/>    |
...
這個屬性會繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，而且會設定為「Microsoft：*GUID* \\ *裝置特定資料*」，其中 *裝置特定資料* 是選擇性的。
...
元素的啟用狀態。 也可以指出這些要求狀態之間的轉換。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))。
...
這個屬性繼承自 [**CIM \_ LogicalPort**](/previous-versions//cc136869(v=vs.85))。
...
要求的埠頻寬（以每秒位數為單位）。 這個屬性繼承自 [**CIM \_ LogicalPort**](/previous-versions//cc136869(v=vs.85))。
...
存取 **Msvm \_ SerialPort** 類別可能受 UAC 篩選所限制。 如需詳細資訊，請參閱 [使用者帳戶控制和 WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi)。
