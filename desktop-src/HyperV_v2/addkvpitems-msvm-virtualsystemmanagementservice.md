...
---
# <a name="addkvpitems-method-of-the-msvm_virtualsystemmanagementservice-class"></a>Msvm VirtualSystemManagementService 類別的 AddKvpItems 方法 \_
...
CIM 系統元件物件 [**\_**](/windows/desktop/CIMWin32Prov/cim-computersystem) 的參考，此物件代表將在其上新增索引鍵/值組的虛擬機器。
...
要加入之索引鍵/值組的陣列。 陣列的每個元素都是 [**Msvm \_ KvpExchangeDataItem**](msvm-kvpexchangedataitem.md) 類別的內嵌實例。 如果目標系統上已經有任何指定的索引鍵/值組，則這個方法會失敗。 這個陣列最多可包含128個元素。
...
下列 c # 範例會將索引鍵/值組加入至虛擬機器。 您可以在 [虛擬化範例 (V2) 的一般公用程式 ](common-utilities-for-the-virtualization-samples-v2.md)中找到參考的公用程式。
...
下列 Visual Basic Scripting Edition (VBScript) 範例會將機碼值組加入至虛擬機器。
