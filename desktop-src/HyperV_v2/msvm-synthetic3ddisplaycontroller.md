...
**Msvm \_ Synthetic3DDisplayController** 類別具有下列類型的成員：
...
**Msvm \_ Synthetic3DDisplayController** 類別具有這些方法。
...
**Msvm \_ Synthetic3DDisplayController** 類別具有這些屬性。
...
顯示控制器的圖形和3D 功能。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))。
...
自由格式字串的陣列，可針對 **AcceleratorCapabilities** 屬性陣列中所指定的任何影片加速器功能提供更詳細的說明。 這個陣列的每個專案都與位於相同索引的 **AcceleratorCapabilities** 屬性陣列中的專案相關。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))。
...
裝置識別碼。 這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，而且一律設定為 "Microsoft：*GUID*"。
...
專案目前的健康情況。 這個屬性工作表示這個專案的健康情況，但不一定是它的子項目。 可能的值是從0到30，其中5表示元素完全狀況良好，而30表示元素完全無法正常運作。 這個屬性繼承自 [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)。
...
支援的最大記憶體數量（以位元組為單位）。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))。
...
提供目前的解析度和可用記憶體所支援的影片頁面數目。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))。
...
當實例的 **VideoMemoryType** 屬性為1時 (其他) 的視訊記憶體類型。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))。
...
上次要求或預期的專案狀態。 元素的實際狀態是由 **EnabledState** 表示。 這個屬性是用來比較最後一個要求和目前啟用或停用的狀態。 特定的 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85)) 實例可能不支援 **RequestStateChange**。 如果發生這種情況，則會使用值 12 (不適用) 。 這個屬性繼承自 **CIM \_ EnabledLogicalElement**，而且一律設定為 2 (啟用) 、3 (停用) 或 12 (不適用) 。
...
視訊記憶體的類型。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))。
...
描述視頻處理器/控制器的字串。 這個屬性繼承自 [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))。
