...
---
# <a name="migrating-virtual-machines"></a>正在遷移虛擬機器

下列 c # 範例示範如何使用 [**MigrateVirtualSystemToHost**](migratevirtualsystemtohost-msvm-virtualsystemmigrationservice.md) 方法來執行簡單的虛擬機器遷移。 此範例會使用資源集區來取得正確的 VHD 路徑。 這段程式碼是從 [hyper-v 虛擬機器遷移範例](https://github.com/microsoftarchive/msdn-code-gallery-microsoft/tree/master/Official%20Windows%20Platform%20Sample/Hyper-V%20virtual%20machine%20migration%20sample)取得。

執行此範例的命令列語法為：

**MigrationSamples.exe vm 和儲存體-簡單的** *SourceHost* *DestinationHost* *VmName*

其中的參數如下：

-   *SourceHost* 是目前虛擬機器主機的名稱。
-   *DestinationHost* 是目的地主機的名稱。
-   *VmName* 是虛擬機器的名稱。
...
下列 c # 程式碼包含從上述範例中呼叫的大部分公用程式方法的執行。
