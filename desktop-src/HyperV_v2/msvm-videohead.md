...
---
# <a name="msvm_videohead-class"></a>Msvm \_ VideoHead 類別
...
| [**RequestStateChange**](msvm-videohead-requeststatechange.md) | 要求狀態變更。<br/>      |
| [**重設**](msvm-videohead-reset.md)                           | 重設虛擬裝置。<br/>    |
...
裝置的任何額外可用性和狀態。 這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，且設定為 6 (不適用) 。
...
用來顯示每個圖元的位數。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
目前的水準圖元數目。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
目前解析度所支援的色彩數目。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
如果在字元模式中，則為這個顯示控制器的資料行數目。 否則為 0。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
如果在字元模式中，則為此影片控制器的資料列數目。 否則為 0。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
目前的重新整理速率（以赫茲為單位）。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
目前的掃描模式。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
<span id="Non-Interlaced_Operation"></span><span id="non-interlaced_operation"></span><span id="NON-INTERLACED_OPERATION"></span>**非交錯操作** (3) 
</dt> <dt>

<span id="Interlaced_Operation"></span><span id="interlaced_operation"></span><span id="INTERLACED_OPERATION"></span>**交錯操作** (4) 
</dt> </dl>
...
目前的垂直圖元數目。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
這個屬性會繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，且會設定為 "Microsoft：*GUID* \\ Head \\ *Index*"。
...
顯示控制器的最大重新整理速率（以赫茲為單位）。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
影片控制器的最小重新整理頻率（以赫茲為單位）。 這個屬性繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))。
...
當實例的 **CurrentScanMode** 屬性為1時 (其他) 的目前掃描模式。 這個屬性會繼承自 [**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85)) ，而且會設定為 **Null**。
...
上次要求或預期的專案狀態。 元素的實際狀態是由 **EnabledState** 表示。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))。
...
字串，指定物件的目前狀態。 這個屬性繼承自 [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)，但不會使用它。
...
存取 **Msvm \_ VideoHead** 類別可能受 UAC 篩選所限制。 如需詳細資訊，請參閱 [使用者帳戶控制和 WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi)。
...
[**CIM \_ VideoHead**](cim-videohead.md)
</dt> <dt>

[**CIM \_ VideoHead**](/previous-versions//cc136948(v=vs.85))
</dt> <dt>
