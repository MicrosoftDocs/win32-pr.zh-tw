...
ms.locfileid: "120107418"
---
# <a name="msvm_transparentbridgingservice-class"></a>Msvm \_ TransparentBridgingService 類別

做為切換內服務的預留位置，以學習 MAC 位址並作為 [**Msvm \_ VirtualEthernetSwitch**](msvm-virtualethernetswitch.md) 和 [**Msvm \_ DynamicForwardingEntry**](msvm-dynamicforwardingentry.md) 類別之間的橋樑。
...
**Msvm \_ TransparentBridgingService** 類別具有下列類型的成員：
...
**Msvm \_ TransparentBridgingService** 類別具有這些方法。



| 方法                                                                           | 描述                         |
|:---------------------------------------------------------------------------------|:------------------------------------|
| [**RequestStateChange**](msvm-transparentbridgingservice-requeststatechange.md) | 要求狀態變更。<br/> |
| [**StartService**](msvm-transparentbridgingservice-startservice.md)             | 啟動服務。<br/>      |
| [**StopService**](msvm-transparentbridgingservice-stopservice.md)               | 停止服務。<br/>       |
...
**Msvm \_ TransparentBridgingService** 類別具有這些屬性。
...
以秒為單位的逾時間隔（以秒為單位）。 這個屬性繼承自 [**CIM \_ TransparentBridgingService**](/previous-versions/windows/desktop/clushyperv/cim-transparentbridgingservice)。
...
物件的簡短描述。 這個屬性繼承自 [**CIM \_ ManagedElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)，一律為「虛擬交換器透明橋接服務」。
...
物件的簡短描述。 這個屬性繼承自 [**CIM \_ 服務**](/windows/desktop/CIMWin32Prov/cim-service)。
...
對物件的描述。 這個屬性繼承自 [**CIM \_ ManagedElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)，一律設定為「Microsoft 虛擬交換器透明橋接服務」。
...
物件的顯示名稱。 除了索引鍵屬性、身分識別資料和描述資訊之外，此屬性還可讓每個實例定義顯示名稱。 這個屬性繼承自 [**CIM \_ ManagedElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)。
...
支援 VLAN 且擁有多個篩選資料庫的參數所使用的篩選資料庫識別碼。 這個屬性繼承自 [**CIM \_ TransparentBridgingService**](/previous-versions/windows/desktop/clushyperv/cim-transparentbridgingservice)。
...
指定物件的安裝時間。 缺少值並不表示物件未安裝。 這個屬性繼承自 [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)，而且不會使用它。
...
請勿使用。 字串的自由格式陣列，提供可在查詢中使用的描述性單字和片語。 因為未標準化，所以不會執行此屬性。 如果這個屬性是必要的查詢結構，則在繼承階層中需要較高的版本。 這個屬性繼承自 [**CIM \_ NetworkService**](/previous-versions//cc136875(v=vs.85))。
...
可唯一識別服務的名稱，並提供所管理功能的指示。 這個屬性繼承自 [**CIM \_ 服務**](/windows/desktop/CIMWin32Prov/cim-service)。
...
當 **EnabledState** 屬性設定為 1 (其他) 時，元素的啟用或停用狀態。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))，而且不會使用它。
...
當 **ProtocolType** 的值為 1 (其他) 時，所轉送的通訊協定類型。 這個屬性繼承自 [**CIM \_ ForwardingService**](/previous-versions/windows/desktop/clushyperv/cim-forwardingservice)。
...
提供有關如何聯繫服務主要擁有者之資訊的字串。 這個屬性繼承自 [**CIM \_ 服務**](/windows/desktop/CIMWin32Prov/cim-service)。
...
服務的主要擁有者名稱（如果有定義的話）。 這個屬性繼承自 [**CIM \_ 服務**](/windows/desktop/CIMWin32Prov/cim-service)。
...
正在轉送的通訊協定類型。 這個屬性繼承自 [**CIM \_ ForwardingService**](/previous-versions/windows/desktop/clushyperv/cim-forwardingservice)。
...
請勿使用。 提供存取服務所需的通訊協定、網路位置和其他服務特定資訊的 URL。 相反地，請使用 **ServiceAccessURI** 類別，它會正確地放置服務存取的語法，並說明資訊的格式。 這個屬性繼承自 [**CIM \_ NetworkService**](/previous-versions//cc136875(v=vs.85))。
...
指出服務是否已啟動 (**True**) ，或停止 (**False**) 。 這個屬性繼承自 [**CIM \_ 服務**](/windows/desktop/CIMWin32Prov/cim-service)。
...
指出服務是否由系統自動啟動、作業系統等，或只在要求時才啟動。 這個屬性繼承自 [**CIM \_ 服務**](/windows/desktop/CIMWin32Prov/cim-service)。
...
請勿使用。 字串的自由格式陣列，指定必須符合才能正確啟動此服務的任何特定前置條件。 這個屬性不會很有用，因為它不是標準化的。 如果這個屬性是必要的結構，則在服務) 上 (的繼承階層架構中需要較高層級的屬性。 這個屬性繼承自 [**CIM \_ NetworkService**](/previous-versions//cc136875(v=vs.85))。
...
請勿使用。 字串的自由格式陣列，指定必須提供給 **StartService** 方法的任何特定參數，才能讓此服務正確啟動。 如果這個方法經過調整，則其參數會更正式地傳達這項資訊。 這個屬性繼承自 [**CIM \_ NetworkService**](/previous-versions//cc136875(v=vs.85))。
...
專案的已啟用狀態上次變更的日期或時間。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))，而且不會使用它。
...
存取 **Msvm \_ TransparentBridgingService** 類別可能受 UAC 篩選所限制。 如需詳細資訊，請參閱 [使用者帳戶控制和 WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi)。
