...
ms.locfileid: "122886969"
---
# <a name="msvm_serialcontroller-class"></a>Msvm \_ SerialController 類別

代表序列控制器的功能和管理。 串列控制器是一律存在於虛擬機器中的邏輯裝置，因此不會透過資源集區配置。 虛擬機器中一律會有一個串列控制器實例。 串列控制器具有固定的埠實例數目。 這種執行支援每個控制器都有兩個埠。

> [!Note]  
> [第2代虛擬機器](virtualization-glossary.md)中的序列控制器是選擇性的。
...
**Msvm \_ SerialController** 類別具有下列類型的成員：
...
**Msvm \_ SerialController** 類別具有這些方法。
...
| [**RequestStateChange**](msvm-serialcontroller-requeststatechange.md) | 要求狀態變更。<br/>      |
| [**重設**](msvm-serialcontroller-reset.md)                           | 重設裝置。<br/>            |
...
**Msvm \_ SerialController** 類別具有這些屬性。
...
在晶片硬體中可能具有固有的序列控制器的緩衝和其他功能。 這個屬性繼承自 [**CIM \_ SerialController**](/windows/desktop/CIMWin32Prov/cim-serialcontroller)。
...
自由格式字串的陣列，可針對 **功能** 陣列中所指出的任何序列控制器功能提供更詳細的說明。 這個屬性繼承自 [**CIM \_ SerialController**](/windows/desktop/CIMWin32Prov/cim-serialcontroller)。
...
自由格式字串的陣列，可提供 **OtherIdentifyingInfo** 屬性陣列中專案背後的說明和詳細資料。 這個屬性繼承自 [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)，而且一律設定為 **Null**。
...
序列控制器支援的最大傳輸速率（以每秒位數為單位）。 這個屬性繼承自 [**CIM \_ SerialController**](/windows/desktop/CIMWin32Prov/cim-serialcontroller)。
...
上次要求或預期的專案狀態。 元素的實際狀態是由 **EnabledState** 表示。 這個屬性是用來比較最後一個要求和目前啟用或停用的狀態。 這個屬性繼承自 [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))。
...
控制器的操作安全性。 這個屬性繼承自 [**CIM \_ SerialController**](/windows/desktop/CIMWin32Prov/cim-serialcontroller)。
...
控制器上次開機的時間。 這個屬性繼承自 [**CIM \_ 控制器**](/windows/desktop/CIMWin32Prov/cim-controller)。
...
存取 **Msvm \_ SerialController** 類別可能受 UAC 篩選所限制。 如需詳細資訊，請參閱 [使用者帳戶控制和 WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi)。
