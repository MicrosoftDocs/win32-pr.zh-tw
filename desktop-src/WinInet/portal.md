...
Microsoft Windows Internet (WinINet) 應用程式開發介面 (API) 可讓應用程式存取標準網際網路通訊協定，例如 FTP 和 HTTP。 為了方便使用，WinINet 將這些通訊協定抽象化為高階介面。
...
WinINet 是設計來供 C/c + + 程式設計人員使用。 它需要對 FTP 和 HTTP 通訊協定有基本的瞭解。

## <a name="run-time-requirements"></a>執行階段需求求

使用 WinINet API 的應用程式需要 Windows NT 4.0 或更新版本，或 Windows Me/98/95。 如需有關使用特定程式設計專案所需之作業系統或元件的詳細資訊，請參閱檔的需求一節。
