---
description: 無論您是設計用來執行線上商務的應用程式、追蹤資訊、存取舊版系統中的資料、訊息、資源規劃、發行和知識管理，或是重要的 LOB (企業營運) 應用程式，您都可以整合 COM + 服務來增強和簡化仲介層的設計。 最常見的方法是將元件群組至 COM + 應用程式。
...
無論您是設計用來執行線上商務的應用程式、追蹤資訊、存取舊版系統中的資料、訊息、資源規劃、發行和知識管理，或是重要的 LOB (企業營運) 應用程式，您都可以整合 COM + 服務來增強和簡化仲介層的設計。 最常見的方法是將元件群組至 COM + 應用程式。

雖然在所有情況下使用 COM + 的完整方法審查已超出本檔的範圍，但下列主題提供您在建立 COM + 應用程式時可使用的資訊。
