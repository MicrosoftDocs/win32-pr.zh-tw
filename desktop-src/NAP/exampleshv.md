...
下列範例會在 NAP 健康原則伺服器上設定 (SHV) 的系統健全狀況驗證程式。

> [!Note]  
> NAP SDK 也包含一組完整的範例程式碼，可在中 \\ 找到。範例 \\ NetDS \\ NAP .。。SDK 安裝的目錄。 此範例集合包含和系統健康情況代理程式， (SHA) 、SHV 和強制用戶端 (EC) 。 它有完整的工作 NAP 案例，可設定 SHA-SHV 與 SHA-1 之間的通訊。
