---
title: 範例 SHV
description: 下列範例會在 NAP 健康原則伺服器上設定 (SHV) 的系統健全狀況驗證程式。
...
ms.locfileid: "104462060"
---
# <a name="example-shv"></a><span data-ttu-id="2692a-103">範例 SHV</span><span class="sxs-lookup"><span data-stu-id="2692a-103">Example SHV</span></span>

> [!Note]  
> <span data-ttu-id="2692a-104">從 Windows 10 開始，無法使用網路存取保護平臺</span><span class="sxs-lookup"><span data-stu-id="2692a-104">The Network Access Protection platform is not available starting with Windows 10</span></span>

 

<span data-ttu-id="2692a-105">下列範例會在 NAP 健康原則伺服器上設定 (SHV) 的系統健全狀況驗證程式。</span><span class="sxs-lookup"><span data-stu-id="2692a-105">The following example sets up a system health validator (SHV) on a NAP health policy server.</span></span>

> [!Note]  
> <span data-ttu-id="2692a-106">NAP SDK 也包含一組完整的範例程式碼，可在中 \\ 找到。範例 \\ NetDS \\ NAP .。。SDK 安裝的目錄。</span><span class="sxs-lookup"><span data-stu-id="2692a-106">The NAP SDK also contains a full set of sample code that can be found in the ...\\Samples\\NetDS\\NAP... directory of your SDK installation.</span></span> <span data-ttu-id="2692a-107">此範例集合包含和系統健康情況代理程式， (SHA) 、SHV 和強制用戶端 (EC) 。</span><span class="sxs-lookup"><span data-stu-id="2692a-107">This sample set includes and system health agent (SHA), SHV, and enforcement client (EC).</span></span> <span data-ttu-id="2692a-108">它有完整的工作 NAP 案例，可設定 SHA-SHV 與 SHA-1 之間的通訊。</span><span class="sxs-lookup"><span data-stu-id="2692a-108">It has full working NAP scenarios setting up communication between SHA-SHV and SHA-EC.</span></span>
