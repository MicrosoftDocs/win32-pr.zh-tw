---
title: " (Windows Media 裝置管理員 11 SDK) 隱私權聲明"
description: 隱私權聲明
ms.assetid: cfa5bc63-9774-418a-b1ae-207b277b63ba
keywords:
- Windows Media 裝置管理員，隱私權聲明
- 裝置管理員，隱私權聲明
- 程式設計指南，隱私權聲明
- 桌面應用程式、隱私權聲明
- 建立 Windows Media 裝置管理員應用程式、隱私權聲明
- 隱私權聲明
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 4b20ff7414d2aa1ab243523bb9eda3ce1ade885f
ms.sourcegitcommit: 8fa6614b715bddf14648cce36d2df22e5232801a
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 12/10/2020
ms.locfileid: "104464100"
---
# <a name="privacy-statement-windows-media-device-manager-11-sdk"></a> (Windows Media 裝置管理員 11 SDK) 隱私權聲明

建立 Windows Media 裝置管理員應用程式的開發人員應該清楚地與他們的使用者溝通其隱私權的方式。 為了協助您建立終端使用者隱私權聲明，Microsoft 已將隱私權指導方針張貼在 [Windows Media 隱私權網頁](https://www.microsoft.com/windows/windowsmedia/privacy/10ssdk.aspx)上。

## <a name="related-topics"></a>相關主題

<dl> <dt>

[**建立 Windows Media 裝置管理員應用程式**](creating-a-windows-media-device-manager-application.md)
</dt> </dl>

 

 




