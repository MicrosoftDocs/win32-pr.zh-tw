...
ms.locfileid: "118585851"
---
# <a name="creating-a-service-provider"></a>建立服務提供者

服務提供者是一種元件，可做為應用程式與裝置之間的中間人。 Windows媒體裝置管理員將應用程式的要求路由傳送到服務提供者，然後負責與裝置通訊或執行要求的動作。 服務提供者通常會與驅動程式進行通訊，以啟用與裝置的通訊。 服務提供者是一種 COM 元件，可執行 Windows 媒體裝置管理員所呼叫的介面。 服務提供者物件的根介面為 [**IMDServiceProvider**](/windows/desktop/api/mswmdm/nn-mswmdm-imdserviceprovider)。 取得此介面之後，Windows Media 裝置管理員可以透過服務提供者的各種方法來取得其他介面。 服務提供者必須實作為的介面會列在 [強制和選擇性的介面](mandatory-and-optional-interfaces.md)中。 介面的階層會顯示在 [服務提供者的介面](interfaces-for-service-providers.md)中。
...
嘗試建立服務提供者之前，您應該徹底瞭解應用程式將對服務提供者進行的呼叫。 閱讀[建立 Windows 媒體裝置管理員應用程式](creating-a-windows-media-device-manager-application.md)，以瞭解應用程式在嘗試與裝置通訊時，會在服務提供者上執行之基本工作和呼叫的一些概念。
...
5.  Windows媒體裝置管理員會藉由呼叫 [**IMDServiceProvider2：： CreateDevice**](/windows/desktop/api/mswmdm/nf-mswmdm-imdserviceprovider2-createdevice)或 [**IMDServiceProvider：： EnumDevices**](/windows/desktop/api/mswmdm/nf-mswmdm-imdserviceprovider-enumdevices)來查詢您的服務提供者，以根據服務提供者是否處理隨插即用裝置來查詢連接的裝置清單。 服務提供者必須傳回代表連線裝置的 [**IMDSPDevice**](/windows/desktop/api/mswmdm/nn-mswmdm-imdspdevice) 物件清單。 如需詳細資訊，請參閱 [列舉裝置](enumerating-devices-service-provider.md) 。
