...
ms.locfileid: "118586733"
---
# <a name="authenticating-the-application"></a>驗證應用程式

您的應用程式必須執行的第一個步驟是驗證。 驗證會驗證應用程式的身分識別，以 Windows 媒體裝置管理員。 驗證您的應用程式之後，您可以呼叫 **QueryInterface** 來取得根 [**IWMDeviceManager**](/windows/desktop/api/mswmdm/nn-mswmdm-iwmdevicemanager) 介面，此介面可針對其他必要的介面進行查詢，而這些介面本身可以針對所有其他介面進行查詢。 在啟動時，驗證只需要進行一次。
