...
ms.locfileid: "120004908"
---
# <a name="sample-service-provider"></a>範例服務提供者

Windows Media 裝置管理員 SDK 包含可供您使用的範例服務提供者。 此服務提供者包含的類別會報告電腦上的每個硬碟，就像是連接的裝置一樣。 將使用此服務提供者的唯一應用程式是範例應用程式;WindowsExplorer 將不會看到此服務提供者所報告的「裝置」。 服務提供者範例是以 ATL 建立的 COM 物件。 下列步驟說明如何使用範例服務提供者：

> [!Note]  
> 範例服務提供者會執行很少的功能，因此不應該用於測試 Windows 媒體裝置管理員應用程式。 若要測試應用程式，請使用完全執行的服務提供者。
