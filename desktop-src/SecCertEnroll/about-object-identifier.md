...
**物件識別碼** 資料類型會編碼成以0X06 的 **標記** 值開頭的 TLV 三個。 點十進位物件識別碼 (OID) 的每個整數都是根據下列規則進行編碼：

-   OID 的前兩個節點會編碼至單一位元組。 第一個節點乘以小數40，並將結果加入至第二個節點的值。
-   小於或等於127的節點值會以一個位元組編碼。
-   大於或等於128的節點值會以多個位元組編碼。 最左邊位元組的位7會設定為1。 每個位元組的位0到6包含編碼的值。

下圖顯示這些點。

![物件識別碼資料類型的 der 編碼](images/der-tlv-oid.png)

下列範例顯示 **ClientId** 屬性如何在憑證要求中進行編碼。
