---
description: 下列範例包含代表 (EOBO) 要求的 CMC 註冊。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
...
下列範例包含代表 (EOBO) 要求的 CMC 註冊。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
