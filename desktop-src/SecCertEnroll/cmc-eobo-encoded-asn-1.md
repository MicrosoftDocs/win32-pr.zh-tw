---
description: 下列範例包含以 asn.1 格式顯示 (EOBO) 要求的 CMC 註冊。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
ms.assetid: 6a339085-de9e-4854-9d17-b8586b0ef5a5
title: CMC EOBO 編碼的 ASN. 1
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "106979944"
---
# <a name="cmc-eobo-encoded-asn1"></a>CMC EOBO 編碼的 ASN. 1

下列範例包含以 asn.1 格式顯示 (EOBO) 要求的 CMC 註冊。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
...
此設定會產生下列範例輸出。 設定會指定 (CMC) 的要求類型、要求註冊之實體的名稱，以及範本的名稱。 使用者範本會指定：

-   要求必須使用 Microsoft 基礎密碼編譯提供者1.0 或 Microsoft 增強型密碼編譯提供者1.0。
-   主體名稱必須從 Active Directory 建立。
-   要求包含憑證範本名稱、增強金鑰使用方法 (EKU) 和金鑰使用延伸模組。 EKU 延伸模組指定已發行的憑證可用於加密檔案系統 (EFS) 、安全電子郵件和用戶端驗證。
