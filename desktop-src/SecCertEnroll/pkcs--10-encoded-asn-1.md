---
description: 下列範例包含 \# 以 asn.1 格式顯示的 PKCS 10 要求。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
ms.assetid: 5001d641-a121-421a-90bf-f2e6e5f88dc8
title: PKCS \# 10 編碼的 ASN. 1
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104195407"
---
# <a name="pkcs-10-encoded-asn1"></a>PKCS \# 10 編碼的 ASN. 1

下列範例包含 \# 以 asn.1 格式顯示的 PKCS 10 要求。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
...
此設定會產生下列範例輸出。 設定會指定主體、要求 (PKCS \# 10) ，以及 (使用者) 的憑證範本。 使用者範本會指定：
