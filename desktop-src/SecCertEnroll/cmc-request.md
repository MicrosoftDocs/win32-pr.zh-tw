---
description: 下列範例包含 CMC 憑證要求。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
...
下列範例包含 CMC 憑證要求。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
...
此設定會產生下列範例輸出。 設定會指定主旨、要求 (CMC) ，以及 (使用者) 的憑證範本。 使用者範本會指定：
...
CMC 憑證要求是以 [**IX509CertificateRequestCmc**](/windows/desktop/api/CertEnroll/nn-certenroll-ix509certificaterequestcmc) 介面表示。
