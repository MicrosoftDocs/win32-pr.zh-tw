...
Asn.1 **八進位字串** 資料類型會編碼成以 0x04 **標記** 位元組開頭的 TLV。 **八進位字串** 和 [位字串](about-bit-string.md)資料類型非常類似。 因此，這兩種類型會以類似的方式進行編碼，但因為 **八位字串** 的尾端位元組不能有未使用的位，所以不能將任何前置位元組新增至內容中。 下列範例是從 [CMC 編碼](cmc-encoded-asn-1.md) 的 asn.1 主題進行調整，顯示憑證範本的名稱如何編碼為位元組陣列。
...
如果位元組陣列包含少於128個位元組，則多位元組的 **長度** 欄位只需要一個位元組來指定內容長度。 如果超過127個位元組，則 [ **長度** ] 欄位中的位7會設定為1，而位6至0則指定用來識別內容長度的其他位元組數目。 這會顯示在下列範例中，第一行的第二個位元組的高序位位設為1，而位元組表示有尾端 **長度** 的位元組。 因此，第三個位元組會指定內容為0x80 位元組的長度。
