---
description: 下列範例包含 \# 代表 (EOBO) 要求的 PKCS 7 註冊。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
ms.assetid: e9e54f26-223d-493a-b4d5-ca7c2bd22756
title: PKCS \# 7 EOBO 要求
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "106975857"
---
# <a name="pkcs-7-eobo-request"></a>PKCS \# 7 EOBO 要求

下列範例包含 \# 代表 (EOBO) 要求的 PKCS 7 註冊。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
...
此設定會產生下列範例輸出。 設定會指定 (PKCS 7) 的要求類型 \# 、要求註冊之實體的名稱，以及範本的名稱。 使用者範本會指定：
