---
description: 下列範例包含以 asn.1 格式顯示的 CMC 更新要求。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
ms.assetid: acb0fdeb-3d7b-4598-bb7a-df97e200984b
title: CMC 更新編碼的 ASN. 1
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104193903"
---
# <a name="cmc-renewal-encoded-asn1"></a>CMC 更新編碼的 ASN. 1

下列範例包含以 asn.1 格式顯示的 CMC 更新要求。 此範例是使用 Certreq.exe 和 Certutil.exe 工具產生的。 用作 Certreq.exe 輸入的 .inf 檔案包含下列設定。
...
設定會指定要求 (CMC) 的類型，以及要求所依據之憑證的序號。 這會產生下列輸出。
