...
這一版的 DirectX SDK 只提供一個版本的 D3DX。 零售 D3DX DLL 包含在 SDK 所提供的可轉散發套件中，並會在 [使用 DirectSetup 安裝 DirectX](https://msdn.microsoft.com/library/ee418267(VS.85).aspx)的過程中自動安裝。 此版本中包含的 D3DX 程式庫相依于此 SDK 隨附的 Direct3D 執行時間。 在此版本中連結至 D3DX 版本的應用程式，也必須從這個 SDK 轉散發執行時間。
