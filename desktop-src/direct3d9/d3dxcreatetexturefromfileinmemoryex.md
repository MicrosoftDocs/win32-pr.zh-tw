...
---
# <a name="d3dxcreatetexturefromfileinmemoryex-function"></a>D3DXCreateTextureFromFileInMemoryEx 函式

從記憶體中的檔案建立紋理。 這是比 [**D3DXCreateTextureFromFileInMemory**](d3dxcreatetexturefromfileinmemory.md)更先進的函式。
...
寬度（以圖元為單位）。 如果這個值為零或 D3DX \_ 預設值，則會從檔案取得維度。
...
高度（以圖元為單位）。 如果這個值為零或 D3DX \_ 預設值，則會從檔案取得維度。
...
一或多個旗標的組合，可控制影像的篩選方式。 指定 \_ 此參數的 D3DX 預設值，相當於指定 D3DX \_ 篩選 \_ 三角形 \| D3DX \_ 篩選 \_ 遞色。 每個有效的篩選準則都必須包含 [D3DX \_ 篩選器](d3dx-filter.md)中的其中一個旗標。
...
一或多個旗標的組合，可控制影像的篩選方式。 指定 \_ 此參數的 D3DX 預設值，相當於指定 D3DX \_ 篩選方塊 \_ 。 每個有效的篩選準則都必須包含 [D3DX \_ 篩選器](d3dx-filter.md)中的其中一個旗標。 此外，使用 bits 27-31 來指定當將 dds 材質載入至記憶體時， (從 mipmap 鏈頂端跳過的 mip 層級數目) 。這可讓您略過32層級。
