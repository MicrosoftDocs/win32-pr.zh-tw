...
[**D3DXMatrixOrthoLH**](d3dxmatrixortholh.md)函式是 **D3DXMatrixOrthoOffCenterLH** 函數的特殊案例。 若要使用 **D3DXMatrixOrthoOffCenterLH** 建立相同的投射，請使用下列值： l =-w/2，r = w/2，b =-h/2，以及 t = h/2。

**D3DXMatrixOrthoOffCenterLH** 函數的所有參數都是相機空間中的距離。 參數描述視圖磁片區的維度。

此函式的傳回值與 *不悅* 參數中所傳回的值相同。 如此一來， **D3DXMatrixOrthoOffCenterLH** 函式就可以用來做為另一個函式的參數。
