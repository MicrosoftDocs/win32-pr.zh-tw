---
description: 本節包含 Direct3D 9 圖形中 D3DX 公用程式程式庫中所包含之結構的相關資訊。
ms.assetid: eee684f1-2d79-444b-86cd-322bf53ab756
title: 'D3DX 結構 (Direct3D 9 圖形) '
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 75d96eae38a8d07d4b89192c255b58fd0f41db0d8c13a3f8d8f0c87c6e477501
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119728208"
---
# <a name="d3dx-structures-direct3d-9-graphics"></a>D3DX 結構 (Direct3D 9 圖形) 

本節包含 D3DX 公用程式程式庫中所包含之結構的相關資訊。

-   [**D3DXATTRIBUTERANGE**](d3dxattributerange.md)
-   [**D3DXATTRIBUTEWEIGHTS**](d3dxattributeweights.md)
-   [**D3DXBONECOMBINATION**](d3dxbonecombination.md)
-   [**D3DXCOLOR**](d3dxcolor.md)
-   [**D3DXCOLOR 延伸模組**](d3dxcolor-extensions.md)
-   [**D3DXCONSTANT \_ DESC**](d3dxconstant-desc.md)
-   [**D3DXCONSTANTTABLE \_ DESC**](d3dxconstanttable-desc.md)
-   [**D3DXFLOAT16**](d3dxfloat16.md)
-   [**D3DXFLOAT16 延伸模組**](d3dxfloat16-extensions.md)
-   [**D3DXFONT \_ DESC**](d3dxfont-desc.md)
-   [**D3DXFRAME**](d3dxframe.md)
-   [**D3DXIMAGE \_ 資訊**](d3dximage-info.md)
-   [**D3DXINTERSECTINFO**](d3dxintersectinfo.md)
-   [**D3DXKEY \_ 回呼**](d3dxkey-callback.md)
-   [**D3DXKEY \_ 四元數**](d3dxkey-quaternion.md)
-   [**D3DXKEY \_ VECTOR3**](d3dxkey-vector3.md)
-   [**D3DXMATERIAL**](d3dxmaterial.md)
-   [**D3DXMATRIX**](d3dxmatrix.md)
-   [D3DXMATRIX 延伸模組](d3dxmatrix-extensions.md)
-   [**D3DXMATRIXA16**](d3dxmatrixa16.md)
-   [**D3DXMESHCONTAINER**](d3dxmeshcontainer.md)
-   [**D3DXMESHDATA**](d3dxmeshdata.md)
-   [**D3DXPATCHINFO**](d3dxpatchinfo.md)
-   [**D3DXPLANE**](d3dxplane.md)
-   [**D3DXPLANE 延伸模組**](d3dxplane-extensions.md)
-   [**D3DXQUATERNION**](d3dxquaternion.md)
-   [**D3DXQUATERNION 延伸模組**](d3dxquaternion-extensions.md)
-   [**D3DXRTE \_ DESC**](d3dxrte-desc.md)
-   [**D3DXRTS \_ DESC**](d3dxrts-desc.md)
-   [**D3DXSHADER \_ CONSTANTINFO**](d3dxshader-constantinfo.md)
-   [**D3DXSHADER \_ CONSTANTTABLE**](d3dxshader-constanttable.md)
-   [**D3DXSHADER \_ STRUCTMEMBERINFO**](d3dxshader-structmemberinfo.md)
-   [**D3DXSHADER \_ TYPEINFO**](d3dxshader-typeinfo.md)
-   [**D3DXSHMATERIAL**](d3dxshmaterial.md)
-   [**D3DXSHPRTSPLITMESHCLUSTERDATA**](d3dxshprtsplitmeshclusterdata.md)
-   [**D3DXSHPRTSPLITMESHVERTDATA**](d3dxshprtsplitmeshvertdata.md)
-   [**D3DXTRACK \_ DESC**](d3dxtrack-desc.md)
-   [**D3DXVECTOR2**](d3dxvector2.md)
-   [D3DXVECTOR2 延伸模組](d3dxvector2-extensions.md)
-   [**D3DXVECTOR3**](d3dxvector3.md)
-   [D3DXVECTOR3 延伸模組](d3dxvector3-extensions.md)
-   [**D3DXVECTOR4**](d3dxvector4.md)
-   [D3DXVECTOR4 延伸模組](d3dxvector4-extensions.md)
-   [**D3DXWeldEpsilons**](d3dxweldepsilons.md)

## <a name="related-topics"></a>相關主題

<dl> <dt>

[D3DX 參考](dx9-graphics-reference-d3dx.md)
</dt> </dl>

 

 



