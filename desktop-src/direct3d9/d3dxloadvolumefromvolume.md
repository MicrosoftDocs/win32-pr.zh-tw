...
[**IDirect3DVolume9**](/windows/win32/api/d3d9helper/nn-d3d9helper-idirect3dvolume9)介面的指標。 指定來源磁片區。
...
寫入至非層級零的音量材質介面，不會讓中途的矩形更新。 如果呼叫 **D3DXLoadVolumeFromVolume** ，但介面尚未變更 (這在正常使用方式下不太可能) ，應用程式必須在介面上明確地呼叫 [**IDirect3DVolumeTexture9：： AddDirtyBox**](/windows/win32/api/d3d9helper/nf-d3d9helper-idirect3dvolumetexture9-adddirtybox) 。
