...
其他編譯器會將 D3DXMATRIXA16 解讀為 D3DXMATRIX。 在未實際對齊矩陣的編譯器上使用此結構可能會造成問題，因為它不會公開會忽略對齊的 bug。 例如，如果 D3DXMATRIXA16 物件位於結構或類別內，則可能會使用緊密封裝完成 [**memcpy**](https://msdn.microsoft.com/library/dswaw1wk(v=VS.71).aspx) ， (忽略16位元組界限) 。 如果編譯器在一段時間加入矩陣時，這會造成組建中斷。
