---
title: 在 ACE 中檢查控制項存取權的範例程式碼
description: 下列 c + + 程式碼範例會在指定物件之 ACL 的 ACE 中，驗證指定的控制項存取權限。
...
---
# <a name="example-code-for-checking-for-a-control-access-right-in-an-ace"></a><span data-ttu-id="46ee7-104">在 ACE 中檢查控制項存取權的範例程式碼</span><span class="sxs-lookup"><span data-stu-id="46ee7-104">Example Code for Checking for a Control Access Right in an ACE</span></span>

<span data-ttu-id="46ee7-105">下列 c + + 程式碼範例會在指定物件之 ACL 的 ACE 中，驗證指定的控制項存取權限。</span><span class="sxs-lookup"><span data-stu-id="46ee7-105">The following C++ code example verifies a specified control access right in an ACE in the ACL of the specified object.</span></span>
