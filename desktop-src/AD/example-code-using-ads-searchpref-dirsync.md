---
title: 使用 ADS_SEARCHPREF_DIRSYNC 的範例程式碼
description: 下列程式碼範例會使用 ADSI 的目錄同步作業 (DirSync) 控制項，在 Active Directory 伺服器的本機網域分割搜尋自上一次呼叫之後變更的使用者物件。
...
---
# <a name="example-code-using-ads_searchpref_dirsync"></a><span data-ttu-id="d7eaf-104">使用 ADS \_ SEARCHPREF DIRSYNC 的範例程式碼 \_</span><span class="sxs-lookup"><span data-stu-id="d7eaf-104">Example Code Using ADS\_SEARCHPREF\_DIRSYNC</span></span>

<span data-ttu-id="d7eaf-105">下列程式碼範例會使用 ADSI 的目錄同步作業 (DirSync) 控制項，在 Active Directory 伺服器的本機網域分割搜尋自上一次呼叫之後變更的使用者物件。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-105">The following code example uses the ADSI implementation of the directory synchronization (DirSync) control to search the local domain partition of an Active Directory server for user objects changed since the previous call.</span></span>

<span data-ttu-id="d7eaf-106">程式碼範例會使用 [**>idirectorysearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) 介面從網域分割的根目錄進行搜尋。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-106">The code example uses the [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) interface to search from the root of the domain partition.</span></span> <span data-ttu-id="d7eaf-107">在呼叫 [**ExecuteSearch**](/windows/desktop/api/iads/nf-iads-idirectorysearch-executesearch) 方法之前，此範例會呼叫 [**SetSearchPreference**](/windows/desktop/api/iads/nf-iads-idirectorysearch-setsearchpreference) 方法來指定 **ads \_ SEARCHPREF \_ 搜尋 \_ 範圍**、 **ads \_ SEARCHPREF \_ DIRSYNC** 和 **ads \_ SEARCHPREF \_** 標記搜尋喜好設定。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-107">Before calling the [**ExecuteSearch**](/windows/desktop/api/iads/nf-iads-idirectorysearch-executesearch) method, the example calls the [**SetSearchPreference**](/windows/desktop/api/iads/nf-iads-idirectorysearch-setsearchpreference) method to specify the **ADS\_SEARCHPREF\_SEARCH\_SCOPE**, **ADS\_SEARCHPREF\_DIRSYNC**, and **ADS\_SEARCHPREF\_TOMBSTONE** search preferences.</span></span> <span data-ttu-id="d7eaf-108">範圍必須指定子樹搜尋。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-108">The scope must specify a subtree search.</span></span> <span data-ttu-id="d7eaf-109">使用 **ads \_ SEARCHPREF \_ DIRSYNC** 搜尋喜好設定，指定 [**ADS \_ >prov \_ 特定**](/windows/win32/api/iads/ns-iads-ads_prov_specific) 結構，其中包含 cookie 的長度和其指標。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-109">With the **ADS\_SEARCHPREF\_DIRSYNC** search preference, specify an [**ADS\_PROV\_SPECIFIC**](/windows/win32/api/iads/ns-iads-ads_prov_specific) structure that contains the length of the cookie and a pointer to it.</span></span>

<span data-ttu-id="d7eaf-110">第一次呼叫此應用程式時，它會指定 null cookie 和零長度。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-110">The first time this application is called, it specifies a null cookie and a zero length.</span></span> <span data-ttu-id="d7eaf-111">這會導致搜尋作業執行完整讀取，並傳回符合篩選準則的所有物件所要求的所有屬性。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-111">This causes the search operation to perform a full read, returning all the requested attributes for all objects that match the filter.</span></span> <span data-ttu-id="d7eaf-112">除了搜尋結果，伺服器會傳回有效的 cookie 和 cookie 長度。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-112">Along with the search results, the server returns a valid cookie and the cookie length.</span></span> <span data-ttu-id="d7eaf-113">在後續執行時，程式會抓取快取的 cookie 和長度，並使用它們來抓取自上一次執行之後的變更。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-113">On subsequent runs, the program retrieves the cached cookie and length and uses them to retrieve changes since the previous run.</span></span>

<span data-ttu-id="d7eaf-114">請注意，此範例只會在登錄中快取 cookie 和 cookie 長度。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-114">Be aware that this sample simply caches the cookie and cookie length in the registry.</span></span> <span data-ttu-id="d7eaf-115">在實際的同步處理應用程式中，將參數儲存在與 Active Directory 伺服器保持一致的相同儲存體中。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-115">In a real synchronization application, store the parameters in the same storage that you are keeping consistent with the Active Directory server.</span></span> <span data-ttu-id="d7eaf-116">這可確保當您的資料庫從備份還原時，參數和物件資料會保持同步。</span><span class="sxs-lookup"><span data-stu-id="d7eaf-116">This ensures that the parameters and object data remain in sync if your database is ever restored from a backup.</span></span>
