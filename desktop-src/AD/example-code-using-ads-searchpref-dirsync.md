...
下列程式碼範例會使用 ADSI 的目錄同步作業 (DirSync) 控制項，在 Active Directory 伺服器的本機網域分割搜尋自上一次呼叫之後變更的使用者物件。

程式碼範例會使用 [**>idirectorysearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) 介面從網域分割的根目錄進行搜尋。 在呼叫 [**ExecuteSearch**](/windows/desktop/api/iads/nf-iads-idirectorysearch-executesearch) 方法之前，此範例會呼叫 [**SetSearchPreference**](/windows/desktop/api/iads/nf-iads-idirectorysearch-setsearchpreference) 方法來指定 **ads \_ SEARCHPREF \_ 搜尋 \_ 範圍**、 **ads \_ SEARCHPREF \_ DIRSYNC** 和 **ads \_ SEARCHPREF \_** 標記搜尋喜好設定。 範圍必須指定子樹搜尋。 使用 **ads \_ SEARCHPREF \_ DIRSYNC** 搜尋喜好設定，指定 [**ADS \_ >prov \_ 特定**](/windows/win32/api/iads/ns-iads-ads_prov_specific) 結構，其中包含 cookie 的長度和其指標。

第一次呼叫此應用程式時，它會指定 null cookie 和零長度。 這會導致搜尋作業執行完整讀取，並傳回符合篩選準則的所有物件所要求的所有屬性。 除了搜尋結果，伺服器會傳回有效的 cookie 和 cookie 長度。 在後續執行時，程式會抓取快取的 cookie 和長度，並使用它們來抓取自上一次執行之後的變更。

請注意，此範例只會在登錄中快取 cookie 和 cookie 長度。 在實際的同步處理應用程式中，將參數儲存在與 Active Directory 伺服器保持一致的相同儲存體中。 這可確保當您的資料庫從備份還原時，參數和物件資料會保持同步。
