...
---
# <a name="creating-a-service-connection-point"></a>建立服務連接點

下列程式碼範例示範如何建立和初始化服務連接點。 程式碼範例會執行額外的步驟，讓服務在執行時間更新 SCP 值。 一般來說，在主機電腦上安裝服務實例時，服務安裝程式會執行這些步驟。

這個程式碼範例會建立 SCP 物件，做為本機電腦物件的子物件。 它會使用 [**GetComputerObjectName**](/windows/desktop/api/secext/nf-secext-getcomputerobjectnamea) 函數來取得本機電腦物件的 DN。 然後，它會使用 DN 來系結至電腦物件的 [**IDirectoryObject**](/windows/desktop/api/iads/nn-iads-idirectoryobject) 指標。 [**IDirectoryObject：： CreateDSObject**](/windows/desktop/api/iads/nf-iads-idirectoryobject-createdsobject)方法會建立 SCP 並指定重要 SCP 屬性的起始值。

[**CreateDSObject**](/windows/desktop/api/iads/nf-iads-idirectoryobject-createdsobject) 會傳回新 scp 的指標，程式碼範例會使用此指標來取得 SCP 的 [**IADs**](/windows/desktop/api/iads/nn-iads-iads) 指標。 程式碼範例會使用 **IADs** 方法來取出 SCP 的 **objectGUID** 和 **distinguishedName** 屬性。 此程式碼範例會使用 **objectGUID** 來撰寫用來系結至 SCP 的字串，然後將 GUID 系結字串快取到本機登錄中，服務可以在執行時間抓取此字串。 **DistinguishedName** 會傳回給函式呼叫端，以便用於撰寫服務主體名稱 (SPN) 作為服務實例。

下列程式碼範例會在安裝啟用目錄的服務的基本步驟中呼叫此函數。 如需詳細資訊，請參閱 [在主機電腦上安裝服務](installing-a-service-on-a-host-computer.md)。
