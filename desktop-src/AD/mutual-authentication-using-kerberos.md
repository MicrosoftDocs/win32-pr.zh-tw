...
Active Directory Domain Services 與 Windows 提供服務主體名稱 (SPN) 的支援，這是 Kerberos 機制中的重要元件，用戶端會用它來驗證服務。 SPN 是唯一的名稱，用來識別服務的實例，並與服務實例執行所在的登入帳戶相關聯。 SPN 的元件可讓用戶端在沒有服務登入帳戶的情況下撰寫服務的 SPN。 這可讓用戶端要求服務驗證其帳戶，即使用戶端沒有帳戶名稱也一樣。
...
本節討論如何使用 Active Directory 網域服務進行相互驗證，特別是在相互驗證中，服務連接點和服務主體名稱的用途。 這並不是如何使用 SSPI 進行相互驗證的完整討論，或是適用于 RPC 和 Windows 通訊端應用程式的驗證和安全性支援。
