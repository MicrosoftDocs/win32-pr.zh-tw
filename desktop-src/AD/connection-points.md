...
| [**serviceInstance**](/windows/desktop/ADSchema/c-serviceinstance)               | Windows 通訊端註冊和解析 (RnR) 名稱服務所使用的連接點物件，可透過 Windows 套 **接字 \* WSA** api 來存取。 如需詳細資訊，請參閱[使用 Windows 通訊端註冊和解析發佈 (RnR) ](publishing-with-windows-sockets-registration-and-resolution.md)。 |
