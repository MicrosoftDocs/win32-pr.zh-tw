...
ms.locfileid: "118190811"
---
# <a name="example-code-for-creating-a-group"></a>建立群組的範例程式碼

下列 c + + 程式碼範例包含一個函式，該函式會使用明確設定的基本屬性 (**cn**、 **sAMAccountType**、 **groupType**) 和不包含任何成員的方式來建立群組。
...
下列 Visual Basic 程式碼範例會建立一個群組，其中只會明確設定必要的屬性 (**cn**、 **sAMAccountType**、 **groupType**) 且不包含任何成員。
