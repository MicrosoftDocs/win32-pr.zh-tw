...
下列程式碼範例會建立兩個物件。 第一個是容器物件，第二個是子容器物件。 子容器物件的值會新增至容器物件的 [**otherWellKnownObjects**](/windows/desktop/ADSchema/a-otherwellknownobjects) 屬性。 程式碼範例會使用 WKGUID 系結來系結至子容器物件，並顯示其 ADsPath。 然後，它會重新命名子容器物件，並使用相同的 WKGUID 系結再次進行系結。
