...
只有在修改現有的架構物件時，才會加諸限制。 架構分為兩個類別。 基底架構中隨附于 Windows 2000 的架構物件屬於類別1。 其他應用程式或使用者稍後透過動態架構延伸新增的任何架構物件都屬於類別2。 架構物件的類別可以由 **classSchema** 物件上 **systemFlags** 屬性中所設定的0x10 位來決定。 這個位只會在類別1物件上設定，且無法變更，也不能在任何類別2物件上設定。

**SystemFlags** 屬性是由 Active Directory Domain Services 在內部使用，以識別基底架構中「基礎結構」物件的特殊特性。 除了識別類別1物件之外， **systemFlags** 還會控制是否可以移動、刪除或重新命名物件。 Windows 2000 相依于執行的物件，將無法執行這些作業。
