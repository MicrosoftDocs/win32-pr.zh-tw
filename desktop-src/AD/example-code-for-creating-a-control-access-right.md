...
ms.locfileid: "118962407"
---
# <a name="example-code-for-creating-a-control-access-right"></a>建立控制項存取權的範例程式碼

下列 Visual Basic 範例會在 Extended-Rights 容器中建立 **controlAccessRight** 物件。
...
下列 c + + 程式碼範例是在 Extended-Rights 容器中建立 **controlAccessRight** 物件的函數。 當您呼叫此函數時，請使用下列格式來指定 *pszRightsGUID* 參數的 GUID 字串。
...
**AppliesTo** 屬性的 **ADSVALUE** 陣列會使用相同的 GUID 格式，並將 **dwType** 成員設定為 **ADSTYPE \_ CASE \_ IGNORE \_ STRING**。
...
您可以使用下列程式碼範例來呼叫這個 **CreateExtendedRight** 範例函數。
