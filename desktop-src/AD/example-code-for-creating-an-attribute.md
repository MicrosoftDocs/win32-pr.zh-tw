...
下列程式碼範例會在架構容器中建立 [**attributeSchema**](/windows/desktop/ADSchema/c-attributeschema) 物件。

**CreateAttribute** 函式會在架構容器中建立 [**attributeSchema**](/windows/desktop/ADSchema/c-attributeschema)物件，但不會將它認可至目錄。 呼叫 [**IADs. SetInfo**](/windows/desktop/api/iads/nf-iads-iads-setinfo) 方法，將新的 **attributeSchema** 物件認可至目錄。

**BytesToVariantArray** 函式是一個公用程式函式，可將八位字串封裝成 variant 陣列。
