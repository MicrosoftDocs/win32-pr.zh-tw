...
下列 C 和 c + + 程式碼範例顯示如何在 ADSI 快取中建立 [**classSchema**](/windows/desktop/ADSchema/c-classschema) 物件。

**CreateClass** 範例函式會示範如何建立 [**classSchema**](/windows/desktop/ADSchema/c-classschema)物件，並將 [**IADs**](/windows/desktop/api/iads/nn-iads-iads)指標傳回至新的物件。 請注意， **CreateClass** 不會呼叫 [**IADs：： SetInfo**](/windows/desktop/api/iads/nf-iads-iads-setinfo) 來認可新的 **classSchema** 物件至目錄。 呼叫端必須使用傳回的指標進行呼叫。

下列 **BytesToVariantArray** 程式碼範例顯示一個公用程式函式，此函式會將八位字串封裝成 variant 陣列。
