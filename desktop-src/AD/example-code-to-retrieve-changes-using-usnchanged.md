...
ms.locfileid: "118189691"
---
# <a name="example-code-to-retrieve-changes-using-usnchanged"></a>使用 USNChanged 取出變更的範例程式碼

下列程式碼範例會在 Active Directory Domain Services 中使用物件的 **uSNChanged** 屬性，以抓取自上一個查詢之後的變更。 程式碼範例可以執行完整同步處理或累加式更新。 若要進行完整的同步處理，範例應用程式會連接到網域控制站的 rootDSE，並讀取它儲存的下列參數，以用於下一個增量同步處理：

-   DC 的 DNS 名稱。 增量同步處理必須在與先前同步處理相同的 DC 上執行。
-   DC 的 **invocationID** GUID。 此程式碼範例會使用此值來偵測是否已從備份還原 DC，在這種情況下，範例必須執行完整的同步處理。
-   **HighestCommittedUSN**。 此值會在下一次遞增同步處理時成為 **uSNChanged** 篩選的下限。

此程式碼範例會使用 [**>idirectorysearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) 介面，並指定搜尋基底的分辨名稱、搜尋範圍和篩選準則。 搜尋基底或範圍沒有任何限制。 除了指定感興趣的物件之外，篩選也必須指定 **uSNChanged** 比較，例如 "uSNChanged >= <lower bound USN> "。 若為完整同步處理，「 &lt; 下限 USN」 &gt; 為零。 針對增量同步處理，它是1加上先前搜尋的 **highestCommittedUSN** 值。

請注意，這個範例應用程式的目的只是要示範如何使用 **uSNChanged** 從 Active Directory 伺服器抓取變更。 它會列印變更，而不會實際同步處理次要儲存體中的資料。 因此，它不會示範如何處理移動物件或「沒有父代」條件等問題。 它會示範如何取出已刪除的物件，但不會顯示應用程式如何使用已刪除物件的 **objectGUID** 來判斷要在儲存體中刪除的對應物件。

此外，此範例會在登錄中快取 DC 名稱、 **invocationID** 和 **highestCommittedUSN** 。 在實際的同步處理應用程式中，您必須將參數儲存在與 Active Directory 伺服器保持一致的相同儲存體中。 這可確保當您的資料庫從備份還原時，參數和物件資料會保持同步。
