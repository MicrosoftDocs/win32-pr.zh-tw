...
ms.locfileid: "118024155"
---
# <a name="backing-up-and-restoring-an-active-directory-server"></a>備份與還原 Active Directory 伺服器

Active Directory Domain Services 提供在目錄資料庫中備份和還原資料的功能。 本節說明如何 [備份](backing-up-an-active-directory-server.md) 和 [還原](restoring-an-active-directory-server.md) Active Directory 的伺服器。 如需使用 Windows 2000 和 Windows server 2003 作業系統中提供的公用程式來備份 Active Directory 伺服器的詳細資訊，請參閱 Microsoft TechNet 網站上提供的適用資源套件。
...
雖然本節中的主題僅說明如何備份和還原 Active Directory 伺服器，但請注意 Windows 2000 和 Windows server 2003 作業系統都有數個必須同時備份和還原的「系統狀態」元件。 這些系統狀態元件包含：
