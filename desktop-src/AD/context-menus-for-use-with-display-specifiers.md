...
Active Directory 系統管理 MMC 嵌入式管理單元和 Windows 2000 shell 提供一種機制，可將專案新增至 Active Directory Domain Services 中物件所顯示的內容功能表。 您可以藉由實作為 *內容功能表延伸* 的 COM 內部進程伺服器來加入內容功能表項目。 您也可以新增操作功能表項目，以叫用任何以 [**ShellExecute**](/windows/win32/api/shellapi/nf-shellapi-shellexecutea) API 啟動的檔案，例如應用程式或網頁 URL。 這就是所謂的 *靜態內容功能表項目*。
