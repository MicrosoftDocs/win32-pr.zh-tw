...
若要初始化主控描繪清單方塊，您的應用程式必須針對每個清單方塊專案載入文字字串和相關聯的點陣圖。

在下列 c + + 程式碼範例中，對話方塊程式會藉由傳送 [**lb \_ ADDSTRING**](lb-addstring.md)訊息來設定文字，然後傳送 [**lb \_ SETITEMDATA**](lb-setitemdata.md)訊息來使點陣圖與每個清單方塊專案產生關聯，藉此初始化清單方塊（ **IDC \_ 列出 \_ 內容**）。 程式碼也會藉由處理 wm [**\_ MEASUREITEM**](wm-measureitem.md) 訊息來設定每個清單方塊專案的高度，並藉由處理 [**wm \_ DRAWITEM**](wm-drawitem.md) 訊息來繪製每個專案的文字和點陣圖。
