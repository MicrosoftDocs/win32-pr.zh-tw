---
title: Rich Edit 訊息
description: Rich Edit 訊息
ms.assetid: 3a20f28f-de78-4a41-8a8d-8fd896c8ae30
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: b07d37a9b99224b590dacd12fafc5da3e94ae7e94481d174ba0872d31c510ea5
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119970298"
---
# <a name="rich-edit-messages"></a>Rich Edit 訊息

## <a name="in-this-section"></a>本節內容

-   [**EM \_ AUTOURLDETECT**](em-autourldetect.md)
-   [**EM \_ CALLAUTOCORRECTPROC**](em-callautocorrectproc.md)
-   [**EM \_ CANPASTE**](em-canpaste.md)
-   [**EM \_ CANREDO**](em-canredo.md)
-   [**EM \_ CONVPOSITION**](em-convposition.md)
-   [**EM \_ DISPLAYBAND**](em-displayband.md)
-   [**EM \_ EXGETSEL**](em-exgetsel.md)
-   [**EM \_ EXLIMITTEXT**](em-exlimittext.md)
-   [**EM \_ EXLINEFROMCHAR**](em-exlinefromchar.md)
-   [**EM \_ EXSETSEL**](em-exsetsel.md)
-   [**EM \_ FINDTEXT**](em-findtext.md)
-   [**EM \_ FINDTEXTEX**](em-findtextex.md)
-   [**EM \_ FINDTEXTEXW**](em-findtextexw.md)
-   [**EM \_ FINDTEXTW**](em-findtextw.md)
-   [**EM \_ FINDWORDBREAK**](em-findwordbreak.md)
-   [**EM \_ FORMATRANGE**](em-formatrange.md)
-   [**EM \_ GETAUTOCORRECTPROC**](em-getautocorrectproc.md)
-   [**EM \_ GETAUTOURLDETECT**](em-getautourldetect.md)
-   [**EM \_ GETBIDIOPTIONS**](em-getbidioptions.md)
-   [**EM \_ GETCHARFORMAT**](em-getcharformat.md)
-   [**EM \_ GETCTFMODEBIAS**](em-getctfmodebias.md)
-   [**EM \_ GETCTFOPENSTATUS**](em-getctfopenstatus.md)
-   [**EM \_ GETEDITSTYLE**](em-geteditstyle.md)
-   [**EM \_ GETEDITSTYLEEX**](em-geteditstyleex.md)
-   [**EM \_ GETELLIPSISMODE**](em-getellipsismode.md)
-   [**EM \_ GETELLIPSISSTATE**](em-getellipsisstate.md)
-   [**EM \_ GETEVENTMASK**](em-geteventmask.md)
-   [**EM \_ GETHYPHENATEINFO**](em-gethyphenateinfo.md)
-   [**EM \_ GETIMECOLOR**](em-getimecolor.md)
-   [**EM \_ GETIMECOMPMODE**](em-getimecompmode.md)
-   [**EM \_ GETIMECOMPTEXT**](em-getimecomptext.md)
-   [**EM \_ GETIMEMODEBIAS**](em-getimemodebias.md)
-   [**EM \_ GETIMEOPTIONS**](em-getimeoptions.md)
-   [**EM \_ GETIMEPROPERTY**](em-getimeproperty.md)
-   [**EM \_ GETLANGOPTIONS**](em-getlangoptions.md)
-   [**EM \_ GETOLEINTERFACE**](em-getoleinterface.md)
-   [**EM \_ GETOPTIONS**](em-getoptions.md)
-   [**EM \_ GETPAGEROTATE**](em-getpagerotate.md)
-   [**EM \_ GETPARAFORMAT**](em-getparaformat.md)
-   [**EM \_ GETPUNCTUATION**](em-getpunctuation.md)
-   [**EM \_ GETREDONAME**](em-getredoname.md)
-   [**EM \_ GETSCROLLPOS**](em-getscrollpos.md)
-   [**EM \_ GETSELTEXT**](em-getseltext.md)
-   [**EM \_ GETSTORYTYPE**](em-getstorytype.md)
-   [**EM \_ GETTABLEPARMS**](em-gettableparms.md)
-   [**EM \_ GETTEXTEX**](em-gettextex.md)
-   [**EM \_ GETTEXTLENGTHEX**](em-gettextlengthex.md)
-   [**EM \_ GETTEXTMODE**](em-gettextmode.md)
-   [**EM \_ GETTEXTRANGE**](em-gettextrange.md)
-   [**EM \_ GETTOUCHOPTIONS**](em-gettouchoptions.md)
-   [**EM \_ GETTYPOGRAPHYOPTIONS**](em-gettypographyoptions.md)
-   [**EM \_ GETUNDONAME**](em-getundoname.md)
-   [**EM \_ GETWORDBREAKPROCEX**](em-getwordbreakprocex.md)
-   [**EM \_ GETWORDWRAPMODE**](em-getwordwrapmode.md)
-   [**EM \_ GETZOOM**](em-getzoom.md)
-   [**EM \_ HIDESELECTION**](em-hideselection.md)
-   [**EM \_ INSERTIMAGE**](em-insertimage.md)
-   [**EM \_ INSERTTABLE**](em-inserttable.md)
-   [**EM \_ ISIME**](em-isime.md)
-   [**EM \_ PASTESPECIAL**](em-pastespecial.md)
-   [**EM \_ 漢字重帶**](em-reconversion.md)
-   [**EM \_ 重做**](em-redo.md)
-   [**EM \_ REQUESTRESIZE**](em-requestresize.md)
-   [**EM \_ SELECTIONTYPE**](em-selectiontype.md)
-   [**EM \_ SETAUTOCORRECTPROC**](em-setautocorrectproc.md)
-   [**EM \_ SETBIDIOPTIONS**](em-setbidioptions.md)
-   [**EM \_ SETBKGNDCOLOR**](em-setbkgndcolor.md)
-   [**EM \_ SETCHARFORMAT**](em-setcharformat.md)
-   [**EM \_ SETCTFMODEBIAS**](em-setctfmodebias.md)
-   [**EM \_ SETCTFOPENSTATUS**](em-setctfopenstatus.md)
-   [**EM \_ SETEDITSTYLE**](em-seteditstyle.md)
-   [**EM \_ SETEDITSTYLEEX**](em-seteditstyleex.md)
-   [**EM \_ SETELLIPSISMODE**](em-setellipsismode.md)
-   [**EM \_ SETEVENTMASK**](em-seteventmask.md)
-   [**EM \_ SETFONTSIZE**](em-setfontsize.md)
-   [**EM \_ SETHYPHENATEINFO**](em-sethyphenateinfo.md)
-   [**EM \_ SETIMECOLOR**](em-setimecolor.md)
-   [**EM \_ SETIMEMODEBIAS**](em-setimemodebias.md)
-   [**EM \_ SETIMEOPTIONS**](em-setimeoptions.md)
-   [**EM \_ SETLANGOPTIONS**](em-setlangoptions.md)
-   [**EM \_ SETOLECALLBACK**](em-setolecallback.md)
-   [**EM \_ >SETOPTIONS**](em-setoptions.md)
-   [**EM \_ SETPARAFORMAT**](em-setparaformat.md)
-   [**EM \_ SETPAGEROTATE**](em-setpagerotate.md)
-   [**EM \_ SETPALETTE**](em-setpalette.md)
-   [**EM \_ SETPUNCTUATION**](em-setpunctuation.md)
-   [**EM \_ SETSCROLLPOS**](em-setscrollpos.md)
-   [**EM \_ SETSTORYTYPE**](em-setstorytype.md)
-   [**EM \_ SETTABLEPARMS**](em-settableparms.md)
-   [**EM \_ SETTARGETDEVICE**](em-settargetdevice.md)
-   [**EM \_ SETTEXTEX**](em-settextex.md)
-   [**EM \_ SETTEXTMODE**](em-settextmode.md)
-   [**EM \_ SETTOUCHOPTIONS**](em-settouchoptions.md)
-   [**EM \_ SETTYPOGRAPHYOPTIONS**](em-settypographyoptions.md)
-   [**EM \_ SETUIANAME**](em-setuianame.md)
-   [**EM \_ SETUNDOLIMIT**](em-setundolimit.md)
-   [**EM \_ SETWORDBREAKPROCEX**](em-setwordbreakprocex.md)
-   [**EM \_ SETWORDWRAPMODE**](em-setwordwrapmode.md)
-   [**EM \_ SETZOOM**](em-setzoom.md)
-   [**EM \_ SHOWSCROLLBAR**](em-showscrollbar.md)
-   [**EM \_ STOPGROUPTYPING**](em-stopgrouptyping.md)
-   [**EM \_ STREAMIN**](em-streamin.md)
-   [**EM \_ STREAMOUT**](em-streamout.md)

 

 




