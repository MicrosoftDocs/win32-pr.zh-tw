...
**安全性警告：** 不當使用此訊息可能會危及程式的安全性。 您必須配置夠大的緩衝區來保存字串。 首先，在 *lParam* 中呼叫傳遞 **Null** 的訊息。 這會傳回所需的字元數，但不包括 **Null**。 然後第二次呼叫訊息以抓取字串。 您應該先複習 [安全性考慮： Microsoft Windows 控制項](sec-comctls.md) ，再繼續進行。
