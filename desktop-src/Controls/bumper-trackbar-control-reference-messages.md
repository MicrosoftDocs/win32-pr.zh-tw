---
title: '[資訊] 控制項訊息'
description: '[資訊] 控制項訊息'
ms.assetid: 1640fc7c-5fe5-4b11-9517-2608b837eedf
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 5fe5d1010fdc737f3c7d098bec6b3ca38e180842e6f0f1ae2a62d378af1870db
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "118414942"
---
# <a name="trackbar-control-messages"></a>[資訊] 控制項訊息

## <a name="in-this-section"></a>本節內容

-   [**TBM \_ CLEARSEL**](tbm-clearsel.md)
-   [**TBM \_ CLEARTICS**](tbm-cleartics.md)
-   [**TBM \_ GETBUDDY**](tbm-getbuddy.md)
-   [**TBM \_ GETCHANNELRECT**](tbm-getchannelrect.md)
-   [**TBM \_ GETLINESIZE**](tbm-getlinesize.md)
-   [**TBM \_ GETNUMTICS**](tbm-getnumtics.md)
-   [**TBM \_ GETPAGESIZE**](tbm-getpagesize.md)
-   [**TBM \_ GETPOS**](tbm-getpos.md)
-   [**TBM \_ GETPTICS**](tbm-getptics.md)
-   [**TBM \_ GETRANGEMAX**](tbm-getrangemax.md)
-   [**TBM \_ GETRANGEMIN**](tbm-getrangemin.md)
-   [**TBM \_ GETSELEND**](tbm-getselend.md)
-   [**TBM \_ GETSELSTART**](tbm-getselstart.md)
-   [**TBM \_ GETTHUMBLENGTH**](tbm-getthumblength.md)
-   [**TBM \_ GETTHUMBRECT**](tbm-getthumbrect.md)
-   [**TBM \_ GETTIC**](tbm-gettic.md)
-   [**TBM \_ GETTICPOS**](tbm-getticpos.md)
-   [**TBM \_ GETTOOLTIPS**](tbm-gettooltips.md)
-   [**TBM \_ GETUNICODEFORMAT**](tbm-getunicodeformat.md)
-   [**TBM \_ SETBUDDY**](tbm-setbuddy.md)
-   [**TBM \_ SETLINESIZE**](tbm-setlinesize.md)
-   [**TBM \_ >SETPAGESIZE METHOD**](tbm-setpagesize.md)
-   [**TBM \_ SETPOS**](tbm-setpos.md)
-   [**TBM \_ SETPOSNOTIFY**](tbm-setposnotify.md)
-   [**TBM \_ SETRANGE**](tbm-setrange.md)
-   [**TBM \_ SETRANGEMAX**](tbm-setrangemax.md)
-   [**TBM \_ SETRANGEMIN**](tbm-setrangemin.md)
-   [**TBM \_ SETSEL**](tbm-setsel.md)
-   [**TBM \_ SETSELEND**](tbm-setselend.md)
-   [**TBM \_ SETSELSTART**](tbm-setselstart.md)
-   [**TBM \_ SETTHUMBLENGTH**](tbm-setthumblength.md)
-   [**TBM \_ SETTIC**](tbm-settic.md)
-   [**TBM \_ SETTICFREQ**](tbm-setticfreq.md)
-   [**TBM \_ SETTIPSIDE**](tbm-settipside.md)
-   [**TBM \_ SETTOOLTIPS**](tbm-settooltips.md)
-   [**TBM \_ SETUNICODEFORMAT**](tbm-setunicodeformat.md)

 

 




