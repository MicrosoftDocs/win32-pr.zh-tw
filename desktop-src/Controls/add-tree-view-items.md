...
---
# <a name="how-to-add-tree-view-items"></a>如何新增 Tree-View 專案

您可以藉由將 [**TVM \_ INSERTITEM**](tvm-insertitem.md) 訊息傳送至控制項，將專案加入至樹狀檢視控制項。 訊息包含 [**TVINSERTSTRUCT**](/windows/win32/api/commctrl/ns-commctrl-tvinsertstructa) 結構的位址，指定父專案、插入新專案之後的專案，以及定義專案屬性的 [**TVITEM**](/windows/win32/api/commctrl/ns-commctrl-tvitema) 結構。 這些屬性包括專案的標籤、其選取的和 nonselected 的影像，以及32位的應用程式定義值。
...
### <a name="add-items-to-the-tree-view"></a>將專案新增至 Tree-View

本節中的範例會示範如何根據應用程式定義陣列中提供的檔標題資訊來建立目錄。 每個陣列元素都包含一個標頭字串，以及一個指出標題層級的整數。 此範例支援三個標題層級 (1、2和 3) 。

此範例包含兩個函數。 第一個函式會將每個標題和伴隨的標題層級解壓縮，然後將它們傳遞給第二個函式。

第二個函式會將專案加入至樹狀檢視控制項。 它使用標題文字作為專案的標籤，並使用標題層級來決定新專案的父專案。 層級的一個標題會加入至樹狀檢視控制項的根目錄中，層級二標題會加入為前一個層級專案的子專案，依此類推。 函數會根據專案是否有子專案，將影像指派給專案。 如果專案有子專案，則會取得代表已關閉資料夾的影像。 否則，它會取得代表檔的影像。 專案會針對選取的和 nonselected 狀態使用相同的影像。
