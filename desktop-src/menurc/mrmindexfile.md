...
ms.locfileid: "118733678"
---
# <a name="mrmindexfile-function"></a>MrmIndexFile 函式

\[某些資訊與預先發行的產品有關，在正式發行之前可能會經過大幅修改。 Microsoft 對此處提供的資訊，不做任何明確或隱含的瑕疵擔保。\]

編制屬於 UWP 應用程式的資源檔索引。 接受明確 (，但) 資源限定詞的選擇性清單。 如需詳細資訊，以及如何使用這些 Api 的案例式逐步解說，請參閱 [封裝資源索引 (PRI) api 和自訂群組建系統](/windows/uwp/app-resources/pri-apis-custom-build-systems)。
...
選擇性的資源限定詞清單，例如 L "language-en-us \_ scale-100 \_ 對比-標準"。 空字串或 **nullptr** 表示中立的資源。 資源限定詞 *不* 會從 *resourceUri* 或 *containerPath* 推斷。
...
如果您想要指定任何資源限定詞，請在 *限定詞* 參數中傳遞它們。 資源限定詞不是從 *resourceUri* 或 *filePath* 推斷 *而* 來。

*ResourceUri* (不是 *filePath*) 的檔案名區段會用來做為資源名稱。
