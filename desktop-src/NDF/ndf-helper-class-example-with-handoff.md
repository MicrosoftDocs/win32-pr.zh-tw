...
ms.locfileid: "119802178"
---
# <a name="ndf-helper-class-extension-with-handoff"></a>具有遞交的 NDF Helper 類別延伸

此協助程式類別對於第一個範例中所編碼的 SimpleFileHelperClass 具有低健康情況的相依性。

第二個遞交 helper 類別是簡單的傳遞 helper 類別，不會執行任何診斷本身。 相反地，它一律會為 SimpleFileHelperClass 產生較低的健康情況假設。 這在作為預留位置，以供未來在此協助程式類別中新增診斷功能時，是很有用的。 「遞交協助程式」類別會執行兩個方法。

LowHealth 方法是用來將診斷狀態設定為 DS \_ 不定。 這會讓 NDF 呼叫 GetLowerHypotheses。
...
接下來，會執行 GetLowerHypotheses，告訴 NDF 要診斷哪一個 Helper 類別。
