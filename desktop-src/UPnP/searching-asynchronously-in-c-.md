...
---
# <a name="searching-asynchronously-in-c"></a><span data-ttu-id="703da-104">在 c + + 中以非同步方式搜尋</span><span class="sxs-lookup"><span data-stu-id="703da-104">Searching Asynchronously in C++</span></span>

<span data-ttu-id="703da-105">您可以使用下列程式碼作為 c + + 應用程式的基礎，以非同步方式搜尋裝置。</span><span class="sxs-lookup"><span data-stu-id="703da-105">The following code can be used as a basis for a C++ application that searches for devices asynchronously.</span></span> <span data-ttu-id="703da-106">程式碼會藉由建立繼承自它的類別，來實 [**IUPnPDeviceFinderCallback**](/windows/desktop/api/Upnp/nn-upnp-iupnpdevicefindercallback) 介面。</span><span class="sxs-lookup"><span data-stu-id="703da-106">The code implements the [**IUPnPDeviceFinderCallback**](/windows/desktop/api/Upnp/nn-upnp-iupnpdevicefindercallback) interface by creating a class that inherits from it.</span></span>

<span data-ttu-id="703da-107">若要同時執行 [**IUPnPDeviceFinderCallback**](/windows/desktop/api/Upnp/nn-upnp-iupnpdevicefindercallback) 和 [**IUPnPDeviceFinderAddCallbackWithInterface**](/windows/desktop/api/Upnp/nn-upnp-iupnpdevicefinderaddcallbackwithinterface)，類別必須繼承自兩個介面。</span><span class="sxs-lookup"><span data-stu-id="703da-107">To implement both [**IUPnPDeviceFinderCallback**](/windows/desktop/api/Upnp/nn-upnp-iupnpdevicefindercallback) and [**IUPnPDeviceFinderAddCallbackWithInterface**](/windows/desktop/api/Upnp/nn-upnp-iupnpdevicefinderaddcallbackwithinterface), the class must inherit from both interfaces.</span></span> <span data-ttu-id="703da-108">**IUPnPDeviceFinderAddCallbackWithInterface** 的執行是選擇性的，但如果您選擇執行，則您也必須執行 **IUPnPDeviceFinderCallback**。</span><span class="sxs-lookup"><span data-stu-id="703da-108">Implementation of **IUPnPDeviceFinderAddCallbackWithInterface** is optional, but if you choose to implement it, you must also implement **IUPnPDeviceFinderCallback**.</span></span>
