...
-   [關於 BC6H/DXGI \_ 格式 \_ BC6H](/windows)
-   [BC6H 執行](#bc6h-implementation)
-   [解碼 BC6H 格式](#decoding-the-bc6h-format)
-   [BC6H 壓縮的端點格式](#bc6h-compressed-endpoint-format)
-   [端點值的正負號延伸](#sign-extension-for-endpoint-values)
-   [端點值的轉換反轉](#transform-inversion-for-endpoint-values)
-   [色彩端點的 Unquantization](#unquantization-of-color-endpoints)
-   [相關主題](#related-topics)

## <a name="about-bc6hdxgi_format_bc6h"></a>關於 BC6H/DXGI \_ 格式 \_ BC6H
...
-   **DXGI \_FORMAT \_ BC6H \_ SF16**。這個 BC6H 格式會在16位浮點色彩通道值中使用正負號位。
...
BC6H 使用固定的 16 位元組 (128 位元) 區塊大小，以及固定之 4x4 材質的磚大小。 與之前的 BC 格式一樣，大於支援之磚大小 (4x4) 的紋理影像，程式會使用多重區塊對其進行壓縮。 此定址身分識別也適用于三維影像、MIP 對應、cubemaps 和材質陣列。 所有影像磚都必須格式相同。

關於 BC6H 格式的一些重要注意事項：

-   BC6H 支援浮點數反正規化，但不支援 INF (無限大) 和 NAN (不是數字)。 例外狀況是 BC6H (DXGI \_ FORMAT \_ BC6H SF16) 的簽署模式 \_ ，它支援-INF (負無限大) 。 請注意，這項對-INF 的支援只是格式本身的成品，而且此格式的編碼器不會特別支援。 一般而言，當編碼器遇到 INF (正面或負面) 或 NaN 輸入資料時，它們應該 \\ 將該資料轉換成允許的最大非 INF 表示值，並在壓縮之前將 NaN 對應至0。
-   BC6H 不支援 Alpha 色板。
-   BC6H 解碼器在執行紋理篩選之前，會先執行解壓縮。
-   BC6H 解壓縮必須是精確的;也就是說，硬體必須傳回與本檔中所述的解碼器相同的結果。

## <a name="bc6h-implementation"></a>BC6H 執行
...
## <a name="decoding-the-bc6h-format"></a>解碼 BC6H 格式
...
### <a name="bc6h-partition-set"></a>BC6H 分割集
...
## <a name="bc6h-compressed-endpoint-format"></a>BC6H 壓縮的端點格式
...
## <a name="sign-extension-for-endpoint-values"></a>端點值的正負號延伸
...
## <a name="transform-inversion-for-endpoint-values"></a>端點值的轉換反轉
...
## <a name="unquantization-of-color-endpoints"></a>色彩端點的 Unquantization
