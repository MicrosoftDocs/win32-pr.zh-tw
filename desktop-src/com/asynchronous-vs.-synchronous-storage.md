...
非同步名字可能也會傳回 [**IBindStatusCallback：： OnDataAvailable**](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/ms775061(v=vs.85))通知中的 [非同步儲存](/windows/desktop/Stg/asynchronous-storage)物件。 當系結仍在進行時，這個儲存物件可能會允許存取某些物件的持續性資料。 用戶端可以選擇兩種儲存模式：封鎖和非封鎖。
