...
VBScript 是 Microsoft Visual Basic for Applications 語言的子集。 由於 VBScript 的目的是要在用戶端應用程式（例如網頁）中安全地使用，因此不會提供檔案輸入和輸出，也不會直接存取基礎作業系統。 這可防止 Web 腳本刪除或改變電腦上的檔案。 如需 VBScript 中不支援之 Visual Basic 功能的完整清單，請參閱 Visual Basic for Applications 檔。
