...
針對相同電腦上的用戶端和伺服器，伺服器的 CLSID 是用戶端的所有需求。 在每部電腦上，COM 會維護一個資料庫 (它會在安裝于系統上的伺服器之所有 Clsid 的所有 Clsid) 使用 Microsoft Windows 和 Macintosh 平臺上的系統登錄。 這是每個 CLSID 和 DLL 或 EXE （裝載該 CLSID 的程式碼）的位置之間的對應。 當用戶端想要建立 COM 類別的實例並使用其服務時，COM 會參考此資料庫，因此用戶端永遠不需要知道程式碼在電腦上的絕對位置。
