...
工作排程器架構會定義用來向工作排程器服務註冊工作的有效 XML。 開發人員可以建立自己的 XML、針對此架構進行驗證，以及使用 [**ITaskFolder：： RegisterTask**](/windows/desktop/api/taskschd/nf-taskschd-itaskfolder-registertask) 方法註冊工作。

工作排程器架構的類型和元素會個別記載于下列各節中。

-   [工作排程器架構元素](task-scheduler-schema-elements.md)
-   [工作排程器架構簡單類型](task-scheduler-schema-simple-types.md)
-   [工作排程器架構複雜類型](task-scheduler-schema-complex-types.md)
-   [工作排程器架構群組](task-scheduler-schema-groups.md)

整個工作排程器架構都是由下列 XSD 檔案所定義。
