---
title: " (XML) 的時間觸發程式範例"
description: 此範例中的 XML 會定義在特定時間開始記事本的工作。
...
ms.locfileid: "119990808"
---
# <a name="time-trigger-example-xml"></a> (XML) 的時間觸發程式範例

此範例中的 XML 會定義在特定時間開始記事本的工作。
...
## <a name="to-define-a-task-to-start-notepad-at-a-specific-time"></a>定義在特定時間開始記事本的工作

下列 XML 範例示範如何使用單一執行動作來定義工作 (啟動記事本) 、在指定時間啟動工作的單一時間觸發程式，以及其他會影響工作排程器如何處理工作的其他工作設定。
...
-   [**動作**](taskschedulerschema-actions-tasktype-element.md)：定義工作執行的動作 (在此案例中，執行記事本) 。
