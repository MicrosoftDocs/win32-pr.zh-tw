...
在呼叫任何其他 [**TaskService**](taskservice.md)方法之前，應該先呼叫 **連線 TaskService** 方法。

如果連線方法失敗，您可以收集錯誤識別碼以找出錯誤的意義。 下表列出錯誤識別碼及其描述。
...
如果您要從 Windows Vista 連接至遠端 Windows Vista 電腦，則需要允許遠端電腦上的遠端排程工作管理防火牆例外。 若要允許此例外，請依序按一下 [開始]、[控制台]、[安全性]、[允許程式通過 Windows 防火牆]，然後選取 [遠端排程工作管理] 核取方塊。 接著按一下 [Windows 防火牆設定] 對話方塊中的 [確定] 按鈕。
