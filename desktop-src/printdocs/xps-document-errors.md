...
<td>出血方塊維度與頁面尺寸不相容。<br/> 出血方塊寬度值必須大於或等於頁面寬度，加上出血方塊來源 x 座標的絕對值。 出血方塊高度值必須大於或等於頁面高度，以及出血方塊來源 y 座標的絕對值。 <br/></td>
</tr>
<tr class="odd">
...
<td><strong>PathGeometry</strong>專案包含一組以<strong>圖形</strong>屬性或子<strong>PathFigure</strong>元素指定的路徑圖形。 幾何的路徑圖形不能同時有 <strong>圖形</strong> 屬性和子 <strong>PathFigure</strong> 元素。 <br/></td>
</tr>
<tr class="even">
...
<td>在其<strong>來源</strong>屬性中指定遠端資源字典的<strong>ResourceDictionary</strong>元素不能包含任何資源定義子系。<br/></td>
</tr>
<tr class="odd">
...
<td>插入號位置值未按順序。 位置值必須以遞增順序排序。 <br/></td>
</tr>
<tr class="even">
...
<td>指定空字串的插入號停止;或者，插入號跳躍索引已超過 Unicode 字串的長度。 <br/></td>
</tr>
<tr class="odd">
...
<td>資源字典中的視覺效果具有 <strong>Name</strong> 屬性，可能不會在 <strong>ResourceDictionary</strong> 元素的任何子系上指定。<br/></td>
</tr>
<tr class="odd">
...
<td>出血方塊矩形包含一或多個無效值。 請參閱參數描述以取得有效的值。 <br/></td>
</tr>
<tr class="odd">
...
<td>Content box 矩形包含一或多個不正確值。 請參閱參數描述以取得有效的值。 <br/></td>
</tr>
<tr class="even">
...
<td><strong>FLOAT</strong>值無效。 它是無限或非數位 (NAN) 。<br/></td>
</tr>
<tr class="even">
...
<td>字型 URI 無效，可能是因為它包含不正確空白片段或字元。<br/></td>
</tr>
<tr class="odd">
...
<td>查閱索引鍵名稱參考的物件不是正確的呼叫類型。例如，如果方法傳回筆刷，但是查閱索引鍵名稱參考幾何物件。<br/></td>
</tr>
<tr class="odd">
...
<td>正在讀取的標記包含不符合 <a href="https://www.ecma-international.org/activities/XML%20Paper%20Specification/XPS%20Standard%20WD%201.6.pdf">XML 檔規格</a>的元素或屬性。<br/>
<blockquote>
[!Note]<br />
為了表示浮點值，XPS OM 會使用 <strong>FLOAT</strong> 資料類型，而不是 <strong>雙精度</strong>浮點數。 如果 XPS 檔的元素具有不符合 <strong>FLOAT</strong> 值的浮點數據，則在還原序列化期間遇到該值時，將會傳回此錯誤。
</blockquote>
<br/> <br/></td>
</tr>
<tr class="even">
...
<td>根據 XML 檔規格，傳遞的字串不是有效的名稱。 <br/></td>
</tr>
<tr class="odd">
...
<td>根據 <a href="https://www.ecma-international.org/activities/XML%20Paper%20Specification/XPS%20Standard%20WD%201.6.pdf">XML 檔規格</a>，查閱索引鍵字串無效。<br/></td>
</tr>
<tr class="even">
...
<td>在一或多個 <a href="/windows/win32/api/xpsobjectmodel/ns-xpsobjectmodel-xps_glyph_mapping"><strong>XPS_GLYPH_MAPPING</strong></a> 結構中，元素不會序列。 <br/></td>
</tr>
<tr class="odd">
...
<td>影像地圖中發生錯誤。<br/> 如果 Unicode 字串是空的，則此錯誤表示也已定義圖像對應。 如果 Unicode 字串是空的，則不得定義圖像對應。<br/> 如果 Unicode 字串不是空的，則此錯誤表示已針對 Unicode 字串以外的字元定義圖像對應。 無法為超出 Unicode 字串長度的字元定義影像地圖。<br/></td>
</tr>
<tr class="odd">
...
<td>色彩設定檔參數為 <strong>Null</strong>，但需要色彩設定檔。 當 XPS_COLOR_TYPE_CONTEXT 色彩類型時，就需要色彩設定檔。 <br/></td>
</tr>
<tr class="even">
...
<td>頁面指的是 discardable 資源，但不會指定 DiscardControl 元件名稱。<br/></td>
</tr>
<tr class="odd">
...
<td><a href="/windows/desktop/api/xpsobjectmodel/nf-xpsobjectmodel-ixpsompackagewriter-addpage"><strong>IXpsOMPackageWriter：： AddPage</strong></a> 是在 <a href="/windows/desktop/api/xpsobjectmodel/nf-xpsobjectmodel-ixpsompackagewriter-startnewdocument"><strong>IXpsOMPackageWriter：： StartNewDocument</strong></a>之前呼叫。<br/></td>
</tr>
<tr class="even">
...
<td>封裝未包含 FixedDocumentSequence。<br/></td>
</tr>
<tr class="odd">
...
<td><a href="/windows/desktop/api/xpsobjectmodel/nn-xpsobjectmodel-ixpsomglyphs"><strong>IXpsOMGlyphs</strong></a>介面需要字型 URI，但未指定一個。<br/></td>
</tr>
<tr class="even">
...
<td>沒有 Unicode 字串的 <a href="/windows/desktop/api/xpsobjectmodel/nn-xpsobjectmodel-ixpsomglyphs"><strong>IXpsOMGlyphs</strong></a> 介面不會指定任何圖像索引。 <strong>IXpsOMGlyphs</strong>介面必須指定 Unicode 字串或圖像索引的陣列。<br/></td>
</tr>
<tr class="odd">
...
<td>頁面尚未命名;只有當頁面具有名稱時，才能設定超連結目標狀態。<br/></td>
</tr>
<tr class="even">
...
<td>FixedDocument 不包含任何 FixedPage 部分。 XPS 檔必須至少包含一個 FixedPage 部分。<br/></td>
</tr>
<tr class="odd">
...
<td>找不到 FixedDocumentSequence 所參考的 FixedDocument 部分。 XPS 檔必須包含至少一個 FixedDocument。<br/></td>
</tr>
<tr class="odd">
...
<td>找不到 FixedDocument 所參考的 FixedPage 部分。 XPS 檔必須至少包含一個 FixedPage 部分。<br/></td>
</tr>
<tr class="even">
...
<td>未指定資源的 <strong>x：Key</strong> 屬性。<br/></td>
</tr>
<tr class="even">
...
<td>頁面或遠端字典內容所參考的資源，不會以頁面關聯性的形式存在。<br/></td>
</tr>
<tr class="odd">
...
<td>呼叫 <a href="/windows/desktop/api/xpsobjectmodel/nf-xpsobjectmodel-ixpsompackagewriter-startnewdocument"><strong>IXpsOMPackageWriter：： StartNewDocument</strong></a>時，未指定參考的受限字型。<br/></td>
</tr>
<tr class="even">
...
<td>已嘗試將 FixedDocumentSequence 新增至已經有一個套件的封裝。 XPS 檔必須只包含一個 FixedDocumentSequence 元件。<br/></td>
</tr>
<tr class="even">
...
<td>嘗試將檔層級列印票證新增至已有一個的 FixedDocument。 XPS 檔中的 FixedDocument 只能包含一個檔層級列印票證。<br/></td>
</tr>
<tr class="odd">
...
<td>嘗試將作業層級列印票證新增至已有一個的 FixedDocumentSequence。 XPS 檔中的 FixedDocumentSequence 只能包含一個作業層級列印票證。<br/></td>
</tr>
<tr class="even">
...
<td>嘗試將頁面層級列印票證新增至已有一個的 FixedPage。 XPS 檔中的 FixedPage 只能包含一個頁面層級的列印票證。<br/></td>
</tr>
<tr class="odd">
...
<td>受限制的字型集合包含重複的受限字型專案。 每個字型專案只可出現在集合中。<br/></td>
</tr>
<tr class="even">
...
<td>嘗試將縮圖影像新增至已經有一個的封裝。 XPS 檔只能包含一個封裝層級的縮圖影像。<br/></td>
</tr>
<tr class="even">
...
<td>嘗試將頁面層級的縮圖影像新增至已有一個的 FixedPage。 XPS 檔中的 FixedPage 只能包含一個頁面層級的縮圖影像。<br/></td>
</tr>
<tr class="odd">
...
<td>嘗試將遠端字典參考新增至遠端字典。 遠端字典無法參考另一個遠端字典。<br/></td>
</tr>
<tr class="odd">
...
<td>介面指標未指向可辨識的介面執行。 不支援自訂 XPS 檔 API 介面的執行。<br/></td>
</tr>
<tr class="even">
...
<td>漸層停駐點的集合少於兩個。 漸層停駐點集合至少必須有兩個漸層停駐點。<br/></td>
</tr>
<tr class="odd">
...
<td>文字字串已指定為面向方向和由右至左。 如果文字是側向方向的，則其雙向層級不能是奇數值 (由右至左) 。 同樣地，如果雙向層級為奇數值，則無法側嚮導向文字。<br/></td>
</tr>
<tr class="even">
...
<td>關聯性是指 XPS 檔之外的部分。 Xps 檔中要轉譯的所有內容都必須包含在 XPS 檔中。<br/></td>
</tr>
<tr class="odd">
...
<td>嘗試將字串複製到新的緩衝區時發生 <strong>size_t</strong> 溢位。<br/></td>
</tr>
<tr class="even">
...
<td>有比 Unicode 程式碼點更多的圖像索引。 如果沒有圖像對應，則圖像索引的數目必須小於或等於 Unicode 程式碼點的數目。<br/></td>
</tr>
<tr class="odd">
...
<td>發生嚴重錯誤，而且 XPS OM 的內容可能無法復原。 XPS OM 的某些元件可能仍可使用，但必須先進行驗證，才能繼續使用。 因為在傳回此錯誤之後無法預測 XPS OM 的狀態，所以應該釋放並捨棄 XPS OM 的所有元件。<br/></td>
</tr>
<tr class="even">
...
<td>當色彩設定檔未預期時，即會出現。 只有在 <a href="/windows/win32/api/xpsobjectmodel/ns-xpsobjectmodel-xps_color"><strong>XPS_COLOR_TYPE_CONTEXT</strong></a>色彩類型時，才允許色彩設定檔。 <br/></td>
</tr>
<tr class="odd">
...
<td>不在資源字典中的路徑幾何已指定 <strong>x：Key</strong> 屬性。 不在資源字典中的路徑幾何不能有 <strong>x：Key</strong> 屬性。<br/></td>
</tr>
</tbody>
</table>
