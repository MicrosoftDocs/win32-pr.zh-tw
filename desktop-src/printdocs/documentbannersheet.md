...
描述要為特定檔輸出的橫幅工作表。 橫幅工作表應該會使用預設 PageMediaType 輸出在預設 PageMediaSize 上。 橫幅工作表也應該與檔的其餘部分隔離。 這表示任何檔完成或處理選項 (例如 DocumentDuplex、DocumentStaple 或 DocumentBinding) 不應包含橫幅工作表。 橫幅工作表可能不會與作業的其餘部分隔離。 這表示任何工作完成或處理選項，都可能包含檔橫幅表。 橫幅工作表應該會以檔的第一頁的形式出現。
