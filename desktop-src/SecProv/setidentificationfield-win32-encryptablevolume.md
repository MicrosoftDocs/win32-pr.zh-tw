...
[**Win32 \_ EncryptableVolume**](win32-encryptablevolume.md)類別的 **SetIdentificationField** 方法會在磁片區的中繼資料中設定指定的識別碼字串。
...
*IdentificationField* \[在中，選擇性\]
</dt> <dd>

類型： **字串**

字串，指定指派給磁片區的識別碼欄位。 如果選擇性字串不存在，則會使用登錄集值。 如果字串存在且不是空的，則會使用指定的值。 *IdentificationField* 參數不區分大小寫。
