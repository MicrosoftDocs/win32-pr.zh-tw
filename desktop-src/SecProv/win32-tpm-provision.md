...
ms.locfileid: "118890806"
---
# <a name="win32_tpmprovision-method"></a>Win32 \_ Tpm：:P rovision 方法

嘗試將 TPM 布建至完全就緒狀態，並將 TPM 的擁有權（如果尚未擁有）。 這種方法的執行成本很高，因為它會執行許多檢查。 建議的應用程式只在必要時才使用這個方法。
...
當設為 **TRUE** 時，方法可能會要求實體存在作業來清除 TPM。 如果設定為 **FALSE**，此方法將不會要求實體存在作業來清除 TPM。

</dd> <dt>

*PhysicalPresencePrompts \_允許* \[ 于\]
</dt> <dd>

當設為 **TRUE** 時，方法可能會要求在開機過程中需要使用者介入的實體存在作業，以確認 TPM 狀態變更。
...
傳回完整布建 TPM 所需內容的最多可用資訊的位元遮罩。 遮罩值（例如資訊 \_ 重新開機）表示方法呼叫應該起始重新開機，以將布建程式轉寄。
...
| <span id="INFORMATION_TPM_SRK_AUTH"></span><span id="information_tpm_srk_auth"></span><dl> <dt>**資訊 \_TPM \_ SRK \_ 驗證**</dt> <dt>0x000000200</dt> </dl>                                  |  (SRK) 授權值的儲存體根金鑰並非全部為零。<br/>                                                                                                                                                            |
