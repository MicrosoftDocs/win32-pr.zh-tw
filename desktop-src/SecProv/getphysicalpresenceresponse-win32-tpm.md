...
[**Win32 \_ tpm**](win32-tpm.md)類別的 **GetPhysicalPresenceResponse** 方法會傳回已執行之 Tpm 實體存在作業的結果。 使用 [**SetPhysicalPresenceRequest**](setphysicalpresencerequest-win32-tpm.md) 方法來要求作業。
...
整數值，指定執行 TPM 實體存在作業的結果。

如果實際存在的使用者確認作業，而且作業執行時沒有錯誤，則實體存在作業會成功。

此值可能包含任何 TPM 錯誤。 下表列出一些常見的錯誤回應。



| 值                                                                                                                                                                                                                                                                    | 意義                                                             |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------|
| <dl> <dt>0</dt> </dl>                                                                                                                                                                                             | 要求的 TPM 操作成功。<br/>              |
| <span id="TPM_E_PPI_USER_ABORT"></span><span id="tpm_e_ppi_user_abort"></span><dl> <dt>**TPM \_E \_ PPI \_ USER \_ ABORT**</dt> <dt>2150171393 (0x80290301)</dt> </dl>       | 使用者已拒絕要求的 TPM 操作。<br/>           |
| <span id="TPM_E_PPI_BIOS_FAILURE"></span><span id="tpm_e_ppi_bios_failure"></span><dl> <dt>**TPM \_E \_ PPI \_ BIOS \_ 失敗**</dt> <dt>2150171394 (0x80290302)</dt> </dl> | 執行 TPM 操作時發生 BIOS 失敗。<br/> |
