...
ms.locfileid: "118155369"
---
# <a name="iamtimelineobjgetsubobject-method"></a>IAMTimelineObj：： GetSubObject 方法
...
方法會抓取 `GetSubObject` 與這個物件相關聯的子物件。
...
接收物件的 **IUnknown** 介面指標。 如果物件沒有子物件，此值會設定為 **Null**。
...
每個時間軸物件都可以保留關聯子物件的指標。

如果 *pVal* 中傳回的值不是 **Null**，則 **IUnknown** 介面具有未處理的參考計數。 使用完畢後，請務必釋放介面。
