...
ms.locfileid: "118402068"
---
# <a name="clock-times"></a>時鐘時間

DirectShow 定義兩個相關的時鐘時間：參考時間和資料流程時間。
...
當應用程式呼叫 [**IMediaControl：： run**](/windows/desktop/api/Control/nf-control-imediacontrol-run)來執行篩選圖形時，篩選 Graph 管理員會在每個篩選器上呼叫 [**IMediaFilter：： run**](/windows/desktop/api/Strmif/nf-strmif-imediafilter-run) 。 為了補償篩選器開始執行所需的時間，篩選 Graph 管理員會在未來指定一段稍微的開始時間。
