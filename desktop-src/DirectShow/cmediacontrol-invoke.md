...
成員的識別碼。 使用 [**CMediaControl：： GetIDsOfNames**](cmediacontrol-getidsofnames.md) 或物件的檔以取得分派識別碼。
...
描述呼叫內容的旗標 `CMediaControl::Invoke` 。
...
\_ \_ 如果 *RIID* 不是 IID Null，則會傳回 \_ 空的 E UNKNOWNINTERFACE。 如果呼叫失敗，則從 [**CMediaControl：： GetTypeInfo**](cmediacontrol-gettypeinfo.md) 傳回其中一個錯誤碼。 否則，會從呼叫 **IDispatch：： Invoke** 傳回 **HRESULT** 。
