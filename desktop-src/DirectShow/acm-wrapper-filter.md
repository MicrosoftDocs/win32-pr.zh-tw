...
作為解壓縮篩選器，LegacyAmFilterCategory) 的 [DirectShow 篩選器] 類別中會出現 [] 的 [DirectShow 篩選器] (類別， \_ 並具有正常的業績 \_ 。 輸入 pin 的連接媒體類型會決定篩選器所使用的編解碼器。 一般而言，應用程式不需要將篩選準則加入至篩選圖形;篩選圖形管理員會在需要時自動提取。 解壓縮只適用于 PCM 音訊。
