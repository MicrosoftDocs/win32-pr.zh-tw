...
-   延展模式。 延展模式會指定 Microsoft® DirectShow®編輯服務 (DES) 如何轉譯大小不符合輸出維度的來源。 根據預設，DES 會伸展影像，而不會保留長寬比。 或者，DES 可以裁剪映射或建立黑邊。 呼叫 [**IAMTimelineSrc：： SetStretchMode**](iamtimelinesrc-setstretchmode.md) 方法來指定延展模式。
