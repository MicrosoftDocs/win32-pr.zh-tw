...
`IAMSetErrorLog`介面會設定或抓取[DirectShow 編輯服務](directshow-editing-services.md) (DES) 中的錯誤記錄檔。 時間軸物件會執行這個介面。 應用程式可以使用這個介面來記錄執行時間的轉譯錯誤。 在您的應用程式中執行 [**IAMErrorLog**](iamerrorlog.md) 介面，然後在時間軸上呼叫 [**IAMSetErrorLog：:p 的 \_ 錯誤**](iamseterrorlog-put-errorlog.md) 記錄檔方法。
