...
當兩個 pin 連接時，其中一個 pin 必須提供配置器。 DirectShow 定義一系列的方法呼叫，用來建立配置器所提供的釘選。 釘選也會同意配置器將建立的緩衝區數目，以及緩衝區的大小。
...
-   [篩選 Graph 中的主題資料 Flow](data-flow-in-the-filter-graph.md)提供 DirectShow 中資料流程的一般總覽。
