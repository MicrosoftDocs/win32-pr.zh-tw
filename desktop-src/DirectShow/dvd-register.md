---
description: DVD \_ 註冊資料類型包含 dvd 一般參數 register 的值 (GPRM) 或 system parameter register (SPRM) 。
ms.assetid: cd1aaeff-241c-4e54-9d05-da5cf61b58df
title: 'DVD_REGISTER (Strmif) '
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: 2e849156b0e23cefa858b361e5ed10a1aecfbcd7f26a86c5194a5166b5213169
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "120043518"
---
# <a name="dvd_register"></a>DVD \_ 註冊

**Dvd \_ 註冊** 資料類型包含 dvd 一般參數 register 的值 (GPRM) 或 system PARAMETER register (SPRM) 。


```C++
typedef WORD DVD_REGISTER;
```



## <a name="requirements"></a>規格需求



| 需求 | 值 |
|-------------------|-------------------------------------------------------------------------------------------------------|
| 標頭<br/> | <dl> <dt>Strmif (包含 Dshow) </dt> </dl> |



## <a name="see-also"></a>另請參閱

<dl> <dt>

[DirectShow資料類型](directshow-data-types.md)
</dt> <dt>

[**GPRMARRAY**](gprmarray.md)
</dt> <dt>

[**SPRMARRAY**](sprmarray.md)
</dt> </dl>

 

 




