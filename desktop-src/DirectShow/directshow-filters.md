...
使用中的包裝函式、AVI 解壓縮程式和 AVI 壓縮程式篩選器可搭配音訊和視訊壓縮管理員使用，讓您可以在 DirectShow 篩選圖形中使用各種編解碼器。

此 (SDK) 的 DirectShow 軟體發展工具組所支援的所有篩選都列在此處。 如果篩選準則出現在 GraphEdit 中，但未記載在此參考區段中，則表示篩選器已由協力廠商安裝，或由其他 Microsoft 技術在內部使用。 DirectShow SDK 不支援這類篩選。
...
| [音訊轉譯器 (WaveOut) ](audio-renderer--waveout--filter.md)                 | 使用 **waveOut \** _ api 來呈現波形音訊。                                                                                                                                                                               |
...
| [SQL-DMO 包裝函式](dmo-wrapper-filter.md)                                           | 讓 DirectShow 應用程式在篩選圖形中使用 [DirectX 媒體物件](directx-media-objects.md) (的) 。                                                                                                            |
...
| [_ *增強的影片* 轉譯器*](enhanced-video-renderer-filter.md)               | 具有與媒體基礎 EVR 媒體接收相同之核心功能和外掛程式模型的影片轉譯器。                                                                                                                           |
...
| [重迭混音器2](overlay-mixer-2-filter.md)                                   | 如同重迭混音器，但可以自動新增至篩選圖形。 (已過時。)                                                                                                                                               |
| [覆蓋混音器](overlay-mixer-filter.md)                                       | 專為使用行21隱藏式字幕的 DVD 播放和廣播影片串流所設計。  (淘汰。 由影片混合轉譯器取代。 )                                                                                  |
...
| [VFW Capture 篩選](vfw-capture-filter.md)                                    | 適用于使用適用于 Windows 的影片的較舊影片捕獲硬體。                                                                                                                                                                |
...
| [Windows Media 來源篩選](windows-media-source-filter.md)                  | 使用 Microsoft MPEG-2 編碼器來播放 Windows Media 和 MPEG 4 內容的預設來源篩選器。 這是 Windows Media Player 6.4 所使用的來源篩選。 (已過時。)                                          |
| [WM ASF 讀取器](wm-asf-reader-filter.md)                                       | Windows Media 內容的檔案播放和使用任何 Microsoft MPEG 4 編碼器 DMOs 建立的內容來源篩選。 必須明確加入至篩選圖形。 此篩選是以 Windows Media Format SDK 為基礎。 |
| [WM ASF 寫入器](wm-asf-writer-filter.md)                                       | 接受未壓縮的輸入資料流程，並使用 Microsoft MPEG-2 編碼器，建立包含 Windows Media 串流或 MPEG 4 串流的 ASF 檔案。 此篩選是以 Windows Media Format SDK 為基礎。                    |
| [WST 編解碼器](wst-codec-filter.md)                                               | 解碼及/或複製 WST 解碼篩選器的已解碼和向前更正錯誤的 Teletext 資料。 (已過時。)                                                                                                             |
| [WST 解碼](wst-decoder-filter.md)                                           | 接受 WST 編解碼器中已解碼的全球標準 Teletext 資料，並使用 Microsoft 提供的字型，將重迭混音器上的點陣圖傳遞給釘選2。 (已過時。)                                                               |
