...
篩選登錄資訊會決定篩選圖形管理員在 [智慧型連接](intelligent-connect.md)期間的功能。 因此，它會影響針對 DirectShow 撰寫的每個應用程式，而不只是將使用您篩選的應用程式。 您應該遵循下列指導方針，以確定您的篩選行為正確。
...
5.  請勿在接受24位 RGB 的「DirectShow 篩選」類別中註冊篩選。 您的篩選準則會干擾色彩空間轉換器篩選。
