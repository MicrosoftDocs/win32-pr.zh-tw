...
**Stop** 方法必須解除封鎖 **接收** 方法，並取消認可篩選器的配置器。 Decommitting 配置器會強制任何暫止的 **GetBuffer** 呼叫傳回，這會解除封鎖正在等候範例的上游篩選。 **Stop** 方法會保存篩選鎖定，然後呼叫 [**CBaseFilter：： Stop**](cbasefilter-stop.md)方法，此方法會在所有篩選器的釘選上呼叫 [**CBasePin：：非**](cbasepin-inactive.md)使用中：
