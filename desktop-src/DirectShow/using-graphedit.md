...
GraphEdit 應用程式的名稱為 "graphedt.exe"。 安裝 SDK 之後，"graphedt.exe" 位於下列目錄： \[ Program Files \] \\ Microsoft sdk \\ Windows \\ \[ version \] \\ Bin \\ 。
...
這些 Dll 可讓 GraphEdit 顯示某些內建的 DirectShow 篩選器的屬性頁。

## <a name="build-a-file-playback-graph"></a>建立檔案播放圖表
...
## <a name="build-a-filter-graph"></a>建立篩選圖形

GraphEdit 可以使用您系統上註冊的任何篩選器來建立自訂篩選圖形。 在 [ **圖形]** 功能表中，按一下 [ **插入篩選**]。 隨即出現一個對話方塊，內含您系統上的篩選器清單，依篩選分類進行組織。 GraphEdit 會從登錄中的資訊建立此清單。 下圖顯示對話方塊。
...
> [!Note]  
> 若要使用 GraphEdit 的內建屬性頁，您必須註冊 proppage.dll。 此 DLL 可在 Windows SDK 中使用。 此 DLL 也包含一些 DirectShow 篩選器的其他屬性頁。 這些屬性頁僅供進行偵錯工具之用。
