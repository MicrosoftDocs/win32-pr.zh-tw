---
description: DirectShow：附錄
ms.assetid: 94cfa199-5be7-4dc2-a78e-a9d1313a26ad
title: DirectShow：附錄
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 35f29034d5b35e5706270b82724df9ef811e597d
ms.sourcegitcommit: a47bd86f517de76374e4fff33cfeb613eb259a7e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 01/06/2021
ms.locfileid: "106972942"
---
# <a name="directshow-appendix"></a>DirectShow：附錄

-   [使用影片畫面](working-with-video-frames.md)
-   [使用編解碼器](working-with-codecs.md)
-   [多媒體串流](multimedia-streaming.md)
-   [國際類比電視微調](international-analog-tv-tuning.md)
-   [使用 C 控制篩選圖形](controlling-filter-graphs-using-c.md)
-   [已淘汰的介面](deprecated-interfaces.md)
-   [AVI 檔案格式](avi-file-format.md)
-   [Windows 中的 DVD 區域變更支援](dvd-region-change-support-in-windows.md)
-   [DVD 光碟機調低時間](dvd-drive-spin-down-timeout.md)
-   [註冊 MPEG2 編解碼器](registering-mpeg2-codecs.md)

## <a name="related-topics"></a>相關主題

<dl> <dt>

[使用 DirectShow](using-directshow.md)
</dt> </dl>

 

 



