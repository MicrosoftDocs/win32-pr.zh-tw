...
影片轉譯器必須正確地處理來自使用者的終止事件。 這表示會正確地隱藏視窗，並知道後續會強制顯示視窗時該怎麼辦。 此外，當轉譯器從篩選圖形中移除時，影片轉譯器必須通知篩選圖形管理員 (或更準確，) 以釋放資源。
