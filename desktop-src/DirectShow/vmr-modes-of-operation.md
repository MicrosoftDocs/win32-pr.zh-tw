...
對於所有設定，VMR 連接到上游篩選器之後，就無法動態新增或移除 pin，但可以連線並中斷連線。 如果應用程式不確定需要多少 pin，則應該將 VMR 設定為可能需要的最大數目。 篩選器上是否有未使用的輸入 pin 不會降低轉譯效能。 不同于舊的覆迭 Mixer，VMR 沒有輸出的 pin，因為它不需要個別的視窗管理篩選。
