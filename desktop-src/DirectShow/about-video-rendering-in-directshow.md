...
ms.locfileid: "119873668"
---
# <a name="about-video-rendering-in-directshow"></a>關於 DirectShow 中的影片轉譯

DirectShow 提供數個轉譯影片的篩選：

-   [影片](video-renderer-filter.md) 轉譯器篩選。 此篩選器適用于所有支援 DirectX 的平臺，而且沒有特定的系統需求。 影片轉譯器會盡可能使用 DirectDraw 來呈現影片;否則，它會使用 GDI。 此篩選是早于 Windows XP 的平臺上的預設影片轉譯器。
...
-   覆迭[Mixer](overlay-mixer-filter.md)濾波器是專門針對 DVD 播放和廣播影片所設計。 它也支援 (VPEs) 的影片埠擴充功能，讓它能夠使用硬體 MPEG-2 解碼器或類比電視調諧器，直接將影片傳送至圖形配接器。
-   從 Windows Vista 開始， (EVR) 篩選器的 [**增強型影片**](enhanced-video-renderer-filter.md)轉譯器可供使用。 相較于舊版的影片轉譯器，它提供改良的影片效能，特別是當 Windows Vista 桌面電腦群組合啟用時。

一般來說，對於以 Windows Vista 或更新版本為目標的應用程式而言，EVR 是慣用的，而 VMR-9 則適用于在舊版 Windows 上執行的應用程式。 如需使用 VMR-7 和 VMR-9 篩選器的詳細資訊，請參閱 [使用影片混合](using-the-video-mixing-renderer.md)轉譯器。
