...
前兩個將來源篩選連接到 AVI 分隔器，並將 AVI 分隔器連接至 DV Mux。 在第一次呼叫時，「捕獲圖形產生器」會自動將 AVI 分隔器加入至圖形，並將其中一個 AVI 分隔器的輸出接點連接至 DV Mux。 在第二個呼叫中，Capture Graph Builder 會尋找 AVI 分隔器的第二個輸出圖釘，並將其連接至 DV Mux。
