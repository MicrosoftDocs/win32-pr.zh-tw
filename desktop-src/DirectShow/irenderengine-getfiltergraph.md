...
使用 [**IRenderEngine：： ConnectFrontEnd**](irenderengine-connectfrontend.md) 方法來建立篩選圖形的前端。 若為預覽版本，請使用 [**IRenderEngine：： RenderOutputPins**](irenderengine-renderoutputpins.md) 來完成圖形。 針對檔案輸出，將前端連接至 mux/檔案寫入器組合。 如需詳細資訊，請參閱轉譯[Project](rendering-a-project.md)。
