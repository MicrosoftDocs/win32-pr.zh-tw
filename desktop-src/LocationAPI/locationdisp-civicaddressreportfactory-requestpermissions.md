...
> [!Note]  
> 如果以受保護模式執行的應用程式（例如瀏覽器協助程式物件） (BHO) 用於 Internet Explorer，則會呼叫 **RequestPermissions**，而使用者在對話方塊中選擇 [ **不要啟用此定位感應器** ] 選項，將不會啟用位置提供者，但是如果相同的使用者再次呼叫 **RequestPermissions** ，則 Windows 會再次顯示此對話方塊。 在受保護模式下執行的應用程式，可能會選擇不在啟動時呼叫 [**RequestPermissions**](/windows/win32/api/locationapi/nf-locationapi-ilocation-requestpermissions) ，讓使用者不會在每次應用程式啟動時看到可能的不必要對話方塊。
