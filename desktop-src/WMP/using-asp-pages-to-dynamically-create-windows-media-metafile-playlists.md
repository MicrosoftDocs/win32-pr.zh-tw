---
title: 使用 ASP 頁面動態建立 Windows 媒體中繼檔播放清單
description: 使用 ASP 頁面動態建立 Windows 媒體中繼檔播放清單
ms.assetid: 9abf04a4-33b9-4502-aaf3-e40de06c7b41
keywords:
- Windows媒體中繼檔播放清單，建立
- 中繼檔播放清單，建立
- 播放清單，建立
- Windows媒體中繼檔播放清單，動態建立
- 中繼檔播放清單，動態建立
- 播放清單，動態建立
- WindowsMedia 中繼檔播放清單，ASP 頁面
- 中繼檔播放清單，ASP 頁面
- 播放清單，ASP 頁面
- 建立 Windows 媒體中繼檔播放清單
- 以動態方式建立 Windows 媒體中繼檔播放清單
- ASP 網頁
ms.topic: article
ms.date: 05/31/2018
topic_type:
...
ms.locfileid: "119762068"
---
# <a name="using-asp-pages-to-dynamically-create-windows-media-metafile-playlists"></a>使用 ASP 頁面動態建立 Windows 媒體中繼檔播放清單

您可以使用 Active Server Pages (ASP 或 .asp 檔案) ，根據使用者提供的資訊動態產生播放清單。 ASP 頁面是一種動態網頁，用來搭配 Microsoft Internet Information Services (IIS) 。 ASP 是一種環境，可讓您合併 HTML、腳本和可重複使用的 ActiveX server 元件，以建立動態且功能強大的 Web 架構商務解決方案。 ASP 頁面為 IIS 提供了伺服器端腳本，其原生支援適用于 Microsoft Visual Basic 腳本撰寫版 (VBScript) 和 microsoft JScript。 本文假設您已熟悉 ASP 並定義變數。

所有的標頭資訊都必須包含在傳回給 Windows Media Player 的 ASP 頁面字串的第一行。

當您使用 ASP 頁面產生播放清單時，必須在 ASP 頁面中指定 **回應** 物件的 **ContentType** 和 **expires** 屬性的值，因為 Windows Media Player 的延遲問題。 **回應. ContentType** 值必須是 Windows 媒體中繼檔的有效副檔名。 可接受的值包括 wma、地板蠟、wmv、300k.wvx、asf 和 asx。
...
下列程式碼是用來產生 Windows 媒體中繼檔播放清單的 ASP 頁面範例。
