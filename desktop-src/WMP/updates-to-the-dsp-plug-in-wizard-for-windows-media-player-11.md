...
-   外掛程式會將執行緒模型註冊為「兩者」。 這可讓外掛程式在 Windows Vista 的媒體基礎管線中執行。 請參閱 *專案名稱*.rgs 檔。
    ...
    5.  您可以藉由執行 **IMFTransform** 來藉由執行 **IMediaObject** 和 MFT，來產生雙重模式外掛程式，以做為 DMO。
