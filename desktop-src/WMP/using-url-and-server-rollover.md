...
您可以使用中繼檔播放清單，在無法存取或播放資料流程時，提供自動變換替代內容來源的方法。 您可以使用這個變換方法，在不同的伺服器或不同類型的伺服器上指定相同內容的來源。 例如，您可以在不同的 Windows Media 伺服器上指定第一個替代。 如果該內容無法播放，用戶端可以變換為 web 伺服器上的第二個替代。 Windows Media Player 會在嘗試播放清單中的換用 Url 之前，根據其 Windows Media 內容設定，自動嘗試變換至不同的通訊協定。
