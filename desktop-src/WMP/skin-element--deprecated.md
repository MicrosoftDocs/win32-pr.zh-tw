...
在 Windows Media 下載套件 (副檔名為 wmd 的) 中， **面板元素可** 讓框線具有內容，並連結至其他網站。 Windows Media 下載套件是壓縮檔案，其中包含 border 檔案和 Windows Media 中繼檔。 副檔名為 wmz 的框線檔 () 會壓縮，並且包含具有 wms 副檔名) 的外觀 (定義檔。
...
Windows Media 下載套件中包含的面板必須是圖形中的矩形。 使用非矩形的外觀建立框線可能會產生非預期的結果。
