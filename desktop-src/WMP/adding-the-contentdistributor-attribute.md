...
ms.locfileid: "119391788"
---
# <a name="adding-the-contentdistributor-attribute"></a>加入 ContentDistributor 屬性

當使用者嘗試播放線上商店內容或將內容複寫到 CD 或裝置時，Windows Media Player 會呼叫 COM 物件中的特定方法。 若要這樣做，播放程式需要一種方式來區別您的內容與其他線上商店提供者的內容。 藉由將您的線上商店金鑰名稱新增為 **ContentDistributor** (的值，這是 Windows 媒體格式 SDK 屬性的別名（名為 **WM/ContentDistributor**) 屬性到 Windows 媒體型內容），可確保播放程式能識別與服務相關聯的內容。
...
-   使用 Windows 媒體格式 SDK，以程式設計方式新增 **WM/ContentDistributor** 屬性。 當您這樣做時，Windows Media Player 會在將數位媒體檔案新增至程式庫時，讀取屬性值並將它新增至資料庫。
...
## <a name="using-the-windows-media-format-sdk"></a>使用 Windows 媒體格式 SDK

Windows Media Player SDK 包含名為 SetContentDistributor 的範例 c + + 檔案，其示範如何使用 Windows 媒體格式9系列 SDK 來新增 **WM/ContentDistributor** 屬性。 您可以在安裝 SDK 的名稱為 Metadata 的資料夾中找到這個範例檔案。 若要使用此程式碼，您必須遵循下列步驟：

1.  安裝 Windows 媒體格式9系列 SDK 並設定執行時間，如檔中所述。
2.  在 Visual Studio 中建立新的空白 c + + 專案，並將名為 SetContentDistributor 的範例檔案新增至專案。
3.  將 Windows 媒體格式9系列 SDK Lib 資料夾的路徑新增至您的檔案路徑清單。 從 [工具]  功能表選擇 [選項] 。
