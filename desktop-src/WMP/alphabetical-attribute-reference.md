---
title: 依字母順序排列屬性參考
description: 依字母順序排列屬性參考
ms.assetid: 12fdeba6-14db-4689-a3cb-cb36dbc9204f
keywords:
- Windows Media Player，媒體專案的屬性
- Windows Media Player行動電話，媒體專案的屬性
- Windows Media Player 物件模型，參考
- 物件模型，參考
- ActiveX 控制項，參考
- Windows Media Player ActiveX 控制項、參考
- Windows Media PlayerMobile ActiveX 控制項，參考
- 物件模型、屬性的參考
- Windows Media Player 物件模型，媒體專案的屬性
- 物件模型、媒體專案的屬性
- Windows Media Player ActiveX 控制項、媒體專案的屬性
- Windows Media Player行動 ActiveX 控制項、媒體專案的屬性
- ActiveX 控制項，媒體專案的屬性
- 屬性，參考
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 41fae63365e9670119d89d11a2faf2d2d9979e7c21aeac2fc5975f072a07f086
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119055316"
---
# <a name="alphabetical-attribute-reference"></a>依字母順序排列屬性參考

本節提供媒體櫃、Cd、Dvd 或其他類型的數位媒體內容中所使用的預先定義屬性清單（依字母順序排列）。 您可以使用其他使用者定義的屬性。

-   [**Abstract 屬性**](abstract-attribute.md)
-   [**AcquisitionTime 屬性**](acquisitiontime-attribute.md)
-   [**AcquisitionTimeDay 屬性**](acquisitiontimeday-attribute.md)
-   [**AcquisitionTimeMonth 屬性**](acquisitiontimemonth-attribute.md)
-   [**AcquisitionTimeYear 屬性**](acquisitiontimeyear-attribute.md)
-   [**AcquisitionTimeYearMonth 屬性**](acquisitiontimeyearmonth-attribute.md)
-   [**AcquisitionTimeYearMonthDay 屬性**](acquisitiontimeyearmonthday-attribute.md)
-   [**AlbumArtistSortOrder 屬性**](albumartistsortorder-attribute.md)
-   [**AlbumID 屬性**](albumid-attribute.md)
-   [**AlbumIDAlbumArtist 屬性**](albumidalbumartist-attribute.md)
-   [**AlbumTitleSortOrder 屬性**](albumtitlesortorder-attribute.md)
-   [**AlternateSourceURL**](alternatesourceurl-attribute.md)
-   [**AudioBitrate 屬性**](audiobitrate-attribute.md)
-   [**AudioFormat 屬性**](audioformat-attribute.md)
-   [**Author 屬性**](author-attribute.md)
-   [**AuthorSortOrder 屬性**](authorsortorder-attribute.md)
-   [**AverageLevel 屬性**](averagelevel-attribute.md)
-   [**位元速率屬性**](bitrate-attribute.md)
-   [**BuyNow 屬性**](buynow-attribute.md)
-   [**BuyTickets 屬性**](buytickets-attribute.md)
-   [**CallLetters 屬性**](callletters-attribute.md)
-   [**CameraManufacturer 屬性**](cameramanufacturer-attribute.md)
-   [**CameraModel 屬性**](cameramodel-attribute.md)
-   [**CDTrackEnabled 屬性**](cdtrackenabled-attribute.md)
-   [**聲道屬性**](channels-attribute.md)
-   [**chapterNum 屬性**](chapternum-attribute.md)
-   [**Comment 屬性**](comment-attribute.md)
-   [**ContentDistributorDuration 屬性**](contentdistributorduration-attribute.md)
-   [**著作權屬性**](copyright-attribute.md)
-   [**Count 屬性**](count-attribute.md)
-   [**CurrentBitrate 屬性**](currentbitrate-attribute.md)
-   [**Description 屬性**](description-attribute.md)
-   [**DisplayArtist 屬性**](displayartist-attribute.md)
-   [**DLNAServerUDN**](dlnaserverudn-attribute.md)
-   [**DLNASourceURI 屬性**](dlnasourceuri-attribute.md)
-   [**DRMIndividualizedVersion 屬性**](drmindividualizedversion-attribute.md)
-   [**DRMKeyID 屬性**](drmkeyid-attribute.md)
-   [**DTCPIPHost**](dtcpiphost-attribute.md)
-   [**DTCPIPPort**](dtcpipport-attribute.md)
-   [**Duration 屬性**](duration-attribute.md)
-   [**DVDID 屬性**](dvdid-attribute.md)
-   [**事件屬性**](event-attribute.md)
-   [**FileSize 屬性**](filesize-attribute.md)
-   [**類型屬性**](filetype-attribute.md)
-   [**FormatTag 屬性**](formattag-attribute.md)
-   [**FourCC 屬性**](fourcc-attribute.md)
-   [**畫面播放速率屬性**](framerate-attribute.md)
-   [**Frequency 屬性**](frequency-attribute.md)
-   [**IsNetworkFeed 屬性**](isnetworkfeed-attribute.md)
-   [**IsVBR 屬性**](isvbr-attribute.md)
-   [**LeadPerformer 屬性**](leadperformer-attribute.md)
-   [**LibraryID**](libraryid-attribute.md)
-   [**>libraryname 屬性**](libraryname-attribute.md)
-   [**Location 屬性**](location-attribute.md)
-   [**媒體屬性**](mediatype-attribute.md)
-   [**ModifiedBy 屬性**](modifiedby-attribute.md)
-   [**MoreInfo 屬性**](moreinfo-attribute.md)
-   [**PartOfSet 屬性**](partofset-attribute.md)
-   [**PeakValue 屬性**](peakvalue-attribute.md)
-   [**PixelAspectRatioX 屬性**](pixelaspectratiox.md)
-   [**PixelAspectRatioY 屬性**](pixelaspectratioy.md)
-   [**PlaylistIndex 屬性**](playlistindex-attribute.md)
-   [**提供者屬性**](provider-attribute.md)
-   [**ProviderLogoURL 屬性**](providerlogourl-attribute.md)
-   [**ProviderURL 屬性**](providerurl-attribute.md)
-   [**RadioBand 屬性**](radioband-attribute.md)
-   [**RadioFormat 屬性**](radioformat-attribute.md)
-   [**RatingOrg 屬性**](ratingorg-attribute.md)
-   [**RecordingTime 屬性**](recordingtime-attribute.md)
-   [**RecordingTimeDay 屬性**](recordingtimeday-attribute.md)
-   [**RecordingTimeMonth 屬性**](recordingtimemonth-attribute.md)
-   [**RecordingTimeYear 屬性**](recordingtimeyear-attribute.md)
-   [**RecordingTimeYearMonth 屬性**](recordingtimeyearmonth-attribute.md)
-   [**RecordingTimeYearMonthDay 屬性**](recordingtimeyearmonthday-attribute.md)
-   [**ReleaseDate 屬性**](releasedate-attribute.md)
-   [**ReleaseDateDay 屬性**](releasedateday-attribute.md)
-   [**ReleaseDateMonth 屬性**](releasedatemonth-attribute.md)
-   [**ReleaseDateYear 屬性**](releasedateyear-attribute.md)
-   [**ReleaseDateYearMonth 屬性**](releasedateyearmonth-attribute.md)
-   [**ReleaseDateYearMonthDay 屬性**](releasedateyearmonthday-attribute.md)
-   [**>webrequest.begingetresponse 屬性**](requeststate-attribute.md)
-   [**ShadowFilePath 屬性**](shadowfilepath-attribute.md)
-   [**SourceURL 屬性**](sourceurl-attribute.md)
-   [**Subject 屬性**](subject-attribute.md)
-   [**SyncState 屬性**](syncstate-attribute.md)
-   **Sync01**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync02**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync03**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync04**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync05**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync06**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync07**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync08**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync09**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync10**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync11**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync12**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync13**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync14**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync15**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   **Sync16**。 請參閱 [**同步屬性**](sync-attributes.md)。
-   [**>synconly 屬性**](synconly-attribute.md)
-   [**暫存屬性**](temporary-attribute.md)
-   [**Title 屬性**](title-attribute.md)
-   [**titleNum 屬性**](titlenum-attribute.md)
-   [**TitleSortOrder 屬性**](titlesortorder-attribute.md)
-   [**TotalDuration 屬性**](totalduration-attribute.md)
-   [**TrackingID 屬性**](trackingid-attribute.md)
-   [**UserCustom1 屬性**](usercustom1-attribute.md)
-   [**UserCustom2 屬性**](usercustom2-attribute.md)
-   [**UserEffectiveRating 屬性**](usereffectiverating-attribute.md)
-   [**UserLastPlayedTime 屬性**](userlastplayedtime-attribute.md)
-   [**UserPlayCount 屬性**](userplaycount-attribute.md)
-   [**UserPlaycountAfternoon 屬性**](userplaycountafternoon-attribute.md)
-   [**UserPlaycountEvening 屬性**](userplaycountevening-attribute.md)
-   [**UserPlaycountMorning 屬性**](userplaycountmorning-attribute.md)
-   [**UserPlaycountNight 屬性**](userplaycountnight-attribute.md)
-   [**UserPlaycountWeekday 屬性**](userplaycountweekday-attribute.md)
-   [**UserPlaycountWeekend 屬性**](userplaycountweekend-attribute.md)
-   [**UserRating 屬性**](userrating-attribute.md)
-   [**UserServiceRating 屬性**](userservicerating-attribute.md)
-   [**VideoBitrate 屬性**](videobitrate-attribute.md)
-   [**VideoFormat 屬性**](videoformat-attribute.md)
-   [**WM/AlbumArtist 屬性**](wm-albumartist-attribute.md)
-   [**WM/AlbumTitle 屬性**](wm-albumtitle-attribute.md)
-   [**WM/Category 屬性**](wm-category-attribute.md)
-   [**WM/Composer 屬性**](wm-composer-attribute.md)
-   [**WM/導體屬性**](wm-conductor-attribute.md)
-   [**WM/ContentDistributor 屬性**](wm-contentdistributor-attribute.md)
-   [**WM/ContentDistributorType 屬性**](wm-contentdistributortype-attribute.md)
-   [**WM/ContentGroupDescription 屬性**](wm-contentgroupdescription-attribute.md)
-   [**WM/Director 屬性**](wm-director-attribute.md)
-   [**WM/EncodingTime 屬性**](wm-encodingtime-attribute.md)
-   [**WM/內容類型屬性**](wm-genre-attribute.md)
-   [**WM/GenreID 屬性**](wm-genreid-attribute.md)
-   [**WM/InitialKey 屬性**](wm-initialkey-attribute.md)
-   [**WM/Language 屬性**](wm-language-attribute.md)
-   [**WM/歌詞屬性**](wm-lyrics-attribute.md)
-   [**WM/MCDI 屬性**](wm-mcdi-attribute.md)
-   [**WM/MediaClassPrimaryID 屬性**](wm-mediaclassprimaryid-attribute.md)
-   [**WM/MediaClassSecondaryID 屬性**](wm-mediaclasssecondaryid-attribute.md)
-   [**WM/MediaOriginalBroadcastDateTime 屬性**](wm-mediaoriginalbroadcastdatetime-attribute.md)
-   [**WM/MediaOriginalChannel 屬性**](wm-mediaoriginalchannel-attribute.md)
-   [**WM/MediaStationName 屬性**](wm-mediastationname-attribute.md)
-   [**WM/情緒屬性**](wm-mood-attribute.md)
-   [**WM/OriginalAlbumTitle 屬性**](wm-originalalbumtitle-attribute.md)
-   [**WM/OriginalArtist 屬性**](wm-originalartist-attribute.md)
-   [**WM/OriginalLyricist 屬性**](wm-originallyricist-attribute.md)
-   [**WM/ParentalRating 屬性**](wm-parentalrating-attribute.md)
-   [**WM/PartOfSet 屬性**](wm-partofset-attribute.md)
-   [**WM/Period 屬性**](wm-period-attribute.md)
-   [**WM/生產者屬性**](wm-producer-attribute.md)
-   [**WM/Set-protectiontype 屬性**](wm-protectiontype-attribute.md)
-   [**WM/提供者屬性**](wm-provider-attribute.md)
-   [**WM/ProviderRating 屬性**](wm-providerrating-attribute.md)
-   [**WM/ProviderStyle 屬性**](wm-providerstyle-attribute.md)
-   [**WM/Publisher 屬性**](wm-publisher-attribute.md)
-   [**WM/SubscriptionContentID 屬性**](wm-subscriptioncontentid-attribute.md)
-   [**WM/子標題屬性**](wm-subtitle-attribute.md)
-   [**WM/SubTitleDescription 屬性**](wm-subtitledescription-attribute.md)
-   [**WM/TrackNumber 屬性**](wm-tracknumber-attribute.md)
-   [**WM/UniqueFileIdentifier 屬性**](wm-uniquefileidentifier-attribute.md)
-   [**WM/VideoFrameRate 屬性**](wm-videoframerate-attribute.md)
-   [**WM/VideoHeight 屬性**](wm-videoheight-attribute.md)
-   [**WM/VideoWidth 屬性**](wm-videowidth-attribute.md)
-   [**WM/WMCollectionGroupID 屬性**](wm-wmcollectiongroupid-attribute.md)
-   [**WM/WMCollectionID 屬性**](wm-wmcollectionid-attribute.md)
-   [**WM/WMContentID 屬性**](wm-wmcontentid-attribute.md)
-   [**WM/WMShadowFileSourceDRMType**](wm-wmshadowfilesourcedrmtype.md)
-   [**WM/WMShadowFileSourceFileType**](wm-wmshadowfilesourcefiletype.md)
-   [**WM/Writer 屬性**](wm-writer-attribute.md)
-   [**WM/Year 屬性**](wm-year-attribute.md)

## <a name="related-topics"></a>相關主題

<dl> <dt>

[**屬性參考**](attribute-reference.md)
</dt> </dl>

 

 




