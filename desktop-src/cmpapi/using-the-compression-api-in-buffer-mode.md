...
下列範例示範如何在緩衝區模式中使用壓縮 API。 緩衝區模式的開發目的是為了方便使用，並自動將輸入緩衝區分割成適合所選壓縮演算法的大社區塊。 緩衝區模式會自動格式化，並將未壓縮的緩衝區大小儲存在可供解壓縮程式使用的壓縮緩衝區中。 壓縮緩衝區的大小不會自動儲存，而且應用程式需要儲存此值以進行解壓縮。 如果您想要在緩衝區模式中使用壓縮 API，請在呼叫 [**CreateCompressor**](/windows/desktop/api/compressapi/nf-compressapi-createcompressor)或 [**CreateDecompressor**](/windows/desktop/api/compressapi/nf-compressapi-createdecompressor)時不要包含 **壓縮 \_ 原始** 旗標。

在大部分情況下，建議使用緩衝區模式。 如需如何使用區塊模式的詳細資訊，請參閱 [在區塊模式中使用壓縮 API](using-the-compression-api-in-block-mode.md)
...
下列程式碼片段示範如何在緩衝區模式中使用壓縮 API 進行檔案解壓縮。
