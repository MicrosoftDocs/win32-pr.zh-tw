...
---
# <a name="using-the-compression-api-in-block-mode"></a><span data-ttu-id="c8da6-103">在區塊模式中使用壓縮 API</span><span class="sxs-lookup"><span data-stu-id="c8da6-103">Using the Compression API in block mode</span></span>

<span data-ttu-id="c8da6-104">下列範例示範如何在區塊模式中使用壓縮 API。</span><span class="sxs-lookup"><span data-stu-id="c8da6-104">The following example demonstrates using the Compression API in block mode.</span></span> <span data-ttu-id="c8da6-105">若要使用區塊模式產生壓縮程式或解壓縮程式，您的應用程式在呼叫 [**CreateCompressor**](/windows/desktop/api/compressapi/nf-compressapi-createcompressor)或 [**CreateDecompressor**](/windows/desktop/api/compressapi/nf-compressapi-createdecompressor)時，必須包含 **壓縮 \_ 原始** 旗標。</span><span class="sxs-lookup"><span data-stu-id="c8da6-105">To generate a compressor or decompressor using block mode, your application must include the **COMPRESS\_RAW** flag when it calls [**CreateCompressor**](/windows/desktop/api/compressapi/nf-compressapi-createcompressor) or [**CreateDecompressor**](/windows/desktop/api/compressapi/nf-compressapi-createdecompressor).</span></span> <span data-ttu-id="c8da6-106">[封鎖] 模式可讓開發人員控制區塊大小，但應用程式需要更多工作。</span><span class="sxs-lookup"><span data-stu-id="c8da6-106">Block mode enables the developer to control the block size, but requires more work be done by the application.</span></span>

<span data-ttu-id="c8da6-107">如果輸入緩衝區的大小大於壓縮演算法的內部區塊大小，區塊模式將會失敗。</span><span class="sxs-lookup"><span data-stu-id="c8da6-107">The block mode will fail if the size of the input buffer is greater than the internal block size of the compression algorithm.</span></span> <span data-ttu-id="c8da6-108">內部區塊大小為 MSZIP 的32KB 和 1 GB，適用于 XPRESS 壓縮演算法。</span><span class="sxs-lookup"><span data-stu-id="c8da6-108">The internal block size is 32KB for the MSZIP and 1GB for the XPRESS compression algorithms.</span></span> <span data-ttu-id="c8da6-109">LZMS 的內部區塊大小可設定為64GB，而且記憶體使用量相對增加。</span><span class="sxs-lookup"><span data-stu-id="c8da6-109">The internal block size for LZMS is configurable up to 64GB with a corresponding increase in memory use.</span></span> <span data-ttu-id="c8da6-110">[**解壓縮**](/windows/desktop/api/compressapi/nf-compressapi-decompress)的 *UncompressedBufferSize* 參數值必須完全等於未壓縮資料的原始大小，而不只是輸出緩衝區的大小。</span><span class="sxs-lookup"><span data-stu-id="c8da6-110">The value of *UncompressedBufferSize* parameter of [**Decompress**](/windows/desktop/api/compressapi/nf-compressapi-decompress) must be exactly equal to the original size of the uncompressed data and not just the size of the output buffer.</span></span> <span data-ttu-id="c8da6-111">這表示您的應用程式將需要指定區塊大小，並儲存解壓縮資料的確切原始大小以供解壓縮程式使用。</span><span class="sxs-lookup"><span data-stu-id="c8da6-111">This means your application will need to specify the block size and save the exact original size of the uncompressed data for use by the decompressor.</span></span> <span data-ttu-id="c8da6-112">壓縮緩衝區的大小不會自動儲存，而且應用程式也需要儲存此值以供解壓縮。</span><span class="sxs-lookup"><span data-stu-id="c8da6-112">The size of the compressed buffer is not automatically saved, and the application also needs to save this for decompression.</span></span>

<span data-ttu-id="c8da6-113">在大部分的情況下，建議使用緩衝區模式，因為它會自動將輸入緩衝區分割成適用于所選壓縮演算法的區塊，並將未壓縮的緩衝區大小儲存在壓縮的緩衝區中。</span><span class="sxs-lookup"><span data-stu-id="c8da6-113">The buffer mode is recommended in most cases because it automatically splits up the input buffer into blocks of a size that is appropriate for the selected compression algorithm stores the uncompressed buffer size in the compressed buffer.</span></span> <span data-ttu-id="c8da6-114">如需如何使用緩衝區模式的詳細資訊，請參閱 [在緩衝區模式中使用壓縮 API](using-the-compression-api-in-buffer-mode.md)。</span><span class="sxs-lookup"><span data-stu-id="c8da6-114">For information on how to use buffer mode, see [Using the Compression API in buffer mode](using-the-compression-api-in-buffer-mode.md).</span></span>

<span data-ttu-id="c8da6-115">使用緩衝區或區塊模式的應用程式，可以選擇在其對 [**CreateCompressor**](/windows/desktop/api/compressapi/nf-compressapi-createcompressor) 或 [**CreateDecompressor**](/windows/desktop/api/compressapi/nf-compressapi-createdecompressor)的呼叫中指定自訂記憶體配置常式。</span><span class="sxs-lookup"><span data-stu-id="c8da6-115">Applications using buffer or block mode have the option to specify a custom memory allocation routine in its call to [**CreateCompressor**](/windows/desktop/api/compressapi/nf-compressapi-createcompressor) or [**CreateDecompressor**](/windows/desktop/api/compressapi/nf-compressapi-createdecompressor).</span></span>

<span data-ttu-id="c8da6-116">**Windows 8 和 Windows Server 2012：** 若要使用下列範例程式碼，您必須執行 Windows 8 或 Windows Server 2012，並使用 "compressapi" 和 "cabinet.dll" 並連結至 ".Cab"。</span><span class="sxs-lookup"><span data-stu-id="c8da6-116">**Windows 8 and Windows Server 2012:** To use the following example code, you must be running Windows 8 or Windows Server 2012 and have "compressapi.h" and "cabinet.dll" and link to the "Cabinet.lib".</span></span>

<span data-ttu-id="c8da6-117">下列範例將示範如何在區塊模式中使用壓縮 API，以使用 LZMS 壓縮演算法和自訂的記憶體配置常式壓縮檔案。</span><span class="sxs-lookup"><span data-stu-id="c8da6-117">The following demonstrates using the Compression API in block mode to compress a file using the LZMS compression algorithm and a customized memory allocation routine.</span></span> <span data-ttu-id="c8da6-118">您的應用程式必須包含 **壓縮 \_ 原始** 旗標，才能在區塊模式中使用壓縮 API。</span><span class="sxs-lookup"><span data-stu-id="c8da6-118">Your application must include the **COMPRESS\_RAW** flag to use the Compression API in block mode.</span></span> <span data-ttu-id="c8da6-119">首先，應用程式會呼叫 [**CreateCompressor**](/windows/desktop/api/compressapi/nf-compressapi-createcompressor) with **壓縮 \_ 演算法 \_ LZMS** \| **壓縮 \_ RAW** 以產生壓縮程式。</span><span class="sxs-lookup"><span data-stu-id="c8da6-119">First the application calls [**CreateCompressor**](/windows/desktop/api/compressapi/nf-compressapi-createcompressor) with **COMPRESS\_ALGORITHM\_LZMS**\|**COMPRESS\_RAW** to generate the compressor.</span></span> <span data-ttu-id="c8da6-120">*AllocationRoutines* 參數會指定記憶體配置常式。</span><span class="sxs-lookup"><span data-stu-id="c8da6-120">The *AllocationRoutines* parameter specifies the memory allocation routine.</span></span> <span data-ttu-id="c8da6-121">然後，應用程式會使用 [**SetCompressorInformation**](/windows/desktop/api/compressapi/nf-compressapi-setcompressorinformation)設定壓縮程式的區塊大小。</span><span class="sxs-lookup"><span data-stu-id="c8da6-121">The application then sets the block size for the compressor using [**SetCompressorInformation**](/windows/desktop/api/compressapi/nf-compressapi-setcompressorinformation).</span></span>

<span data-ttu-id="c8da6-122">應用程式會重複呼叫以 [**壓縮**](/windows/desktop/api/compressapi/nf-compressapi-compress) ，以依區塊壓縮資料區塊。</span><span class="sxs-lookup"><span data-stu-id="c8da6-122">The application makes repeated calls to [**Compress**](/windows/desktop/api/compressapi/nf-compressapi-compress) to compress the data block by block.</span></span> <span data-ttu-id="c8da6-123">應用程式會將未壓縮的區塊大小、壓縮的區塊大小和壓縮的資料寫入輸出緩衝區。</span><span class="sxs-lookup"><span data-stu-id="c8da6-123">The application writes the uncompressed block size, compressed block size, and compressed data into the output buffer.</span></span>
...
<span data-ttu-id="c8da6-124">以下示範如何在區塊模式中使用壓縮 API 進行檔案解壓縮。</span><span class="sxs-lookup"><span data-stu-id="c8da6-124">The following demonstrates file decompression using the Compression API in block mode.</span></span>
...
<span data-ttu-id="c8da6-125">使用緩衝區或區塊模式的應用程式可選擇自訂壓縮 API 在呼叫 [**CreateCompressor**](/windows/desktop/api/compressapi/nf-compressapi-createcompressor) 或 [**CreateDecompressor**](/windows/desktop/api/compressapi/nf-compressapi-createdecompressor)時所使用的記憶體配置。</span><span class="sxs-lookup"><span data-stu-id="c8da6-125">An application using buffer or block mode has the option to customize the memory allocation used by the Compression API when it calls [**CreateCompressor**](/windows/desktop/api/compressapi/nf-compressapi-createcompressor) or [**CreateDecompressor**](/windows/desktop/api/compressapi/nf-compressapi-createdecompressor).</span></span> <span data-ttu-id="c8da6-126">在封鎖模式中，應用程式必須處理壓縮區塊資訊，例如壓縮的資料大小和未壓縮的資料大小，否則 [**解壓縮**](/windows/desktop/api/compressapi/nf-compressapi-decompress) 將無法解壓縮資訊。</span><span class="sxs-lookup"><span data-stu-id="c8da6-126">In block mode, the application must handle compression block information, such as compressed data size and uncompressed data size, otherwise [**Decompress**](/windows/desktop/api/compressapi/nf-compressapi-decompress) will be unable to decompress the information.</span></span>

<span data-ttu-id="c8da6-127">下列程式碼片段顯示簡單的自訂配置常式。</span><span class="sxs-lookup"><span data-stu-id="c8da6-127">The following snippet shows a simple customized allocation routine.</span></span>
