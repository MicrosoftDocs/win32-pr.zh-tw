...
取用者必須知道事件資料的格式，才能使用事件特定資料。 如果提供者使用 MOF 來發佈事件資料的格式，您可以使用 MOF 類別來剖析事件資料。 所有核心事件都會使用 MOF 來發佈事件資料的格式。 如需發佈事件的詳細資訊，請參閱 [發行您的事件架構](publishing-your-event-schema.md)。

剖析事件資料需要使用 Windows 管理基礎結構 (WMI) API。 提供者發佈其 MOF 類別的 ETW 命名空間是根 \\ wmi。 ETW 命名空間包含三種類型的 MOF 類別：提供者 MOF 類別、事件 MOF 類別，以及事件種類 MOF 類別。 事件 MOF 類別會以邏輯方式將一或多個事件種類 MOF 類別分組。 事件種類 MOF 類別會定義實際的事件資料。

事件 MOF 類別包含 **guid** 類別限定詞，其值必須符合 **標頭** 中的值。 [**事件 \_ 追蹤**](/windows/win32/api/evntrace/ns-evntrace-event_trace) 結構的 guid 成員。 若要確保您擁有正確版本的類別，也請比較 **EventVersion** 類別限定詞與 **事件 \_ 追蹤** 結構的 **Header. 版本** 成員。

找到正確的事件類別之後，列舉其子事件種類類別，以找出包含事件資料格式的類別。 正確的事件種類類別包含符合專案 **的類別辨識** 符號，其值符合 **標頭** 中的值。 [**事件 \_ 追蹤**](/windows/win32/api/evntrace/ns-evntrace-event_trace) 結構的類型成員。

然後，您可以使用 WMI API 來列舉 MOF 類別的屬性。 您可以使用每個屬性的限定詞和資料類型來決定要讀取的事件資料中的資料元素大小，以及如何將它格式化。 如需 ETW 支援的 MOF 限定詞清單，請參閱 [事件追蹤 MOF 限定詞](event-tracing-mof-qualifiers.md)。

由於 ETW 不會強制在事件資料值之間對齊，因此直接從緩衝區 typecasting 或指派值可能會造成對齊錯誤;您不應該從 MOF 類別建立結構，並嘗試使用它來取用事件資料。 例如，如果您的字元後面接著 ULONGLONG，ULONGLONG 就不會對齊8位元組的界限，因此指派會造成對齊例外狀況。  (在64位的電腦上，這會更頻繁地發生。 ) 基於這個原因，您應該使用 CopyMemory 將緩衝區中的資料複製到本機變數。 此外，如果稍後再修改事件，當您嘗試使用結構時，您的取用者可能無法運作。

從 Windows Vista 開始，建議您使用追蹤資料協助程式 (TDH) 函式來取用使用 MOF 類別發佈的事件。 如需詳細資訊，請參閱 [使用 TDH 來取出事件資料](retrieving-event-data-using-tdh.md)。
