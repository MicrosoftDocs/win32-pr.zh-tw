...
---
# <a name="retrieving-event-data-using-mof"></a><span data-ttu-id="a5bbe-103">使用 MOF 取出事件資料</span><span class="sxs-lookup"><span data-stu-id="a5bbe-103">Retrieving Event Data Using MOF</span></span>

<span data-ttu-id="a5bbe-104">取用者必須知道事件資料的格式，才能使用事件特定資料。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-104">To consume event specific data, the consumer must know the format of the event data.</span></span> <span data-ttu-id="a5bbe-105">如果提供者使用 MOF 來發佈事件資料的格式，您可以使用 MOF 類別來剖析事件資料。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-105">If the provider used MOF to publish the format of the event data, you can use the MOF class to parse the event data.</span></span> <span data-ttu-id="a5bbe-106">所有核心事件都會使用 MOF 來發佈事件資料的格式。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-106">All the kernel events use MOF to publish the format of the event data.</span></span> <span data-ttu-id="a5bbe-107">如需發佈事件的詳細資訊，請參閱 [發行您的事件架構](publishing-your-event-schema.md)。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-107">For information on publishing events, see [Publishing Your Event Schema](publishing-your-event-schema.md).</span></span>

<span data-ttu-id="a5bbe-108">剖析事件資料需要使用 Windows 管理基礎結構 (WMI) API。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-108">Parsing the event data requires using the Windows Management Infrastructure (WMI) API.</span></span> <span data-ttu-id="a5bbe-109">提供者發佈其 MOF 類別的 ETW 命名空間是根 \\ wmi。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-109">The ETW namespace where providers publish their MOF class is root\\wmi.</span></span> <span data-ttu-id="a5bbe-110">ETW 命名空間包含三種類型的 MOF 類別：提供者 MOF 類別、事件 MOF 類別，以及事件種類 MOF 類別。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-110">The ETW namespace contains three types of MOF classes: the provider MOF class, the event MOF class, and the event type MOF class.</span></span> <span data-ttu-id="a5bbe-111">事件 MOF 類別會以邏輯方式將一或多個事件種類 MOF 類別分組。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-111">The event MOF class logically groups one or more event type MOF classes.</span></span> <span data-ttu-id="a5bbe-112">事件種類 MOF 類別會定義實際的事件資料。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-112">The event type MOF class defines the actual event data.</span></span>

<span data-ttu-id="a5bbe-113">事件 MOF 類別包含 **guid** 類別限定詞，其值必須符合 **標頭** 中的值。 [**事件 \_ 追蹤**](/windows/win32/api/evntrace/ns-evntrace-event_trace) 結構的 guid 成員。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-113">An event MOF class contains a **Guid** class qualifier whose value must match the value in the **Header.Guid** member of the [**EVENT\_TRACE**](/windows/win32/api/evntrace/ns-evntrace-event_trace) structure.</span></span> <span data-ttu-id="a5bbe-114">若要確保您擁有正確版本的類別，也請比較 **EventVersion** 類別限定詞與 **事件 \_ 追蹤** 結構的 **Header. 版本** 成員。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-114">To ensure you have the correct version of the class, also compare the **EventVersion** class qualifier to the **Header.Class.Version** member of the **EVENT\_TRACE** structure.</span></span>

<span data-ttu-id="a5bbe-115">找到正確的事件類別之後，列舉其子事件種類類別，以找出包含事件資料格式的類別。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-115">After you find the correct event class, enumerate its child event type classes to find the class that contains the format of the event data.</span></span> <span data-ttu-id="a5bbe-116">正確的事件種類類別包含符合專案 **的類別辨識** 符號，其值符合 **標頭** 中的值。 [**事件 \_ 追蹤**](/windows/win32/api/evntrace/ns-evntrace-event_trace) 結構的類型成員。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-116">The correct event type class contains an **EventType** class qualifier whose value matches the value in the **Header.Class.Type** member of the [**EVENT\_TRACE**](/windows/win32/api/evntrace/ns-evntrace-event_trace) structure.</span></span>

<span data-ttu-id="a5bbe-117">然後，您可以使用 WMI API 來列舉 MOF 類別的屬性。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-117">You can then use the WMI API to enumerate the properties of the MOF class.</span></span> <span data-ttu-id="a5bbe-118">您可以使用每個屬性的限定詞和資料類型來決定要讀取的事件資料中的資料元素大小，以及如何將它格式化。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-118">Use the qualifiers and data type of each property to determine the size of the data element in the event data to read and how to format it.</span></span> <span data-ttu-id="a5bbe-119">如需 ETW 支援的 MOF 限定詞清單，請參閱 [事件追蹤 MOF 限定詞](event-tracing-mof-qualifiers.md)。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-119">For a list of MOF qualifiers that ETW supports, see [Event Tracing MOF Qualifiers](event-tracing-mof-qualifiers.md).</span></span>

<span data-ttu-id="a5bbe-120">由於 ETW 不會強制在事件資料值之間對齊，因此直接從緩衝區 typecasting 或指派值可能會造成對齊錯誤;您不應該從 MOF 類別建立結構，並嘗試使用它來取用事件資料。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-120">Because ETW does not force an alignment between event data values, typecasting or assigning the value directly from a buffer may cause an alignment fault; you should not create a structure from the MOF class and try to use it to consume event data.</span></span> <span data-ttu-id="a5bbe-121">例如，如果您的字元後面接著 ULONGLONG，ULONGLONG 就不會對齊8位元組的界限，因此指派會造成對齊例外狀況。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-121">For example, if you have a character followed by ULONGLONG, the ULONGLONG would not be aligned to an 8-byte boundary, so an assignment would cause an alignment exception.</span></span> <span data-ttu-id="a5bbe-122"> (在64位的電腦上，這會更頻繁地發生。 ) 基於這個原因，您應該使用 CopyMemory 將緩衝區中的資料複製到本機變數。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-122">(On 64-bit computers, this happens more often.) For this reason, you should use CopyMemory to copy the data from the buffer to a local variable.</span></span> <span data-ttu-id="a5bbe-123">此外，如果稍後再修改事件，當您嘗試使用結構時，您的取用者可能無法運作。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-123">Also, if the event is later revised, your consumer may not work if you try to use a structure.</span></span>

<span data-ttu-id="a5bbe-124">從 Windows Vista 開始，建議您使用追蹤資料協助程式 (TDH) 函式來取用使用 MOF 類別發佈的事件。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-124">Beginning with Windows Vista, you are encouraged to use the trace data helper (TDH) functions to consume events that were published using MOF classes.</span></span> <span data-ttu-id="a5bbe-125">如需詳細資訊，請參閱 [使用 TDH 來取出事件資料](retrieving-event-data-using-tdh.md)。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-125">For details, see [Retrieving Event Data Using TDH](retrieving-event-data-using-tdh.md).</span></span>

<span data-ttu-id="a5bbe-126">下列範例示範如何使用 MOF 類別所定義的事件。</span><span class="sxs-lookup"><span data-stu-id="a5bbe-126">The following example shows how to consume events that are defined by a MOF class.</span></span>
