---
description: 下列範例顯示如何使用事件來計算一組指示的 CPU 使用量。 此範例假設提供者包裝具有 start 事件種類和 end 事件種類的指令集。
ms.assetid: 7bc15ed3-d299-40dd-9838-520f896921e3
title: 使用事件計算 CPU 使用量
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104972265"
---
# <a name="using-events-to-calculate-cpu-usage"></a><span data-ttu-id="e1af9-104">使用事件計算 CPU 使用量</span><span class="sxs-lookup"><span data-stu-id="e1af9-104">Using Events to Calculate CPU Usage</span></span>

<span data-ttu-id="e1af9-105">下列範例顯示如何使用事件來計算一組指示的 CPU 使用量。</span><span class="sxs-lookup"><span data-stu-id="e1af9-105">The following example shows you how to use events to calculate the CPU usage for a set of instructions.</span></span> <span data-ttu-id="e1af9-106">此範例假設提供者包裝具有 start 事件種類和 end 事件種類的指令集。</span><span class="sxs-lookup"><span data-stu-id="e1af9-106">This example assumes the provider wraps the instruction set with a start event type and an end event type.</span></span>
