...
---
# <a name="dml_element_wise_bit_shift_right_operator_desc-structure-directmlh"></a>DML_ELEMENT_WISE_BIT_SHIFT_RIGHT_OPERATOR_DESC 結構 (directml .h) 

依 *BTensor* 的對應元素所指定的位數，對 *ATensor* 的每個元素執行邏輯右移位，將結果放入 *OutputTensor* 的對應元素。
...
| 張 | 類型 | 支援的維度計數 | 支援的資料類型 |
| ------ | ---- | -------------------------- | -------------------- |
| ATensor | 輸入 | 4 | UINT32、UINT16、UINT8 |
| BTensor | 輸入 | 4 | UINT32、UINT16、UINT8 |
| OutputTensor | 輸出 | 4 | UINT32、UINT16、UINT8 |
