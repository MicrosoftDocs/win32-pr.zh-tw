...
---
# <a name="dml_element_wise_atan_yx_operator_desc-directmlh"></a>DML_ELEMENT_WISE_ATAN_YX_OPERATOR_DESC (directml) 

針對 *ATensor* 和 *BTensor* 的每個專案計算雙引數反正切值，其中 *ATensor* 是 *Y 軸* ，而 *BTensor* 是 *X 軸*，將結果放入 *OutputTensor* 的對應元素。 來源 (的這個運算子是未定義的，也就是當對應元素) 的 *ATensor* 和 *BTensor* 都是0時。
...
這個運算子支援就地執行，這表示在系結期間允許輸出 tensor 別名 *ATensor* 或 *BTensor* 。
...
要從中讀取 Y 軸值的輸入 tensor。

`BTensor`

Type： **const [DML_TENSOR_DESC](/windows/win32/api/directml/ns-directml-dml_tensor_desc) \***

要從中讀取 X 軸值的輸入 tensor。
...
| ------ | ---- | -------------------------- | -------------------- |
| ATensor | 輸入 | 1至8 | FLOAT32、FLOAT16 |
| BTensor | 輸入 | 1至8 | FLOAT32、FLOAT16 |
