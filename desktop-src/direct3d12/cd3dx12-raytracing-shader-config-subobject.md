---
title: 'CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT 類別 (D3dx12) '
description: 建立 raytracing 著色器設定狀態子物件的 helper 類別。
keywords:
- CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT 類別
topic_type:
- apiref
api_name:
...
ms.locfileid: "121812913"
---
# <a name="cd3dx12_raytracing_shader_config_subobject-class"></a>CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT 類別

建立 raytracing 著色器設定狀態子物件的 helper 類別。
...
預設建構函式。 建立 **CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT** 預設初始化的新實例。

`CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC&)`

建立新實例的函式， **CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT** 以 [**CD3DX12_STATE_OBJECT_DESC**](cd3dx12-state-object-desc.md) 物件的內容初始化。

`Config(UINT, UINT)`

用來設定承載大小上限的函式，以及最大的屬性大小 (（以位元組為單位）) 。

`Type`

抓取子物件的類型，由 [D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_SHADER_CONFIG](/windows/win32/api/d3d12/ne-d3d12-d3d12_state_subobject_type) 常數表示。
...
轉換運算子，會傳回描述狀態物件之常數 [D3D12_RAYTRACING_SHADER_CONFIG](/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_shader_config) 物件的參考。
