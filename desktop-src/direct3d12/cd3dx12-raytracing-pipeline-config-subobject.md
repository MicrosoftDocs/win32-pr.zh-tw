---
title: 'CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT 類別 (D3dx12) '
description: 建立 raytracing 管線設定狀態子物件的 helper 類別。
keywords:
- CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT 類別
topic_type:
- apiref
api_name:
...
ms.locfileid: "121812210"
---
# <a name="cd3dx12_raytracing_pipeline_config_subobject-class"></a>CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT 類別

建立 raytracing 管線設定狀態子物件的 helper 類別。 另請參閱 [CD3DX12_RAYTRACING_PIPELINE_CONFIG1_SUBOBJECT](cd3dx12-raytracing-pipeline-config1-subobject.md)。
...
預設建構函式。 建立 **CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT** 預設初始化的新實例。

`CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT(CD3DX12_STATE_OBJECT_DESC&)`

建立新實例的函式， **CD3DX12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT** 以 [**CD3DX12_STATE_OBJECT_DESC**](cd3dx12-state-object-desc.md) 物件的內容初始化。

`Config(UINT)`

用來設定 raytracing 管線的光線遞迴限制的函式。
