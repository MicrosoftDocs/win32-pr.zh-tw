...
---
# <a name="directml-helper-functions"></a>DirectML helper 函式

## <a name="dmlcalcbuffertensorsize"></a>DMLCalcBufferTensorSize

此 helper 函式會計算以指定的類型、大小和進展儲存緩衝區 tensor 所需的最小位元組數目。 公式可以表示如下。
...
換句話說，tensor 的大小下限是一個後端元素的索引，乘上元素大小 (例如， **FLOAT16** tensor) 的2個位元組。 此外，DirectML 需要所有的系結緩衝區都必須有 **DWORD** 對齊的總大小，因此，最小的隱含大小（以位元組為單位）必須四捨五入到最接近的4位元組界限。
...
此協助程式函式會計算有 NCHW 或 NHWC 配置的4D 張量以及選擇性廣播的進展。
