---
title: Windows先進的點陣化平臺 (變形) 指南
description: 本文描述 Windows 的 Advanced 點陣化平臺 (變形) 以及下列各方面的變形。
...
ms.locfileid: "119745988"
---
# <a name="windows-advanced-rasterization-platform-warp-guide"></a>Windows先進的點陣化平臺 (變形) 指南

本文描述 Windows 的 Advanced 點陣化平臺 (變形) 以及下列各方面的變形。
...
變形是高速、完全一致的軟體轉譯器。 它是由 Direct3D 11 執行時間引進的 DirectX 圖形技術元件。 Direct3D 11 執行時間安裝在 Windows 7、Windows Server 2008 R2 和 Windows Vista （含 \[ KB971644 更新）上 \] 。 上述 Windows 8、Windows 10 Windows Server 2012 &，而 Windows RT 包含具有更新版本之變形的 Direct3D 11.1 執行時間。 Windows 10 Fall Creators Update (1709) 包含支援 Direct3D 11 和 direct3d 12 執行時間的變形版本。
...
當您在 Windows 7 SP1 或 Windows Server 2008 R2 SP1 上安裝[Windows 7 的平臺更新](https://support.microsoft.com/kb/2670838)時，該作業系統接著會包含 direct3d 11.1 執行時間，以及在搭配[功能等級](/windows/desktop/direct3d11/overviews-direct3d-11-devices-downlevel-intro)9 \_ 1、9 \_ 2、9 \_ 3、10 \_ 0、10 \_ 1 和 11 \_ 0 時，支援 direct3d 11. x 的變形版本。

上述 Windows 8、Windows 10 Windows Server 2012 &，而 Windows RT 包含 Direct3D 11.1 執行時間和新版本的變形。 此版本在與 [功能層級](/windows/desktop/direct3d11/overviews-direct3d-11-devices-downlevel-intro) 9 \_ 1、9 \_ 2、9 \_ 3、10 \_ 0、10 \_ 1、11 \_ 0 和 11 \_ 1 搭配使用時，支援 Direct3D 11. x。
...
遊戲通常有簡單的呈現需求。 不過，它們也需要使用可能需要硬體加速的令人印象深刻的視覺效果。 大部分適用于 Windows 的最暢銷遊戲專案都是模擬或休閒遊戲，而這兩種遊戲都不需要高效能圖形。 不過，這兩種遊戲樣式大幅受益于新式著色器式圖形以及調整硬體的能力。
...
變形是以參考轉譯器程式碼基底為基礎。 因此，「變形」會對 Direct3D 10 和更新版本和 DXGI 使用相同的軟體介面。 Windows 7 中的變形包含在 D3d10warp.dll 的 [Windows systems] 資料夾中。 64位電腦上安裝了兩個版本的變形，也就是 x86 和 x64 版。 在特定情況下，x64 版本的執行速度可能較快，因為包含在變形中的程式碼產生器可以利用使用者執行64位應用程式時可用的其他暫存器。
...
「變形」會使用 Windows Vista 中引進的執行緒集區和複雜的工作管理和相依性追蹤，讓轉譯管線的所有部分能有效率地分散到可用的 CPU 核心上。
...
由於變形會使用多個 CPU 核心，因此最適合用於新式的四核心 Cpu。 在已安裝 SSE 4.1 擴充功能的電腦上，變形也會明顯地執行。 Microsoft 在具有八個或更多核心和 sse 4.1 的電腦上執行了大幅的測試和效能調整，因為這些高階電腦在 Windows 7 和更新版本的作業系統存留期間將會更為普遍。
...
| CPU                         | 時間   | 平均 FPS | 最小 FPS | 最小框架 | 最大 FPS | 最大框架 |
|-----------------------------|--------|---------|---------|-----------|---------|-----------|
...
| 顯卡         | 時間   | 平均 FPS | 最小 FPS | 最小框架 | 最大 FPS | 最大框架 |
|-----------------------|--------|---------|---------|-----------|---------|-----------|
...
在測試中使用了 Windows 的[PIX](https://msdn.microsoft.com/library/ee417062(v=VS.85).aspx)偵錯工具和分析工具，Microsoft 有一個大型的應用程式框架庫，可用來在硬體和變形之間進行比較。 大部分的影像在硬體和變形之間看起來幾乎相同;有時可能會發生微小的差異，而是在 Direct3D 10 規格所定義的容限範圍內。