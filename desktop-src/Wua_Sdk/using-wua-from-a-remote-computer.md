...
-   [**IAutomaticUpdates 的 Settings 屬性**](/windows/desktop/api/Wuapi/nf-wuapi-iautomaticupdates-get_settings)
...
您必須將下列埠和例外狀況新增至適用于 Windows Vista 和 Windows Server 2008 的 Windows 防火牆設定，才能從遠端呼叫 WUA API。
...
下列清單包含可用於設定 Windows 防火牆設定的工具：

-   [具有進階安全性的 Windows 防火牆] 嵌入式管理單元
-   群組原則
-   Netsh advfirewall 命令列工具

如需如何使用工具來設定 Windows 防火牆設定的詳細資訊，請參閱使用 [具有 Advanced Security 的 Windows 防火牆開始使用](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc748991(v=ws.10))。
