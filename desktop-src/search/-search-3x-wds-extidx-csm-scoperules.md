...
> [!Note]  
> 在 Windows Vista 中，如果使用者是透過主控台中的使用者設定檔移除，CSM 會移除所有包含其 SID 的規則和根，並從目錄移除其索引項目目。 在 Windows XP 上，您必須手動移除使用者的根和規則。
...
如果您認為應該排除檔案路徑，但它列為包含的，請確定排除規則的結尾為 " <path> \\ \* "。 如果您認為應該包含檔案或檔案路徑，但是它不是，請務必檢查檔案或路徑的 FANCI 位設定。 若要這樣做，請以滑鼠右鍵按一下檔案或檔案路徑，然後選取 [ **屬性**]，然後確定已選取 [ **快速搜尋]、[允許編制索引服務來為此資料夾編制索引** ] 核取方塊。 如果檔案或檔案路徑未標示為要在這裡進行索引編制，就不會將它編制索引，即使它是在包含規則中也一樣。
