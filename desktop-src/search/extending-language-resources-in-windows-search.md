...
---
# <a name="extending-language-resources"></a>擴充語言資源

Windows Search 使用語言資源（例如斷詞工具和字幹分析器），在建立索引和查詢處理期間中斷文字的原生地區設定。 Microsoft 提供數種語言的斷詞工具和字幹分析器。 本節說明如何在 Microsoft 提供的語言和地區設定之外，執行及使用自訂斷詞工具和字幹分析器。
