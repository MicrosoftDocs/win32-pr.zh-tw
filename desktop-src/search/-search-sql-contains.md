...
述詞的 LCID 部分會指定搜尋地區設定。 這會指示搜尋引擎針對搜尋查詢使用適當的斷詞工具和字形變化表單。 若要指定地區設定，請 (LCID) 提供 Windows 的標準語言代碼識別碼。 例如，1033是美國的 LCID （英文）。 將 LCID 放在 CONTAINS 子句括弧內的最後一個專案。 如需搜尋和語言的重要資訊，請參閱 [使用當地語系化的搜尋](-search-sql-usinglocsearches.md)。
