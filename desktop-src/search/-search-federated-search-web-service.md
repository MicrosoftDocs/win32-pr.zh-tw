...
-   [連線您的 web 服務](#connect-your-web-service)
-   [註冊現有的遠端資料存放區](#register-an-existing-remote-data-store)
-   [其他資源](#additional-resources)
-   [相關主題](#related-topics)

## <a name="connect-your-web-service"></a>連線您的 web 服務
...
使用者 Windows 同盟搜尋來註冊新的遠端資料存放區，方法是開啟 OpenSearch 描述 (. .osdx) 檔。 當使用者這樣做時，會發生下列事件：

1.  會在 Windows 搜尋資料夾中建立 searchconnector-ms 檔案 (搜尋連接器)  (% userprofile%/Searches) 。
2.  Searchconnector-ms 檔案的快捷方式建立于連結資料夾 (% userprofile%/Links) 。
3.  快捷方式會出現在 Windows 檔案總管導覽的 [我的最 **愛] 窗格** 中，讓使用者流覽至新的資料存放區，然後查詢 web 服務。

> [!Note]  
> 當使用者開啟 .osdx 檔案時，可以將已經有[OpenSearch](https://github.com/dewitt/opensearch) web 服務與 Windows 同盟搜尋相容的資料存放區新增至 Windows 檔案總管。
