...
ms.locfileid: "117863731"
---
# <a name="ifiltersample"></a>IFilterSample

IFilterSample 程式碼範例會示範如何建立用來執行 [**ifilter**](/windows/win32/api/filter/nn-filter-ifilter) 介面的 ifilter 基類。
...
1. 開啟 Windows 檔案總管，然後流覽至 **FilterBase** 專案目錄。
2. 按兩下 FilterBase .sln 檔案的圖示，在 Visual Studio 中開啟專案。
...
2. 在命令提示字元中，輸入 `FilterBase.exe` ，或從 Windows 檔案總管中，按兩下 FilterBase.exe 的圖示。
