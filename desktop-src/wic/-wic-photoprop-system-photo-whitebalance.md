---
description: WhiteBalance 屬性的相片中繼資料原則。
ms.assetid: 7b3a31e6-a929-41e4-b7f0-c5b3beac2519
title: WhiteBalance 相片中繼資料原則
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: cc333cedcd4665ace37033452ec3c7f0336f13e5
ms.sourcegitcommit: 831e8f3db78ab820e1710cede244553c70e50500
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 01/08/2021
ms.locfileid: "107001738"
---
# <a name="systemphotowhitebalance-photo-metadata-policy"></a>WhiteBalance 相片中繼資料原則

[WhiteBalance](../properties/props-system-photo-whitebalance.md)屬性的相片中繼資料原則。

### <a name="pkey"></a>PKEY

PKEY \_ 相片 \_ WhiteBalance

### <a name="containers"></a>容器

JPEG、TIFF

### <a name="read-only"></a>唯讀

No

### <a name="output-propvariant-type"></a>輸出 PROPVARIANT 類型

VT \_ UI4

### <a name="input-type"></a>輸入類型

UShort

### <a name="conflict-resolution-policy"></a>衝突解決原則

不同架構的值會進行協調。

### <a name="jpeg-policy"></a>JPEG 原則

### <a name="read-paths"></a>讀取路徑



| 單 | 路徑                          | 磁片格式 |
|-------|-------------------------------|-------------|
| 1     | /app1/ifd/exif/{ushort = 41987} | ushort      |
| 2     | /xmp/exif:WhiteBalance        | Unicode     |



 

### <a name="write-paths"></a>寫入路徑



| 單 | 路徑                          | 磁片格式 |
|-------|-------------------------------|-------------|
| 1     | /app1/ifd/exif/{ushort = 41987} | ushort      |
| 2     | /xmp/exif:WhiteBalance        | Unicode     |



 

### <a name="remove-paths"></a>移除路徑



| 單 | 路徑                          |
|-------|-------------------------------|
| 1     | /app1/ifd/exif/{ushort = 41987} |
| 2     | /xmp/exif:whitebalance        |



 

### <a name="tiff-policies"></a>TIFF 原則

### <a name="read-paths"></a>讀取路徑



| 單 | 路徑                       | 磁片格式 |
|-------|----------------------------|-------------|
| 1     | /ifd/exif/{ushort = 41987}   | ushort      |
| 2     | /ifd/xmp/exif:WhiteBalance | Unicode     |



 

### <a name="write-paths"></a>寫入路徑



| 單 | 路徑                       | 磁片格式 |
|-------|----------------------------|-------------|
| 1     | /ifd/exif/{ushort = 41987}   | ushort      |
| 2     | /ifd/xmp/exif:WhiteBalance | Unicode     |



 

### <a name="remove-paths"></a>移除路徑



| 單 | 路徑                       |
|-------|----------------------------|
| 1     | /ifd/exif/{ushort = 41987}   |
| 2     | /ifd/xmp/exif:whitebalance |



 

## <a name="remarks"></a>備註

## <a name="related-topics"></a>相關主題

<dl> <dt>

[WhiteBalance](../properties/props-system-photo-whitebalance.md)
</dt> </dl>

 

 
