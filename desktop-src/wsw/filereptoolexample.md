...
這是驅動 [FileRep 服務](filerepserviceexample.md)的命令列工具。 您可以在該處找到此範例的更詳細描述。

命令列參數如下所示：

WsFileRep.exe <Client Service Url> <Server Service Url> <Source File> <Destination File> \[ /encoding： <binary/text/MTOM>\] \[ /sync\]

用戶端服務 Url：必要。 用戶端服務的 Url。

伺服器服務 Url：必要。 伺服器服務的 Url。

來源檔案：必要。 來源檔案的完整本機名稱。 檔案位於伺服器服務執行所在的電腦上。

目的地檔案：必要。 目的地檔案的完整本機名稱。 檔案將位於用戶端服務執行所在的電腦上。

編碼：選擇性。 指定在用戶端和伺服器服務之間傳送之訊息的訊息編碼方式。 為了簡單起見，無法變更命令列工具與用戶端服務之間的通訊所使用的編碼方式。 有效的參數為 binary、text 和 MTOM。 如果未指定參數，則會使用傳輸的預設編碼方式。

同步：選擇性。 當設定時，要求會以同步方式完成。 否則，要求將會以非同步方式完成。

-   [工具 .cpp](#toolcpp)
-   [一般。h](#commonh)
-   [相關主題](#related-topics)

## <a name="toolcpp"></a>工具 .cpp
