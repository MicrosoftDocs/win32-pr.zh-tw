...
description: 此範例顯示接受通道的 TCP 伺服器，並透過 SSL 混合模式安全性來接收和回復訊息。
ms.assetid: d70ae988-0d7c-4494-a54d-1cd73ab8a8ce
keywords:
- HttpServerWithUsernameOverSslExample 原生 Web 服務
- WWSAPI
- WWS
ms.topic: article
ms.date: 05/31/2018
...
---
# <a name="httpserverwithusernameoversslexample"></a><span data-ttu-id="903bc-106">HttpServerWithUsernameOverSslExample</span><span class="sxs-lookup"><span data-stu-id="903bc-106">HttpServerWithUsernameOverSslExample</span></span>

<span data-ttu-id="903bc-107">此範例顯示接受通道的 TCP 伺服器，並透過 SSL 混合模式安全性來接收和回復訊息。</span><span class="sxs-lookup"><span data-stu-id="903bc-107">This example shows a TCP server that accepts a channel and receives and replies to messages, with username over SSL mixed-mode security.</span></span> <span data-ttu-id="903bc-108">在此設定中，傳輸連線會受到 SSL (簽署、加密) 的 SSL，也提供伺服器驗證。</span><span class="sxs-lookup"><span data-stu-id="903bc-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="903bc-109">用戶端驗證是由訊息中 WS-Security 標頭中的使用者名稱/密碼組所提供。</span><span class="sxs-lookup"><span data-stu-id="903bc-109">Client authentication is provided by a username/password pair in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="903bc-110">HttpServerWithUsernameOverSsl .cpp</span><span class="sxs-lookup"><span data-stu-id="903bc-110">HttpServerWithUsernameOverSsl.cpp</span></span>](#httpserverwithusernameoversslcpp)
-   [<span data-ttu-id="903bc-111">PurchaseOrder .wsdl</span><span class="sxs-lookup"><span data-stu-id="903bc-111">PurchaseOrder.wsdl</span></span>](#purchaseorderwsdl)
-   [<span data-ttu-id="903bc-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="903bc-112">Makefile</span></span>](#makefile)

## <a name="httpserverwithusernameoversslcpp"></a><span data-ttu-id="903bc-113">HttpServerWithUsernameOverSsl .cpp</span><span class="sxs-lookup"><span data-stu-id="903bc-113">HttpServerWithUsernameOverSsl.cpp</span></span>
...
## <a name="purchaseorderwsdl"></a><span data-ttu-id="903bc-114">PurchaseOrder .wsdl</span><span class="sxs-lookup"><span data-stu-id="903bc-114">PurchaseOrder.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="903bc-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="903bc-115">Makefile</span></span>
...
all: $(OUTDIR) $(OUTDIR)\WsHttpServerWithUsernameOverSsl.exe
...
$(OUTDIR)\HttpServerWithUsernameOverSsl.obj: HttpServerWithUsernameOverSsl.cpp $(OUTDIR)\PurchaseOrder.wsdl.c
    $(cc) $(cdebug) $(cflags) $(cvarsmt) /WX -I$(OUTDIR) /Fo"$(OUTDIR)\\" /Fd"$(OUTDIR)\\" HttpServerWithUsernameOverSsl.cpp

$(OUTDIR)\WsHttpServerWithUsernameOverSsl.exe: $(OUTDIR)\HttpServerWithUsernameOverSsl.obj $(OUTDIR)\PurchaseOrder.wsdl.obj
    $(link) $(ldebug) $(conlflags) $(conlibsmt) $(EXTRA_LIBS) -out:$(OUTDIR)\WsHttpServerWithUsernameOverSsl.exe $(OUTDIR)\HttpServerWithUsernameOverSsl.obj $(OUTDIR)\PurchaseOrder.wsdl.obj /PDB:$(OUTDIR)\WsHttpServerWithUsernameOverSsl.PDB
