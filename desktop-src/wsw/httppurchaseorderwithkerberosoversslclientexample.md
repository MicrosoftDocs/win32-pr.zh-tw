...
此範例示範使用服務 proxy 與 PurchaseOrder 服務通訊的 HTTP 用戶端，並透過 SSL 混合模式安全性。 在此設定中，傳輸連線會受到 SSL (簽署、加密) 的 SSL，也提供伺服器驗證。 用戶端驗證是由訊息的 WS-Security 標頭中的 kerberos APREQ 票證所提供。
