...
description: 此範例顯示傳送要求-回復訊息的 HTTP 用戶端，其使用者名稱為 SSL 混合模式安全性。
ms.assetid: aaaad584-d861-4abd-9d76-95aba5090676
keywords:
- HttpClientWithUsernameOverSslExample 原生 Web 服務
- WWSAPI
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104021639"
---
# <a name="httpclientwithusernameoversslexample"></a><span data-ttu-id="4c605-106">HttpClientWithUsernameOverSslExample</span><span class="sxs-lookup"><span data-stu-id="4c605-106">HttpClientWithUsernameOverSslExample</span></span>

<span data-ttu-id="4c605-107">此範例顯示傳送要求-回復訊息的 HTTP 用戶端，其使用者名稱為 SSL 混合模式安全性。</span><span class="sxs-lookup"><span data-stu-id="4c605-107">This example shows a HTTP client that sends request-reply messages, with username over SSL mixed-mode security.</span></span> <span data-ttu-id="4c605-108">在此設定中，傳輸連線會受到 SSL (簽署、加密) 的 SSL，也提供伺服器驗證。</span><span class="sxs-lookup"><span data-stu-id="4c605-108">In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.</span></span> <span data-ttu-id="4c605-109">用戶端驗證是由訊息中 WS-Security 標頭中的使用者名稱/密碼組所提供。</span><span class="sxs-lookup"><span data-stu-id="4c605-109">Client authentication is provided by a username/password pair in a WS-Security header in the message.</span></span>

-   [<span data-ttu-id="4c605-110">HttpClientWithUsernameOverSsl .cpp</span><span class="sxs-lookup"><span data-stu-id="4c605-110">HttpClientWithUsernameOverSsl.cpp</span></span>](#httpclientwithusernameoversslcpp)
-   [<span data-ttu-id="4c605-111">PurchaseOrder .wsdl</span><span class="sxs-lookup"><span data-stu-id="4c605-111">PurchaseOrder.wsdl</span></span>](#purchaseorderwsdl)
-   [<span data-ttu-id="4c605-112">Makefile</span><span class="sxs-lookup"><span data-stu-id="4c605-112">Makefile</span></span>](#makefile)

## <a name="httpclientwithusernameoversslcpp"></a><span data-ttu-id="4c605-113">HttpClientWithUsernameOverSsl .cpp</span><span class="sxs-lookup"><span data-stu-id="4c605-113">HttpClientWithUsernameOverSsl.cpp</span></span>
...
## <a name="purchaseorderwsdl"></a><span data-ttu-id="4c605-114">PurchaseOrder .wsdl</span><span class="sxs-lookup"><span data-stu-id="4c605-114">PurchaseOrder.wsdl</span></span>
...
## <a name="makefile"></a><span data-ttu-id="4c605-115">Makefile</span><span class="sxs-lookup"><span data-stu-id="4c605-115">Makefile</span></span>
...
all: $(OUTDIR) $(OUTDIR)\WsHttpClientWithUsernameOverSsl.exe
...
$(OUTDIR)\HttpClientWithUsernameOverSsl.obj: HttpClientWithUsernameOverSsl.cpp $(OUTDIR)\PurchaseOrder.wsdl.c
    $(cc) $(cdebug) $(cflags) $(cvarsmt) /WX -I$(OUTDIR) /Fo"$(OUTDIR)\\" /Fd"$(OUTDIR)\\" HttpClientWithUsernameOverSsl.cpp

$(OUTDIR)\WsHttpClientWithUsernameOverSsl.exe: $(OUTDIR)\HttpClientWithUsernameOverSsl.obj $(OUTDIR)\PurchaseOrder.wsdl.obj
    $(link) $(ldebug) $(conlflags) $(conlibsmt) $(EXTRA_LIBS) -out:$(OUTDIR)\WsHttpClientWithUsernameOverSsl.exe $(OUTDIR)\HttpClientWithUsernameOverSsl.obj $(OUTDIR)\PurchaseOrder.wsdl.obj /PDB:$(OUTDIR)\WsHttpClientWithUsernameOverSsl.PDB
