...
description: FileRep 會從伺服器抓取檔案，並將其複製到用戶端。
ms.assetid: 9f446999-8f10-4ce4-86eb-e9289e131733
keywords:
- 適用于 Windows 的 FileRepServiceExample Web 服務
- WWSAPI
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104021626"
---
# <a name="filerepserviceexample"></a><span data-ttu-id="ad895-106">FileRepServiceExample</span><span class="sxs-lookup"><span data-stu-id="ad895-106">FileRepServiceExample</span></span>

<span data-ttu-id="ad895-107">FileRep 會從伺服器抓取檔案，並將其複製到用戶端。</span><span class="sxs-lookup"><span data-stu-id="ad895-107">FileRep retrieves files from a server and copies them to a client.</span></span> <span data-ttu-id="ad895-108">若要這樣做，它會採用三個元件：在電腦上使用原始程式檔執行的伺服器服務、將儲存目的地檔案的電腦上執行的用戶端服務，以及用來控制複製的命令列工具。</span><span class="sxs-lookup"><span data-stu-id="ad895-108">To do so it employs three components - the server service running on the machine with the source file, the client service running on the machine where the destination file will be stored and a command line tool to control the copying.</span></span> <span data-ttu-id="ad895-109">用戶端和伺服器服務會在使用者啟動命令列工具時，持續執行 web 服務，並在一個要求之後結束。</span><span class="sxs-lookup"><span data-stu-id="ad895-109">The client and server services are constantly running web services while the command line tool is started by the user and exits after one request.</span></span>

<span data-ttu-id="ad895-110">此範例說明如何使用通道和序列化層。</span><span class="sxs-lookup"><span data-stu-id="ad895-110">This sample illustrates the use of the channel and serialization layer.</span></span>

<span data-ttu-id="ad895-111">這是服務。</span><span class="sxs-lookup"><span data-stu-id="ad895-111">This is the service.</span></span> <span data-ttu-id="ad895-112">您可以在 [這裡](filereptoolexample.md)找到命令列工具。此服務具有用戶端和伺服器模式，伺服器會在其中傳送檔案和用戶端接收檔案。</span><span class="sxs-lookup"><span data-stu-id="ad895-112">The command line tool can be found [here](filereptoolexample.md).The service has a client and a server mode, where the server sends files and the client receives files.</span></span>

<span data-ttu-id="ad895-113">用戶端模式的命令列參數如下所示：</span><span class="sxs-lookup"><span data-stu-id="ad895-113">The command line parameters for the client mode are as follows:</span></span>

<span data-ttu-id="ad895-114">**WsFileRepService.exe 用戶端** *<Service Url>\*\*\*\[ /reporting： <error/info/verbose>\] \[ /encoding： <TEXT/binary/MTOM>\] \[ /connections： ***<number of connections>*** \]*\*</span><span class="sxs-lookup"><span data-stu-id="ad895-114">**WsFileRepService.exe client** *<Service Url>* **\[/reporting:<error/info/verbose>\] \[/encoding:<text/binary/MTOM>\] \[/connections:***<number of connections>***\]**</span></span>

<dl> <dt>

<span data-ttu-id="ad895-115"><span id="Client"></span><span id="client"></span><span id="CLIENT"></span>客戶</span><span class="sxs-lookup"><span data-stu-id="ad895-115"><span id="Client"></span><span id="client"></span><span id="CLIENT"></span>Client</span></span>
</dt> <dd>

<span data-ttu-id="ad895-116">必要。</span><span class="sxs-lookup"><span data-stu-id="ad895-116">Required.</span></span> <span data-ttu-id="ad895-117">表示服務會以用戶端的形式執行。</span><span class="sxs-lookup"><span data-stu-id="ad895-117">Denotes that the service runs as client.</span></span>

</dd> <dt>

<span data-ttu-id="ad895-118"><span id="Service_Url"></span><span id="service_url"></span><span id="SERVICE_URL"></span>服務 Url</span><span class="sxs-lookup"><span data-stu-id="ad895-118"><span id="Service_Url"></span><span id="service_url"></span><span id="SERVICE_URL"></span>Service Url</span></span>
</dt> <dd>

<span data-ttu-id="ad895-119">必要。</span><span class="sxs-lookup"><span data-stu-id="ad895-119">Required.</span></span> <span data-ttu-id="ad895-120">表示服務接聽的 URL。</span><span class="sxs-lookup"><span data-stu-id="ad895-120">Denotes the URL the service listens on.</span></span>

</dd> <dt>

<span data-ttu-id="ad895-121"><span id="Encoding"></span><span id="encoding"></span><span id="ENCODING"></span>編碼</span><span class="sxs-lookup"><span data-stu-id="ad895-121"><span id="Encoding"></span><span id="encoding"></span><span id="ENCODING"></span>Encoding</span></span>
</dt> <dd>

<span data-ttu-id="ad895-122">選擇性。</span><span class="sxs-lookup"><span data-stu-id="ad895-122">Optional.</span></span> <span data-ttu-id="ad895-123">指定使用命令列工具進行通訊時所使用的編碼方式。</span><span class="sxs-lookup"><span data-stu-id="ad895-123">Specifies the encoding used when communicating with the command line tool.</span></span> <span data-ttu-id="ad895-124">請注意，目前的工具不支援指定此傳輸的編碼，因此變更這種設定可能會產生錯誤。</span><span class="sxs-lookup"><span data-stu-id="ad895-124">Note that the current tool does not support specifying an encoding for this transfer, so changing this setting will likely produce an error.</span></span> <span data-ttu-id="ad895-125">這項設定可讓您獨立于伺服器之外變更和擴充工具。</span><span class="sxs-lookup"><span data-stu-id="ad895-125">The setting is there so that the tool can be changed and extended independently of the server.</span></span>

</dd> <dt>

<span data-ttu-id="ad895-126"><span id="Reporting"></span><span id="reporting"></span><span id="REPORTING"></span>報告</span><span class="sxs-lookup"><span data-stu-id="ad895-126"><span id="Reporting"></span><span id="reporting"></span><span id="REPORTING"></span>Reporting</span></span>
</dt> <dd>

<span data-ttu-id="ad895-127">選擇性。</span><span class="sxs-lookup"><span data-stu-id="ad895-127">Optional.</span></span> <span data-ttu-id="ad895-128">啟用錯誤、資訊或詳細資訊層級的報告。</span><span class="sxs-lookup"><span data-stu-id="ad895-128">Enables error, information or verbose level reporting.</span></span> <span data-ttu-id="ad895-129">預設值為 error。</span><span class="sxs-lookup"><span data-stu-id="ad895-129">The default is error.</span></span> <span data-ttu-id="ad895-130">訊息會列印到主控台。</span><span class="sxs-lookup"><span data-stu-id="ad895-130">Messages are printed to the console.</span></span>

</dd> <dt>

<span data-ttu-id="ad895-131"><span id="Connections"></span><span id="connections"></span><span id="CONNECTIONS"></span>連接</span><span class="sxs-lookup"><span data-stu-id="ad895-131"><span id="Connections"></span><span id="connections"></span><span id="CONNECTIONS"></span>Connections</span></span>
</dt> <dd>

<span data-ttu-id="ad895-132">選擇性。</span><span class="sxs-lookup"><span data-stu-id="ad895-132">Optional.</span></span> <span data-ttu-id="ad895-133">指定將處理的並行要求數目上限。</span><span class="sxs-lookup"><span data-stu-id="ad895-133">Specifies the maximum number of concurrent requests that will be processed.</span></span> <span data-ttu-id="ad895-134">如果省略，則預設值為100。</span><span class="sxs-lookup"><span data-stu-id="ad895-134">If omitted the default is 100.</span></span>

</dd> </dl>

<span data-ttu-id="ad895-135">伺服器模式的命令列參數如下所示：</span><span class="sxs-lookup"><span data-stu-id="ad895-135">The command line parameters for the server mode are as follows:</span></span>

<span data-ttu-id="ad895-136">**WsFileRepService.exe 伺服器** *<Service Url>\*\*\*\[ /reporting： <error/info/verbose>\] \[ /encoding： <text/binary/MTOM>\] \[ /connections： ***<number of connections>*** \] \[ /chunk： ***<size of the payload per message in bytes>*** \]*\*</span><span class="sxs-lookup"><span data-stu-id="ad895-136">**WsFileRepService.exe server** *<Service Url>* **\[/reporting:<error/info/verbose>\] \[/encoding:<text/binary/MTOM>\] \[/connections:***<number of connections>***\] \[/chunk:***<size of the payload per message in bytes>***\]**</span></span>

<dl> <dt>

<span data-ttu-id="ad895-137"><span id="Server"></span><span id="server"></span><span id="SERVER"></span>伺服器</span><span class="sxs-lookup"><span data-stu-id="ad895-137"><span id="Server"></span><span id="server"></span><span id="SERVER"></span>Server</span></span>
</dt> <dd>

<span data-ttu-id="ad895-138">必要。</span><span class="sxs-lookup"><span data-stu-id="ad895-138">Required.</span></span> <span data-ttu-id="ad895-139">表示服務會以檔案伺服器的形式執行。</span><span class="sxs-lookup"><span data-stu-id="ad895-139">Denotes that the service runs as file server.</span></span>

</dd> <dt>

<span data-ttu-id="ad895-140"><span id="Chunk"></span><span id="chunk"></span><span id="CHUNK"></span>塊</span><span class="sxs-lookup"><span data-stu-id="ad895-140"><span id="Chunk"></span><span id="chunk"></span><span id="CHUNK"></span>Chunk</span></span>
</dt> <dd>

<span data-ttu-id="ad895-141">選擇性。</span><span class="sxs-lookup"><span data-stu-id="ad895-141">Optional.</span></span> <span data-ttu-id="ad895-142">傳輸的檔案會分成指定大小的區塊。</span><span class="sxs-lookup"><span data-stu-id="ad895-142">The transferred files are broken into chunks of the specified size.</span></span> <span data-ttu-id="ad895-143">每個訊息都包含一個區塊。</span><span class="sxs-lookup"><span data-stu-id="ad895-143">Each message contains one chunk.</span></span> <span data-ttu-id="ad895-144">預設值為32768個位元組。</span><span class="sxs-lookup"><span data-stu-id="ad895-144">The default is 32768 bytes.</span></span>

</dd> </dl>

<span data-ttu-id="ad895-145">執行詳細資料。</span><span class="sxs-lookup"><span data-stu-id="ad895-145">Implementation details.</span></span> <span data-ttu-id="ad895-146">因為此範例是針對通道層而撰寫，所以必須手動執行服務模型層可能自動執行的特定工作。</span><span class="sxs-lookup"><span data-stu-id="ad895-146">As this sample is written against the channel layer, it has to manually perform certain tasks that the service model layer could do automatically.</span></span> <span data-ttu-id="ad895-147">其中一項工作就是通道管理。</span><span class="sxs-lookup"><span data-stu-id="ad895-147">One such task is channel management.</span></span> <span data-ttu-id="ad895-148">為了提供對要求的快速回應，一定會有 (的限制，) 數個通道可以接受要求。</span><span class="sxs-lookup"><span data-stu-id="ad895-148">In order to provide a fast response to requests, there are always (up to a limit) several channels ready to accept requests.</span></span> <span data-ttu-id="ad895-149">在多個要求的情況下，如果多個通道在建立通道時只需建立一個通道，則擁有多個通道的效能會更好。</span><span class="sxs-lookup"><span data-stu-id="ad895-149">Having multiple channels ready is more performant in the case of multiple requests than having only one channel as the creation of a channel takes time.</span></span> <span data-ttu-id="ad895-150">此外，重複使用狀態也可改善效能。</span><span class="sxs-lookup"><span data-stu-id="ad895-150">Further, reuse of state also improves performance.</span></span> <span data-ttu-id="ad895-151">Sapphire 會提供 Api 來重設大部分狀態，以避免必須釋放並重新建立。</span><span class="sxs-lookup"><span data-stu-id="ad895-151">Sapphire provides APIs to reset most state to avoid having to free and recreate it.</span></span> <span data-ttu-id="ad895-152">此範例會在可能的情況下重複使用通道和要求狀態，並只在傳遞特定閾值時建立或終結狀態，以利用這種方式。</span><span class="sxs-lookup"><span data-stu-id="ad895-152">This sample takes advantage of that by reusing channel and request state wherever possible and only creating or destroying state when certain thresholds are passed.</span></span> <span data-ttu-id="ad895-153">通道管理的相關程式碼可在 CChannelManager 中找到。</span><span class="sxs-lookup"><span data-stu-id="ad895-153">The code related to channel management can be found in CChannelManager.</span></span>

<span data-ttu-id="ad895-154">主要訊息處理迴圈是在 CRequest 中。</span><span class="sxs-lookup"><span data-stu-id="ad895-154">The main message processing loop is in CRequest.</span></span> <span data-ttu-id="ad895-155">該類別包含非同步 Sapphire 訊息處理迴圈所需的應用程式獨立狀態和方法。</span><span class="sxs-lookup"><span data-stu-id="ad895-155">That class contains the application-independent state and methods needed for an asynchronous Sapphire messaging processing loop.</span></span> <span data-ttu-id="ad895-156">應用程式特定的程式碼位於 CFileRepClient (用戶端服務) 和 CFileRepServer (server 服務) 。</span><span class="sxs-lookup"><span data-stu-id="ad895-156">The application-specific code is in CFileRepClient (client service) and CFileRepServer (server service).</span></span> <span data-ttu-id="ad895-157">這兩個類別都繼承自 CFileRep，其中包含一般服務相關的程式碼。</span><span class="sxs-lookup"><span data-stu-id="ad895-157">Both those classes inherit from CFileRep, which contains generic service-related code.</span></span>

<span data-ttu-id="ad895-158">此範例會使用序列化程式，並執行自訂序列化。</span><span class="sxs-lookup"><span data-stu-id="ad895-158">The sample both uses the serializer and performs custom serialization.</span></span> <span data-ttu-id="ad895-159">自訂序列化是在處理大型資料區塊時使用，以針對特定目的手動優化記憶體配置，以將記憶體耗用量降至最低。</span><span class="sxs-lookup"><span data-stu-id="ad895-159">Custom serialization is used when dealing with large chunks of data to minimize memory consumption by manually optimizing memory allocation for the specific purpose.</span></span> <span data-ttu-id="ad895-160">因為這會導致複雜的低層級程式碼進行手動序列化，因此應該只在絕對必要時才執行。</span><span class="sxs-lookup"><span data-stu-id="ad895-160">As this leads to complex, low-level code doing manual serialization should only be done when absolutely necessary.</span></span>

<span data-ttu-id="ad895-161">訊息交換模式：</span><span class="sxs-lookup"><span data-stu-id="ad895-161">Message exchange pattern:</span></span>

-   <span data-ttu-id="ad895-162">用戶端服務會從命令列工具取得要求訊息。</span><span class="sxs-lookup"><span data-stu-id="ad895-162">The client service gets a request message from the command line tool.</span></span>
-   <span data-ttu-id="ad895-163">如果要求是非同步，請立即傳送確認。</span><span class="sxs-lookup"><span data-stu-id="ad895-163">If the request is asynchronous send back a confirmation immediately.</span></span>
-   <span data-ttu-id="ad895-164">用戶端服務會將檔案資訊的要求傳送到伺服器服務。</span><span class="sxs-lookup"><span data-stu-id="ad895-164">The client service sends a request for file information to the server service.</span></span> <span data-ttu-id="ad895-165">探索要求是以-1 的區塊位置表示。</span><span class="sxs-lookup"><span data-stu-id="ad895-165">A discovery request is denoted by a chunk position of -1.</span></span>
-   <span data-ttu-id="ad895-166">伺服器服務會傳回檔案資訊。</span><span class="sxs-lookup"><span data-stu-id="ad895-166">The server service returns the file information.</span></span>
-   <span data-ttu-id="ad895-167">用戶端服務會依序一次從伺服器要求個別的區塊。</span><span class="sxs-lookup"><span data-stu-id="ad895-167">The client service requests the individual chunks sequentially one by one from the server.</span></span> <span data-ttu-id="ad895-168">區塊的識別方式是在檔案中的位置。</span><span class="sxs-lookup"><span data-stu-id="ad895-168">Chunks are identified by their position within the file.</span></span>
-   <span data-ttu-id="ad895-169">重複直到檔案傳輸完成或發生錯誤。</span><span class="sxs-lookup"><span data-stu-id="ad895-169">Repeat until the file transfer is completed or a failure occurred.</span></span>
-   <span data-ttu-id="ad895-170">如果要求是同步傳送成功或失敗訊息至命令列工具。</span><span class="sxs-lookup"><span data-stu-id="ad895-170">If the request is synchronous send success or failure message to the command line tool.</span></span>

<span data-ttu-id="ad895-171">如需與每個訊息相關聯的個別資料結構，請參閱一般 .h。</span><span class="sxs-lookup"><span data-stu-id="ad895-171">For the individual data structures associated with each message, see common.h.</span></span>

-   [<span data-ttu-id="ad895-172">服務 .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-172">Service.cpp</span></span>](#servicecpp)
-   [<span data-ttu-id="ad895-173">服務。h</span><span class="sxs-lookup"><span data-stu-id="ad895-173">Service.h</span></span>](#serviceh)
-   [<span data-ttu-id="ad895-174">CChannelManager .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-174">CChannelManager.cpp</span></span>](#cchannelmanagercpp)
-   [<span data-ttu-id="ad895-175">CFileRep .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-175">CFileRep.cpp</span></span>](#cfilerepcpp)
-   [<span data-ttu-id="ad895-176">CRequest .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-176">CRequest.cpp</span></span>](#crequestcpp)
-   [<span data-ttu-id="ad895-177">CFileRepClient .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-177">CFileRepClient.cpp</span></span>](#cfilerepclientcpp)
-   [<span data-ttu-id="ad895-178">CFileRepServer .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-178">CFileRepServer.cpp</span></span>](#cfilerepservercpp)
-   [<span data-ttu-id="ad895-179">一般。h</span><span class="sxs-lookup"><span data-stu-id="ad895-179">common.h</span></span>](#commonh)
-   [<span data-ttu-id="ad895-180">FileRep.mc</span><span class="sxs-lookup"><span data-stu-id="ad895-180">FileRep.mc</span></span>](#filerepmc)
-   [<span data-ttu-id="ad895-181">FileRep .rc</span><span class="sxs-lookup"><span data-stu-id="ad895-181">FileRep.rc</span></span>](#filereprc)
-   [<span data-ttu-id="ad895-182">Makefile</span><span class="sxs-lookup"><span data-stu-id="ad895-182">Makefile</span></span>](#makefile)
-   [<span data-ttu-id="ad895-183">相關主題</span><span class="sxs-lookup"><span data-stu-id="ad895-183">Related topics</span></span>](#related-topics)

## <a name="servicecpp"></a><span data-ttu-id="ad895-184">服務 .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-184">Service.cpp</span></span>
...
## <a name="serviceh"></a><span data-ttu-id="ad895-185">服務。h</span><span class="sxs-lookup"><span data-stu-id="ad895-185">Service.h</span></span>
...
## <a name="cchannelmanagercpp"></a><span data-ttu-id="ad895-186">CChannelManager .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-186">CChannelManager.cpp</span></span>
...
## <a name="cfilerepcpp"></a><span data-ttu-id="ad895-187">CFileRep .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-187">CFileRep.cpp</span></span>
...
## <a name="crequestcpp"></a><span data-ttu-id="ad895-188">CRequest .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-188">CRequest.cpp</span></span>
...
## <a name="cfilerepclientcpp"></a><span data-ttu-id="ad895-189">CFileRepClient .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-189">CFileRepClient.cpp</span></span>
...
## <a name="cfilerepservercpp"></a><span data-ttu-id="ad895-190">CFileRepServer .cpp</span><span class="sxs-lookup"><span data-stu-id="ad895-190">CFileRepServer.cpp</span></span>
...
## <a name="commonh"></a><span data-ttu-id="ad895-191">common.h</span><span class="sxs-lookup"><span data-stu-id="ad895-191">common.h</span></span>
...
## <a name="filerepmc"></a><span data-ttu-id="ad895-192">FileRep.mc</span><span class="sxs-lookup"><span data-stu-id="ad895-192">FileRep.mc</span></span>
...
## <a name="filereprc"></a><span data-ttu-id="ad895-193">FileRep .rc</span><span class="sxs-lookup"><span data-stu-id="ad895-193">FileRep.rc</span></span>
...
## <a name="makefile"></a><span data-ttu-id="ad895-194">Makefile</span><span class="sxs-lookup"><span data-stu-id="ad895-194">Makefile</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="ad895-195">相關主題</span><span class="sxs-lookup"><span data-stu-id="ad895-195">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="ad895-196">FileRepToolExample</span><span class="sxs-lookup"><span data-stu-id="ad895-196">FileRepToolExample</span></span>](filereptoolexample.md)
</dt> </dl>
