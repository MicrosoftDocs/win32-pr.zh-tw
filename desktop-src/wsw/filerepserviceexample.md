...
FileRep 會從伺服器抓取檔案，並將其複製到用戶端。 若要這樣做，它會採用三個元件：在電腦上使用原始程式檔執行的伺服器服務、將儲存目的地檔案的電腦上執行的用戶端服務，以及用來控制複製的命令列工具。 用戶端和伺服器服務會在使用者啟動命令列工具時，持續執行 web 服務，並在一個要求之後結束。

此範例說明如何使用通道和序列化層。

這是服務。 您可以在 [這裡](filereptoolexample.md)找到命令列工具。此服務具有用戶端和伺服器模式，伺服器會在其中傳送檔案和用戶端接收檔案。

用戶端模式的命令列參數如下所示：

**WsFileRepService.exe 用戶端** *<Service Url>***\[ /reporting： <error/info/verbose>\] \[ /encoding： <TEXT/binary/MTOM>\] \[ /connections： ***<number of connections>*** \]**

<dl> <dt>

<span id="Client"></span><span id="client"></span><span id="CLIENT"></span>客戶
</dt> <dd>
...
<span id="Service_Url"></span><span id="service_url"></span><span id="SERVICE_URL"></span>服務 Url
</dt> <dd>
...
<span id="Encoding"></span><span id="encoding"></span><span id="ENCODING"></span>編碼
</dt> <dd>

選擇性。 指定使用命令列工具進行通訊時所使用的編碼方式。 請注意，目前的工具不支援指定此傳輸的編碼，因此變更這種設定可能會產生錯誤。 這項設定可讓您獨立于伺服器之外變更和擴充工具。

</dd> <dt>

<span id="Reporting"></span><span id="reporting"></span><span id="REPORTING"></span>報告
</dt> <dd>

選擇性。 啟用錯誤、資訊或詳細資訊層級的報告。 預設值為 error。 訊息會列印到主控台。

</dd> <dt>

<span id="Connections"></span><span id="connections"></span><span id="CONNECTIONS"></span>連接
</dt> <dd>

選擇性。 指定將處理的並行要求數目上限。 如果省略，則預設值為100。

</dd> </dl>

伺服器模式的命令列參數如下所示：

**WsFileRepService.exe 伺服器** *<Service Url>***\[ /reporting： <error/info/verbose>\] \[ /encoding： <text/binary/MTOM>\] \[ /connections： ***<number of connections>*** \] \[ /chunk： ***<size of the payload per message in bytes>*** \]**

<dl> <dt>

<span id="Server"></span><span id="server"></span><span id="SERVER"></span>伺服器
</dt> <dd>
...
<span id="Chunk"></span><span id="chunk"></span><span id="CHUNK"></span>塊
</dt> <dd>

選擇性。 傳輸的檔案會分成指定大小的區塊。 每個訊息都包含一個區塊。 預設值為32768個位元組。

</dd> </dl>

執行詳細資料。 因為此範例是針對通道層而撰寫，所以必須手動執行服務模型層可能自動執行的特定工作。 其中一項工作就是通道管理。 為了提供對要求的快速回應，一定會有 (的限制，) 數個通道可以接受要求。 在多個要求的情況下，如果多個通道在建立通道時只需建立一個通道，則擁有多個通道的效能會更好。 此外，重複使用狀態也可改善效能。 Sapphire 會提供 Api 來重設大部分狀態，以避免必須釋放並重新建立。 此範例會在可能的情況下重複使用通道和要求狀態，並只在傳遞特定閾值時建立或終結狀態，以利用這種方式。 通道管理的相關程式碼可在 CChannelManager 中找到。

主要訊息處理迴圈是在 CRequest 中。 該類別包含非同步 Sapphire 訊息處理迴圈所需的應用程式獨立狀態和方法。 應用程式特定的程式碼位於 CFileRepClient (用戶端服務) 和 CFileRepServer (server 服務) 。 這兩個類別都繼承自 CFileRep，其中包含一般服務相關的程式碼。

此範例會使用序列化程式，並執行自訂序列化。 自訂序列化是在處理大型資料區塊時使用，以針對特定目的手動優化記憶體配置，以將記憶體耗用量降至最低。 因為這會導致複雜的低層級程式碼進行手動序列化，因此應該只在絕對必要時才執行。

訊息交換模式：

-   用戶端服務會從命令列工具取得要求訊息。
-   如果要求是非同步，請立即傳送確認。
-   用戶端服務會將檔案資訊的要求傳送到伺服器服務。 探索要求是以-1 的區塊位置表示。
-   伺服器服務會傳回檔案資訊。
-   用戶端服務會依序一次從伺服器要求個別的區塊。 區塊的識別方式是在檔案中的位置。
-   重複直到檔案傳輸完成或發生錯誤。
-   如果要求是同步傳送成功或失敗訊息至命令列工具。

如需與每個訊息相關聯的個別資料結構，請參閱一般 .h。
