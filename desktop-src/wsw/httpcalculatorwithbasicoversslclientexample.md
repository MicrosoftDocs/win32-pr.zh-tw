...
ms.locfileid: "117841562"
---
# <a name="httpcalculatorwithbasicoversslclientexample"></a>HttpCalculatorWithBasicOverSslClientExample

此範例說明如何使用服務 proxy 與具有 SSL 傳輸安全性和 HTTP 標頭基本驗證的計算機服務通訊。 在此設定中，傳輸連線會受到 SSL (簽署、加密) 的 SSL，也提供伺服器驗證。 用戶端驗證是透過使用基本配置的 HTTP 標頭中提供的使用者名稱/密碼組來提供。
