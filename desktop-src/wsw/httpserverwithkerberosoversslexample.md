...
description: 此範例顯示可接受通道並接收和回復訊息的 TCP 伺服器，並具有 kerberos over SSL 混合模式安全性。
ms.assetid: 2910f594-2f38-4c1e-bb47-89258b752572
keywords:
- HttpServerWithKerberosOverSslExample 原生 Web 服務
- WWSAPI
- WWS
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "119927282"
---
# <a name="httpserverwithkerberosoversslexample"></a>HttpServerWithKerberosOverSslExample

此範例顯示可接受通道並接收和回復訊息的 TCP 伺服器，並具有 kerberos over SSL 混合模式安全性。 在此設定中，傳輸連線會受到 SSL (簽署、加密) 的 SSL，也提供伺服器驗證。 用戶端驗證是由訊息的 WS-Security 標頭中的 kerberos APREQ 票證所提供。
