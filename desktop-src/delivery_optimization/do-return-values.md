...
---
# <a name="do-return-values"></a>傳回值

下列常數代表傳遞最佳化 () 產生的傳回值，以及會捕捉的 HTTP 傳回值。 您可以接收的所有其他傳回值為 COM、RPC 或轉換的 Windows 傳回值 (使用 [HRESULT_FROM_WIN32](/windows/win32/api/winerror/nf-winerror-hresult_from_win32) 宏將 windows 傳回值轉換成 HRESULT 值) 。
...
使用未知的屬性識別碼呼叫 SetProperty () 或 GetProperty () 。
...
伺服器不支援必要的 HTTP 通訊協定。 傳遞最佳化 () 需要伺服器支援範圍通訊協定標頭。
