...
<span data-ttu-id="ae42d-110">測試案例的名稱。</span><span class="sxs-lookup"><span data-stu-id="ae42d-110">Name of the test case.</span></span>
...
<li><span data-ttu-id="ae42d-174">確認使用者 Jane 封鎖 EXE/名稱時，不會啟動使用者 Toby 的遊戲。</span><span class="sxs-lookup"><span data-stu-id="ae42d-174">Verify that the game does not launch for User Toby when EXE/Name is blocked by User Jane.</span></span></li>
...
<li><span data-ttu-id="ae42d-205">16:10 &quot; 寬銀幕 &quot; (1152 720、1680 1050 或 800 480) </span><span class="sxs-lookup"><span data-stu-id="ae42d-205">16:10 &quot;widescreen&quot; (1152 720, 1680 1050, or 800 480)</span></span></li>
</ul></td>
</tr>
<tr class="even">
...
<td><dl> <span data-ttu-id="ae42d-237"><dt><span id="Manual"></span><span id="manual"></span><span id="MANUAL"></span>手動</dt> </span><span class="sxs-lookup"><span data-stu-id="ae42d-237"><dt><span id="Manual"></span><span id="manual"></span><span id="MANUAL"></span>Manual</dt> </span></span><dd>
<ol>
...
<td><span data-ttu-id="ae42d-263">手動測試</span><span class="sxs-lookup"><span data-stu-id="ae42d-263">Manual Test</span></span><br/>
<ol>
...
<ul>
<li><span data-ttu-id="ae42d-428">基本概念：控制碼、堆積、鎖定、記憶體、TLS</span><span class="sxs-lookup"><span data-stu-id="ae42d-428">Basics: Handles, Heaps, Locks, Memory, TLS</span></span></li>
...
<li><span data-ttu-id="ae42d-433">找出遊戲可執行檔，選取它，然後按一下 [ &quot; 開啟] &quot; 按鈕。</span><span class="sxs-lookup"><span data-stu-id="ae42d-433">Locate the game executable, select it, and click the &quot;Open&quot; button.</span></span></li>
<li><span data-ttu-id="ae42d-434">在 [ &quot; 應用程式] &quot; 區段中，選取 [遊戲可執行檔]。</span><span class="sxs-lookup"><span data-stu-id="ae42d-434">In the &quot;Applications&quot; section, select the game executable.</span></span></li>
<li><span data-ttu-id="ae42d-435">在 [ &quot; 測試] &quot; 區段中，選取 [分類基本和其他] 底下列出的測試 (取消核取 [ &quot; ThreadPool] 和 [TimeRollOver] &quot; &quot; &quot;) ，並確定未選取所有其他測試。</span><span class="sxs-lookup"><span data-stu-id="ae42d-435">In the &quot;Tests&quot; section, select the tests listed above under the categories &quot;Basics&quot; and &quot;Miscellaneous&quot; (uncheck ThreadPool and TimeRollOver), and ensure all other tests are not selected.</span></span></li>
<li><span data-ttu-id="ae42d-436">啟動遊戲。</span><span class="sxs-lookup"><span data-stu-id="ae42d-436">Launch the game.</span></span></li>
<li><span data-ttu-id="ae42d-437">確認遊戲在應用程式驗證器下執行時，不會產生失敗。</span><span class="sxs-lookup"><span data-stu-id="ae42d-437">Verify that the game does not generate failures when run under Application Verifier.</span></span></li>
</ol>
<blockquote>
[!Note]<br />
<span data-ttu-id="ae42d-438">有些測試需要完整執行偵錯工具。</span><span class="sxs-lookup"><span data-stu-id="ae42d-438">Some tests require a debugger to be fully run.</span></span> <span data-ttu-id="ae42d-439">這可能需要不受保護的遊戲可執行檔版本，因為反盜版/反盜版技術可能會干擾 AppVerifer。</span><span class="sxs-lookup"><span data-stu-id="ae42d-439">This may require an unprotected release version of the game executable, since anti-cheat/anti-piracy technology may interfere with AppVerifer.</span></span>
</blockquote>
<br/></td>
</tr>
</tbody>
...
### <a name="43-support-windows-error-reporting"></a><span data-ttu-id="ae42d-440">4.3 支援 Windows 錯誤報告</span><span class="sxs-lookup"><span data-stu-id="ae42d-440">4.3 Support Windows Error Reporting</span></span>
...
<tbody>
<tr class="odd">
<td><span data-ttu-id="ae42d-441">Windows 7</span><span class="sxs-lookup"><span data-stu-id="ae42d-441">Windows 7</span></span><br/> <span data-ttu-id="ae42d-442">Windows Vista</span><span class="sxs-lookup"><span data-stu-id="ae42d-442">Windows Vista</span></span><br/> <span data-ttu-id="ae42d-443">Windows XP</span><span class="sxs-lookup"><span data-stu-id="ae42d-443">Windows XP</span></span><br/></td>
<td><span data-ttu-id="ae42d-444">遊戲必須只處理已知和預期的例外狀況，Windows 錯誤報告不得停用。</span><span class="sxs-lookup"><span data-stu-id="ae42d-444">Games must handle only exceptions that are known and expected, and Windows Error Reporting must not be disabled.</span></span> <span data-ttu-id="ae42d-445">如果將錯誤 (（例如存取違規) ）插入遊戲中，則必須允許 Windows 錯誤報告報告損毀。</span><span class="sxs-lookup"><span data-stu-id="ae42d-445">If a fault (such as an Access Violation) is injected into a game, it must allow Windows Error Reporting to report the crash.</span></span></td>
</tr>
<tr class="even">

<td><span data-ttu-id="ae42d-446">使用工具：執行緒劫匪</span><span class="sxs-lookup"><span data-stu-id="ae42d-446">Use Tool: Thread Hijacker</span></span> <br/>
<ul>
<li><span data-ttu-id="ae42d-447">如果應用程式在測試時當機，請確認該遊戲已正確顯示 Windows 錯誤報告，並收集損毀資料。</span><span class="sxs-lookup"><span data-stu-id="ae42d-447">If the application crashes while testing, verify that the game displays Windows Error Reporting properly and collects crash data.</span></span></li>
</ul></td>
</tr>
</tbody>
</table>
...
<tbody>
<tr class="odd">
<td><span data-ttu-id="ae42d-448">Windows 7</span><span class="sxs-lookup"><span data-stu-id="ae42d-448">Windows 7</span></span><br/> <span data-ttu-id="ae42d-449">Windows Vista</span><span class="sxs-lookup"><span data-stu-id="ae42d-449">Windows Vista</span></span><br/> <span data-ttu-id="ae42d-450">Windows XP</span><span class="sxs-lookup"><span data-stu-id="ae42d-450">Windows XP</span></span><br/></td>
<td><span data-ttu-id="ae42d-451">所有可執行檔 (例如，.exe 或 .dll 檔) 都必須包含正確的產品名稱、公司名稱和檔案版本。</span><span class="sxs-lookup"><span data-stu-id="ae42d-451">All executable files (for example, .exe or .dll files) must contain an accurate Product Name, Company Name, and File Version.</span></span></td>
</tr>
<tr class="even">

<td><dl> <span data-ttu-id="ae42d-452"><dt><span id="Manual_test_"></span><span id="manual_test_"></span><span id="MANUAL_TEST_"></span>手動測試：</dt> </span><span class="sxs-lookup"><span data-stu-id="ae42d-452"><dt><span id="Manual_test_"></span><span id="manual_test_"></span><span id="MANUAL_TEST_"></span>Manual test:</dt> </span></span><dd>
<ol>
<li><span data-ttu-id="ae42d-453">在安裝媒體上，以滑鼠右鍵按一下遊戲的可執行檔 (s) ，然後安裝在電腦硬碟上。</span><span class="sxs-lookup"><span data-stu-id="ae42d-453">Right-click the game's executable file(s) on both the installation media and those installed to the computer hard drive.</span></span></li>
<li><span data-ttu-id="ae42d-454">選取 [屬性]。</span><span class="sxs-lookup"><span data-stu-id="ae42d-454">Select Properties.</span></span></li>
<li><span data-ttu-id="ae42d-455">Windows XP：按一下 [ <strong>版本</strong> ] 索引標籤。確認已正確填入 [產品名稱]、[公司名稱] 和 [檔案版本] 欄位。</span><span class="sxs-lookup"><span data-stu-id="ae42d-455">Windows XP: Click the <strong>Version</strong> tab. Verify that the Product Name, Company Name, and File Version fields are properly populated.</span></span></li>
<li><span data-ttu-id="ae42d-456">Windows Vista 或 Windows 7：按一下 [ <strong>詳細資料</strong> ] 索引標籤。確認 [產品名稱] 和 [檔案版本] 欄位已正確填入。</span><span class="sxs-lookup"><span data-stu-id="ae42d-456">Windows Vista or Windows 7: Click the <strong>Details</strong> tab. Verify that the Product name and File Version fields are properly populated.</span></span> <span data-ttu-id="ae42d-457">在 Windows Vista 或 Windows 7 的 [屬性] 頁面中看不到公司名稱。</span><span class="sxs-lookup"><span data-stu-id="ae42d-457">Company Name is not visible in the Windows Vista or Windows 7 properties page.</span></span></li>
</ol>
</dd> <span data-ttu-id="ae42d-458"><dt><span id="Automated_test_"></span><span id="automated_test_"></span><span id="AUTOMATED_TEST_"></span>自動化測試：</dt> </span><span class="sxs-lookup"><span data-stu-id="ae42d-458"><dt><span id="Automated_test_"></span><span id="automated_test_"></span><span id="AUTOMATED_TEST_"></span>Automated test:</dt> </span></span><dd>
<ul>
<li><span data-ttu-id="ae42d-459">使用 Microsoft 遊戲進行 Windows 測試控管;請參閱 <a href="#64-microsoft-games-for-windows-test-tool">第6.4 節</a>。</span><span class="sxs-lookup"><span data-stu-id="ae42d-459">Use the Microsoft Games for Windows Test Tool; see <a href="#64-microsoft-games-for-windows-test-tool">section 6.4</a>.</span></span></li>
</ul>
</dd> </dl></td>
</tr>
</tbody>
...
<tbody>
<tr class="odd">
<td><span data-ttu-id="ae42d-460">Windows 7</span><span class="sxs-lookup"><span data-stu-id="ae42d-460">Windows 7</span></span><br/> <span data-ttu-id="ae42d-461">Windows Vista</span><span class="sxs-lookup"><span data-stu-id="ae42d-461">Windows Vista</span></span><br/> <span data-ttu-id="ae42d-462">Windows XP</span><span class="sxs-lookup"><span data-stu-id="ae42d-462">Windows XP</span></span><br/></td>
<td><span data-ttu-id="ae42d-463">正常結束遊戲時，不能造成不明的例外狀況錯誤。</span><span class="sxs-lookup"><span data-stu-id="ae42d-463">Normal exit of the game must not result in an unknown exception fault.</span></span></td>
</tr>
<tr class="even">

<td><ul>
<li><span data-ttu-id="ae42d-464">播放正常遊戲會話的遊戲之後，請確認遊戲未在結束時產生錯誤。</span><span class="sxs-lookup"><span data-stu-id="ae42d-464">After playing the game for a normal gaming session, verify that the game does not generate errors on exit.</span></span></li>
</ul></td>
</tr>
</tbody>
</table>
...
## <a name="5-sample-test-script"></a><span data-ttu-id="ae42d-465">5. 範例測試腳本</span><span class="sxs-lookup"><span data-stu-id="ae42d-465">5. Sample Test Script</span></span>

<span data-ttu-id="ae42d-466">這是使用上述測試需求作為指南的一般測試階段範例。</span><span class="sxs-lookup"><span data-stu-id="ae42d-466">This is an example of a typical test pass using the preceding test requirements as a guide.</span></span>

### <a name="51-tools"></a><span data-ttu-id="ae42d-467">5.1 工具</span><span class="sxs-lookup"><span data-stu-id="ae42d-467">5.1 Tools</span></span>

-   <span data-ttu-id="ae42d-468">Windows Vista SP1 和（或） Windows 7 在 AMD CPU 上的32位版本</span><span class="sxs-lookup"><span data-stu-id="ae42d-468">32-bit edition of Windows Vista SP1 and/or Windows 7 on an AMD CPU</span></span>
-   <span data-ttu-id="ae42d-469">Intel CPU 上的 Windows Vista SP1 和（或） Windows 7 的32位版本</span><span class="sxs-lookup"><span data-stu-id="ae42d-469">32-bit edition of Windows Vista SP1 and/or Windows 7 on an Intel CPU</span></span>
-   <span data-ttu-id="ae42d-470">Windows Vista SP1 和（或） Windows 7 在 AMD CPU 上的64位版本</span><span class="sxs-lookup"><span data-stu-id="ae42d-470">64-bit edition of Windows Vista SP1 and/or Windows 7 on an AMD CPU</span></span>
-   <span data-ttu-id="ae42d-471">Intel CPU 上的 Windows Vista SP1 和（或） Windows 7 的64位版本</span><span class="sxs-lookup"><span data-stu-id="ae42d-471">64-bit edition of Windows Vista SP1 and/or Windows 7 on an Intel CPU</span></span>
-   <span data-ttu-id="ae42d-472">AMD CPU 上的32位 edition Windows XP SP2</span><span class="sxs-lookup"><span data-stu-id="ae42d-472">32-bit edition Windows XP SP2 on an AMD CPU</span></span>
-   <span data-ttu-id="ae42d-473">Intel CPU 上的32位版 Windows XP SP2</span><span class="sxs-lookup"><span data-stu-id="ae42d-473">32-bit edition Windows XP SP2 on an Intel CPU</span></span>
-   <span data-ttu-id="ae42d-474">支援 1680 1050 的全螢幕監視器</span><span class="sxs-lookup"><span data-stu-id="ae42d-474">Wide Screen Monitor that supports 1680 1050</span></span>
-   <span data-ttu-id="ae42d-475">適用于 Windows 的 Xbox 360 控制器</span><span class="sxs-lookup"><span data-stu-id="ae42d-475">Xbox 360 Controller for Windows</span></span>

### <a name="52-pre-install"></a><span data-ttu-id="ae42d-476">5.2 預先安裝</span><span class="sxs-lookup"><span data-stu-id="ae42d-476">5.2 Pre-Install</span></span>

1.  <span data-ttu-id="ae42d-477">Windows Vista 和 Windows 7：建立兩位標準使用者： Jane 和 Toby</span><span class="sxs-lookup"><span data-stu-id="ae42d-477">Windows Vista and Windows 7: Create two Standard Users: Jane and Toby</span></span>
2.  <span data-ttu-id="ae42d-478">Windows Vista 和 Windows 7：確定已啟用使用者帳戶控制</span><span class="sxs-lookup"><span data-stu-id="ae42d-478">Windows Vista and Windows 7: Ensure User Account Control is enabled</span></span>
3.  <span data-ttu-id="ae42d-479">建立 System32 的預先安裝快照集</span><span class="sxs-lookup"><span data-stu-id="ae42d-479">Create a pre-install snapshot of System32</span></span>

    1.  <span data-ttu-id="ae42d-480">建立名為 G4Wtest 的目錄</span><span class="sxs-lookup"><span data-stu-id="ae42d-480">Make a directory called G4Wtest</span></span>
    2.  <span data-ttu-id="ae42d-481">顯示命令視窗 (啟動-> 執行-> cmd) </span><span class="sxs-lookup"><span data-stu-id="ae42d-481">Bring up a command Window (Start -> Run -> cmd)</span></span>
    3.  <span data-ttu-id="ae42d-482">流覽至 c： \\ windows \\ system32</span><span class="sxs-lookup"><span data-stu-id="ae42d-482">Navigate to c:\\windows\\system32</span></span>
    4.  <span data-ttu-id="ae42d-483">輸入 dir/o：-g/o：-d >> c： \\ G4Wtest \\pregame.txt</span><span class="sxs-lookup"><span data-stu-id="ae42d-483">Type dir /o:-g /o:-d >> c:\\G4Wtest\\pregame.txt</span></span>

4.  <span data-ttu-id="ae42d-484">Windows Vista 和 Windows 7：設定為 150% DPI \[ 1。8\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-484">Windows Vista and Windows 7: Set to 150% DPI \[1.8\]</span></span>
5.  <span data-ttu-id="ae42d-485">繼續 [安裝](#3-installation)</span><span class="sxs-lookup"><span data-stu-id="ae42d-485">Proceed to [Install](#3-installation)</span></span>

### <a name="53-install"></a><span data-ttu-id="ae42d-486">5.3 安裝</span><span class="sxs-lookup"><span data-stu-id="ae42d-486">5.3 Install</span></span>

1.  <span data-ttu-id="ae42d-487">以使用者 Jane 登入</span><span class="sxs-lookup"><span data-stu-id="ae42d-487">Log on as User Jane</span></span>
2.  <span data-ttu-id="ae42d-488">將遊戲光碟插入 CD/DVD 光碟機中，確認 [安裝/執行] 對話方塊自動出現 \[ 3。7\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-488">Insert the game disc into the CD/DVD drive, verify that the install / run dialog box comes up automatically \[3.7\]</span></span>
3.  <span data-ttu-id="ae42d-489">確認遊戲安裝程式會提示使用者 Jane 提升系統管理員認證 \[ 3。2\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-489">Verify that the game installation process prompts User Jane to elevate Administrator Credentials \[3.2\]</span></span>
4.  <span data-ttu-id="ae42d-490">確認遊戲自動執行程式本身不會提示使用者 Jane 透過系統管理員認證3.7 進行提升 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-490">Verify that the game Autorun program itself does not prompt User Jane to elevate through Administrator Credentials \[3.7\]</span></span>
5.  <span data-ttu-id="ae42d-491">確認遊戲未顯示超過一份 End-User 授權合約 (EULA) \[ 3。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-491">Verify that the game does not display more than one End-User License Agreement (EULA) \[3.1\]</span></span>
6.  <span data-ttu-id="ae42d-492">確認遊戲顯示預設/簡單和自訂/Advanced 安裝選項 \[ 3。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-492">Verify that the game displays Default/Easy and Custom/Advanced installation options \[3.1\]</span></span>
7.  <span data-ttu-id="ae42d-493">確認 [預設/簡單安裝] 選項會略過安裝程式的所有使用者輸入選項， (選取 [安裝資料夾]、[元件選取] 等等。 ) \[ 3。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-493">Verify that the Default/Easy installation option bypasses all user input selections for the install process (selection of installation folder, components selection, and so on.) \[3.1\]</span></span>
8.  <span data-ttu-id="ae42d-494">確認遊戲安裝程式不會提示 OS 元件安裝 (DirectX 安裝程式、C Run-Time 程式庫等等。 ) \[ 3。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-494">Verify that the game installation process does not prompt for OS component setup (DirectX setup, C Run-Time libraries, and so on.) \[3.1\]</span></span>
9.  <span data-ttu-id="ae42d-495">確認遊戲安裝程式未提示防火牆互動 \[ 3。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-495">Verify that the game installation process does not prompt for firewall interaction \[3.1\]</span></span>
10. <span data-ttu-id="ae42d-496">確認遊戲安裝程式未遇到關於 OS 版本 \[ 2.5 \] \[ 4.2 的錯誤\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-496">Verify that the game installation process does not encounter an error regarding OS Version \[2.5\] \[4.2\]</span></span>
11. <span data-ttu-id="ae42d-497">確認遊戲安裝程式未顯示未簽署的驅動程式對話方塊 (s) \[ 2。4\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-497">Verify that the game installation process does not display unsigned driver dialog(s) \[2.4\]</span></span>
12. <span data-ttu-id="ae42d-498">確認未在安裝程式期間出現任何 Windows 資源保護 (WRP) 對話方塊： \[ 3。4\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-498">Verify that no Windows Resource Protection (WRP) dialogs appear during the install process \[3.4\]</span></span>
13. <span data-ttu-id="ae42d-499">確認在安裝後將光碟重新插入磁片磁碟機，不會再次自動開始安裝</span><span class="sxs-lookup"><span data-stu-id="ae42d-499">Verify that re-inserting the disc in the drive after installation does not cause installation to automatically begin again</span></span>
14. <span data-ttu-id="ae42d-500">確認遊戲不需要在安裝3.5 後重新開機系統 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-500">Verify that the game does not require the system to be rebooted after installation \[3.5\]</span></span>
15. <span data-ttu-id="ae42d-501">確認您可以將遊戲安裝為使用者 Jane \[ 3。2\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-501">Verify that you can install the game as User Jane \[3.2\]</span></span>
16. <span data-ttu-id="ae42d-502">確認遊戲會自動執行，或在安裝程式結束時出現啟動器功能表 \[ 3。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-502">Verify that the game automatically runs or that a launcher menu is present at the end of the installation process \[3.1\]</span></span>
17. <span data-ttu-id="ae42d-503">如果遊戲在安裝後自動執行，請跳至[運行](#55-runtime)時間</span><span class="sxs-lookup"><span data-stu-id="ae42d-503">If the game does auto-run after installation, skip to [Runtime](#55-runtime)</span></span>
18. <span data-ttu-id="ae42d-504">如果遊戲離開啟動功能表，或無法卸載，請參閱[安裝後](#54-post-install)的章節</span><span class="sxs-lookup"><span data-stu-id="ae42d-504">If the game left a launch menu up, or failed to uninstall see section [Post-Install](#54-post-install)</span></span>

### <a name="54-post-install"></a><span data-ttu-id="ae42d-505">5.4 安裝後</span><span class="sxs-lookup"><span data-stu-id="ae42d-505">5.4 Post-Install</span></span>

1.  <span data-ttu-id="ae42d-506">確認遊戲未在使用者桌面1.1 上放置啟動快捷方式 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-506">Verify that the game does not place launch shortcuts on user desktop \[1.1\]</span></span>
2.  <span data-ttu-id="ae42d-507">確認遊戲未在 [開始] 功能表1.1 中放置啟動快捷方式 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-507">Verify that the game does not place launch shortcuts in the Start Menu \[1.1\]</span></span>
3.  <span data-ttu-id="ae42d-508">確認遊戲圖示顯示在 Windows 遊戲瀏覽器 \[ 1.1 中\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-508">Verify that the game icon displays in Windows Games Explorer \[1.1\]</span></span>
4.  <span data-ttu-id="ae42d-509">確認底部的 [發行者]、[開發人員]、[內容類型]、[發行日期]、[版本])  (的中繼資料，以及是否正確 \[ 1。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-509">Verify that the metadata (publisher, developer, genre, release date, version) at the bottom displays and is correct \[1.1\]</span></span>
5.  <span data-ttu-id="ae42d-510">確認遊戲圖示顯示 Windows 體驗指數 (Windows 遊戲瀏覽器1.1 中) 資訊的 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-510">Verify that the game icon displays Windows Experience Index (WEI) information in Windows Games Explorer \[1.1\]</span></span>
6.  <span data-ttu-id="ae42d-511">確認適用于中繼資料的遊戲超連結在 Windows 遊戲瀏覽器1.1 中正確運作 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-511">Verify that game hyperlinks for metadata work correctly in Windows Games Explorer \[1.1\]</span></span>
7.  <span data-ttu-id="ae42d-512">確認該遊戲在 Windows 遊戲瀏覽器1.1 中顯示精確的家長監護控制項分級 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-512">Verify that the game displays accurate parental control rating in Windows Games Explorer \[1.1\]</span></span>
8.  <span data-ttu-id="ae42d-513">建立 System32 的後置安裝快照集</span><span class="sxs-lookup"><span data-stu-id="ae42d-513">Create a post-install snapshot of System32</span></span>

    1.  <span data-ttu-id="ae42d-514">顯示命令視窗 (啟動-> 執行-> cmd) </span><span class="sxs-lookup"><span data-stu-id="ae42d-514">Bring up a command Window (Start -> Run -> cmd)</span></span>
    2.  <span data-ttu-id="ae42d-515">流覽至 c： \\ windows \\ system32</span><span class="sxs-lookup"><span data-stu-id="ae42d-515">Navigate to c:\\windows\\system32</span></span>
    3.  <span data-ttu-id="ae42d-516">輸入 dir/o：-g/o：-d >> c： \\ G4Wtest \\postgame.txt</span><span class="sxs-lookup"><span data-stu-id="ae42d-516">Type dir /o:-g /o:-d >> c:\\G4Wtest\\postgame.txt</span></span>
    4.  <span data-ttu-id="ae42d-517">藉由比較 pregame.txt 與 postgame.txt 3.6，確認遊戲未回歸兩份檔中所列檔案的任何檔案版本 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-517">Verify that the game does not regress any file versions of files listed in the two documents by comparing pregame.txt to postgame.txt \[3.6\]</span></span>

9.  <span data-ttu-id="ae42d-518">繼續進行[運行](#55-runtime)時間</span><span class="sxs-lookup"><span data-stu-id="ae42d-518">Proceed to [Runtime](#55-runtime)</span></span>

### <a name="55-runtime"></a><span data-ttu-id="ae42d-519">5.5 執行時間</span><span class="sxs-lookup"><span data-stu-id="ae42d-519">5.5 Runtime</span></span>

1.  <span data-ttu-id="ae42d-520">執行時間1：如果出現 [啟動] 功能表，請從該處啟動遊戲。</span><span class="sxs-lookup"><span data-stu-id="ae42d-520">RUNTIME 1: If the launch menu is present, launch the game from there.</span></span> <span data-ttu-id="ae42d-521">如果在安裝後自動執行遊戲或從遊戲啟動器功能表啟動，請執行下列動作：如果沒有，請跳至執行時間2：</span><span class="sxs-lookup"><span data-stu-id="ae42d-521">If the game auto-ran or was launched from the game launcher menu after install, perform the following; if not, skip to RUNTIME 2:</span></span>

    1.  <span data-ttu-id="ae42d-522">如果遊戲允許， (建立設定檔) </span><span class="sxs-lookup"><span data-stu-id="ae42d-522">Create a profile (if the game allows)</span></span>
    2.  <span data-ttu-id="ae42d-523">開始新的遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-523">Start a new game</span></span>
    3.  <span data-ttu-id="ae42d-524">儲存遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-524">Save the game</span></span>
    4.  <span data-ttu-id="ae42d-525">離開遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-525">Exit the game</span></span>
    5.  <span data-ttu-id="ae42d-526">從遊戲瀏覽器啟動遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-526">Launch the game from Games Explorer</span></span>
    6.  <span data-ttu-id="ae42d-527">確認遊戲從遊戲瀏覽器圖示 \[ 1.2 啟動\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-527">Verify that the game launches from the Games Explorer icon \[1.2\]</span></span>
    7.  <span data-ttu-id="ae42d-528">確認遊戲不會在啟動1.2 時提示輸入系統管理員認證 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-528">Verify that the game does not prompt for Administrator Credentials on launch \[1.2\]</span></span>
    8.  <span data-ttu-id="ae42d-529">確認使用者 Jane 帳戶3.2 可存取使用者設定檔和儲存遊戲 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-529">Verify that User Profiles and Save Games can be accessed by User Jane account \[3.2\]</span></span>
    9.  <span data-ttu-id="ae42d-530">繼續執行執行時間3</span><span class="sxs-lookup"><span data-stu-id="ae42d-530">Proceed to RUNTIME 3</span></span>

2.  <span data-ttu-id="ae42d-531">執行時間2：如果遊戲未從遊戲啟動器功能表自動執行或顯示啟動，這會是3.1 的失敗 \[ \] ; 不過，測試可以正常地繼續：</span><span class="sxs-lookup"><span data-stu-id="ae42d-531">RUNTIME 2: If the game did not auto-run or display a launch from the game launcher menu, this is a failure of \[3.1\]; however, testing can continue normally:</span></span>

    1.  <span data-ttu-id="ae42d-532">從遊戲瀏覽器啟動遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-532">Launch the game from Games Explorer</span></span>
    2.  <span data-ttu-id="ae42d-533">確認遊戲從遊戲瀏覽器圖示 \[ 1.2 啟動\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-533">Verify that the game launches from the Games Explorer icon \[1.2\]</span></span>
    3.  <span data-ttu-id="ae42d-534">確認遊戲不會在啟動1.2 時提示輸入系統管理員認證 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-534">Verify that the game does not prompt for Administrator Credentials on launch \[1.2\]</span></span>
    4.  <span data-ttu-id="ae42d-535">繼續執行執行時間3</span><span class="sxs-lookup"><span data-stu-id="ae42d-535">Proceed to RUNTIME 3</span></span>

3.  <span data-ttu-id="ae42d-536">執行時間3：如果遊戲支援遊戲台，請確認遊戲可將 Windows Xbox 360 控制器辨識為輸入裝置 \[ 1。4\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-536">RUNTIME 3: If the game supports a game pad, verify that the game recognizes Xbox 360 Controller for Windows as an input device \[1.4\]</span></span>

    1.  <span data-ttu-id="ae42d-537">如有需要，請透過 [選項] 功能表啟用控制器</span><span class="sxs-lookup"><span data-stu-id="ae42d-537">If needed, enable the controller via the options menu</span></span>
    2.  <span data-ttu-id="ae42d-538">使用 Xbox 360 名稱確認遊戲參考控制器按鈕和觸發程式</span><span class="sxs-lookup"><span data-stu-id="ae42d-538">Verify that the game refers to the controller buttons and triggers using Xbox 360 names</span></span>
    3.  <span data-ttu-id="ae42d-539">確認遊戲和功能表系統可控制適用于 Windows 的 Xbox 360 控制器</span><span class="sxs-lookup"><span data-stu-id="ae42d-539">Verify that the game and menu system are controllable with the Xbox 360 Controller for Windows</span></span>
    4.  <span data-ttu-id="ae42d-540">確認適用于 Windows 的 Xbox 360 控制器根據接受的標準運作</span><span class="sxs-lookup"><span data-stu-id="ae42d-540">Verify that the Xbox 360 Controller for Windows behaves according to accepted standards</span></span>

4.  <span data-ttu-id="ae42d-541">將影片設定為 \[ 1.5 \] ：</span><span class="sxs-lookup"><span data-stu-id="ae42d-541">Set the video to \[1.5\]:</span></span>

    1.  <span data-ttu-id="ae42d-542">確認遊戲以4:3 的外觀比例解析度執行 (800 600 或 1024 768) </span><span class="sxs-lookup"><span data-stu-id="ae42d-542">Verify that the game runs at a 4:3 Aspect Ratio resolution (800 600 or 1024 768)</span></span>
    2.  <span data-ttu-id="ae42d-543">確認遊戲以16:9 的外觀比例解析度執行 (1280 720) </span><span class="sxs-lookup"><span data-stu-id="ae42d-543">Verify that the game runs at a 16:9 Aspect Ratio resolution (1280 720)</span></span>
    3.  <span data-ttu-id="ae42d-544">確認遊戲以16:10 的外觀比例解析度（ (1680 1050、800 480 或 1152 720）執行) </span><span class="sxs-lookup"><span data-stu-id="ae42d-544">Verify that the game runs at a 16:10 Aspect Ratio resolution (1680 1050, 800 480, or 1152 720)</span></span>
    4.  <span data-ttu-id="ae42d-545">確認當解決問題時，遊戲提示使用者</span><span class="sxs-lookup"><span data-stu-id="ae42d-545">Verify that the game prompts the user when a change is made to the resolution</span></span>
    5.  <span data-ttu-id="ae42d-546">如果您未在15秒內接受，請確認顯示器會還原為先前的設定</span><span class="sxs-lookup"><span data-stu-id="ae42d-546">Verify that the display reverts to the previous setting if you do not accept within 15 seconds</span></span>
    6.  <span data-ttu-id="ae42d-547">確認遊戲不會延展圖片，而是會顯示更廣大的視圖區域</span><span class="sxs-lookup"><span data-stu-id="ae42d-547">Verify that the game does not stretch the picture and in turn presents a wider area of view</span></span>
    7.  <span data-ttu-id="ae42d-548">確認遊戲未在遊戲播放區域的左邊和右邊新增黑色橫條</span><span class="sxs-lookup"><span data-stu-id="ae42d-548">Verify that the game does not add black bars to the left and right of the game play area</span></span>

5.  <span data-ttu-id="ae42d-549">如果影片設定中有提供，請確認遊戲轉譯選項預設為 Direct3D \[ 1.7 \] ; 否則，請繼續進行 [自動化測試](#58-automated-tests)</span><span class="sxs-lookup"><span data-stu-id="ae42d-549">If available in the video settings, verify that the game render options default to Direct3D \[1.7\]; otherwise, proceed to [Automated Tests](#58-automated-tests)</span></span>
6.  <span data-ttu-id="ae42d-550">如果出現提示或選項可供使用，請建立使用者設定檔。</span><span class="sxs-lookup"><span data-stu-id="ae42d-550">If prompted or if the option is available, create a user profile.</span></span> <span data-ttu-id="ae42d-551">確認在使用長檔名時，遊戲不會遇到錯誤 \[ 2。7\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-551">Verify that the game does not encounter errors when using long file names \[2.7\]</span></span>
7.  <span data-ttu-id="ae42d-552">開始新的遊戲、建立遊戲儲存，並確認在處理不支援的檔案系統字元時，遊戲不會遇到錯誤 \[ 2。7\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-552">Start a new game, create a game save, and verify that the game does not encounter errors when handling unsupported file system characters \[2.7\]</span></span>
8.  <span data-ttu-id="ae42d-553">確認遊戲正確地將 ALT + Tab 切換至 Windows 桌面 \[ 2。6\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-553">Verify that the game properly ALT+TABs to the Windows desktop \[2.6\]</span></span>

    1.  <span data-ttu-id="ae42d-554">按一下 [開始-> 切換使用者]，切換執行遊戲的使用者</span><span class="sxs-lookup"><span data-stu-id="ae42d-554">Switch users with the game running by clicking Start -> Switch User</span></span>
    2.  <span data-ttu-id="ae42d-555">以 Toby 登入</span><span class="sxs-lookup"><span data-stu-id="ae42d-555">Log on as Toby</span></span>
    3.  <span data-ttu-id="ae42d-556">確認在以使用者 Jane 2.6 的形式執行時，遊戲以使用者 Toby 的形式啟動 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-556">Verify that the game launches as User Toby while still running as User Jane \[2.6\]</span></span>
    4.  <span data-ttu-id="ae42d-557">確認在使用者切換程式期間，此遊戲不會遇到使用者 Toby 或使用者 Jane 的錯誤 \[ 2。6\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-557">Verify that the game does not encounter errors for User Toby or User Jane during User Switch process \[2.6\]</span></span>
    5.  <span data-ttu-id="ae42d-558">確認您無法聽到原始遊戲會話2.6 的音訊 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-558">Verify that you cannot hear audio from the original game session \[2.6\]</span></span>
    6.  <span data-ttu-id="ae42d-559">離開遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-559">Exit the game</span></span>
    7.  <span data-ttu-id="ae42d-560">登出 Toby</span><span class="sxs-lookup"><span data-stu-id="ae42d-560">Log off Toby</span></span>
    8.  <span data-ttu-id="ae42d-561">切換回正在執行遊戲的原始使用者</span><span class="sxs-lookup"><span data-stu-id="ae42d-561">Switch back to the original user where the game is running</span></span>
    9.  <span data-ttu-id="ae42d-562">ALT + TAB 回到遊戲中</span><span class="sxs-lookup"><span data-stu-id="ae42d-562">ALT+TAB back into the game</span></span>

9.  <span data-ttu-id="ae42d-563">離開遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-563">Exit the game</span></span>
10. <span data-ttu-id="ae42d-564">繼續進行[後置運行](#56-post-runtime)時間</span><span class="sxs-lookup"><span data-stu-id="ae42d-564">Proceed to [Post-Runtime](#56-post-runtime)</span></span>

### <a name="56-post-runtime"></a><span data-ttu-id="ae42d-565">5.6 後置執行時間</span><span class="sxs-lookup"><span data-stu-id="ae42d-565">5.6 Post-Runtime</span></span>

1.  <span data-ttu-id="ae42d-566">確認遊戲未在結束4.3 時產生錯誤 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-566">Verify that the game does not generate errors on exit \[4.3\]</span></span>
2.  <span data-ttu-id="ae42d-567">確認已將遊戲安裝至 Program Files \[ 3。3\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-567">Verify that the game installed to Program Files \[3.3\]</span></span>
3.  <span data-ttu-id="ae42d-568">繼續進行[家長](/windows)監護</span><span class="sxs-lookup"><span data-stu-id="ae42d-568">Proceed to [Parental Controls](/windows)</span></span>

### <a name="57-parental-controls"></a><span data-ttu-id="ae42d-569">5.7 家長監護</span><span class="sxs-lookup"><span data-stu-id="ae42d-569">5.7 Parental Controls</span></span>

1.  <span data-ttu-id="ae42d-570">在主控台中開啟家長監護</span><span class="sxs-lookup"><span data-stu-id="ae42d-570">Open Parental Controls in Control Panel</span></span>
2.  <span data-ttu-id="ae42d-571">確認該遊戲在家長監護的遊戲標題下方顯示精確的家長監護，主控台 \[ 1。2\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-571">Verify that the game displays accurate Parental Control Rating below the game title in Parental Controls Control Panel \[1.2\]</span></span>
3.  <span data-ttu-id="ae42d-572">請參閱測試案例 \[ 1.2 \] 以進行下列測試：</span><span class="sxs-lookup"><span data-stu-id="ae42d-572">See Test Case \[1.2\] for the following tests:</span></span>

    1.  <span data-ttu-id="ae42d-573">將家長監護設定為 [開啟] 之後，請確認遊戲以使用者 Jane 1.2 的這些設定執行。 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-573">After setting Parental Controls to "On", verify that the game runs with these settings as User Jane \[1.2\]</span></span>
    2.  <span data-ttu-id="ae42d-574">登出並以 Toby 登入</span><span class="sxs-lookup"><span data-stu-id="ae42d-574">Log off and log on as Toby</span></span>
    3.  <span data-ttu-id="ae42d-575">確認遊戲以這些設定作為使用者 Toby 1.2 來執行 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-575">Verify that the game runs with these settings as User Toby \[1.2\]</span></span>
    4.  <span data-ttu-id="ae42d-576">登出並以 Jane 登入</span><span class="sxs-lookup"><span data-stu-id="ae42d-576">Log off and log on as Jane</span></span>
    5.  <span data-ttu-id="ae42d-577">在 [家長監護] 區段中，封鎖使用者 Toby 從您剛剛安裝的遊戲中，讓使用者看得到一個 ESRB 等級以上的遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-577">In the Parental Control section, block User Toby from seeing games one ESRB level up and higher from the game that you just installed</span></span>

        <span data-ttu-id="ae42d-578">範例：如果遊戲的分級為 E，請設定它，讓 Toby 只能播放評為 C 的遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-578">Example: If the game is rated E, set it so Toby can only play games that are rated C</span></span>

    6.  <span data-ttu-id="ae42d-579">確認遊戲以使用者 Jane 1.2 的這些設定執行 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-579">Verify that the game runs with these settings as User Jane \[1.2\]</span></span>
    7.  <span data-ttu-id="ae42d-580">登出並以使用者 Toby 登入</span><span class="sxs-lookup"><span data-stu-id="ae42d-580">Log off and log on as user Toby</span></span>
    8.  <span data-ttu-id="ae42d-581">當使用者 Jane 1.2 封鎖 ESRB 時，確認遊戲未在使用者 Toby 上啟動 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-581">Verify that the game does not launch on User Toby when ESRB is blocked by User Jane \[1.2\]</span></span>
    9.  <span data-ttu-id="ae42d-582">以使用者 Jane 的形式登出使用者 Toby 及重新登入</span><span class="sxs-lookup"><span data-stu-id="ae42d-582">Log off as user Toby and back on as user Jane</span></span>
    10. <span data-ttu-id="ae42d-583">如果之前有變更，請還原 ESRB 設定</span><span class="sxs-lookup"><span data-stu-id="ae42d-583">If changed previously, restore the ESRB settings</span></span>
    11. <span data-ttu-id="ae42d-584">如果沒有任何 ESRB 設定，請選取 [封鎖或允許特定遊戲]，然後依名稱選取遊戲。</span><span class="sxs-lookup"><span data-stu-id="ae42d-584">If there are no ESRB settings, then select "Block or Allow Specific Games" and select the game by name</span></span>
    12. <span data-ttu-id="ae42d-585">以 Jane 的形式登出，並以 Toby</span><span class="sxs-lookup"><span data-stu-id="ae42d-585">Log off as Jane and on as Toby</span></span>
    13. <span data-ttu-id="ae42d-586">確認使用者 Jane 1.2 封鎖了 EXE/名稱時，該遊戲未在使用者 Toby 上啟動 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-586">Verify that the game does not launch on User Toby when EXE/Name is blocked by User Jane \[1.2\]</span></span>
    14. <span data-ttu-id="ae42d-587">以 Jane 的形式登出 Toby 和上一頁</span><span class="sxs-lookup"><span data-stu-id="ae42d-587">Log off as Toby and back on as Jane</span></span>
    15. <span data-ttu-id="ae42d-588">像 Jane 一樣，開啟使用者控制項-> 應用程式限制</span><span class="sxs-lookup"><span data-stu-id="ae42d-588">As Jane, open User Controls -> Application Restrictions</span></span>
    16. <span data-ttu-id="ae42d-589">按一下 [Toby 只能使用我允許的程式]，然後按一下 [確定] (即 [不允許 exe]) </span><span class="sxs-lookup"><span data-stu-id="ae42d-589">Click "Toby can only use the programs I allow", and then click OK (i.e. allow no exes)</span></span>
    17. <span data-ttu-id="ae42d-590">按一下 [全選] 方塊，然後按一下 [確定]。</span><span class="sxs-lookup"><span data-stu-id="ae42d-590">Click the Uncheck All box, and then click OK</span></span>
    18. <span data-ttu-id="ae42d-591">前往使用者控制項 \| 遊戲控制，並允許使用 ESRB 分級的特定遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-591">Go to User Controls \| Games Controls and allow the specific game using the ESRB rating</span></span>
    19. <span data-ttu-id="ae42d-592">以 Jane 的身份登入，並以 Toby 登入，然後嘗試播放遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-592">Log off as Jane and log on as Toby and try to play the game</span></span>
    20. <span data-ttu-id="ae42d-593">確認遊戲未被封鎖，而且 Toby 可以在 [允許沒有 exe] 設定為1.2 時播放 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-593">Verify that the game is NOT blocked and that Toby can play it when "allow no exes" is set \[1.2\]</span></span>
    21. <span data-ttu-id="ae42d-594">以使用者 Jane 的形式登出使用者 Toby 及重新登入</span><span class="sxs-lookup"><span data-stu-id="ae42d-594">Log off as user Toby and back on as user Jane</span></span>
    22. <span data-ttu-id="ae42d-595">移至主控台中的 [家長監護]，並移除限制</span><span class="sxs-lookup"><span data-stu-id="ae42d-595">Go to Parental Controls in Control Panel and remove the restrictions</span></span>
    23. <span data-ttu-id="ae42d-596">確認這兩個使用者現在都可以玩遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-596">Verify that both users can now play the game</span></span>

4.  <span data-ttu-id="ae42d-597">繼續進行 [自動化測試](#58-automated-tests)</span><span class="sxs-lookup"><span data-stu-id="ae42d-597">Proceed to [Automated Tests](#58-automated-tests)</span></span>

### <a name="58-automated-tests"></a><span data-ttu-id="ae42d-598">5.8 自動化測試</span><span class="sxs-lookup"><span data-stu-id="ae42d-598">5.8 Automated Tests</span></span>

1.  <span data-ttu-id="ae42d-599">確認在應用程式驗證器下執行時，遊戲未產生失敗-請參閱商標測試控管檔 \[ 4。2\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-599">Verify that the game does not generate failures when run under Application Verifier - See Branding Test Tool Documentation \[4.2\]</span></span>
2.  <span data-ttu-id="ae42d-600">確認遊戲可執行檔包含資訊清單-請參閱商標測試控管檔 \[ 2。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-600">Verify that the game executable files contain manifests - see Branding Test Tool Documentation \[2.1\]</span></span>
3.  <span data-ttu-id="ae42d-601">確認遊戲可執行檔資訊清單並無 requestedexecutionlevel 為 "AsInvoker"-請參閱商標測試控管檔 \[ 2。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-601">Verify that the game executable file manifest requestedExecutionLevel is "AsInvoker" - see Branding Test Tool Documentation \[2.1\]</span></span>
4.  <span data-ttu-id="ae42d-602">繼續進行 [其他測試](#59-other-tests)</span><span class="sxs-lookup"><span data-stu-id="ae42d-602">Proceed to [Other Tests](#59-other-tests)</span></span>

### <a name="59-other-tests"></a><span data-ttu-id="ae42d-603">5.9 其他測試</span><span class="sxs-lookup"><span data-stu-id="ae42d-603">5.9 Other Tests</span></span>

1.  <span data-ttu-id="ae42d-604">確認遊戲可執行檔包含數位簽章 \[ 2。3\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-604">Verify that the game executable files contain a digital signature \[2.3\]</span></span>
2.  <span data-ttu-id="ae42d-605">確認遊戲安裝程式在 Windows Vista 及/或 Windows 7 2.3 的64位版本上正常執行 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-605">Verify that the game installation process runs normally on 64-bit editions of Windows Vista and/or Windows 7 \[2.3\]</span></span>
3.  <span data-ttu-id="ae42d-606">在 Windows Vista 及/或 Windows 7 2.3 的64位版本上執行16位可執行檔時，請確認該遊戲未遇到錯誤 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-606">Verify that the game does not encounter an error as a result of 16-bit executables on 64-bit editions of Windows Vista and/or Windows 7 \[2.3\]</span></span>
4.  <span data-ttu-id="ae42d-607">在測試時強制應用程式損毀，並確認遊戲顯示 Windows 錯誤報告正確，並收集損毀資料 \[ 4。3\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-607">Force the application to crash while testing, and verify that the game displays Windows Error Reporting properly and collects crash data \[4.3\]</span></span>
5.  <span data-ttu-id="ae42d-608">確定正確的檔案摘要 \[ 4。3\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-608">Ensure proper file summaries \[4.3\]</span></span>

    1.  <span data-ttu-id="ae42d-609">按一下 [開始-> 電腦]</span><span class="sxs-lookup"><span data-stu-id="ae42d-609">Click Start -> Computer</span></span>
    2.  <span data-ttu-id="ae42d-610">流覽至遊戲目錄</span><span class="sxs-lookup"><span data-stu-id="ae42d-610">Navigate to the game directory</span></span>
    3.  <span data-ttu-id="ae42d-611">在 [搜尋] 視窗中，輸入 \* .dll</span><span class="sxs-lookup"><span data-stu-id="ae42d-611">In the search window, type \*.dll</span></span>
    4.  <span data-ttu-id="ae42d-612">針對每個檔案：以滑鼠右鍵按一下檔案，然後按一下 [屬性]</span><span class="sxs-lookup"><span data-stu-id="ae42d-612">For each file: Right-click the file and click Properties</span></span>

        -   <span data-ttu-id="ae42d-613">在 Windows XP 中：按一下 [版本] 索引標籤。確認已正確填入 [產品名稱]、[公司名稱] 和 [檔案版本] 欄位。</span><span class="sxs-lookup"><span data-stu-id="ae42d-613">In Windows XP: Click the Version tab. Verify that the Product Name, Company Name, and File Version fields are properly populated.</span></span> <span data-ttu-id="ae42d-614">\[4.3\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-614">\[4.3\]</span></span>
        -   <span data-ttu-id="ae42d-615">在 Windows Vista 和 Windows 7 中：按一下 [詳細資料] 索引標籤。確認 [產品名稱] 和 [檔案版本] 欄位已正確填入。</span><span class="sxs-lookup"><span data-stu-id="ae42d-615">In Windows Vista and Windows 7: Click the Details tab. Verify that the Product name and File Version fields are properly populated.</span></span> <span data-ttu-id="ae42d-616">在 Windows Vista 或 Windows 7 屬性頁面4.3 中看不到公司名稱 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-616">Company Name is not visible in the Windows Vista or Windows 7 properties page \[4.3\]</span></span>

    5.  <span data-ttu-id="ae42d-617">針對 .exe 檔案重複此檢查</span><span class="sxs-lookup"><span data-stu-id="ae42d-617">Repeat this check for .exe files</span></span>

6.  <span data-ttu-id="ae42d-618">啟動遊戲。</span><span class="sxs-lookup"><span data-stu-id="ae42d-618">Launch the game.</span></span>

    1.  <span data-ttu-id="ae42d-619">按 CTRL + ALT + DEL</span><span class="sxs-lookup"><span data-stu-id="ae42d-619">Press CTRL+ALT+DEL</span></span>
    2.  <span data-ttu-id="ae42d-620">按一下 [關機選項] 箭號</span><span class="sxs-lookup"><span data-stu-id="ae42d-620">Click the "Shutdown Options" arrow</span></span>
    3.  <span data-ttu-id="ae42d-621">按一下 [重新開機]</span><span class="sxs-lookup"><span data-stu-id="ae42d-621">Click Restart</span></span>
    4.  <span data-ttu-id="ae42d-622">驗證遊戲不會封鎖關機 \[ 3。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-622">Verify game does not block shutdown \[3.1\]</span></span>

7.  <span data-ttu-id="ae42d-623">繼續 [卸載](#510-uninstall)</span><span class="sxs-lookup"><span data-stu-id="ae42d-623">Proceed to [Uninstall](#510-uninstall)</span></span>

### <a name="510-uninstall"></a><span data-ttu-id="ae42d-624">5.10 卸載</span><span class="sxs-lookup"><span data-stu-id="ae42d-624">5.10 Uninstall</span></span>

-   <span data-ttu-id="ae42d-625">確認遊戲卸載程式會移除所有已安裝、未轉散發的作業系統元件檔案，並清除所有設定 \[ 3。1\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-625">Verify that the game uninstallation process removes all installed, non-redistributed operating system component files and clears all settings \[3.1\]</span></span>

    -   <span data-ttu-id="ae42d-626">在 Windows Vista 或 Windows 7 中確認主控台是移除程式1.1 的唯一方法 \[\]</span><span class="sxs-lookup"><span data-stu-id="ae42d-626">Verify in Windows Vista or Windows 7 that Control Panel is the only way to remove the program \[1.1\]</span></span>

## <a name="test-tool-notes"></a><span data-ttu-id="ae42d-627">測試控管注意事項</span><span class="sxs-lookup"><span data-stu-id="ae42d-627">Test Tool Notes</span></span>

<span data-ttu-id="ae42d-628">這些是上述測試需求中所列的每個測試控管的附注。</span><span class="sxs-lookup"><span data-stu-id="ae42d-628">These are notes for each of the test tools listed in the above test requirements.</span></span>

### <a name="61-appverifier-40-or-higher"></a><span data-ttu-id="ae42d-629">6.1 Appverifier 4.0 (或更新版本) </span><span class="sxs-lookup"><span data-stu-id="ae42d-629">6.1 Appverifier 4.0 (or higher)</span></span>

<span data-ttu-id="ae42d-630">**測試案例：2.5、4。2**</span><span class="sxs-lookup"><span data-stu-id="ae42d-630">**Test Case: 2.5, 4.2**</span></span>

> [!Note]  
> <span data-ttu-id="ae42d-631">有些應用程式無法執行，因為禁止複製的 AppVerifier 正在執行中。</span><span class="sxs-lookup"><span data-stu-id="ae42d-631">Some applications fail to run with AppVerifier running, because of copy protection.</span></span> <span data-ttu-id="ae42d-632">您可以使用遊戲可執行檔的未受保護髮行版來執行，以解決此問題。</span><span class="sxs-lookup"><span data-stu-id="ae42d-632">This can be resolved by running with an unprotected release version of the game executable.</span></span>

 

1.  <span data-ttu-id="ae42d-633">在執行 Windows XP 的電腦上安裝 AppVerifier 4.0 (或更高版本) </span><span class="sxs-lookup"><span data-stu-id="ae42d-633">Install AppVerifier 4.0 (or higher) on a computer running Windows XP</span></span>
2.  <span data-ttu-id="ae42d-634">啟動 AppVerifier，然後按一下 [檔案-> 新增應用程式]</span><span class="sxs-lookup"><span data-stu-id="ae42d-634">Launch AppVerifier and click File -> Add Application</span></span>
3.  <span data-ttu-id="ae42d-635">找出遊戲可執行檔、選取它，然後按一下 [開啟]。</span><span class="sxs-lookup"><span data-stu-id="ae42d-635">Locate the game executable, select it and click Open</span></span>
4.  <span data-ttu-id="ae42d-636">在 [應用程式] 區段中，選取 [遊戲可執行檔]</span><span class="sxs-lookup"><span data-stu-id="ae42d-636">In the "Applications" section, select the game executable</span></span>
5.  <span data-ttu-id="ae42d-637">在 [基本概念] 區段中，選取下列測試：</span><span class="sxs-lookup"><span data-stu-id="ae42d-637">Select the following tests in the "Basics" section:</span></span>

    -   <span data-ttu-id="ae42d-638">處理</span><span class="sxs-lookup"><span data-stu-id="ae42d-638">Handles</span></span>
    -   <span data-ttu-id="ae42d-639">堆積</span><span class="sxs-lookup"><span data-stu-id="ae42d-639">Heaps</span></span>
    -   <span data-ttu-id="ae42d-640">鎖定</span><span class="sxs-lookup"><span data-stu-id="ae42d-640">Locks</span></span>
    -   <span data-ttu-id="ae42d-641">記憶體</span><span class="sxs-lookup"><span data-stu-id="ae42d-641">Memory</span></span>
    -   <span data-ttu-id="ae42d-642">TLS</span><span class="sxs-lookup"><span data-stu-id="ae42d-642">TLS</span></span>

6.  <span data-ttu-id="ae42d-643">在 [其他] 區段中，選取下列測試：</span><span class="sxs-lookup"><span data-stu-id="ae42d-643">Select the following tests in the "Miscellaneous" section:</span></span>

    -   <span data-ttu-id="ae42d-644">DangerousAPIs</span><span class="sxs-lookup"><span data-stu-id="ae42d-644">DangerousAPIs</span></span>
    -   <span data-ttu-id="ae42d-645">DirtyStacks</span><span class="sxs-lookup"><span data-stu-id="ae42d-645">DirtyStacks</span></span>

7.  <span data-ttu-id="ae42d-646">確定未選取所有其他測試</span><span class="sxs-lookup"><span data-stu-id="ae42d-646">Ensure all other tests are not selected</span></span>
8.  <span data-ttu-id="ae42d-647">啟動遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-647">Launch the game</span></span>
9.  <span data-ttu-id="ae42d-648">玩遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-648">Play the game</span></span>
10. <span data-ttu-id="ae42d-649">關閉遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-649">Close the game</span></span>
11. <span data-ttu-id="ae42d-650">在 AppVerifier 中選取 View-> 記錄</span><span class="sxs-lookup"><span data-stu-id="ae42d-650">In AppVerifier select View -> Logs</span></span>
12. <span data-ttu-id="ae42d-651">在 [應用程式] 區段中，選取應用程式 .exe 檔案</span><span class="sxs-lookup"><span data-stu-id="ae42d-651">In the "Applications" section select the app .exe file</span></span>
13. <span data-ttu-id="ae42d-652">在 [記錄檔] 區段中，選取記錄檔並觀察錯誤計數。</span><span class="sxs-lookup"><span data-stu-id="ae42d-652">In the "Logs" section, select the log file and observe the error count.</span></span> <span data-ttu-id="ae42d-653">如果沒有任何錯誤，請結束 AppVerifier 測試。</span><span class="sxs-lookup"><span data-stu-id="ae42d-653">If there are no errors, then end AppVerifier tests.</span></span> <span data-ttu-id="ae42d-654">如果發生錯誤，請按一下 [流覽] 按鈕</span><span class="sxs-lookup"><span data-stu-id="ae42d-654">If there are errors, click the View button</span></span>
14. <span data-ttu-id="ae42d-655">搜尋檔 (CTRL + F) 嚴重性 = "錯誤</span><span class="sxs-lookup"><span data-stu-id="ae42d-655">Search the document (CTRL+F) for Severity="Error</span></span>
15. <span data-ttu-id="ae42d-656">根據失敗的 LayerName = 部分建立 bug</span><span class="sxs-lookup"><span data-stu-id="ae42d-656">Create bugs based on the LayerName= portion of the failure</span></span>

### <a name="62-manifest-test---mtexe"></a><span data-ttu-id="ae42d-657">6.2 資訊清單測試-mt.exe</span><span class="sxs-lookup"><span data-stu-id="ae42d-657">6.2 Manifest Test - mt.exe</span></span>

<span data-ttu-id="ae42d-658">**測試案例：1.8、2。1**</span><span class="sxs-lookup"><span data-stu-id="ae42d-658">**Test Case: 1.8, 2.1**</span></span>

<span data-ttu-id="ae42d-659">此工具是從 MT.exe 所在的命令提示字元執行。</span><span class="sxs-lookup"><span data-stu-id="ae42d-659">This tool is run from a command prompt where MT.exe is located.</span></span>

<span data-ttu-id="ae42d-660">範例：</span><span class="sxs-lookup"><span data-stu-id="ae42d-660">Example:</span></span>

``` syntax
mt.exe -inputresource:"c:\yourdir\YourGame.exe";#1 -out:yourgame.manifest
```

1.  <span data-ttu-id="ae42d-661">按一下 [開始-> 執行-> 輸入 cmd，然後按一下 [確定] 按鈕</span><span class="sxs-lookup"><span data-stu-id="ae42d-661">Click Start -> Run -> type cmd and click the OK button</span></span>
2.  <span data-ttu-id="ae42d-662">執行 mt.exe 工具，為隨遊戲安裝的每個 .exe 檔案產生資訊清單檔。</span><span class="sxs-lookup"><span data-stu-id="ae42d-662">Run the mt.exe tool to generate a .manifest file for each .exe file that installs with the game</span></span>
3.  <span data-ttu-id="ae42d-663">開啟產生的資訊清單檔案</span><span class="sxs-lookup"><span data-stu-id="ae42d-663">Open the generated .manifest file</span></span>
4.  <span data-ttu-id="ae42d-664">確定每個 .exe 檔案都包含下列 (要求：</span><span class="sxs-lookup"><span data-stu-id="ae42d-664">Ensure that each .exe file contains the following (requested:</span></span>
...
> [!Note]  
> <span data-ttu-id="ae42d-665">每個檔案都應該要有要求的執行層級，而且至少要有遊戲的可執行檔的 DPIAware。</span><span class="sxs-lookup"><span data-stu-id="ae42d-665">Requested execution level should be present for every file, and dpiAware should be present for at least the game s executable file.</span></span>

 

### <a name="63-thread-hijacker---threadhijackerexe"></a><span data-ttu-id="ae42d-666">6.3 執行緒劫匪-threadhijacker.exe</span><span class="sxs-lookup"><span data-stu-id="ae42d-666">6.3 Thread Hijacker - threadhijacker.exe</span></span>

<span data-ttu-id="ae42d-667">此工具是從 threadhijacker.exe 所在的命令提示字元執行。</span><span class="sxs-lookup"><span data-stu-id="ae42d-667">This tool is run from a command prompt where threadhijacker.exe is located.</span></span>

<span data-ttu-id="ae42d-668">範例：</span><span class="sxs-lookup"><span data-stu-id="ae42d-668">Example:</span></span>

``` syntax
threadhijacker.exe /process:str
```

<span data-ttu-id="ae42d-669">其中 str 是program.exe 的 \_ 名稱 \_</span><span class="sxs-lookup"><span data-stu-id="ae42d-669">Where str is the name\_of\_program.exe</span></span>

1.  <span data-ttu-id="ae42d-670">顯示工作管理員，按一下 [處理常式] 索引標籤，然後找出遊戲可執行檔的名稱。</span><span class="sxs-lookup"><span data-stu-id="ae42d-670">Bring up Task Manager, click the Processes tab, and locate the name of the game executable.</span></span>
2.  <span data-ttu-id="ae42d-671">以系統管理員模式開啟命令提示字元</span><span class="sxs-lookup"><span data-stu-id="ae42d-671">Open a command prompt in Admin mode</span></span>
3.  <span data-ttu-id="ae42d-672">流覽至 threadhijacker.exe 所在的目錄</span><span class="sxs-lookup"><span data-stu-id="ae42d-672">Navigate to the directory where threadhijacker.exe is</span></span>
4.  <span data-ttu-id="ae42d-673">Type： **threadhijacker.exe/process：** str，其中 str 是您想要叫用之可執行檔的名稱</span><span class="sxs-lookup"><span data-stu-id="ae42d-673">Type: **threadhijacker.exe /process:** str, where str is the name of the executable that you want to hit</span></span>

### <a name="64-microsoft-games-for-windows-test-tool"></a><span data-ttu-id="ae42d-674">6.4 Windows 測試控管的 Microsoft 遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-674">6.4 Microsoft Games for Windows Test Tool</span></span>

<span data-ttu-id="ae42d-675">此工具位於 DirectX SDK 中。</span><span class="sxs-lookup"><span data-stu-id="ae42d-675">This tool is located in the DirectX SDK.</span></span> <span data-ttu-id="ae42d-676">一旦 SDK 安裝在電腦上，就可以將 Windows 測試控管的安裝程式放置在測試電腦上，然後安裝。</span><span class="sxs-lookup"><span data-stu-id="ae42d-676">Once the SDK is installed on a computer, the installer for the Games for Windows Test Tool can be placed on the test computer and installed.</span></span>

<span data-ttu-id="ae42d-677">在安裝 DirectX SDK 的開發電腦上，找出適用于 Windows Test Tool 安裝程式的 Microsoft 遊戲。</span><span class="sxs-lookup"><span data-stu-id="ae42d-677">Locate the Microsoft Games for Windows Test Tool installer on the development computer where the DirectX SDK is installed.</span></span> <span data-ttu-id="ae42d-678">依預設，它位於下列位置：</span><span class="sxs-lookup"><span data-stu-id="ae42d-678">By default, it is placed in the following location:</span></span>

``` syntax
%SystemDrive%\Program Files (x86)\Microsoft DirectX SDK (Date)\Utilities\bin\x86\Microsoft Games for Windows Test Tools\
```

1.  <span data-ttu-id="ae42d-679">將安裝程式 (MicrosoftGFWTestTool.msi/setup.exe) 複製到測試電腦。</span><span class="sxs-lookup"><span data-stu-id="ae42d-679">Copy the installer (MicrosoftGFWTestTool.msi / setup.exe) to the test computer.</span></span>
2.  <span data-ttu-id="ae42d-680">執行安裝程式。</span><span class="sxs-lookup"><span data-stu-id="ae42d-680">Run the installer.</span></span>
3.  <span data-ttu-id="ae42d-681">啟動 Windows 測試控管的 Microsoft 遊戲。</span><span class="sxs-lookup"><span data-stu-id="ae42d-681">Launch the Microsoft Games for Windows Test Tool.</span></span>
4.  <span data-ttu-id="ae42d-682">在 [ **專案清單** ] 欄位中，將 [ **建立新專案** ] 取代為您的標題名稱， **然後按一下 [新建]**。</span><span class="sxs-lookup"><span data-stu-id="ae42d-682">In the **Project List** field replace **Create New Project** with your title name and click **Create New**.</span></span>

    <span data-ttu-id="ae42d-683">等候基準完成。</span><span class="sxs-lookup"><span data-stu-id="ae42d-683">Wait for the Baseline to complete.</span></span>

5.  <span data-ttu-id="ae42d-684">在 [ **遊戲資訊** ] 區段中填寫任何您可能擁有的資訊，然後按一下 [ **更新遊戲資訊**]。</span><span class="sxs-lookup"><span data-stu-id="ae42d-684">Fill in any information that you may have in the **Game Information** section, and click **Update Game Information**.</span></span>
6.  <span data-ttu-id="ae42d-685">按一下 [ **測試案例** ] 索引標籤。</span><span class="sxs-lookup"><span data-stu-id="ae42d-685">Click on **Test Cases** tab.</span></span>
7.  <span data-ttu-id="ae42d-686">從頂端開始，繼續進行測試案例，適當地按一下 [ **通過** ] 或 [ **失敗** ]。</span><span class="sxs-lookup"><span data-stu-id="ae42d-686">Starting at the top, proceed through the test cases, clicking **Pass** or **Fail** as appropriate.</span></span>

    <span data-ttu-id="ae42d-687">如需有關在報表中包含錯誤的詳細資訊，請參閱本節稍後的「撰寫 Bug」。</span><span class="sxs-lookup"><span data-stu-id="ae42d-687">See "Writing a Bug", later in this section, for details on including a bug in the report.</span></span>

8.  <span data-ttu-id="ae42d-688">核取 [**報表**] 和 [ **Bug 編輯**] 索引標籤，以在查看報表 (之後返回 [**專案**] 索引標籤) 。</span><span class="sxs-lookup"><span data-stu-id="ae42d-688">Return to the **Projects** tab after reviewing the report (by checking the **Report** and **Bug Edit** tabs).</span></span>
9.  <span data-ttu-id="ae42d-689">按一下 [ **編譯報告**]。</span><span class="sxs-lookup"><span data-stu-id="ae42d-689">Click **Compile Report**.</span></span>

    <span data-ttu-id="ae42d-690">當報表完成編譯時，就會開啟一個視窗。</span><span class="sxs-lookup"><span data-stu-id="ae42d-690">A window will open when the report is finished compiling.</span></span> <span data-ttu-id="ae42d-691">您將在這裡找到。ZIP 檔案名 *專案* 名稱 \_report.zip。</span><span class="sxs-lookup"><span data-stu-id="ae42d-691">Here you will find a .ZIP file names *ProjectName*\_report.zip.</span></span> <span data-ttu-id="ae42d-692">此檔案包含在測試階段期間收集的所有記錄和結果。</span><span class="sxs-lookup"><span data-stu-id="ae42d-692">This file contains all of the logs and results collected during the test pass.</span></span>

### <a name="writing-a-bug"></a><span data-ttu-id="ae42d-693">撰寫 Bug</span><span class="sxs-lookup"><span data-stu-id="ae42d-693">Writing a Bug</span></span>

<span data-ttu-id="ae42d-694">有兩種方式可以撰寫 bug 報告：您可以完成測試案例，然後按一下 [當標題無法測試案例時 **失敗** ]，或者按一下 [ **bug 編輯** ] 索引標籤並手動加入 bug 報表。</span><span class="sxs-lookup"><span data-stu-id="ae42d-694">There are two ways to write a bug report: you can go through the test cases and click **Fail** when the title fails a test case, or you can click the **Bug Edit** tab and manually add a bug report.</span></span>

### <a name="clicking-fail-on-a-test-case"></a><span data-ttu-id="ae42d-695">在測試案例上按一下 [失敗]</span><span class="sxs-lookup"><span data-stu-id="ae42d-695">Clicking Fail on a test case</span></span>

1.  <span data-ttu-id="ae42d-696">當您按一下測試案例的 [ **失敗** ] 時，[ **問題類型** ] 下拉式清單會自動設定為測試案例類型。</span><span class="sxs-lookup"><span data-stu-id="ae42d-696">When you click **Fail** on a test case, the **Issue Type** drop-down list will automatically be set to the test case type.</span></span>
2.  <span data-ttu-id="ae42d-697">將簡短描述新增至簡要描述問題的 [ **標題** ] 欄位。</span><span class="sxs-lookup"><span data-stu-id="ae42d-697">Add a short description to the **Title** field that briefly describes the issue.</span></span>
3.  <span data-ttu-id="ae42d-698">將問題的詳細描述新增至 [觀察到的 **行為** ] 欄位。</span><span class="sxs-lookup"><span data-stu-id="ae42d-698">Add a detailed description of the issue to the **Observed Behavior** field.</span></span>
4.  <span data-ttu-id="ae42d-699">視需要新增預期的 (，而不是 [ **預期行為** ] 欄位) 問題的描述。</span><span class="sxs-lookup"><span data-stu-id="ae42d-699">As appropriate, add what was expected (as opposed to a description of the issue) to the **Expected Behavior** field.</span></span>
5.  <span data-ttu-id="ae42d-700">新增如何重現問題至 **重現步驟** 欄位的詳細描述。</span><span class="sxs-lookup"><span data-stu-id="ae42d-700">Add a detailed description of how to reproduce the issue to the **Repro-Steps** field.</span></span>
6.  <span data-ttu-id="ae42d-701">完成時，按一下 [ **儲存** ] 按鈕。</span><span class="sxs-lookup"><span data-stu-id="ae42d-701">When done, click the **Save** button.</span></span>

### <a name="manually-adding-a-bug"></a><span data-ttu-id="ae42d-702">手動新增 Bug</span><span class="sxs-lookup"><span data-stu-id="ae42d-702">Manually Adding a Bug</span></span>

<span data-ttu-id="ae42d-703">此程式與按一下 [ **失敗**] 相同，但自動填入的下拉式清單除外。</span><span class="sxs-lookup"><span data-stu-id="ae42d-703">This process is the same as clicking **Fail**, with the exception of the auto-populated drop-down list.</span></span> <span data-ttu-id="ae42d-704">在此情況下，請選取適當的 TCR 失敗類型，或針對落在 TR 範圍之外但仍應回報的 bug 選取 **\* \* 非 TR 問題 \* \*** 。</span><span class="sxs-lookup"><span data-stu-id="ae42d-704">In this case, either select the appropriate TCR failure type or select **\*\* Non-TR Issue \*\*** for bugs that fall outside of the TR range but should still be reported.</span></span>

## <a name="resources"></a><span data-ttu-id="ae42d-705">資源</span><span class="sxs-lookup"><span data-stu-id="ae42d-705">Resources</span></span>

<dl> <dt>

<span data-ttu-id="ae42d-706"><span id="Games_for_Windows__Technical_Requirements"></span><span id="games_for_windows__technical_requirements"></span><span id="GAMES_FOR_WINDOWS__TECHNICAL_REQUIREMENTS"></span>適用于 Windows 的遊戲：技術需求</span><span class="sxs-lookup"><span data-stu-id="ae42d-706"><span id="Games_for_Windows__Technical_Requirements"></span><span id="games_for_windows__technical_requirements"></span><span id="GAMES_FOR_WINDOWS__TECHNICAL_REQUIREMENTS"></span>Games for Windows: Technical Requirements</span></span>
</dt> <dd>

[<span data-ttu-id="ae42d-707">Windows 技術需求的遊戲： Windows XP、Windows Vista 和 Windows 7 遊戲的最佳作法</span><span class="sxs-lookup"><span data-stu-id="ae42d-707">Games for Windows Technical Requirements: Best Practices for Games on Windows XP, Windows Vista, and Windows 7</span></span>](./games-for-windows-technical-requirements-1-1-0006.md)

</dd> <dt>

<span data-ttu-id="ae42d-708"><span id="Windows_SDK"></span><span id="windows_sdk"></span><span id="WINDOWS_SDK"></span>Windows SDK</span><span class="sxs-lookup"><span data-stu-id="ae42d-708"><span id="Windows_SDK"></span><span id="windows_sdk"></span><span id="WINDOWS_SDK"></span>Windows SDK</span></span>
</dt> <dd>

[<span data-ttu-id="ae42d-709">Windows Sdk</span><span class="sxs-lookup"><span data-stu-id="ae42d-709">Windows SDKs</span></span>](https://msdn.microsoft.com/bb980924.aspx)

</dd> <dt>

<span data-ttu-id="ae42d-710"><span id="User_Account_Control_Guidelines"></span><span id="user_account_control_guidelines"></span><span id="USER_ACCOUNT_CONTROL_GUIDELINES"></span>使用者帳戶控制指導方針</span><span class="sxs-lookup"><span data-stu-id="ae42d-710"><span id="User_Account_Control_Guidelines"></span><span id="user_account_control_guidelines"></span><span id="USER_ACCOUNT_CONTROL_GUIDELINES"></span>User Account Control Guidelines</span></span>
</dt> <dd>

<span data-ttu-id="ae42d-711">[使用者帳戶控制相容性的 Windows Vista 應用程式開發需求](/previous-versions/dotnet/articles/bb530410(v=msdn.10))</span><span class="sxs-lookup"><span data-stu-id="ae42d-711">[Windows Vista Application Development Requirements for User Account Control Compatibility](/previous-versions/dotnet/articles/bb530410(v=msdn.10))</span></span>

</dd> <dt>

<span data-ttu-id="ae42d-712"><span id="Windows_Installer_Information"></span><span id="windows_installer_information"></span><span id="WINDOWS_INSTALLER_INFORMATION"></span>Windows Installer 資訊</span><span class="sxs-lookup"><span data-stu-id="ae42d-712"><span id="Windows_Installer_Information"></span><span id="windows_installer_information"></span><span id="WINDOWS_INSTALLER_INFORMATION"></span>Windows Installer Information</span></span>
</dt> <dd>

[<span data-ttu-id="ae42d-713">Windows Installer</span><span class="sxs-lookup"><span data-stu-id="ae42d-713">Windows Installer</span></span>](../msi/windows-installer-portal.md)

</dd> <dt>

<span data-ttu-id="ae42d-714"><span id="WinQual_Developer_Portal__"></span><span id="winqual_developer_portal__"></span><span id="WINQUAL_DEVELOPER_PORTAL__"></span>WinQual 開發人員入口網站</span><span class="sxs-lookup"><span data-stu-id="ae42d-714"><span id="WinQual_Developer_Portal__"></span><span id="winqual_developer_portal__"></span><span id="WINQUAL_DEVELOPER_PORTAL__"></span>WinQual Developer Portal</span></span> 
</dt> <dd>

[<span data-ttu-id="ae42d-715">Windows Quality Online Services (Winqual) </span><span class="sxs-lookup"><span data-stu-id="ae42d-715">Windows Quality Online Services (Winqual)</span></span>](/windows-hardware/drivers/dashboard/winqual-submission-tool--winqualexe-)

</dd> <dt>

<span data-ttu-id="ae42d-716"><span id="DirectX_Developer_Portal"></span><span id="directx_developer_portal"></span><span id="DIRECTX_DEVELOPER_PORTAL"></span>DirectX 開發人員入口網站</span><span class="sxs-lookup"><span data-stu-id="ae42d-716"><span id="DirectX_Developer_Portal"></span><span id="directx_developer_portal"></span><span id="DIRECTX_DEVELOPER_PORTAL"></span>DirectX Developer Portal</span></span>
</dt> <dd>

<span data-ttu-id="ae42d-717">[DirectX 開發人員中心](/previous-versions/windows/apps/hh452744(v=win.10))</span><span class="sxs-lookup"><span data-stu-id="ae42d-717">[DirectX Developer Center](/previous-versions/windows/apps/hh452744(v=win.10))</span></span>

</dd> <dt>

<span data-ttu-id="ae42d-718"><span id="Games_for_Windows_and_DirectX_SDK_Blog"></span><span id="games_for_windows_and_directx_sdk_blog"></span><span id="GAMES_FOR_WINDOWS_AND_DIRECTX_SDK_BLOG"></span>Windows 和 DirectX SDK Blog 的遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-718"><span id="Games_for_Windows_and_DirectX_SDK_Blog"></span><span id="games_for_windows_and_directx_sdk_blog"></span><span id="GAMES_FOR_WINDOWS_AND_DIRECTX_SDK_BLOG"></span>Games for Windows and DirectX SDK Blog</span></span>
</dt> <dd>

[<span data-ttu-id="ae42d-719">適用於 Windows 與 DirectX SDK 的遊戲</span><span class="sxs-lookup"><span data-stu-id="ae42d-719">Games for Windows and the DirectX SDK</span></span>](https://walbourn.github.io/)

</dd> <dt>

<span data-ttu-id="ae42d-720"><span id="Additional_DirectX_Articles"></span><span id="additional_directx_articles"></span><span id="ADDITIONAL_DIRECTX_ARTICLES"></span>其他 DirectX 文章</span><span class="sxs-lookup"><span data-stu-id="ae42d-720"><span id="Additional_DirectX_Articles"></span><span id="additional_directx_articles"></span><span id="ADDITIONAL_DIRECTX_ARTICLES"></span>Additional DirectX Articles</span></span>
</dt> <dd>

[<span data-ttu-id="ae42d-721">DirectX 技術文章</span><span class="sxs-lookup"><span data-stu-id="ae42d-721">DirectX Technical Articles</span></span>](./dx9-technical-articles.md)
