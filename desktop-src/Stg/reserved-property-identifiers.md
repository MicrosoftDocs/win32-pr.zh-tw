...
| 1                    | 保留為字碼頁的指標 (Windows) 或腳本 (Macintosh) 在解讀屬性集中的字串時使用。<br/> 屬性集中的所有字串值都必須以相同的字碼頁儲存。 屬性集標頭中的原始作業系統值 (PROPERTYSETHEADER：:d wOSVer) 決定字碼頁指標是否對應至 Windows 字碼頁或 Macintosh 腳本。<br/>                                                                                                                                                        |
