...
---
# <a name="cpapfile-class-and-methods"></a>CPapFile 類別和方法

**StoClien** 會將它的複合檔案作業封裝在 CPapFile c + + 物件中。

以下是來自 Papfile 的 **CPapFile** 類別宣告。
...
CPapFile 物件會在成員 **m \_ szCurFileName** 中保留目前的檔案名。 當 [**Load**](load-method---cpapfile.md) 和 [**Save**](save-method---cpapfile.md) 方法未明確接收檔案名時，會使用這個檔案名作為預設值。

成員 **m \_ pIPaper** 會保留 COPaper [**IPaper**](ipaper-methods.md) 介面的介面指標。 成員 **m \_ pIStorage** 會針對 **StoClien** 用於結構化儲存的目前複合檔案，保留 [**IStorage**](/windows/desktop/api/Objidl/nn-objidl-istorage)介面的指標。
