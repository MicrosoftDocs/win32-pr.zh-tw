---
description: Microsoft Windows 安全性模型可讓您控制處理物件的存取權。 如需安全性的詳細資訊，請參閱 Access-Control 模型。
...
ms.locfileid: "119418708"
---
# <a name="process-security-and-access-rights"></a>流程安全性和存取權限

Microsoft Windows 安全性模型可讓您控制處理物件的存取權。 如需安全性的詳細資訊，請參閱 [存取控制模型](../secauthz/access-control-model.md)。
...
| 值                                             | 意義                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
|---------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **處理 \_ 所有 \_ 存取**                          | 處理常式物件的所有可能存取權限。**Windows Server 2003 和 Windows XP：** Windows Server 2008 和 Windows Vista 增加的 **進程 \_ 所有 \_ 存取** 旗標大小。 如果針對 Windows server 2008 和 Windows Vista 編譯的應用程式在 Windows Server 2003 或 Windows XP 上執行，則「**處理 \_ 所有 \_ 存取**」旗標會太大，而指定此旗標的函式會失敗，並 **\_ \_ 拒絕錯誤存取**。 若要避免這個問題，請指定作業所需的最小存取權限集合。 如果必須使用 [ **處理 \_ 所有 \_ 存取** ]，請將 \_ WIN32 \_ WINNT 設定為您的應用程式設為目標的最小作業系統 (例如 `#define _WIN32_WINNT _WIN32_WINNT_WINXP`) 。 如需詳細資訊，請參閱[使用 Windows 標頭](../winprog/using-the-windows-headers.md)。 <br/> |
...
| **進程 \_設定 \_ 配額** (0x0100)                   | 使用 [**SetProcessWorkingSetSize**](/windows/desktop/api/memoryapi/nf-memoryapi-setprocessworkingsetsize)設定記憶體限制所需。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
WindowsVista 引進 *受保護* 的程式以增強對數位 Rights Management 的支援。 系統會限制受保護進程的存取，以及受保護進程的執行緒。
