---
title: LicenseVersion2 屬性
description: 每個使用者 CAL 的第二部終端機伺服器版本。
ms.assetid: 9e4bc586-365a-4a46-a623-e686d305a159
ms.tgt_platform: multiple
keywords:
- LicenseVersion2 屬性 AD 架構
- msTSLicenseVersion2 屬性 AD 架構
topic_type:
- apiref
api_name:
- MS-TS-LicenseVersion2
api_type:
- Schema
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: e54437fc9de7e622b147abe7855fdbe658f0b453dbf8eb7ca9fde255171c5466
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "118960157"
---
# <a name="ms-ts-licenseversion2-attribute"></a>LicenseVersion2 屬性

每個使用者 CAL 的第二部終端機伺服器版本。



| 進入 | 值 |
|-------------------|---------------------------------------------|
| CN                | MS-TS-LicenseVersion2                       |
| Ldap-顯示名稱 | msTSLicenseVersion2                         |
| 大小              | \-                                          |
| 更新許可權  | \-                                          |
| 更新頻率  | \-                                          |
| Attribute-Id      | 1.2.840.113556.1.4.2001                     |
| 系統識別碼-Guid    | 4b0df103-8d97-45d9-ad69-85c3080ba4e7        |
| Syntax            | [**String(Unicode)**](s-string-unicode.md) |



## <a name="implementations"></a>實作

-   [**Windows Server 2008**](#windows-server-2008)
-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## <a name="windows-server-2008"></a>Windows Server 2008



| 進入 | 值 |
|------------------------|--------------|
| 連結識別碼                | \-           |
| MAPI-Id                | \-           |
| System-Only            | 否        |
| 是-單一值       | 是         |
| 已編制索引             | 是         |
| 在通用類別目錄中      | 否        |
| NT-Security-描述元 | O:BAG：不正確： S： |
| Range-Lower            | 0            |
| Range-Upper            | 255          |
| Search-Flags           | 0x00000001   |
| System-Flags           | 0x00000010   |
| 中使用的類別        | \-           |



## <a name="windows-server-2008-r2"></a>Windows Server 2008 R2



| 進入 | 值 |
|------------------------|--------------|
| 連結識別碼                | \-           |
| MAPI-Id                | \-           |
| System-Only            | 否        |
| 是-單一值       | 是         |
| 已編制索引             | 是         |
| 在通用類別目錄中      | 否        |
| NT-Security-描述元 | O:BAG：不正確： S： |
| Range-Lower            | 0            |
| Range-Upper            | 255          |
| Search-Flags           | 0x00000001   |
| System-Flags           | 0x00000010   |
| 中使用的類別        | \-           |



## <a name="windows-server-2012"></a>Windows Server 2012



| 進入 | 值 |
|------------------------|--------------|
| 連結識別碼                | \-           |
| MAPI-Id                | \-           |
| System-Only            | 否        |
| 是-單一值       | 是         |
| 已編制索引             | 是         |
| 在通用類別目錄中      | 否        |
| NT-Security-描述元 | O:BAG：不正確： S： |
| Range-Lower            | 0            |
| Range-Upper            | 255          |
| Search-Flags           | 0x00000001   |
| System-Flags           | 0x00000010   |
| 中使用的類別        | \-           |



 

 




