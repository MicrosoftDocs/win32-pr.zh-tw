---
title: DS-複寫-同步處理擴充許可權
description: 從指定 NC 同步處理複寫所需的擴充許可權。
ms.assetid: ea22cee4-9107-49f4-9755-9979ba5e5e04
ms.tgt_platform: multiple
keywords:
- DS-複寫-同步處理擴充的正確 AD 架構
topic_type:
- apiref
api_name:
- DS-Replication-Synchronize
api_type:
- Schema
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: ae1cebc1d3940f2d54f97b7b07ece3549125d3148c06e42eccc542cce3f80250
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119532018"
---
# <a name="ds-replication-synchronize-extended-right"></a>DS-複寫-同步處理擴充許可權

從指定 NC 同步處理複寫所需的擴充許可權。



| 進入 | 值 |
|--------------|--------------------------------------|
| CN           | DS 複寫-同步處理           |
| 顯示名稱 | 複寫同步處理          |
| 許可權-GUID  | 1131f6ab-9c07-11d1-f79f-00c04fc2dcd2 |



## <a name="implementations"></a>實作

-   [**Windows 2000 Server**](#windows-2000-server)
-   [**Windows Server 2003**](#windows-server-2003)
-   [**亞當**](#adam)
-   [**Windows Server 2003 R2**](#windows-server-2003-r2)
-   [**Windows Server 2008**](#windows-server-2008)
-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## <a name="windows-2000-server"></a>Windows 2000 Server



| 進入 | 值 |
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Applies-To              | [**網域 DNS**](c-domaindns.md)<br/> [**組態**](c-configuration.md)<br/> [**DMD**](c-dmd.md)<br/> |
| 當地語系化-顯示識別碼 | 26                                                                                                                               |



## <a name="windows-server-2003"></a>Windows Server 2003



| 進入 | 值 |
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Applies-To              | [**網域 DNS**](c-domaindns.md)<br/> [**組態**](c-configuration.md)<br/> [**DMD**](c-dmd.md)<br/> |
| 當地語系化-顯示識別碼 | 26                                                                                                                               |



## <a name="adam"></a>亞當



| 進入 | 值 |
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Applies-To              | [**網域 DNS**](c-domaindns.md)<br/> [**組態**](c-configuration.md)<br/> [**DMD**](c-dmd.md)<br/> |
| 當地語系化-顯示識別碼 | 26                                                                                                                               |



## <a name="windows-server-2003-r2"></a>Windows Server 2003 R2



| 進入 | 值 |
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Applies-To              | [**網域 DNS**](c-domaindns.md)<br/> [**組態**](c-configuration.md)<br/> [**DMD**](c-dmd.md)<br/> |
| 當地語系化-顯示識別碼 | 26                                                                                                                               |



## <a name="windows-server-2008"></a>Windows Server 2008



| 進入 | 值 |
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Applies-To              | [**網域 DNS**](c-domaindns.md)<br/> [**組態**](c-configuration.md)<br/> [**DMD**](c-dmd.md)<br/> |
| 當地語系化-顯示識別碼 | 26                                                                                                                               |



## <a name="windows-server-2008-r2"></a>Windows Server 2008 R2



| 進入 | 值 |
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Applies-To              | [**網域 DNS**](c-domaindns.md)<br/> [**組態**](c-configuration.md)<br/> [**DMD**](c-dmd.md)<br/> |
| 當地語系化-顯示識別碼 | 26                                                                                                                               |



## <a name="windows-server-2012"></a>Windows Server 2012



| 進入 | 值 |
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Applies-To              | [**網域 DNS**](c-domaindns.md)<br/> [**組態**](c-configuration.md)<br/> [**DMD**](c-dmd.md)<br/> |
| 當地語系化-顯示識別碼 | 26                                                                                                                               |



 

 





