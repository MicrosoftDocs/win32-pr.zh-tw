---
title: ms-DFS-架構-主要版本屬性
description: 包含 DFS 中繼資料架構的主要版本。
ms.assetid: 830aba30-6e76-4149-bd42-b0ea97cb2d61
ms.tgt_platform: multiple
keywords:
- ms-DFS-架構-主要版本屬性 AD 架構
- msDFS-SchemaMajorVersion 屬性 AD 架構
topic_type:
- apiref
api_name:
- ms-DFS-Schema-Major-Version
api_type:
- Schema
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: 26b6b9f6338f90b631cfdb2a50128d9edf8fa91c
ms.sourcegitcommit: b77ace27b0432e7cd3863191b11926be032fbe2f
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 12/14/2020
ms.locfileid: "104509720"
---
# <a name="ms-dfs-schema-major-version-attribute"></a>ms-DFS-架構-主要版本屬性

包含 DFS 中繼資料架構的主要版本。



| 進入 | 值 |
|-------------------|--------------------------------------|
| CN                | ms-DFS-Schema-Major-Version          |
| Ldap-顯示名稱 | msDFS-SchemaMajorVersion             |
| 大小              | \-                                   |
| 更新許可權  | \-                                   |
| 更新頻率  | \-                                   |
| Attribute-Id      | 1.2.840.113556.1.4.2030              |
| 系統識別碼-Guid    | ec6d7855-704a-4f61-9aa6-c49a7c1d54c7 |
| Syntax            | [**列舉型別**](s-enumeration.md) |



## <a name="implementations"></a>實作

-   [**Windows Server 2008**](#windows-server-2008)
-   [**Windows Server 2008 R2**](#windows-server-2008-r2)
-   [**Windows Server 2012**](#windows-server-2012)

## <a name="windows-server-2008"></a>Windows Server 2008



| 進入 | 值 |
|------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| 連結識別碼                | \-                                                                                                                                  |
| MAPI-Id                | \-                                                                                                                                  |
| System-Only            | 否                                                                                                                               |
| 是-單一值       | 對                                                                                                                                |
| 已編制索引             | 否                                                                                                                               |
| 在通用類別目錄中      | 否                                                                                                                               |
| NT-Security-描述元 | O:BAG：不正確： S：                                                                                                                        |
| Range-Lower            | 2                                                                                                                                   |
| Range-Upper            | 2                                                                                                                                   |
| Search-Flags           | 0x00000000                                                                                                                          |
| System-Flags           | 0x00000010                                                                                                                          |
| 中使用的類別        | [**ms-DFS-Namespace-Anchor**](c-msdfs-namespaceanchor.md)<br/> [**ms-DFS-Namespace-v2**](c-msdfs-namespacev2.md)<br/> |



## <a name="windows-server-2008-r2"></a>Windows Server 2008 R2



| 進入 | 值 |
|------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| 連結識別碼                | \-                                                                                                                                  |
| MAPI-Id                | \-                                                                                                                                  |
| System-Only            | 否                                                                                                                               |
| 是-單一值       | 對                                                                                                                                |
| 已編制索引             | 否                                                                                                                               |
| 在通用類別目錄中      | 否                                                                                                                               |
| NT-Security-描述元 | O:BAG：不正確： S：                                                                                                                        |
| Range-Lower            | 2                                                                                                                                   |
| Range-Upper            | 2                                                                                                                                   |
| Search-Flags           | 0x00000000                                                                                                                          |
| System-Flags           | 0x00000010                                                                                                                          |
| 中使用的類別        | [**ms-DFS-Namespace-Anchor**](c-msdfs-namespaceanchor.md)<br/> [**ms-DFS-Namespace-v2**](c-msdfs-namespacev2.md)<br/> |



## <a name="windows-server-2012"></a>Windows Server 2012



| 進入 | 值 |
|------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| 連結識別碼                | \-                                                                                                                                  |
| MAPI-Id                | \-                                                                                                                                  |
| System-Only            | 否                                                                                                                               |
| 是-單一值       | 對                                                                                                                                |
| 已編制索引             | 否                                                                                                                               |
| 在通用類別目錄中      | 否                                                                                                                               |
| NT-Security-描述元 | O:BAG：不正確： S：                                                                                                                        |
| Range-Lower            | 2                                                                                                                                   |
| Range-Upper            | 2                                                                                                                                   |
| Search-Flags           | 0x00000000                                                                                                                          |
| System-Flags           | 0x00000010                                                                                                                          |
| 中使用的類別        | [**ms-DFS-Namespace-Anchor**](c-msdfs-namespaceanchor.md)<br/> [**ms-DFS-Namespace-v2**](c-msdfs-namespacev2.md)<br/> |



 

 





