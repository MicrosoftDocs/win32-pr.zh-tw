...
ms.locfileid: "119879808"
---
# <a name="ms-pki-key-recovery-agent-class"></a>ms-PKI-Key Recovery-Agent 類別

針對每個已安裝的憑證伺服器，會為每個已安裝的憑證伺服器建立一個金鑰復原代理 (KRA) 物件實例， (在憑證伺服器安裝期間) 唯一的一般名稱 如果在 CA 設定期間，為兩個 Ca 指定相同的一般名稱，則會共用單一的 KRA 物件實例。
...
|-------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CN                | ms-PKI-金鑰復原-代理程式                                                                                                                                                    |
| Ldap-顯示名稱 | Mspki-certificate-name-flag-金鑰-復原-代理程式                                                                                                                                                     |
| 更新許可權  | 安裝 CA 的系統管理員必須能夠在 KRA 容器中建立一個 KRA 實例。 已安裝的憑證伺服器必須能夠更新 userCertificate 屬性。 |
| 更新頻率  | 幾個月內最多會新增幾個憑證。                                                                                                                   |
| 架構識別碼-Guid    | 26ccf238-a08e-4b86-9a82-a8c9ac7ee5cb                                                                                                                                         |
