...
ms.locfileid: "117809063"
---
# <a name="memory-protection"></a>記憶體保護

屬於進程的記憶體是由其私用虛擬位址空間隱含保護。 此外，Windows 使用虛擬記憶體硬體來提供記憶體保護。 這項保護的執行會因處理器而異，例如，進程位址空間中的字碼頁可以標示為唯讀，而不受使用者模式執行緒修改。
...
當載入相同 Windows 應用程式的多個實例時，每個實例都會在它自己的受保護虛擬位址空間中執行。 不過，其實例會處理 (*hInstance*) 通常具有相同的值。 此值代表應用程式在其虛擬位址空間中的基底位址。 如果每個實例都可以載入至其預設基底位址，則可以使用「寫入時複製」保護，將相同的實體頁面對應至其他實例，並與其他實例共用。 系統可讓這些實例共用相同的實體頁面，直到其中一個修改頁面為止。 如果基於某些原因而無法在所需的基底位址中載入其中一個實例，它會收到自己的實體頁面。
