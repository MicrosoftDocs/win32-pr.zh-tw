...
---
# <a name="creating-a-view-within-a-file"></a>在檔案中建立視圖

如果您想要查看不是從檔開頭開始的部分檔案，您必須建立檔案對應物件。 此物件是您想要查看之檔案的部分大小以及檔案中的位移。 例如，如果您想要查看 1 kb)  (1 kb 開始131072個位元組 (128K) 到檔案中，您必須建立至少132096個位元組大小的檔案對應物件 (129K) 的大小。 View 會開始131072個位元組 (128K) 至檔案，並至少延長1024個位元組。 此範例假設檔案配置細微性為64K。

檔案配置細微性會影響地圖視圖可以啟動的位置。 地圖視圖的開頭必須是檔案配置細微性的多個檔案的位移。 因此，您想要查看的資料可能是將配置細微性模數分割至視野的檔案位移。 視圖的大小是資料模數與配置細微性的位移，加上您想要檢查的資料大小。

例如，假設 [**GetSystemInfo**](/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsysteminfo) 函式指出了64k 的配置細微性。 若要檢查138240個位元組的 1 kb 資料 (135K) 入檔案中，請執行下列動作：

1.  建立至少139264個位元組的檔案對應物件 (136K) 的大小。
2.  建立從檔案位移開始的檔案，此檔案會在檔案配置細微性的最大倍數小於所需的位移。 在此情況下，檔案視圖會從位移131072開始 (128K) 至檔案中。 此視圖為139264個位元組 (136K) 減去131072位元組 (128K) ，或8192位元組 (8K) 的大小。
3.  建立7K 至視圖的指標位移，以存取您感興趣的1千。

如果您想要跨越檔案配置細微性界限的資料，您可以讓視圖大於檔案配置細微性。 這可避免將資料細分成片段。
