...
)。 您可以使用 [**Windows。UI：： Xaml：： Controls：： SwapChainPanel**](/uwp/api/Windows.UI.Xaml.Controls.SwapChainPanel?view=winrt-19041)類別，用來建立具有 XAML UI 覆迭的高效能 DirectX 應用程式。 如需在 Windows 應用程式中結合 XAML 和 DirectX 的詳細資訊，請參閱[DirectX 和 XAML interop](/previous-versions/windows/apps/hh825871(v=win.10))。
