...
您可以將子專案放 <formulas> 在或中， `<shape>` `<shapetype>` 以定義可以改變圖形路徑的公式。 在 `<formulas>` 子項目內，一個 **f** 子項目會定義一個公式，因此會根據該公式來評估一個值。 例如，公式 `<v:f eqn="prod 10 4 5"/>` 會定義相當於 "10 x 4/5" 的值。
