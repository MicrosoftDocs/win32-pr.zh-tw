...
---
# <a name="process2dquadtessfactorsavg-function"></a>Process2DQuadTessFactorsAvg 函式
...
會產生四個修補程式的已更正鑲嵌因數，並計算出鑲嵌因數作為邊緣鑲嵌因數的平均值。 您和 V 鑲嵌式因素的計算方式是使用網域的平均相反邊來個別計算，然後以 InsideScale 進行調整。 結果接著會根據資料分割模式舍入，但使用 UnroundedInsideTessFactors 參數可使用 unrounded 結果。
