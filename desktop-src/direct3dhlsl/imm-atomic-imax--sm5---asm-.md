...
---
# <a name="imm_atomic_imax-sm5---asm"></a>imm 不可部分完成 \_ \_ imax (sm5-asm) 

立即可部分完成的帶正負號的最大記憶體。 傳回 max 作業之前的記憶體值。



| imm \_ 不可 \_ 部分完成 imax dst0 \[ 。單一 \_ 元件 \_ 遮罩 \] 、dst1、dstAddress \[ . swizzle \] 、src0 \[ 。選取 \_ 元件\] |
|--------------------------------------------------------------------------------------------------------------|
...
此指令會在每個元件位址 *dstAddress* 的32位上執行具有 *dst1* 的運算元 *src0* 的單一元件32位帶正負號的最大值。
