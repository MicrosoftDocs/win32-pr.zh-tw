...
ms.locfileid: "118516780"
---
# <a name="atomic_umin-sm5---asm"></a>不可部分完成 \_ 的 umin (sm5-asm) 

記憶體最小的不可部分完成帶正負號整數。



| 不可 \_ 部分完成的 umin dst，dstAddress \[ . swizzle \] ，src0 \[ . select \_ component\] |
|----------------------------------------------------------------------|
...
此指令會32在每個元件位址 DstAddress 的每個元件位址中，執行每個元件32位不帶正負號的最小運算元 *src0* 至 *dst* 的單一元件。
