---
title: Texture3D：： Texture3D 範例方法
description: 範例材質。 |Texture3D：： Texture3D 範例方法
ms.assetid: 769AE904-5F0C-4427-862E-2BC5BC995546
keywords:
- 範例方法 HLSL
topic_type:
- apiref
api_type:
- NA
ms.topic: reference
ms.date: 05/31/2018
api_name: ''
api_location: ''
ms.openlocfilehash: 8edf90276ca7d08272d9b945f4abd273df067047751ca8d3f69c077283be3a19
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "118283717"
---
# <a name="texture3dsample-methods"></a>Texture3D：： Sample 方法

範例材質。

### <a name="overload-list"></a>多載清單



| 方法                                                                       | 描述                                                                                                                               |
|:-----------------------------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------|
| [**範例 (S、float、int)**](dx-graphics-hlsl-to-sample.md)                    | 範例材質。<br/>                                                                                                             |
| [**範例 (S、float、int、float)**](t3d-sample-s-float-int-float-.md)           | 使用選擇性值來取樣材質，以將範例詳細資料 (」 LOD) 值到。<br/>                                      |
| [**範例 (S、float、int、float、uint)**](t3d-sample-s-float-int-float-uint-.md) | 使用選擇性值來取樣材質，以將範例詳細資料 (」 LOD) 值，並傳回作業的狀態。<br/> |



## <a name="see-also"></a>另請參閱

<dl> <dt>

[**Texture3D**](sm5-object-texture3d.md)
</dt> <dt>

[材質物件](dx-graphics-hlsl-to-type.md)
</dt> </dl>

 

 





