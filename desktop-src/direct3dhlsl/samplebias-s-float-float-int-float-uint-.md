...
---
# <a name="samplebiassamplebiassfloatfloatintfloatuint-function-for-texture2d"></a>Texture2D 的 SampleBias：： SampleBias (S、float、float、int、float、uint) 函數

在將偏差值套用至 mipmap 層級之後，針對 [**Texture2D**](sm5-object-texture2d.md)進行取樣，並使用選擇性的值來壓入範例詳細資料層級 (」 lod) 值為。 傳回操作的相關狀態。
...
[SampleBias 方法](texture2d-samplebias.md)
</dt> </dl>
