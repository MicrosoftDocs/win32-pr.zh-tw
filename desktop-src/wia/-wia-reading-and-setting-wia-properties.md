---
description: IWiaPropertyStorage 介面會提供方法，以讀取和寫入 Windows 影像取得 (WIA) 專案的屬性。 專案屬性包括裝置命令、專案格式資訊和裝置資訊。
...
ms.locfileid: "118439872"
---
# <a name="reading-and-setting-wia-properties"></a>讀取及設定 WIA 屬性

[**IWiaPropertyStorage**](/windows/desktop/api/wia_xp/nn-wia_xp-iwiapropertystorage)介面會提供方法，以讀取和寫入 Windows 影像取得 (WIA) 專案的屬性。 專案屬性包括裝置命令、專案格式資訊和裝置資訊。
...
[**IWiaPropertyStorage**](/windows/desktop/api/wia_xp/nn-wia_xp-iwiapropertystorage)繼承自 [IPropertyStorage](/windows/win32/api/propidlbase/nn-propidlbase-ipropertystorage) ，而繼承的方法會依照 Windows 軟體開發套件 (SDK) 中結構化儲存體的參考一節所述的方式來執行。
