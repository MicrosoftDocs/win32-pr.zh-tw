---
description: 媒體類型的子類型 GUID。
ms.assetid: 8e600943-92f1-4936-8c00-842fc7f4cb57
title: 'MF_MT_SUBTYPE 屬性 (Mfapi) '
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: 5e739bba5acc4816b8f8b9c2e9506496de3468865afca2d262cf56379c600889
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "118060010"
---
# <a name="mf_mt_subtype-attribute"></a>MF \_ MT \_ 子類型屬性

媒體類型的子類型 GUID。

## <a name="data-type"></a>資料類型

**GUID**

## <a name="remarks"></a>備註

子類型 GUID 會定義主要類型內的特定媒體格式類型。 例如，在影片中，子類型包括 RGB-24、RGB-32、UYVY、AYUV 等等。 在音訊內，子類型包含 PCM 音訊、Windows Media 音訊9等等。

如需可能的值，請參閱下列主題：

-   [音訊子類型 Guid](audio-subtype-guids.md)
-   [影片子類型 Guid](video-subtype-guids.md)

這個屬性的 GUID 常數是從 mfuuid 匯出。

## <a name="requirements"></a>規格需求



| 需求 | 值 |
|-------------------------------------|------------------------------------------------------------------------------------|
| 最低支援的用戶端<br/> | WindowsVista \[ desktop apps \| UWP 應用程式\]<br/>                              |
| 最低支援的伺服器<br/> | WindowsServer 2008 \[ desktop app \| UWP 應用程式\]<br/>                        |
| 標頭<br/>                   | <dl> <dt>Mfapi。h</dt> </dl> |



## <a name="see-also"></a>另請參閱

<dl> <dt>

[依字母順序排列的媒體基礎屬性清單](alphabetical-list-of-media-foundation-attributes.md)
</dt> <dt>

[**IMFAttributes：： GetGUID**](/windows/desktop/api/mfobjects/nf-mfobjects-imfattributes-getguid)
</dt> <dt>

[**IMFAttributes：： SetGUID**](/windows/desktop/api/mfobjects/nf-mfobjects-imfattributes-setguid)
</dt> <dt>

[**IMFMediaType**](/windows/desktop/api/mfobjects/nn-mfobjects-imfmediatype)
</dt> <dt>

[媒體類型屬性](media-type-attributes.md)
</dt> </dl>

 

 




