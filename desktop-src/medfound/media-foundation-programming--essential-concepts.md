...
在大部分的其他網域中，無法接受失真的壓縮。  (Imagine 取回電子表格的「近似值」！ ) 但失真的壓縮配置非常適合音訊和影片，原因有幾個。
...
媒體容器的早期範例是 AVI 檔案格式。 其他範例包括 [的] 和 [Advanced] 系統格式， (ASF) 。 您可以使用副檔名來識別容器 (例如 .mp4) 或 MIME 類型。
...
這種事實很自然地指向軟體設計，其中會使用不同的元件來剖析容器和解碼資料流程。 此外，此方法也適合用於外掛程式模型，讓協力廠商可以提供自己的剖析器和編解碼器。 在 Windows 上， (COM) 的元件物件模型會提供一種標準方式，將 API 與其實作為隔離，這是任何外掛程式模型的需求。 基於這個理由， (其他) ，媒體基礎會使用 COM 介面。
