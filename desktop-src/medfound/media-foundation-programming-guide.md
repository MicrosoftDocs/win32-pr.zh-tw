---
description: 媒體基礎程式設計指南
ms.assetid: 5258b2ff-96d5-490f-9a18-c01dd6924f1c
title: 媒體基礎程式設計指南
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 7ffc8e0d9197f3a9528a6586cb4ddc8da9a3a45f
ms.sourcegitcommit: c16214e53680dc71d1c07111b51f72b82a4512d8
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 03/03/2021
ms.locfileid: "104195726"
---
# <a name="media-foundation-programming-guide"></a>媒體基礎程式設計指南

本章節包含在 c + + 應用程式中使用 Microsoft 媒體基礎的總覽主題。

### <a name="getting-started"></a>開始使用

-   [基本概念](media-foundation-programming--essential-concepts.md)
-   [媒體基礎架構](media-foundation-architecture.md)

### <a name="writing-media-applications"></a>撰寫媒體應用程式

-   [音訊/視訊播放](audio-video-playback.md)
-   [音訊/視訊擷取](audio-video-capture.md)
-   [編碼與檔案編寫](encoding-and-file-authoring.md)
-   [媒體中繼資料](media-metadata.md)
-   [媒體基礎中的網路功能](networking-in-media-foundation.md)

### <a name="media-format-support"></a>媒體格式支援

-   [媒體基礎中支援的媒體格式](supported-media-formats-in-media-foundation.md)
-   [媒體基礎編解碼器物件](codecobjects.md)
-   [媒體基礎中的 ASF 支援](asf-support-in-media-foundation.md)
-   [媒體基礎中的 MPEG 4 支援](mpeg-4-support-in-media-foundation.md)
-   [Windows Media 轉碼器](windows-media-codecs.md)

### <a name="low-level-video-apis"></a>Low-Level 影片 Api

-   [Direct3D 9 影片 Api](direct3d-video-apis.md)
-   [Direct3D 11 影片 Api](direct3d-11-video-apis.md)
-   [DirectX Video 加速2。0](directx-video-acceleration-2-0.md)
-   [輸出保護管理員](output-protection-manager.md)

### <a name="writing-media-foundation-plug-ins"></a>撰寫媒體基礎外掛程式

-   [媒體基礎轉換](media-foundation-transforms.md)
-   [媒體接收器](media-sinks.md)
-   [媒體來源](media-sources.md)

## <a name="related-topics"></a>相關主題

<dl> <dt>

[媒體基礎架構](media-foundation-architecture.md)
</dt> <dt>

[Microsoft 媒體基礎](microsoft-media-foundation-sdk.md)
</dt> </dl>

 

 



