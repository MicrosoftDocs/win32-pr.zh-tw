...
ms.locfileid: "119034776"
---
# <a name="seeking-fast-forward-and-reverse-play"></a>搜尋、向前快轉及反向播放

本主題說明使用 [媒體會話](media-session.md) 進行播放時，管理搜尋和速率變更的範例程式碼。

使用媒體會話進行播放時，應用程式可以搜尋並變更播放速率，如下所示：

-   若要搜尋，請呼叫 [**IMFMediaSession：： Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start) 並指定搜尋位置。
-   若要變更播放速率，請使用 [**IMFRateControl**](/windows/desktop/api/mfidl/nn-mfidl-imfratecontrol) 介面，如 [速率控制](rate-control.md)中所述。
-   若要在搜尋時取得更新的影片畫面，請在呼叫 [**IMFMediaSession：： Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start)之前，將播放速率設定為零。 這項作業稱為「 *清理*」。  (瞭解 [如何執行清理](how-to-perform-scrubbing.md)。 ) 

不過，若要建立最佳的使用者體驗，您必須考慮下列行為：

-   搜尋是非同步，而媒體會話會將 FIFO 佇列上的所有搜尋要求排入佇列。 如果您提交多個搜尋要求，UI 可能會在實際播放狀態之前執行。 例如，假設您的應用程式會實行搜尋列。 如果使用者向前及向後拖曳搜尋列，則在執行正向搜尋時可能會有延遲。 進行搜尋時，您應該快取使用者的搜尋要求。 當目前的搜尋作業完成時，請提交使用者最新的搜尋要求，並捨棄其他要求。
-   某些傳輸狀態不允許某些速率轉換。 例如，播放時不允許從向前播放切換至反向播放。 支援的轉換如 [**IMFRateControl：： SetRate**](/windows/desktop/api/mfidl/nf-mfidl-imfratecontrol-setrate)所述。 速率變更也是非同步。

下列 helper 類別可以用來管理搜尋要求和速率變更。 非同步作業暫止時，類別會快取任何搜尋或速率變更要求。 當目前的作業完成時，類別會提交最近的要求（如果有的話）。 此類別也會管理傳輸狀態，以避免不正確速率變更。

以下是 PlayerSeeking 類別的宣告。
