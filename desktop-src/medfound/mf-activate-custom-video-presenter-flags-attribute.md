...
ms.locfileid: "119723665"
---
# <a name="mf_activate_custom_video_presenter_flags-attribute"></a>MF \_ 啟用 \_ 自訂 \_ 影片展示 \_ 者 \_ 旗標屬性

指定如何建立增強型影片轉譯器 (EVR) 的自訂展示者。
...
| 值                                          | 描述                                                                                                                                                                                                                                                                                                                          |
|------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **MF \_ 啟用 \_ 自訂展示 \_ 者 \_ ALLOWFAIL** | 如果 [**IMFActivate：： ActivateObject**](/windows/desktop/api/mfobjects/nf-mfobjects-imfactivate-activateobject) 方法無法建立應用程式的自訂展示者，則會改為使用預設的 EVR 展示者。 根據預設，如果 [**IMFActivate**](/windows/desktop/api/mfobjects/nn-mfobjects-imfactivate) 物件嘗試建立自訂的簡報者時失敗， **ActivateObject** 方法就會失敗。 |



 

應用程式可以使用 [ [**MF \_ 啟用 \_ 自訂 \_ 影片展示 \_ 者 \_ CLSID**](mf-activate-custom-video-presenter-clsid-attribute.md) ] 屬性來指定 EVR 的自訂展示者。
