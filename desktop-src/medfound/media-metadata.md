...
-    (shell 中繼資料) 的 Windows shell [**IPropertyStore**](/windows/win32/api/propsys/nn-propsys-ipropertystore)介面。
...
| 媒體基礎 v1 中繼資料 | Shell 中繼資料 | 
|------------------------------|----------------|
| 需要 Windows Vista 或更新版本。 | 需要 Windows 7。<blockquote>[!Note]<br />一般而言，shell 中繼資料不需要 Windows 7，但媒體基礎不支援 Windows 7 之前的 shell 中繼資料。</blockquote><br /> | 
| 屬性與 Shell 屬性系統不相容。 | 屬性與 Shell 屬性系統相容。 | 
| 屬性可套用至整個檔案，或在資料流程層級套用。 | 僅支援檔案層級的屬性。 不支援資料流程層級屬性。 | 
| 屬性可以有多種語言的值。 | 不支援多種語言的值。 | 
| 屬性索引鍵是寬字元字串。 | 屬性索引鍵是 <a href="/windows/desktop/api/wtypes/ns-wtypes-propertykey"><strong>PROPERTYKEY</strong></a> 值。 | 
| 屬性值為 <a href="/windows/win32/api/propidl/ns-propidl-propvariant"><strong>PROPVARIANT</strong></a> 值。 | 屬性值為 <a href="/windows/win32/api/propidl/ns-propidl-propvariant"><strong>PROPVARIANT</strong></a> 值。 | 
