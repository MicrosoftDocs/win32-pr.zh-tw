...
本主題包含使用 [輸出保護管理員](output-protection-manager.md)的範例程式碼。

本主題中的範例程式碼示範如何執行 OPM 交握、傳送狀態要求，以及傳送 OPM 命令。 針對密碼編譯作業，程式碼會使用密碼編譯 API：新一代 (CNG) 。 本主題的重點是要顯示 OPM 功能，因此不會顯示與 x.509 憑證相關的工作，例如剖析和驗證憑證。

本主題所顯示的程式會在 [使用「輸出保護管理員](using-output-protection-manager.md)」中有更詳細的說明。

-   [執行 OPM 交握](#performing-the-opm-handshake)
-   [傳送 OPM 狀態要求](#sending-an-opm-status-request)
-   [傳送 OPM 命令](#sending-an-opm-command)
-   [計算 OMAC-1 值](#computing-the-omac-1-value)
-   [相關主題](#related-topics)

## <a name="performing-the-opm-handshake"></a>執行 OPM 交握

1.  列舉 OPM 裝置並選取影片輸出之後 (未顯示) ，第一個步驟是呼叫 [**IOPMVideoOutput：： StartInitialization**](/windows/desktop/api/opmapi/nf-opmapi-iopmvideooutput-startinitialization) 來取得裝置的 x.509 憑證鏈：
...
2.  應用程式必須驗證憑證鏈，並從鏈中的分葉憑證取得公開金鑰。 這些步驟不會顯示在這裡。
3.  擁有公開金鑰之後，您就可以將金鑰匯入至 CNG 演算法提供者。 呼叫 **BCryptOpenAlgorithmProvider** 函數以載入提供者。 應用程式定義的函式會匯 `ImportRsaPublicKey` 入金鑰，並將控制碼傳回至匯入的金鑰：
    ```C++
...
4.  接下來，準備包含起始序號和 AES 工作階段金鑰的緩衝區。
    ```C++
...
5.  使用驅動程式的公開金鑰，以 RSAES-PKCS1-OAEP 加密加密此緩衝區。
    ```C++
        //--------------------------------------------------------------------
...
6.  呼叫 [**IOPMVideoOutput：： FinishInitialization**](/windows/desktop/api/opmapi/nf-opmapi-iopmvideooutput-finishinitialization) 來完成交握。
    ```C++
...
## <a name="sending-an-opm-status-request"></a>傳送 OPM 狀態要求

下一個範例顯示如何傳送 [**OPM \_ GET \_ CONNECTOR \_ 類型**](opm-get-connector-type.md) 狀態要求。

1.  使用狀態要求的資訊填入 [**OPM \_ 取得 \_ 資訊 \_ 參數**](/windows/desktop/api/ksopmapi/ns-ksopmapi-opm_get_info_parameters) 結構。
    ```C++
        //--------------------------------------------------------------------
...
2.  [**OPM \_ 取得 \_ 資訊 \_ 參數**](/windows/desktop/api/ksopmapi/ns-ksopmapi-opm_get_info_parameters)結構的 **OMAC** 成員是單一金鑰的 CBC MAC (針對結構的其餘部分計算的 omac) 。 稍後所示的 ComputeOMAC 函式 () 宣告如下：
    ```C++
    HRESULT ComputeOMAC(
        OPM_RANDOM_NUMBER&  AesKey,     // Session key
        PUCHAR pb,                      // Data
...
3.  呼叫 [**IOPMVideoOutput：： GetInformation**](/windows/desktop/api/opmapi/nf-opmapi-iopmvideooutput-getinformation) 以傳送狀態要求。
    ```C++
...
4.  驅動程式會將回應寫入至 [**OPM \_ 所要求的 \_ 資訊**](/windows/desktop/api/ksopmapi/ns-ksopmapi-opm_requested_information) 結構。 回應結構包含 OMAC 值，此值是針對結構的其餘部分所計算。 信任回應資料之前，請先確認此值：
    ```C++
        //--------------------------------------------------------------------
...
5.  [**OPM 所 \_ 要求之 \_ 資訊**](/windows/desktop/api/ksopmapi/ns-ksopmapi-opm_requested_information)結構的 **abRequestedInformation** 成員包含回應資料。 針對 [**OPM \_ GET \_ CONNECTOR \_ 型**](opm-get-connector-type.md) 別要求，回應資料是由 [**OPM \_ 標準 \_ 資訊**](/windows/desktop/api/ksopmapi/ns-ksopmapi-opm_standard_information) 結構所組成。
    ```C++
...
## <a name="sending-an-opm-command"></a>傳送 OPM 命令

下一個範例顯示如何藉由傳送 [**OPM \_ SET \_ Protection \_ LEVEL**](opm-set-protection-level.md) 命令，來啟用 High-Bandwidth 數位內容保護 (HDCP) 。

1.  所有 OPM 命令都使用 [**OPM \_ 設定 \_ 參數**](/windows/desktop/api/opmapi/ns-opmapi-opm_configure_parameters) 結構來輸入資料。 此結構中的 **abParameters** 陣列包含命令特有的資料。 針對 [**OPM \_ set \_ protection \_ level**](opm-set-protection-level.md) 命令， **abParameters** 陣列包含 [**OPM \_ SET \_ 保護 \_ 層級的 \_ 參數**](/windows/desktop/api/opmapi/ns-opmapi-opm_set_protection_level_parameters) 結構。 填寫此結構，如下所示：
    ```C++
        //--------------------------------------------------------------------
...
2.  接下來，填寫 [**OPM \_ 設定 \_ 參數**](/windows/desktop/api/opmapi/ns-opmapi-opm_configure_parameters) 結構並計算 OMAC。
    ```C++
...
3.  若要傳送命令，請呼叫 [**IOPMVideoOutput：： Configure**](/windows/desktop/api/opmapi/nf-opmapi-iopmvideooutput-configure)。 請記得在每個命令之後遞增命令序號。
    ```C++
        //  Send the command.
...
4.  若要確認是否已啟用 HDCP，請傳送 [**OPM \_ 取得 \_ 虛擬 \_ 保護 \_ 層級**](opm-get-virtual-protection-level.md) 狀態要求 (未顯示) 。

## <a name="computing-the-omac-1-value"></a>計算 OMAC-1 值

下列程式碼說明如何計算用來簽署 OPM 命令和要求結構的 OMAC-1 值。
...
OMAC-1 演算法的詳細說明 <https://www.nuee.nagoya-u.ac.jp/labs/tiwata/omac/omac.html> 。
