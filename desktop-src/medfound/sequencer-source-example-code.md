...
ms.locfileid: "120012353"
---
# <a name="sequencer-source-example-code"></a>Sequencer 來源範例程式碼

本主題說明在 Microsoft 媒體基礎中使用 [Sequencer 來源](sequencer-source.md) 的範例程式碼。 包含以下幾節。

-   [CPlaylist 類別](#cplaylist-class)
-   [建立 CPlaylist 的實例](#creating-an-instance-of-cplaylist)
-   [新增和移除播放清單區段](#adding-and-removing-playlist-segments)
-   [處理會話事件](#handling-session-events)
-   [相關主題](#related-topics)

## <a name="cplaylist-class"></a>CPlaylist 類別

`CPlaylist`類別衍生自 `CPlayer` 教學課程中所顯示的類別，[如何使用媒體基礎來播放媒體](how-to-play-unprotected-media-files.md)檔案。
...
## <a name="creating-an-instance-of-cplaylist"></a>建立 CPlaylist 的實例

`CPlaylist::CreateInstance`方法會建立新的 `CPlaylist` 物件。 就內部而言，這個方法會呼叫 `CPlaylist::Initialize` 來初始化物件。 `Initialize`方法會呼叫 [**MFCreateSequencerSource**](/windows/desktop/api/mfidl/nf-mfidl-mfcreatesequencersource)來建立順序來源。 它也會呼叫 [**IMFMediaSession：： GetClock**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-getclock) 來取得簡報時鐘的指標。
...
## <a name="adding-and-removing-playlist-segments"></a>新增和移除播放清單區段

方法會新增 `AddSegment` 播放清單區段。

這個方法會執行下列步驟：

1.  建立播放拓撲。 此步驟的程式碼會顯示在 [建立播放拓撲](creating-playback-topologies.md)的主題中。
2.  呼叫 [**IMFSequencerSource：： AppendTopology**](/windows/desktop/api/mfidl/nf-mfidl-imfsequencersource-appendtopology) 將拓撲新增至播放清單。
3.  在第一個區段上，取得包含播放持續時間的 [ [**MF \_ PD \_ 持續時間**](mf-pd-duration-attribute.md) ] 屬性的值。
4.  將區段識別碼和拓撲識別碼儲存在查閱資料表中。
...
`AddSegment`方法是私用的 `CPlaylist` ，而且是從下列公用方法呼叫：
...
當您建立第一個播放清單區段時，您必須在媒體會話上將區段拓撲排入佇列，如下所示：

1.  查詢 [**IMFMediaSourceTopologyProvider**](/windows/desktop/api/mfidl/nn-mfidl-imfmediasourcetopologyprovider) 介面的 sequencer 來源。
2.  將表示描述項傳遞給 [**IMFMediaSourceTopologyProvider：： GetMediaSourceTopology**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasourcetopologyprovider-getmediasourcetopology) 方法。 這個方法會取得區段拓撲的指標。 請注意，此拓撲與您建立 earliers 的播放拓撲並不完全相同。 相反地，它是該拓撲的修改版本。 如需詳細資訊，請參閱 [關於 Sequencer 來源](about-the-sequencer-source.md)。
3.  藉由呼叫 [**IMFMediaSession：： SetTopology**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-settopology)，在媒體會話上將拓撲排入佇列。

下列程式碼顯示這些步驟。 當播放清單 prerolls 下一個區段時，也會使用相同的程式碼。
...
若要刪除播放清單區段，請呼叫 [**IMFSequencerSource：:D eletetopology**](/windows/desktop/api/mfidl/nf-mfidl-imfsequencersource-deletetopology)。 區段是由區段識別碼指定。  (這就是應用程式應該快取區段識別碼清單的原因。 ) 
...
## <a name="handling-session-events"></a>處理會話事件
