...
---
# <a name="meaudiosessionservershutdown-event"></a>MEAudioSessionServerShutdown 事件

當 Windows 音訊伺服器系統關閉時，由音訊轉譯器引發。 音訊轉譯器現在無效。
...
此事件是由音訊轉譯器的資料流程接收所傳送。 當音訊轉譯器從音訊會話接收 [**IAudioSessionEvents：： OnSessionDisconnected**](/windows/win32/api/audiopolicy/nf-audiopolicy-iaudiosessionevents-onsessiondisconnected) 事件，而中斷連線原因等於 **DisconnectReasonServerShutdown** 時，就會觸發此事件。
