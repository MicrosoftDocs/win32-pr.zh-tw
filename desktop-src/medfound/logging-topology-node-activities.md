...
ms.locfileid: "119465518"
---
# <a name="logging-topology-node-activities"></a>記錄拓撲節點活動

TopoEdit 會提供選項，讓您收集轉換節點或拓撲輸出節點的記錄資訊。

## <a name="to-setup-logging"></a>設定記錄

1.  在 [ **拓撲] 窗格** 上，按一下 [轉換] 節點或輸出節點來選取它。

2.  從 [ **工具** ] 功能表中，按一下 [ **Spy 選取的節點**]。

在拓撲建立期間，會將所選節點上的所有方法通話記錄至文字檔。 這會儲存在媒體檔案所在的資料夾中。 記錄檔會與節點名稱和唯一拓撲節點識別碼一併儲存。 這可確保不會有其他節點寫入記錄檔。 若要以程式設計方式取得識別碼，請呼叫 [**IMFTopologyNode：： GetTopoNodeID**](/windows/desktop/api/mfidl/nf-mfidl-imftopologynode-gettoponodeid)。
