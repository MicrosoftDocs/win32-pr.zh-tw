---
description: 輸出保護管理員 (OPM) 可讓應用程式在透過實體連接器傳送至顯示裝置時，保護影片內容。
ms.assetid: daae615b-37c4-4044-91c6-693357e0016a
title: 輸出保護管理員
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: a17d52c909596dba2304a38960b708da0b92cc17fd3530cae886641de1f69e3f
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119101867"
---
# <a name="output-protection-manager"></a>輸出保護管理員

輸出保護管理員 (OPM) 可讓應用程式在透過實體連接器傳送至顯示裝置時，保護影片內容。

此章節包含下列主題。

-   [使用輸出保護管理員](using-output-protection-manager.md)
-   [OPM 憑證撤銷](opm-certificate-revocation.md)
-   [OPM 範例程式碼](opm-example-code.md)
-   [OPM 程式設計參考](opm-programming-reference.md)

## <a name="related-topics"></a>相關主題

<dl> <dt>

[媒體基礎程式設計指南](media-foundation-programming-guide.md)
</dt> </dl>

 

 



