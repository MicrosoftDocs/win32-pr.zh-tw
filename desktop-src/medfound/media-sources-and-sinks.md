---
description: 本節說明 Microsoft 媒體基礎所支援的媒體來源和接收器。
ms.assetid: 560A3EA8-AB78-4326-893B-A28DCBC0A7E0
title: 媒體來源和接收
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 668e2c609987f50911b2074035def9aad22e6432
ms.sourcegitcommit: c16214e53680dc71d1c07111b51f72b82a4512d8
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 03/03/2021
ms.locfileid: "107000514"
---
# <a name="media-sources-and-sinks"></a>媒體來源和接收

本節說明 Microsoft 媒體基礎所支援的媒體來源和接收器。

## <a name="in-this-section"></a>本節內容



| 主題                                                   | 描述                                                                                                   |
|---------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|
| [**MP3 檔案來源**](mp3-file-source.md)<br/>   | MP3 檔案來源會剖析 MP3 檔案。<br/>                                                              |
| [**MPEG-2 File 接收**](mpeg-4-file-sink.md)<br/> | MPEG-2 檔案接收會建立檔案。<br/>                                                            |
| [MPEG-2 檔案來源](mpeg-4-file-source.md)<br/> | MPEG-2 檔案來源會剖析3GPP 檔案。<br/>                                                  |
| [薩米媒體來源](sami-media-source.md)<br/>   | 已同步處理的可存取媒體交換 (薩米) 是一種將標題新增至數位媒體的格式。<br/> |



 

## <a name="related-topics"></a>相關主題

<dl> <dt>

[媒體基礎架構](media-foundation-architecture.md)
</dt> <dt>

[媒體基礎程式設計參考](media-foundation-programming-reference.md)
</dt> <dt>

[媒體接收器](media-sinks.md)
</dt> <dt>

[媒體來源](media-sources.md)
</dt> <dt>

[媒體基礎中支援的媒體格式](supported-media-formats-in-media-foundation.md)
</dt> </dl>

 

 




