---
title: WinSAT 架構
description: 您可以使用 Windows 系統評定工具 (WinSAT) 架構來判斷要執行的有效 XPath 查詢。
...
\[在 Windows 8.1 之後，可能會變更或無法釋放 WinSAT 列舉。\]

您可以使用 Windows 系統評定工具 (WinSAT) 架構來判斷要執行的有效 XPath 查詢。 若要執行 XPath 查詢，請參閱下列其中一種方法：
...
當您呼叫 [**get \_ AllXML**](/windows/desktop/api/Winsatcominterfacei/nf-winsatcominterfacei-iqueryallwinsatassessments-get_allxml)方法時， **WinSAT** 元素可將 **WinsatAssessments** 元素作為父元素。

以下是您可以用來判斷 XPath 的 WinSAT 架構。
