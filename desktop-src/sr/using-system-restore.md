...
ms.locfileid: "120111258"
---
# <a name="using-system-restore"></a>使用系統還原

下列範例示範如何使用 [**SRSetRestorePoint**](/windows/desktop/api/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa) 函式來建立和取消還原點。

使用系統還原的第一個步驟是設定 [**CoInitializeEx**](/windows/win32/api/combaseapi/nf-combaseapi-coinitializeex) 和 [**CoInitializeSecurity**](/windows/win32/api/combaseapi/nf-combaseapi-coinitializesecurity)的 COM 呼叫。 任何使用 [**SRSetRestorePoint**](/windows/desktop/api/SRRestorePtAPI/nf-srrestoreptapi-srsetrestorepointa) 函式的進程都需要這項功能。 NetworkService、LocalService 和 System 必須允許呼叫進程。 下列 **InitializeCOMSecurity** 函數是如何初始化 COM 安全性的範例。 您可能需要修改應用程式的 **CoInitializeSecurity** 函式的參數。
...
## <a name="example-1-create-a-restore-point"></a>範例1：建立還原點。
...
## <a name="example-2-create-and-cancel-a-restore-point"></a>範例2：建立和取消還原點。
