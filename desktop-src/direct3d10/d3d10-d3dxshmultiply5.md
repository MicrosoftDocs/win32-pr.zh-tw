...
ms.locfileid: "118990768"
---
# <a name="d3dxshmultiply5-function"></a>D3DXSHMultiply5 函式

計算兩個球形諧波函式的乘積 (f 和 g) 。 這兩個函數的順序都是 N = 5。
...
Order N = 5 的兩個 SH 函數的乘積會產生 order 2 × *n-1* = 9 的 sh 函式，但結果會被截斷。 這表示產品與 ( *f* × *g*  =  *g* × *f* ) 但不會關聯 ( *f* x ( *g* × *h* ) ≠ ( *f* × *g* ) × *h* ) 。
