...
---
# <a name="d3dxvec3hermite-function-d3dx10mathh"></a>D3DXVec3Hermite 函式 (D3DX10Math) 

使用指定的3D 向量，執行 Hermite 曲線插補。
...
D3DXVECTOR3 結構的指標，此結構是 Hermite 曲線插補的結果。

## <a name="remarks"></a>備註

**D3DXVec3Hermite** 函式會將 (PositionA、tangentA) 從 (PositionB、tangentB) （使用 Hermite 曲線插補）進行插補。
...
此函式的傳回值與不悅參數中所傳回的值相同。 如此一來， **D3DXVec3Hermite** 函式就可以用來做為另一個函式的參數。
