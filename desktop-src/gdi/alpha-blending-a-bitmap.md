...
下列程式碼範例會將一個視窗分成三個水準區域。 然後，它會在每個視窗區域中繪製 Alpha 混合點陣圖，如下所示：

-   在頂端區域中，常數 Alpha = 50% 但沒有來源 Alpha。
-   在中間區域中，常數 Alpha = 100% (已停用) 而來源 Alpha 為 0 (透明) 在點陣圖中間，而 0xff (不透明) 于其他位置。
-   在底部區域中，常數 Alpha = 75% 和來源 Alpha 變更。
