---
description: 本章節包含的程式碼範例，可讓使用者使用 [選擇字型] 對話方塊) 來選取特定點大小的字型 (，請使用以這個字型) 繪製的文字來選取剪輯路徑 (，然後查看裁剪至文字的結果。
...
ms.locfileid: "104115297"
---
# <a name="using-paths"></a><span data-ttu-id="74b2a-103">使用路徑</span><span class="sxs-lookup"><span data-stu-id="74b2a-103">Using Paths</span></span>

<span data-ttu-id="74b2a-104">本章節包含的程式碼範例，可讓使用者使用 [選擇字型] 對話方塊) 來選取特定點大小的字型 (，請使用以這個字型) 繪製的文字來選取剪輯路徑 (，然後查看裁剪至文字的結果。</span><span class="sxs-lookup"><span data-stu-id="74b2a-104">This section contains a code sample that enables the user to select a font of a particular point size (by using the Choose Font dialog box), select a clip path (by using text drawn with this font), and then view the result of clipping to the text.</span></span>

<span data-ttu-id="74b2a-105">這個程式碼範例是用來建立顯示在 [剪輯路徑](clip-paths.md)中的圖例。</span><span class="sxs-lookup"><span data-stu-id="74b2a-105">This code sample was used to create the illustration that appears in [Clip Paths](clip-paths.md).</span></span>
