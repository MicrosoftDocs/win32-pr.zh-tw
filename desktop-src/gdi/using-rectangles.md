...
---
# <a name="using-rectangles-windows-gdi"></a>使用 (Windows GDI) 的矩形

本節中的範例說明如何使用矩形函數。 它是由應用程式中的主視窗程式所組成，可讓使用者移動點陣圖並調整其大小。

當應用程式啟動時，它會在畫面左上角的32圖元點陣圖繪製 32-圖元。 使用者可以拖曳點陣圖以移動點陣圖。 若要調整點陣圖大小，使用者可以拖曳滑鼠來建立目標矩形，然後在目標矩形上拖曳點陣圖並「卸載」。 應用程式會藉由將點陣圖複製到目標矩形來回應。

下列範例提供允許使用者移動和調整點陣圖大小的視窗程式。
