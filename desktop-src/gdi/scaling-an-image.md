...
某些應用程式會調整影像;亦即，它們會顯示影像的縮放或減少視圖。 例如，繪圖應用程式可能會提供縮放功能，讓使用者以圖元為單位來查看和編輯繪圖。

應用程式會藉由呼叫 [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) 函數來調整影像。 如同 [**BitBlt**](/windows/desktop/api/Wingdi/nf-wingdi-bitblt) 函式， **StretchBlt** 會從來源裝置內容中的點陣圖複製點陣圖資料 ([**dc**](/windows/desktop/api/Winuser/nf-winuser-getdcex)) 到目標 DC 中的點陣圖。 但是，與 **BitBlt** 函式不同的是， **StretchBlt** 會根據來源和目標矩形的指定維度來調整影像。 如果來源矩形大於目標矩形，結果影像將會顯示為已壓縮;如果來源矩形小於目標矩形，結果影像將會顯示為已展開。

如果目標矩形小於來源矩形， [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) 會根據指定的延展模式來移除影像中的色彩資料，如下表所示。



| Stretch 模式 | 方法                                                                                                                    |
|--------------|---------------------------------------------------------------------------------------------------------------------------|
| BLACKONWHITE | 針對已刪除的圖元以及剩餘圖元的色彩資料，在色彩資料上執行邏輯 AND 運算。 |
| WHITEONBLACK | 針對已刪除的圖元以及剩餘圖元的色彩資料執行邏輯 OR 運算。  |
| COLORONCOLOR | 完全消除已刪除圖元的色彩資料。                                                               |
| 半 色調     | 近似原始 (源) 目的地中的色彩資料。                                                         |



 

您可以藉由呼叫 [**SetStretchBltMode**](/windows/desktop/api/Wingdi/nf-wingdi-setstretchbltmode) 函數來設定延展模式。

下列範例程式碼取自應用程式，該應用程式示範 [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) 函式所提供的四種延展模式。
