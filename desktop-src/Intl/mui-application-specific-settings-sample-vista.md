...
此應用程式會先剖析文字檔中的分隔語言清單，然後將它轉換成多字串語言清單，以定義應用程式專屬的語言喜好設定。 範例中支援的分隔符號為 "、"、";"、"：" 和 ""。 剖析清單之後，程式碼會尋找並載入已識別語言的資源，就像系統設定範例一樣。 此程式碼會使用對資源載入器函式 [**LoadLibraryEx**](/windows/win32/api/libloaderapi/nf-libloaderapi-loadlibraryexa) 和 [**FreeLibrary**](/windows/win32/api/libloaderapi/nf-libloaderapi-freelibrary)的呼叫來載入和釋放資源檔。
