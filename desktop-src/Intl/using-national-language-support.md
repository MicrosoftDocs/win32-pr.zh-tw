---
description: 使用國家語言支援
ms.assetid: 48d96954-90ca-4a4b-b62d-3dd2348fc61c
title: 使用國家語言支援
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 34bfe31221c08bb66a10d4756909743a2eb4b16da004c0fd647e550f8ac0d5a2
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119764568"
---
# <a name="using-national-language-support"></a>使用國家語言支援

有關使用國家語言支援 (NLS) 的討論分為下列主題：

-   [呼叫「地區設定名稱」函數](calling-the--locale-name--functions.md)
-   [處理國際化功能變數名稱 (IDNs) ](handling-internationalized-domain-names--idns.md)
-   [處理應用程式中的排序](handling-sorting-in-your-applications.md)
-   [對應地區設定資料](mapping-locale-data.md)
-   [正在抓取和設定地區設定資訊](retrieving-and-setting-locale-information.md)
-   [正在抓取時間和日期資訊](retrieving-time-and-date-information.md)
-   [日本曆的紀元處理方式](era-handling-for-the-japanese-calendar.md)
-   [使用永久性地區設定資料](using-persistent-locale-data.md)
-   [使用 Pseudo-Locales 進行當地語系化測試](using-pseudo-locales-for-localization-testing.md)
-   [使用 Unicode 正規化來代表字串](using-unicode-normalization-to-represent-strings.md)
-   [使用自訂地區設定](working-with-custom-locales.md)
-   [NLS：國際化功能變數名稱 (IDN) 轉換範例](nls--internationalized-domain-name--idn--conversion-sample.md)
-   [NLS：國際化功能變數名稱 (IDN) 緩和範例](nls--internationalized-domain-name--idn--mitigation-sample.md)
-   [NLS：以名稱為基礎的 Api 範例](nls--name-based-apis-sample.md)
-   [NLS： Unicode 正規化範例](nls--unicode-normalization-sample.md)

 

 



