---
description: 多語系消費者介面 (MUI) 是一種技術，可為使用者提供在 Windows 作業系統中的全球化應用程式和使用者介面語言資源管理的當地語系化使用者介面。
...
ms.locfileid: "120041658"
---
# <a name="about-multilingual-user-interface"></a>關於多語系消費者介面

多語系消費者介面 (MUI) 是一種技術，可為使用者提供在 Windows 作業系統中的全球化應用程式和使用者介面語言資源管理的當地語系化使用者介面。 提供支援，可將 MUI 功能新增至全球化應用程式，以在 Windows Vista 和更新版本上執行，以及許多預先 Windows 的 Vista 作業系統。 MUI 當地語系化和資源管理模型可增強對全球化軟體的開發、測試和支援。
...
-    (API) 的完整應用程式設計介面。 WindowsVista 和更新版本會公開 MUI API，供您用來將 MUI 功能新增至全球化應用程式。 語言管理功能可讓您的應用程式支援各種不同的使用者介面語言。 此 API 也包含資源載入功能，可讓資源載入器載入 Windows vista 和更新版本的資源，以及預先 Windows 的 Vista 作業系統。
-   一組語言套件管理工具，可在國際部署映射管理中提供彈性。 這些工具並非以開發人員為目標，因此不會涵蓋在 SDK 中。 如需詳細資訊，請參閱 Technet 上的 Windows 自動化安裝套件（英文）。
