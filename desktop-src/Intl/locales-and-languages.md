...
「地區設定」是以值清單表示的語言相關使用者喜好設定資訊的集合。 WindowsXP 支援150以上的地區設定，Windows Vista 支援大約200。 每個地區設定都是由語言和排序次序所定義，而且具有地區設定名稱和地區設定識別碼。 德文 (德國) 的地區設定名稱範例是「de de \_ 電話簿」。
