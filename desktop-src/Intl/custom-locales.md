...
ms.locfileid: "119754918"
---
# <a name="custom-locales"></a>自訂地區設定

**Windows Vista 和更新版本：** 自訂地區設定支援國際屬性，提供比使用 Microsoft 隨附的標準地區設定更具文化特性的使用者體驗。 使用自訂地區設定，可讓系統管理員擴充 Microsoft 提供的地區設定，或以 Windows 所附的地區設定來取代資料，例如，貨幣符號或年度月份的名稱。
...
| fr-US          | Woodlawn bank 使用 Windows XP Embedded，將其自動化的出納電腦 (atm) ，而銀行會透過以法文、英文和西班牙文的使用者介面北美洲的使用者介面。 為了提供一致的體驗，銀行會建立美國法文的地區設定，其中包含美國格式但法文日和月份名稱。 |
