---
title: Windows色彩系統一般配置檔案類型架構、版本控制和當地語系化策略
description: Windows色彩系統一般配置檔案類型架構、版本控制和當地語系化策略
ms.assetid: 295522c6-7c53-47f6-9b98-aeee2b0e34fc
keywords:
- Windows色彩系統 (WCS) 、一般配置檔案類型
...
- 色彩管理，設定檔
- 色彩，設定檔
- Windows色彩系統 (WCS) ，版本控制
- WCS (Windows 色彩系統) ，版本控制
- 影像色彩管理，版本控制
- 色彩管理，版本控制
- 色彩，版本控制
- Windows色彩系統 (WCS) ，當地語系化
...
ms.locfileid: "119442288"
---
# <a name="windows-color-system-common-profile-types-schema-versioning-and-localization-strategies"></a>Windows色彩系統一般配置檔案類型架構、版本控制和當地語系化策略
...
在 Windows Vista 上，語言選擇的完成方式如下：每個執行緒都有相關聯的「慣用 UI 語言」清單，可以藉由呼叫 [**GetThreadPreferredUILanguages**](/windows/win32/api/winnls/nf-winnls-getthreadpreferreduilanguages)來取得。 此清單會依使用者喜好設定順序傳回。 比方說，在設定為美式英文的系統上， **GetThreadPreferredUILanguages** 傳回的清單是 {"en-us"，"en"}。 這表示： 1) 美式英文（如果有的話）。 2) 否則，請使用英文的任何區域性變化，例如 "en-us" 或單純的 "en"。
...
隨附于 Windows Vista 的標準 WCS 設定檔（例如 cdmp）具有與其他設定檔相同的可當地語系化屬性。

標準 Windows 解決方案會將當地語系化的字串放在 MUI DLL 中，並將其參考如下：
...
在 Windows 系統上，會從適當當地語系化版本的 MUI 資源中的資源識別碼101解壓縮描述文字，以進行 mscms.dll。 非 Windows 系統會使用 wcs： Text 子項目，如上所述。
...
在 windows 隨附的 WCS 色彩設定檔中，色彩 CPL 會顯示資源中的描述性資訊，而不是來自設定檔中的 wcs： Text 元素。 這可讓 Windows 在所有支援的語言中顯示這些設定檔的當地語系化資訊，而不需要設定檔 themlselves 包含所有語言的描述性資訊。
