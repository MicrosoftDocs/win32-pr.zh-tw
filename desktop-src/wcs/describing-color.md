---
title: 描述色彩
description: 色彩可以在元件方面描述。
ms.assetid: 441d4f8d-57ef-4ecb-90b4-9d7d0e804ea2
keywords:
- Windows色彩系統 (WCS) 、色彩元件
- WCS (Windows 色彩系統) 、色彩元件
- 影像色彩管理、色彩元件
- 色彩管理、色彩元件
- 色彩、元件
- 色調
- 色度
- 亮度
- 色調
- 語氣
- 陰影
- 零飽和度
ms.localizationpriority: high
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: ebad23d619c4fbe0c03f9e4278b734fd6fbd8e6f43feb68e1337354d36619ac3
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "119028776"
---
# <a name="describing-color"></a>描述色彩

色彩可以在元件方面描述。 影像專業人員最常使用的元件為 *色調*、 *色度* 和 *亮度*。 色調通常會視為色彩。 色彩的色調會修正其在可見光線範圍中的位置。 色彩的色度是色彩的「純粹」或「強式」。 中性灰色表示有零飽和度。 色彩的亮度是指影像所反映或傳輸的光線濃度。

色彩影像處理文獻中也可以使用詞彙的 *色調*、 *語氣* 和 *陰影* 。 色彩的色調是藉由將色調與白色混合來取得。 色彩的色調是藉由將色調與灰色混合來建立。 色彩的陰影是藉由在色調中加上黑色來進行。

 

 




