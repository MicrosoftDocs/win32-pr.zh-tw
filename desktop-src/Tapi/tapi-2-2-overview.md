...
Microsoft 電話語音應用程式開發介面 (TAPI) 2.2 版是以 C 為基礎的介面，可讓開發人員執行一系列的通訊應用程式。 下列內容提供使用 TAPI 2.2 的特定總覽資訊。 主要總覽內容包含在 [Microsoft 電話語音程式設計模型](./microsoft-telephony-programming-model.md)中。 我們會假設您熟悉這項教材。

-   [Tapi 版本控制](tapi-versioning.md) 涵蓋 tapi 的可用版本、其散發方式，以及您的應用程式如何協商要使用的 tapi 版本。
-   [會話存取](session-access.md) 涵蓋了 tapi 2.2 (Tapi/C) 特定的一些通話會話內容。 如需有關呼叫會話作業和資訊的主要參考，請參閱 [會話控制](./session-control.md) 的一般總覽。
