...
媒體串流終端 (MST) 是根據使用 DirectShow 篩選器的動態終端機。 使用 [TAPI 3 MSP 基類](tapi-3-msp-base-classes.md) 撰寫的 msp 會併入 MST，但 msp 作者可以選擇在不使用基類的情況下加以執行。

若要叫用 MST 建立，應用程式會呼叫 [**ITTerminalSupport：： CreateTerminal**](/windows/win32/api/tapi3if/nf-tapi3if-itterminalsupport-createterminal) ，並將 *PTERMINALCLASS* 設定為 CLSID \_ MediaStreamTerminal。
