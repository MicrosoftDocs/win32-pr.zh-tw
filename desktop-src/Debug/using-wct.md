...
---
# <a name="using-wct"></a>使用 WCT

下列範例程式碼示範如何使用 wait chain 遍歷 API。 它會列舉系統中的所有線程，並列印每個執行緒的等候鏈。

若要列舉系統中的所有線程，請執行不含參數的範例。 若只要列舉指定進程中的執行緒，請執行範例，並將處理序識別碼傳遞為參數。 此範例會執行下列步驟：

1. 呼叫 [RegisterWaitChainCOMCallback](/windows/desktop/api/Wct/nf-wct-registerwaitchaincomcallback) 函式以註冊 COM 回呼函數。
2. 呼叫 [OpenThreadWaitChainSession](/windows/desktop/api/Wct/nf-wct-openthreadwaitchainsession) 函式來建立等候鏈會話。
3. 呼叫 [AdjustTokenPrivileges](/windows/win32/api/securitybaseapi/nf-securitybaseapi-adjusttokenprivileges) 函數，以啟用 SE \_ DEBUG \_ NAME 許可權。
4. 呼叫 [EnumProcesses](/windows/win32/api/psapi/nf-psapi-enumprocesses) 和 [CreateToolhelp32Snapshot](/windows/win32/api/tlhelp32/nf-tlhelp32-createtoolhelp32snapshot) 函數來列舉指定的執行緒。
5. 呼叫 [GetThreadWaitChain](/windows/desktop/api/Wct/nf-wct-getthreadwaitchain) ，以抓取 [WAITCHAIN \_ 節點 \_ 資訊](/windows/desktop/api/Wct/ns-wct-waitchain_node_info) 結構的陣列，其中包含等候鏈的節點。
6. 從等候鏈列印資訊。
7. 呼叫 [CloseThreadWaitChainSession](/windows/desktop/api/Wct/nf-wct-closethreadwaitchainsession) 函數來清除等候鏈。
...
[等候鏈](wait-chain-traversal.md)對， [WCT 參考](wct-reference.md)， [MSDN 雜誌 2007 7 月-bugslayer.web：等候鏈對](/archive/msdn-magazine/2007/july/bugslayer-wait-chain-traversal)， [Microsoft 支援服務：修正應用程式從 Microsoft Store 的問題](https://support.microsoft.com/help/4027498/microsoft-store-fix-problems-with-apps)
