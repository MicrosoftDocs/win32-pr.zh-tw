---
description: 此規格描述可執行檔的結構 (映射) 檔案和 Windows 系列作業系統下的物件檔案。 這些檔案分別稱為可攜式可執行檔 (PE) 和通用物件檔案格式 (COFF) 檔。
...
ms.locfileid: "121812201"
---
# <a name="pe-format"></a>PE 格式

此規格描述可執行檔的結構 (映射) 檔案和 Windows 系列作業系統下的物件檔案。 這些檔案分別稱為可攜式可執行檔 (PE) 和通用物件檔案格式 (COFF) 檔。
...
本檔指定 Microsoft Windows 系列作業系統下可執行檔 (映射的結構) 檔案和物件檔案。 這些檔案分別稱為可攜式可執行檔 (PE) 和通用物件檔案格式 (COFF) 檔。 「可移植的可執行檔」名稱是指格式不是架構特定的事實。
...
|---------------------|----------------------|-------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0 <br/>       | 28/24 <br/>    | 標準欄位 <br/>         | 針對所有 COFF 的執行所定義的欄位，包括 UNIX。 <br/>                                                                                          |
...
接下來的21個欄位是 COFF 選用標頭格式的延伸。 其中包含 Windows 中連結器和載入器所需的其他資訊。
...
| 64/64 <br/>    | 4 <br/>      | 校驗 <br/>                    | 影像檔案總和檢查碼。 計算總和檢查碼的演算法會併入 IMAGHELP.DLL 中。 系統會在載入時檢查下列各項的驗證：所有驅動程式、在開機時載入的任何 dll，以及載入至重要 Windows 進程的任何 dll。 <br/>                                          |
...
##### <a name="windows-subsystem"></a>Windows子系統

針對選擇性標頭的子系統欄位所定義的下列值，會決定執行映射需要任何) 時 (哪個 Windows 子系統。

| 常數                                                  | 值          | 描述                                                      |
|-----------------------------------------------------------|----------------|------------------------------------------------------------------|
| 映射 \_ 子系統 \_ 不明 <br/>                     | 0 <br/>  | 未知的子系統 <br/>                                 |
| 映射 \_ 子系統 \_ 原生 <br/>                      | 1 <br/>  | 設備磁碟機和原生 Windows 進程 <br/>          |
| 映射 \_ 子系統 \_ WINDOWS \_ GUI <br/>                | 2 <br/>  | Windows 的圖形化使用者介面 (GUI) 子系統 <br/> |
...
| 映射 \_ 子系統 \_ WINDOWS \_ 開機 \_ 應用程式 <br/>  | 16 <br/> | Windows boot 應用程式。 <br/>                            |
...
| 影像 \_ DLLCHARACTERISTICS \_ NO \_ SEH <br/>                      | 0x0400 <br/> | 不使用結構化例外狀況 (SE) 處理。 在此影像中，不會呼叫任何 SE 處理常式。 <br/> |
...
| 影像 \_ DLLCHARACTERISTICS \_ 防護 \_ CF <br/>                     | 0x4000 <br/> | 映射支援控制 Flow 防護。 <br/>                                                          |
...
每個資料目錄都會提供 Windows 使用之資料表或字串的位址和大小。 這些資料目錄專案都會載入至記憶體中，讓系統可以在執行時間使用它們。 資料目錄是具有下列宣告的8位元組欄位：
...
| 168/184 <br/> | 8 <br/> | TLS 資料表 <br/>               | 執行緒區域儲存 (TLS) 資料表位址和大小。 如需詳細資訊，請參閱 [Tls 一節](#the-tls-section)。<br/>                                                        |
| 176/192 <br/> | 8 <br/> | 載入設定資料表 <br/>       | 載入設定資料表位址和大小。 如需詳細資訊，請參閱 [僅) 的負載設定結構 (映射 ](#the-load-configuration-structure-image-only)。<br/>       |
...
| Offset         | 大小          | 欄位                            | 描述                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
|----------------|---------------|----------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0 <br/>  | 8 <br/> | 名稱 <br/>                 | 8位元組、以 null 填補的 UTF-8 編碼字串。 如果字串長度剛好為8個字元，則不會有結束的 null。 若為較長的名稱，此欄位包含斜線 (/) ，後面接著十進位數的 ASCII 標記法，即字串資料表中的位移。 可執行檔映射不會使用字串資料表，且不支援長度超過8個字元的區段名稱。 如果物件檔中的長名稱是發出至可執行檔，則會被截斷。 <br/>                                         |
...
| 16 <br/> | 1 <br/> | StorageClass <br/>       | 表示儲存類別的列舉值。 如需詳細資訊，請參閱[儲存體類別](#storage-class)。 <br/>                                                                                                                   |
...
| 影像 \_ SYM \_ 類型 \_ INT <br/>    | 4 <br/>  | 自然整數類型 (在 Windows 中通常是4個位元組)  <br/>                       |
...
| 12 <br/> | 2 <br/> | 數字 <br/>              | 關聯區段的區段資料表中，以一個為基的索引。 當 COMDAT 選項設定為5時，就會使用此選項。 <br/>                                                                                     |
...
| 4 <br/>  | 4 <br/> | 名稱 <br/>                       | 要載入之 DLL 名稱的 RVA。 名稱位於影像的唯讀資料區段中。 <br/>                                                                                                                                                                                                        |
...
例如，在 Windows XP 的 Kernel32.dll 中，會將名為 "HeapAlloc" 的匯出轉送至字串 "ntdll.dll"。RtlAllocateHeap." 這可讓應用程式使用 Windows XP 專屬模組 Ntdll.dll，而不需要實際包含其匯入參考。 應用程式的匯入資料表只會參考 Kernel32.dll。 因此，應用程式並不是 Windows XP 特有的，而且可以在任何 Win32 系統上執行。
...
| 2 <br/>  | 變動 <br/> | 名稱 <br/> | 包含要匯入之名稱的 ASCII 字串。 這是必須與 DLL 中的公用名稱相符的字串。 這個字串會區分大小寫，並以 null 位元組終止。 <br/> |
...
Tls 區段為靜態執行緒區域儲存 (TLS) 提供直接 PE 和 COFF 支援。 TLS 是特殊的儲存類別，Windows 支援，其中資料物件不是自動 (堆疊) 變數，而是在執行程式碼的每個個別執行緒的本機。 因此，每個執行緒都可以針對使用 TLS 所宣告的變數維護不同的值。
...
預先保留的 SEH 負載設定結構的資料目錄專案必須指定負載設定結構的特定大小，因為作業系統載入器一律會預期它是特定的值。 就這一點而言，大小其實只是版本檢查。 為了與 Windows XP 及舊版的 Windows 相容，x86 映射的大小必須為64。
...
| 44/64 <br/>   | 4/8 <br/> | ProcessAffinityMask <br/>            | 將此欄位設定為非零值相當於在進程啟動期間使用此值來呼叫 SetProcessAffinityMask， (.exe 只)  <br/>                                                                                                                                       |
...
| 72/112 <br/>  | 4/8 <br/> | GuardCFCheckFunctionPointer <br/>    | 用來儲存 Control Flow Guard 檢查函式指標的 VA。 <br/>                                                                                                                                                                                                               |
| 76/120 <br/>  | 4/8 <br/> | GuardCFDispatchFunctionPointer <br/> | 用來儲存 Control Flow Guard 分派函式指標的 VA。 <br/>                                                                                                                                                                                                            |
| 80/128 <br/>  | 4/8 <br/> | GuardCFFunctionTable <br/>           | 影像中每個控制項之 rva 的已排序資料表的 VA Flow Guard 函數。 <br/>                                                                                                                                                                                            |
| 84/136 <br/>  | 4/8 <br/> | GuardCFFunctionCount <br/>           | 上表中唯一 Rva 的計數。 <br/>                                                                                                                                                                                                                                    |
| 88/144 <br/>  | 4 <br/>   | GuardFlags <br/>                     | 控制 Flow 防護相關的旗標。 <br/>                                                                                                                                                                                                                                               |
| 92/148 <br/>  | 12 <br/>  | CodeIntegrity <br/>                  | 程式碼完整性資訊。 <br/>                                                                                                                                                                                                                                                     |
| 104/160 <br/> | 4/8 <br/> | GuardAddressTakenIatEntryTable <br/> | 已儲存控制 Flow 防護位址取得 IAT 表格的 VA。 <br/>                                                                                                                                                                                                              |
| 108/168 <br/> | 4/8 <br/> | GuardAddressTakenIatEntryCount <br/> | 上表中唯一 Rva 的計數。 <br/>                                                                                                                                                                                                                                    |
| 112/176 <br/> | 4/8 <br/> | GuardLongJumpTargetTable <br/>       | 已儲存 Control Flow Guard long 跳躍目標資料表的 VA。 <br/>                                                                                                                                                                                                               |
...
-   包含控制項跨距的子欄位遮罩 Flow 防護函式資料表專案， (也就是每個資料表專案) 的其他位元組計數。

    ` #define IMAGE_GUARD_CF_FUNCTION_TABLE_SIZE_MASK  0xF0000000`

此外，Windows SDK winnt. h 標頭也會定義這個宏，以將 GuardFlags 值向右調整以向右對齊控制項 Flow Guard 函數資料表 stride：
...
| Offset         | 大小           | 欄位                     | 描述                                                                                                                                                                                                 |
|----------------|----------------|---------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0 <br/>  | 16 <br/> | 名稱 <br/>          | 封存成員的名稱，加上斜線 (/) 附加以終止名稱。 如果第一個字元是斜線，則名稱具有特殊的解讀，如下表所述。 <br/> |
| 16 <br/> | 12 <br/> | 日期 <br/>          | 封存成員的建立日期和時間：這是自 1/1/1970 UCT 以來的秒數的 ASCII 小數表示。 <br/>                                                    |
| 28 <br/> | 6 <br/>  | 使用者識別碼 <br/>       | 使用者識別碼的 ASCII 十進位標記法。 此欄位不會在 Windows 平臺上包含有意義的值，因為 Microsoft 工具會發出所有的空白。 <br/>                                    |
| 34 <br/> | 6 <br/>  | 群組識別碼 <br/>      | 群組識別碼的 ASCII 十進位標記法。 此欄位不會在 Windows 平臺上包含有意義的值，因為 Microsoft 工具會發出所有的空白。 <br/>                                   |
...
- **選用標頭 Windows 特定欄位的 [檔案總和檢查碼] 欄位。** 此總和檢查碼包含整個檔案 (包括) 檔案中的任何屬性憑證。 在所有可能性中，總和檢查碼在插入 Authenticode 簽章之後將會與原始值不同。
...
[Windows 的下載和工具 (包含 Windows SDK) ](https://developer.microsoft.com/windows/downloads)
...
[WindowsAuthenticode 可攜式可執行檔簽章格式 (.docx) ](https://download.microsoft.com/download/9/c/5/9c5b2167-8017-4bae-9fde-d599bac8184a/Authenticode_PE.docx)
