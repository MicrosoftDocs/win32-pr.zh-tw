...
---
# <a name="working-with-xstate-context"></a><span data-ttu-id="42b5d-103">使用 XState 內容</span><span class="sxs-lookup"><span data-stu-id="42b5d-103">Working with XState Context</span></span>

<span data-ttu-id="42b5d-104">本檔包含的範例示範如何使用 XState 內容函式，線上程上取得和設定擴充功能。</span><span class="sxs-lookup"><span data-stu-id="42b5d-104">This document contains an example that demonstrates how to use the XState context functions to retrieve and set extended features on a thread.</span></span> <span data-ttu-id="42b5d-105">下列範例會操作 Intel Advanced Vector Extensions (AVX) 狀態，此狀態是由 FeatureId 2 (功能遮罩 4) 所定義。</span><span class="sxs-lookup"><span data-stu-id="42b5d-105">The following examples manipulate Intel Advanced Vector Extensions (AVX) state which is defined by FeatureId 2 (Feature Mask 4).</span></span> <span data-ttu-id="42b5d-106">Intel AVX 是定義在提供的「Intel Advanced Vector Extensions 程式設計參考」中 <https://go.microsoft.com/fwlink/p/?linkid=212716> 。</span><span class="sxs-lookup"><span data-stu-id="42b5d-106">Intel AVX is defined in the "Intel Advanced Vector Extensions Programming Reference" available from <https://go.microsoft.com/fwlink/p/?linkid=212716>.</span></span>

<span data-ttu-id="42b5d-107">**Windows 7 SP1：**[AVX API](avx-support-portal.md)首次在 WINDOWS 7 SP1 上執行。</span><span class="sxs-lookup"><span data-stu-id="42b5d-107">**Windows 7 with SP1:** The [AVX API](avx-support-portal.md) is first implemented on Windows 7 with SP1.</span></span> <span data-ttu-id="42b5d-108">由於沒有適用于 Windows 7 SP1 的 SDK，因此沒有可供使用的標頭和程式庫檔案。</span><span class="sxs-lookup"><span data-stu-id="42b5d-108">Since there is no SDK for Windows 7 with SP1, that means there are no available headers and library files to work with.</span></span> <span data-ttu-id="42b5d-109">在這種情況下，呼叫端必須宣告此檔所需的函式，並使用 [**GetModuleHandle**](/windows/win32/api/libloaderapi/nf-libloaderapi-getmodulehandlea) 在 "Kernel32.dll" 上取得其指標，然後再呼叫 [**GetProcAddress**](/windows/win32/api/libloaderapi/nf-libloaderapi-getprocaddress)。</span><span class="sxs-lookup"><span data-stu-id="42b5d-109">In this situation, a caller must declare the needed functions from this documentation and get pointers to them using [**GetModuleHandle**](/windows/win32/api/libloaderapi/nf-libloaderapi-getmodulehandlea) on "Kernel32.dll", followed by calls to [**GetProcAddress**](/windows/win32/api/libloaderapi/nf-libloaderapi-getprocaddress).</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="42b5d-110">相關主題</span><span class="sxs-lookup"><span data-stu-id="42b5d-110">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="42b5d-111">**CopyCoNtext**</span><span class="sxs-lookup"><span data-stu-id="42b5d-111">**CopyContext**</span></span>](/windows/desktop/api/WinBase/nf-winbase-copycontext)
</dt> <dt>

[<span data-ttu-id="42b5d-112">**InitializeCoNtext**</span><span class="sxs-lookup"><span data-stu-id="42b5d-112">**InitializeContext**</span></span>](/windows/desktop/api/WinBase/nf-winbase-initializecontext)
</dt> <dt>

[<span data-ttu-id="42b5d-113">**GetEnabledXStateFeatures**</span><span class="sxs-lookup"><span data-stu-id="42b5d-113">**GetEnabledXStateFeatures**</span></span>](/windows/desktop/api/WinBase/nf-winbase-getenabledxstatefeatures)
</dt> <dt>

[<span data-ttu-id="42b5d-114">**LocateXStateFeature**</span><span class="sxs-lookup"><span data-stu-id="42b5d-114">**LocateXStateFeature**</span></span>](/windows/desktop/api/WinBase/nf-winbase-locatexstatefeature)
</dt> <dt>

[<span data-ttu-id="42b5d-115">**GetXStateFeaturesMask**</span><span class="sxs-lookup"><span data-stu-id="42b5d-115">**GetXStateFeaturesMask**</span></span>](/windows/desktop/api/WinBase/nf-winbase-getxstatefeaturesmask)
</dt> <dt>

[<span data-ttu-id="42b5d-116">**SetXStateFeaturesMask**</span><span class="sxs-lookup"><span data-stu-id="42b5d-116">**SetXStateFeaturesMask**</span></span>](/windows/desktop/api/WinBase/nf-winbase-setxstatefeaturesmask)
</dt> </dl>
