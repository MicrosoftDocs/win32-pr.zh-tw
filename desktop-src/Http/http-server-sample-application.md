---
title: HTTP 伺服器範例應用程式
description: 本主題包含範例應用程式，示範如何使用 HTTP 伺服器 API 來執行伺服器端工作。
...
下列範例應用程式顯示如何使用 HTTP 伺服器 API 來執行伺服器端工作。 第一個範例中包含的 "precomp .h" 檔案包含執行範例所需的所有標頭，例如：
...
## <a name="main-and-preliminaries"></a>Main 和準備工作
...
## <a name="cleanup-the-http-server-api"></a>清除 HTTP 伺服器 API
...
## <a name="receive-a-request"></a>接收要求
...
## <a name="send-an-http-response"></a>傳送 HTTP 回應
...
## <a name="send-an-http-post-response"></a>傳送 HTTP POST 回應
