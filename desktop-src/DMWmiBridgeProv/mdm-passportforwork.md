...
description: MDM \_ PassportForWork 可用來布建商務 Windows Hello。
...
以全域唯一識別碼 (GUID) 的格式指定租使用者識別碼，而不含大括弧 ( {，} ) ，這將作為用於商務布建和管理之 Windows Hello 的一部分。
