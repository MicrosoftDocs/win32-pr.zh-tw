...
本主題包含範例程式碼，示範如何執行可讓用戶端接收和處理 Microsoft 消費者介面自動化事件的介面。 本主題將討論下列主題：

-   [處理一般消費者介面自動化事件](#handling-general-ui-automation-events)
-   [處理 Focus-Changed 事件](#handling-focus-changed-events)
-   [處理 Property-Changed 事件](#handling-property-changed-events)
-   [處理 Structure-Changed 事件](#handling-structure-changed-events)
-   [相關主題](#related-topics)
