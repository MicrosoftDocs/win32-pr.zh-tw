...
-   Riched20.dll (在文字編輯器（例如 Windows) 98 中的 Microsoft WordPad）所提供的 rich edit 控制項中的插入號物件，在文字選取期間變更其位置時，不會傳送任何 [WinEvents](winevents-infrastructure.md) 。 當使用者按下 SHIFT 和方向鍵選取文字時，插入號物件不會觸發 [**事件 \_ 物件 \_ LOCATIONCHANGE**](event-constants.md) new-winevent。 同樣地，當您透過 rich edit 訊息以程式設計方式設定選取專案時，插入號物件不會傳送任何事件來指出其新位置。
