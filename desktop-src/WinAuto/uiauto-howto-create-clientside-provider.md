---
title: 如何) 消費者介面自動化提供者建立 Client-Side (Proxy
description: 本主題包含範例程式碼，示範如何執行用戶端或 proxy 的 Microsoft 消費者介面自動化提供者。
...
本主題包含範例程式碼，示範如何執行用戶端或 proxy 的 Microsoft 消費者介面自動化提供者。

-   [範例1：列舉 Proxy Factory 資料表](#example-1-enumerating-the-proxy-factory-table)
-   [範例2：針對按鈕控制項執行簡單的 Proxy](#example-2-implementing-a-simple-proxy-for-button-controls)
-   [相關主題](#related-topics)

## <a name="example-1-enumerating-the-proxy-factory-table"></a>範例1：列舉 Proxy Factory 資料表

下列範例程式碼會列舉 proxy factory 資料表中的專案，並顯示所支援控制項的類別名稱。 針對不是由作業系統提供的 proxy，系統會顯示映射名稱。
...
[執行 Client-Side 消費者介面自動化提供者](uiauto-serversideprovider.md)
</dt> <dt>
