...
ms.locfileid: "119052956"
---
# <a name="background-information"></a>背景資訊

Microsoft Active Accessibility 元件（oleacc.dll）會建立代表標準 Windows 控制項來執行 [**IAccessible**](/windows/desktop/api/oleacc/nn-oleacc-iaccessible)的 proxy 物件。 因為這些 proxy 使用標準 Windows 訊息和控制特定 api 來收集每個控制項的相關資訊，所以沒有直接機制可以自訂這些 proxy 透過 **IAccessible** 公開的資訊。
