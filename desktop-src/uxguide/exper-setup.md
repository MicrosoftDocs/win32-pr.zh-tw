...
雖然所有 Microsoft Windows 程式都需要某種安裝程式，但您可以選擇要在哪裡放置程式設定：
...
同樣地，這類複雜的安裝程式應該是例外狀況，而不是規則。 大部分的 Windows 程式都應該盡力以簡單的單一步驟來啟動安裝程式。
...
-   **避免重新開機 Windows。** 大部分的程式都應該安裝，而不需要重新開機 Windows。 程式安裝或更新需要系統重新開機的主要原因是，執行中的程式目前正在使用一些相關檔案。 在此情況下，更好的替代方式是讓使用者知道情況、允許使用者關閉這些程式，然後重試此動作。 如需避免重新開機的詳細資訊，請參閱 [重新開機管理員](../rstmgr/restart-manager-portal.md)。
-   **如果您的安裝程式必須重新開機 Windows：**
    -   **使用單一重新開機。** 延遲任何必要條件所需的重新開機，直到完整安裝程式及其更新為止。
    -   **讓使用者判斷何時發生。** 不要自動重新開機 Windows，因為使用者可能會遺失工作。 請確定使用者有選擇的明確使用者。
    ...
    -   **如果使用者選擇不立即重新開機 Windows，請提出任何最終意見反應成功，而不是失敗。** 在技術上，安裝程式在重新開機之前不會完成，而是從使用者的觀點來看成功。
...
-   **消除不必要的選項和問題。**
    ...
    -   請勿詢問有關不重要詳細資料的問題。 比方說，對於一般的 Windows 程式來說，您應該將程式檔案複製到 [Program Files] 資料夾。
...
-   **如果安裝程式在安裝後有後續的步驟，請將它們列在完成頁面上。** 但為了論證完成頁面的合理性，請確定使用者可能會執行這些步驟，而且真的中所述的步驟都 (也就是) 不明顯。
    ...
    **不正確：**
    ...
    在此範例中，不必要的完成頁面會說明明顯的。 Windows Update 會自動執行，因此使用者不需要手動執行。
