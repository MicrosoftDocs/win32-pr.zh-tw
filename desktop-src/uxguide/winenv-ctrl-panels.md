...
<span data-ttu-id="fdb21-116">[控制台] 首頁是所有控制台專案的主要進入點。</span><span class="sxs-lookup"><span data-stu-id="fdb21-116">The control panel home page is the main entry point for all control panel items.</span></span> <span data-ttu-id="fdb21-117">它會依類別目錄列出專案，以及最常見的工作。</span><span class="sxs-lookup"><span data-stu-id="fdb21-117">It lists the items by their category, along with the most common tasks.</span></span> <span data-ttu-id="fdb21-118">當使用者按一下 [開始] 功能表中的主控台時，就會顯示此畫面。</span><span class="sxs-lookup"><span data-stu-id="fdb21-118">It is displayed when users click Control Panel in the Start menu.</span></span>
...
### <a name="create-a-new-control-panel-item-or-extend-an-existing-one"></a><span data-ttu-id="fdb21-153">建立新的控制台專案或擴充現有的控制台專案？</span><span class="sxs-lookup"><span data-stu-id="fdb21-153">Create a new control panel item or extend an existing one?</span></span>
...
<span data-ttu-id="fdb21-197">**輪輻頁面**</span><span class="sxs-lookup"><span data-stu-id="fdb21-197">**Spoke pages**</span></span>
...
-   <span data-ttu-id="fdb21-244">**將工作連結和按鈕的位置作為重要性，以及可搜尋性的需求。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-244">**Base the location of task links and buttons on importance and need for discoverability.**</span></span>
...
-   <span data-ttu-id="fdb21-287">執行次要的獨立工作。</span><span class="sxs-lookup"><span data-stu-id="fdb21-287">To perform secondary, stand-alone tasks.</span></span> <span data-ttu-id="fdb21-288">使用非 [模式](glossary.md)的次要視窗可讓您獨立執行這類工作，並在主要工作流程之外執行。</span><span class="sxs-lookup"><span data-stu-id="fdb21-288">Using a [modeless](glossary.md), secondary window allows such tasks to be performed independently and outside the main task flow.</span></span>
...
<span data-ttu-id="fdb21-306">**互動**</span><span class="sxs-lookup"><span data-stu-id="fdb21-306">**Interaction**</span></span>
...
-   <span data-ttu-id="fdb21-325">**如果是以工作為基礎的中樞頁面，請使用工作連結和圖示來識別每項工作。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-325">**For task-based hub pages, identify each task with a task link and an icon.**</span></span> <span data-ttu-id="fdb21-326">您也可以為每個連結提供選擇性的描述。</span><span class="sxs-lookup"><span data-stu-id="fdb21-326">You can also provide an optional description for each link.</span></span> <span data-ttu-id="fdb21-327">不過，請試著讓工作連結一目了然，並只針對真正需要的連結提供選擇性描述。</span><span class="sxs-lookup"><span data-stu-id="fdb21-327">However, try to make the task links self-explanatory and provide optional descriptions only to links that really need them.</span></span>
    ...
    ![<span data-ttu-id="fdb21-328">[電腦效能中樞] 頁面的螢幕擷取畫面</span><span class="sxs-lookup"><span data-stu-id="fdb21-328">screen shot of computer performance hub page</span></span> ](images/winenv-ctrl-panels-image8.png)
...
<span data-ttu-id="fdb21-355">**另請參閱連結**</span><span class="sxs-lookup"><span data-stu-id="fdb21-355">**See also links**</span></span>
...
<span data-ttu-id="fdb21-376">這樣做會將主要工作的認可按鈕意義保持明確且明確。</span><span class="sxs-lookup"><span data-stu-id="fdb21-376">Doing so keeps the meaning of the commit buttons for the main task clear and unambiguous.</span></span> <span data-ttu-id="fdb21-377">使用者應該一律安心瞭解他們正在認可哪些內容。</span><span class="sxs-lookup"><span data-stu-id="fdb21-377">Users should always be confident in understanding what they are committing to.</span></span>
...
-   <span data-ttu-id="fdb21-414">**使用對主要指示的特定回應來確認工作，而不是一般標籤（例如 [確定]）。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-414">**Confirm tasks using commit buttons that are specific responses to the main instruction, instead of generic labels such as OK.**</span></span> <span data-ttu-id="fdb21-415">認可按鈕上的標籤應該有意義。</span><span class="sxs-lookup"><span data-stu-id="fdb21-415">The labels on commit buttons should make sense on their own.</span></span> <span data-ttu-id="fdb21-416">請避免使用 [確定]，因為它不是主要指示的特定回應，因此更容易誤解。</span><span class="sxs-lookup"><span data-stu-id="fdb21-416">Avoid using OK because it isn't a specific response to the main instruction, and therefore easier to misunderstand.</span></span> <span data-ttu-id="fdb21-417">此外，[確定] 通常會搭配強制回應對話方塊使用，並不正確地表示關閉 [控制台] 專案視窗。</span><span class="sxs-lookup"><span data-stu-id="fdb21-417">Furthermore, OK is typically used with modal dialog boxes and incorrectly implies closing the control panel item window.</span></span>
    -   <span data-ttu-id="fdb21-418">**異常：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-418">**Exceptions:**</span></span>
        ...
        -   <span data-ttu-id="fdb21-421">如果頁面有回應主要指示的選項按鈕，請使用 [確定]。</span><span class="sxs-lookup"><span data-stu-id="fdb21-421">Use OK if the page has radio buttons that are responses to the main instruction.</span></span> <span data-ttu-id="fdb21-422">若要維護延遲的認可模型，您無法使用最終輪輻頁面上的工作連結。</span><span class="sxs-lookup"><span data-stu-id="fdb21-422">To maintain the delayed commit model, you can't use task links on a final spoke page.</span></span>
            ...
            ![<span data-ttu-id="fdb21-423">具有 [確定] 按鈕之 web 限制的螢幕擷取畫面</span><span class="sxs-lookup"><span data-stu-id="fdb21-423">screen shot of web restrictions with ok button</span></span> ](images/winenv-ctrl-panels-image10.png)
...
-   <span data-ttu-id="fdb21-445">**請考慮使用 [控制台] 頁面的 [預覽] 按鈕，而不是 [套用] 按鈕。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-445">**Consider using a Preview button instead of an Apply button for control panel pages.**</span></span> <span data-ttu-id="fdb21-446">預覽按鈕可讓使用者更容易瞭解，並且可用於任何輪輻頁面上。</span><span class="sxs-lookup"><span data-stu-id="fdb21-446">Preview buttons are easier for users to understand and can be used on any spoke page.</span></span>
...
-   <span data-ttu-id="fdb21-462">**不要提供 [套用] 按鈕。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-462">**Don't provide an Apply button.**</span></span> <span data-ttu-id="fdb21-463">Live preview 會套用不必要的應用。</span><span class="sxs-lookup"><span data-stu-id="fdb21-463">The live preview makes Apply unnecessary.</span></span>
-   <span data-ttu-id="fdb21-464">如果使用者使用 [上一頁]、[關閉] 或 [網址] 列離開，請 **還原任何變更**。</span><span class="sxs-lookup"><span data-stu-id="fdb21-464">**Restore any changes if users navigate away** using Back, Close, or the Address bar.</span></span> <span data-ttu-id="fdb21-465">若要保留變更，使用者必須明確地認可這些變更。</span><span class="sxs-lookup"><span data-stu-id="fdb21-465">To preserve changes, users must commit them explicitly.</span></span>

<span data-ttu-id="fdb21-466">**套用按鈕**</span><span class="sxs-lookup"><span data-stu-id="fdb21-466">**Apply buttons**</span></span>

-   <span data-ttu-id="fdb21-467">請確定 **[套用] 按鈕表示套用工作啟動後 (所做的暫止變更，或上次套用) ，但仍留在目前的頁面上。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-467">Make sure **the Apply button means apply the pending changes (made since the task was started or the last Apply), but remain on the current page.**</span></span> <span data-ttu-id="fdb21-468">這麼做可讓使用者在繼續進行其他工作之前，先評估變更。</span><span class="sxs-lookup"><span data-stu-id="fdb21-468">Doing so allows users to evaluate the changes before moving on to other tasks.</span></span>
-   <span data-ttu-id="fdb21-469">**只在最終輪輻頁面上使用 [套用] 按鈕。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-469">**Use Apply buttons only on final spoke pages.**</span></span> <span data-ttu-id="fdb21-470">[套用] 按鈕不應該用於中繼輪輻頁面，以維護立即認可模型。</span><span class="sxs-lookup"><span data-stu-id="fdb21-470">Apply buttons should not be used on intermediate spoke pages to maintain an immediate commit model.</span></span>
    -   <span data-ttu-id="fdb21-471">**例外狀況：** 如果變更設定需要提高 [許可權](glossary.md)，您可以使用混合式中樞頁面上的 [套用] 按鈕。</span><span class="sxs-lookup"><span data-stu-id="fdb21-471">**Exception:** You can use Apply buttons on a hybrid hub page if changing a setting requires [elevation](glossary.md).</span></span> <span data-ttu-id="fdb21-472">如需詳細資訊，請參閱 [中樞頁面互動](#hub-pages)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-472">For more details, see [hub page interaction](#hub-pages).</span></span>
-   <span data-ttu-id="fdb21-473">**只有當頁面的設定 (至少一個) 具有使用者可透過有意義的方式評估的效果時，才提供 [套用] 按鈕。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-473">**Provide an Apply button only if the page has settings (at least one) with effects that users can evaluate in a meaningful way.**</span></span> <span data-ttu-id="fdb21-474">一般情況下，[套用] 按鈕會在設定進行可見變更時使用。</span><span class="sxs-lookup"><span data-stu-id="fdb21-474">Typically, Apply buttons are used when settings make visible changes.</span></span> <span data-ttu-id="fdb21-475">使用者應該能夠套用變更、評估變更，然後根據該評估進行進一步的變更。</span><span class="sxs-lookup"><span data-stu-id="fdb21-475">Users should be able to apply a change, evaluate the change, and make further changes based on that evaluation.</span></span>
-   <span data-ttu-id="fdb21-476">**只有在有暫止的變更時，才啟用 [套用] 按鈕;** 否則，請將它停用。</span><span class="sxs-lookup"><span data-stu-id="fdb21-476">**Enable the Apply button only when there are pending changes;** otherwise, disable it.</span></span>
-   <span data-ttu-id="fdb21-477">**指派 "A" 作為存取金鑰。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-477">**Assign "A" as the access key.**</span></span>

### <a name="control-panel-integration"></a><span data-ttu-id="fdb21-478">控制台整合</span><span class="sxs-lookup"><span data-stu-id="fdb21-478">Control panel integration</span></span>

<span data-ttu-id="fdb21-479">若要整合您的控制台專案與 Windows，您可以：</span><span class="sxs-lookup"><span data-stu-id="fdb21-479">To integrate your control panel item with Windows, you can:</span></span>

-   <span data-ttu-id="fdb21-480">**註冊您的控制台專案 (包括其名稱、描述和圖示)**，讓 Windows 知道它。</span><span class="sxs-lookup"><span data-stu-id="fdb21-480">**Register your control panel item (including its name, description, and icon)**, so that Windows is aware of it.</span></span>
-   <span data-ttu-id="fdb21-481">如果您的控制台專案是最上層 (請參閱以下) ：</span><span class="sxs-lookup"><span data-stu-id="fdb21-481">If your control panel item is top level (see below):</span></span>
    -   <span data-ttu-id="fdb21-482">將它與適當的 **類別頁面** 產生關聯。</span><span class="sxs-lookup"><span data-stu-id="fdb21-482">Associate it with the appropriate **category page**.</span></span>
    -   <span data-ttu-id="fdb21-483">**提供工作連結 (包括其名稱、描述、關鍵字及命令列)** 來指出主要工作，並允許使用者直接流覽至工作。</span><span class="sxs-lookup"><span data-stu-id="fdb21-483">**Provide task links (including their name, description, keywords, and command line)** to indicate primary tasks and allow users to navigate directly to the tasks.</span></span>
-   <span data-ttu-id="fdb21-484">**提供搜尋字詞** ，以協助使用者使用主控台搜尋功能來尋找您的工作連結。</span><span class="sxs-lookup"><span data-stu-id="fdb21-484">**Provide search terms** to help users find your task links using the Control Panel search feature.</span></span>

    <span data-ttu-id="fdb21-485">請注意，您只能為個別的控制台專案提供此資訊，您無法針對您延伸的現有控制台專案新增或變更此資訊。</span><span class="sxs-lookup"><span data-stu-id="fdb21-485">Note that you can provide this information only for individual control panel items you can't add or change this information for existing control panel items that you extend.</span></span>

<span data-ttu-id="fdb21-486">**類別頁面**</span><span class="sxs-lookup"><span data-stu-id="fdb21-486">**Category pages**</span></span>

-   <span data-ttu-id="fdb21-487">**只有在下列情況時，才將 [控制台] 專案新增至 [類別] 頁面：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-487">**Add your control panel item to a category page only if:**</span></span>

    -   <span data-ttu-id="fdb21-488">大部分的使用者都需要它。</span><span class="sxs-lookup"><span data-stu-id="fdb21-488">Most users need it.</span></span> <span data-ttu-id="fdb21-489">範例：網路和共用中心</span><span class="sxs-lookup"><span data-stu-id="fdb21-489">Example: Network and Sharing Center</span></span>
    -   <span data-ttu-id="fdb21-490">使用的次數很多。</span><span class="sxs-lookup"><span data-stu-id="fdb21-490">It is used many times.</span></span> <span data-ttu-id="fdb21-491">範例：系統</span><span class="sxs-lookup"><span data-stu-id="fdb21-491">Example: System</span></span>
    -   <span data-ttu-id="fdb21-492">它提供的重要功能不會在其他地方公開。</span><span class="sxs-lookup"><span data-stu-id="fdb21-492">It provides important functionality that isn't exposed elsewhere.</span></span> <span data-ttu-id="fdb21-493">範例：印表機</span><span class="sxs-lookup"><span data-stu-id="fdb21-493">Example: Printers</span></span>

    <span data-ttu-id="fdb21-494">符合這些準則的控制台專案稱為最上層。</span><span class="sxs-lookup"><span data-stu-id="fdb21-494">Control panel items that meet these criteria are referred to as top level.</span></span>

-   <span data-ttu-id="fdb21-495">**如果有下列情況，請不要將您的控制台專案加入至類別頁面：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-495">**Don't add your control panel item to a category page if:**</span></span>

    -   <span data-ttu-id="fdb21-496">它很少使用或用於一次性設定。</span><span class="sxs-lookup"><span data-stu-id="fdb21-496">It is rarely used or used for one-time setup.</span></span> <span data-ttu-id="fdb21-497">範例：歡迎中心</span><span class="sxs-lookup"><span data-stu-id="fdb21-497">Example: Welcome Center</span></span>
    -   <span data-ttu-id="fdb21-498">其目標物件為 advanced users 或 IT 專業人員。</span><span class="sxs-lookup"><span data-stu-id="fdb21-498">It is targeted at advanced users or IT professionals.</span></span> <span data-ttu-id="fdb21-499">範例：色彩管理</span><span class="sxs-lookup"><span data-stu-id="fdb21-499">Example: Color Management</span></span>
    -   <span data-ttu-id="fdb21-500">不適用於目前的硬體或軟體設定。</span><span class="sxs-lookup"><span data-stu-id="fdb21-500">It doesn't apply to the current hardware or software configuration.</span></span> <span data-ttu-id="fdb21-501">範例： Windows SideShow (目前的硬體) 不支援的情況。</span><span class="sxs-lookup"><span data-stu-id="fdb21-501">Example: Windows SideShow (if not supported by current hardware).</span></span>

    <span data-ttu-id="fdb21-502">從類別頁面移除這類的 [控制台] 專案，可讓您更輕鬆地尋找最上層專案。</span><span class="sxs-lookup"><span data-stu-id="fdb21-502">Removing such control panel items from the category pages makes the top-level items easier to find.</span></span> <span data-ttu-id="fdb21-503">由於這些控制台專案的使用方式，可以透過搜尋或內容進入點充分探索。</span><span class="sxs-lookup"><span data-stu-id="fdb21-503">Given their usage, these control panel items are sufficiently discoverable through search or contextual entry points.</span></span>

-   <span data-ttu-id="fdb21-504">**將最上層的 [控制台] 專案與使用者最有可能尋找的類別建立關聯。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-504">**Associate your top-level control panel item with the category under which users are most likely to look for it.**</span></span> <span data-ttu-id="fdb21-505">此決定應以使用者測試為基礎。</span><span class="sxs-lookup"><span data-stu-id="fdb21-505">This decision should be based on user testing.</span></span>
-   <span data-ttu-id="fdb21-506">**請考慮將最上層的控制台專案與第二個最可能的類別建立關聯。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-506">**Consider associating your top-level control panel item with the second most likely category as well.**</span></span> <span data-ttu-id="fdb21-507">如果使用者可能會在多個位置中尋找其主要工作，您應該將 [控制台] 專案與兩個類別建立關聯。</span><span class="sxs-lookup"><span data-stu-id="fdb21-507">You should associate a control panel item with two categories if users are likely to look for its main tasks in more than one place.</span></span>
-   <span data-ttu-id="fdb21-508">**請勿將您的 [控制台] 專案與兩個以上的類別建立關聯。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-508">**Don't associate your control panel item with more than two categories.**</span></span> <span data-ttu-id="fdb21-509">如果 [控制台] 專案出現在數個類別中，則分類的值為破壞。</span><span class="sxs-lookup"><span data-stu-id="fdb21-509">The value of the categorization is undermined if control panel items appear in several categories.</span></span>

<span data-ttu-id="fdb21-510">**工作連結**</span><span class="sxs-lookup"><span data-stu-id="fdb21-510">**Task links**</span></span>

-   <span data-ttu-id="fdb21-511">**將您的 [控制台] 專案與其主要工作產生關聯。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-511">**Associate your control panel item with its primary tasks.**</span></span> <span data-ttu-id="fdb21-512">您最多可以在 [類別目錄] 頁面上顯示五個工作，但會使用其他工作來進行控制台搜尋作業。</span><span class="sxs-lookup"><span data-stu-id="fdb21-512">You can display up to five tasks on a Category page, but additional tasks are used for control panel searching.</span></span> <span data-ttu-id="fdb21-513">使用與工作連結相同的片語，可能會移除一些文字，讓工作連結更簡潔。</span><span class="sxs-lookup"><span data-stu-id="fdb21-513">Use the same phrasing as you do for task links, possibly removing some words to make the task links more succinct.</span></span>
-   <span data-ttu-id="fdb21-514">**偏好讓工作連結出現在 [控制台] 專案中的不同位置。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-514">**Prefer to have task links lead to different places in your control panel item.**</span></span> <span data-ttu-id="fdb21-515">有多個相同位置的連結可能會造成混淆。</span><span class="sxs-lookup"><span data-stu-id="fdb21-515">Having multiple links to the same place can be confusing.</span></span>

<span data-ttu-id="fdb21-516">**搜尋詞彙**</span><span class="sxs-lookup"><span data-stu-id="fdb21-516">**Search terms**</span></span>

-   <span data-ttu-id="fdb21-517">**註冊您的控制台專案的搜尋詞彙，使用者最有可能用來描述它。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-517">**Register search terms for your control panel item that users are most likely to use to describe it.**</span></span> <span data-ttu-id="fdb21-518">這些搜尋詞彙應包括：</span><span class="sxs-lookup"><span data-stu-id="fdb21-518">These search terms should include:</span></span>

    -   <span data-ttu-id="fdb21-519">已設定的功能或物件。</span><span class="sxs-lookup"><span data-stu-id="fdb21-519">The features or objects configured.</span></span>
    -   <span data-ttu-id="fdb21-520">主要工作。</span><span class="sxs-lookup"><span data-stu-id="fdb21-520">The primary tasks.</span></span>

    <span data-ttu-id="fdb21-521">這些搜尋詞彙應以使用者測試為基礎。</span><span class="sxs-lookup"><span data-stu-id="fdb21-521">These search terms should be based on user testing.</span></span>

-   <span data-ttu-id="fdb21-522">**也包含這些搜尋詞彙的一般同義字。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-522">**Also include common synonyms for these search terms.**</span></span> <span data-ttu-id="fdb21-523">例如，「監視」和「顯示」是同義字，因此應該包含這兩個字。</span><span class="sxs-lookup"><span data-stu-id="fdb21-523">For example, monitor and display are synonyms, so both words should be included.</span></span>
-   <span data-ttu-id="fdb21-524">**包含替代的拼寫或斷詞。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-524">**Include alternative spellings or word breaks.**</span></span> <span data-ttu-id="fdb21-525">例如，使用者可能會搜尋網站和網站。</span><span class="sxs-lookup"><span data-stu-id="fdb21-525">For example, users might search for either web site and website.</span></span> <span data-ttu-id="fdb21-526">也請考慮提供常見的拼寫錯誤。</span><span class="sxs-lookup"><span data-stu-id="fdb21-526">Consider providing common misspellings as well.</span></span>
-   <span data-ttu-id="fdb21-527">**請考慮單數與複數名詞形式。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-527">**Consider singular vs. plural noun forms.**</span></span> <span data-ttu-id="fdb21-528">[控制台] 搜尋功能不會自動搜尋這兩種表單;提供使用者可能搜尋的表單。</span><span class="sxs-lookup"><span data-stu-id="fdb21-528">The control panel search feature doesn't automatically search for both forms; supply the forms for which users are likely to search.</span></span>
-   <span data-ttu-id="fdb21-529">**使用簡單的現在時動詞動詞。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-529">**Use simple present tense verbs.**</span></span> <span data-ttu-id="fdb21-530">如果您將 [連線] 註冊為搜尋字詞，搜尋功能將不會自動尋找連接、連接和連線。</span><span class="sxs-lookup"><span data-stu-id="fdb21-530">If you register connect as a search term, the search feature won't automatically look for connects, connecting, and connected.</span></span>
-   <span data-ttu-id="fdb21-531">**不要擔心大小寫。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-531">**Don't worry about case.**</span></span> <span data-ttu-id="fdb21-532">搜尋功能不區分大小寫。</span><span class="sxs-lookup"><span data-stu-id="fdb21-532">The search feature is not case-sensitive.</span></span>

### <a name="standard-users-and-protected-administrators"></a><span data-ttu-id="fdb21-533">標準使用者和受保護的系統管理員</span><span class="sxs-lookup"><span data-stu-id="fdb21-533">Standard users and Protected administrators</span></span>

<span data-ttu-id="fdb21-534">**許多設定都需要系統管理員許可權才能變更。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-534">**Many settings require administrator privileges to change.**</span></span> <span data-ttu-id="fdb21-535">如果進程需要系統管理員許可權，則 Windows Vista 和更新版本需要 [標準使用者](glossary.md) 和 [受保護](glossary.md) 的系統管理員明確地提升其許可權。</span><span class="sxs-lookup"><span data-stu-id="fdb21-535">If a process requires administrator privileges, Windows Vista and later requires [Standard users](glossary.md) and [Protected administrators](glossary.md) to elevate their privileges explicitly.</span></span> <span data-ttu-id="fdb21-536">這樣做有助於防止惡意程式碼以系統管理員許可權執行。</span><span class="sxs-lookup"><span data-stu-id="fdb21-536">Doing so helps prevent malicious code from running with administrator privileges.</span></span>

<span data-ttu-id="fdb21-537">如需詳細資訊和範例，請參閱 [使用者帳戶控制](winenv-uac.md)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-537">For more information and examples, see [User Account Control](winenv-uac.md).</span></span>

### <a name="schemes-and-themes"></a><span data-ttu-id="fdb21-538">配置和主題</span><span class="sxs-lookup"><span data-stu-id="fdb21-538">Schemes and themes</span></span>

<span data-ttu-id="fdb21-539">配置是視覺設定的命名集合。</span><span class="sxs-lookup"><span data-stu-id="fdb21-539">A scheme is a named collection of visual settings.</span></span> <span data-ttu-id="fdb21-540">主題是跨系統設定的命名集合。</span><span class="sxs-lookup"><span data-stu-id="fdb21-540">A theme is a named collection of settings across the system.</span></span> <span data-ttu-id="fdb21-541">配置和主題的範例包括顯示、滑鼠、電話和數據機、電源選項，以及音效和音訊選項。</span><span class="sxs-lookup"><span data-stu-id="fdb21-541">Examples of schemes and themes include Display, Mouse, Phone and Modem, Power Options, and Sound and Audio Options.</span></span>

-   <span data-ttu-id="fdb21-542">**允許使用者在下列情況建立配置：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-542">**Allow users to create schemes when:**</span></span>

    -   <span data-ttu-id="fdb21-543">**使用者很可能會變更設定。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-543">**Users are likely to change the settings.**</span></span>
    -   <span data-ttu-id="fdb21-544">**使用者最有可能將設定變更為集合。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-544">**Users are most likely to change settings as a collection.**</span></span>

    <span data-ttu-id="fdb21-545">當使用者位於不同的環境（例如 (國家/地區、時區) 的不同實體位置）時，配置會很有用;在不同的情況下使用其電腦 (電池、停駐/取消插接) ;或者，將電腦用於不同的功能 (簡報、影片播放) 。</span><span class="sxs-lookup"><span data-stu-id="fdb21-545">Schemes are useful when users are in a different environment, such as a different physical location (country/region, time zone); using their computer in a different situation (on batteries, docked/undocked); or using their computer for a different function (presentations, video playback).</span></span>

-   <span data-ttu-id="fdb21-546">**至少提供一個預設配置。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-546">**Provide at least one default scheme.**</span></span> <span data-ttu-id="fdb21-547">在大多數情況下，預設配置應妥善命名並套用至大部分的使用者。</span><span class="sxs-lookup"><span data-stu-id="fdb21-547">The default scheme should be well named and apply to most users in most circumstances.</span></span> <span data-ttu-id="fdb21-548">使用者不應該建立自己的配置。</span><span class="sxs-lookup"><span data-stu-id="fdb21-548">Users shouldn't have to create a scheme of their own.</span></span>
-   <span data-ttu-id="fdb21-549">**提供預覽** 或其他機制，讓使用者可以查看配置內的設定。</span><span class="sxs-lookup"><span data-stu-id="fdb21-549">**Provide a preview** or other mechanism so that users can see the settings within the scheme.</span></span>

    ![<span data-ttu-id="fdb21-550">[個人化] 對話方塊的螢幕擷取畫面</span><span class="sxs-lookup"><span data-stu-id="fdb21-550">screen shot of personalization dialog box</span></span> ](images/winenv-ctrl-panels-image13.png)

    <span data-ttu-id="fdb21-551">在此範例中，[個人化] 控制台專案會顯示桌面和外觀設定的預覽。</span><span class="sxs-lookup"><span data-stu-id="fdb21-551">In this example, the Personalization control panel item shows a preview of the desktop and appearance settings.</span></span>

-   <span data-ttu-id="fdb21-552">**提供 [另存新檔] 和 [刪除] 命令。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-552">**Provide Save As and Delete commands.**</span></span> <span data-ttu-id="fdb21-553">重新命名命令不是必要的，使用者可以藉由儲存所需的名稱並刪除原始配置來重新命名配置。</span><span class="sxs-lookup"><span data-stu-id="fdb21-553">A rename command isn't necessary users can rename schemes by saving under the desired name and deleting the original scheme.</span></span>
-   <span data-ttu-id="fdb21-554">如果無法在沒有配置的情況下套用設定，則 **不允許使用者刪除所有配置。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-554">If the settings can't be applied without a scheme, **don't allow users to delete all the schemes.**</span></span> <span data-ttu-id="fdb21-555">使用者不應該建立自己的配置。</span><span class="sxs-lookup"><span data-stu-id="fdb21-555">Users shouldn't have to create a scheme of their own.</span></span>
-   <span data-ttu-id="fdb21-556">如果配置不是完全獨立的 (例如，電源配置相依于目前的膝上型電腦模式) ，請 **確定有一個簡單的方法可變更套用到所有配置的設定。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-556">If the schemes are not completely independent (for example, power schemes depend upon the current laptop mode of operation), **make sure there is an easy way to change settings that apply across all schemes.**</span></span> <span data-ttu-id="fdb21-557">例如，使用電源配置時，請確定使用者可以設定在單一位置中關閉可攜式電腦的蓋時所發生的情況。</span><span class="sxs-lookup"><span data-stu-id="fdb21-557">For example with power schemes, make sure that users can set what happens when a portable computer's lid is closed in a single location.</span></span>

### <a name="miscellaneous"></a><span data-ttu-id="fdb21-558">其他</span><span class="sxs-lookup"><span data-stu-id="fdb21-558">Miscellaneous</span></span>

-   <span data-ttu-id="fdb21-559">**使用主控台擴充功能來取代或擴充現有的 Windows 功能。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-559">**Use Control Panel extensions for features that replace or extend existing Windows functionality.**</span></span> <span data-ttu-id="fdb21-560">下列控制台專案可延伸：藍牙裝置、顯示器、網際網路、鍵盤、滑鼠、網路、電源、系統、無線 (紅外線) 。</span><span class="sxs-lookup"><span data-stu-id="fdb21-560">The following control panel items are extensible: Bluetooth Devices, Display, Internet, Keyboard, Mouse, Network, Power, System, Wireless (infrared).</span></span>

### <a name="default-values"></a><span data-ttu-id="fdb21-561">預設值</span><span class="sxs-lookup"><span data-stu-id="fdb21-561">Default values</span></span>

-   <span data-ttu-id="fdb21-562">**[控制台] 專案內的設定必須反映功能的目前狀態。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-562">**The settings within a control panel item must reflect the current state of the feature.**</span></span> <span data-ttu-id="fdb21-563">否則會產生誤導，可能會導致不想要的結果。</span><span class="sxs-lookup"><span data-stu-id="fdb21-563">Doing otherwise would be misleading and possibly lead to undesired results.</span></span> <span data-ttu-id="fdb21-564">例如，如果設定反映出建議，而不是目前的狀態，則使用者可能會按一下 [取消]，而不是進行變更，以為不需要任何變更。</span><span class="sxs-lookup"><span data-stu-id="fdb21-564">For example, if settings reflect the recommendations but not the current state, users might click Cancel instead of making changes, thinking that no changes are needed.</span></span>
-   <span data-ttu-id="fdb21-565">**選擇最安全的 (，以防止資料遺失或系統存取) 和最安全的初始狀態。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-565">**Choose the safest (to prevent loss of data or system access) and most secure initial state.**</span></span> <span data-ttu-id="fdb21-566">假設大部分的使用者不會變更設定。</span><span class="sxs-lookup"><span data-stu-id="fdb21-566">Assume that most users won't change the settings.</span></span>
-   <span data-ttu-id="fdb21-567">**如果安全性和安全性不是因素，請選擇最有可能或方便的初始狀態。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-567">**If safety and security aren't factors, choose the initial state that is most likely or convenient.**</span></span>

## <a name="text"></a><span data-ttu-id="fdb21-568">Text</span><span class="sxs-lookup"><span data-stu-id="fdb21-568">Text</span></span>

### <a name="item-names"></a><span data-ttu-id="fdb21-569">專案名稱</span><span class="sxs-lookup"><span data-stu-id="fdb21-569">Item names</span></span>

-   <span data-ttu-id="fdb21-570">**選擇明確通訊的描述性名稱，並區分 [控制台] 專案的用途。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-570">**Choose a descriptive name that clearly communicates and differentiates what the control panel item does.**</span></span> <span data-ttu-id="fdb21-571">大部分的名稱會描述要設定的 Windows 功能或物件，並顯示在 [控制台] 首頁的 [傳統] 視圖中。</span><span class="sxs-lookup"><span data-stu-id="fdb21-571">Most names describe the Windows feature or object being configured, and are displayed in the Classic View of the control panel home page.</span></span>
-   <span data-ttu-id="fdb21-572">**請勿在名稱中包含單字「設定」、「選項」、「屬性」或「設定」。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-572">**Don't include the words "Settings," "Options," "Properties," or "Configuration" in the name.**</span></span> <span data-ttu-id="fdb21-573">這是隱含的，而不是讓使用者能夠更輕鬆地進行掃描。</span><span class="sxs-lookup"><span data-stu-id="fdb21-573">This is implied, and leaving it off makes it easier for users to scan.</span></span>

    <span data-ttu-id="fdb21-574">**不正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-574">**Incorrect:**</span></span>

    <span data-ttu-id="fdb21-575">協助工具選項</span><span class="sxs-lookup"><span data-stu-id="fdb21-575">Accessibility Options</span></span>

    <span data-ttu-id="fdb21-576">數據機設定</span><span class="sxs-lookup"><span data-stu-id="fdb21-576">Modem Settings</span></span>

    <span data-ttu-id="fdb21-577">電源選項</span><span class="sxs-lookup"><span data-stu-id="fdb21-577">Power Options</span></span>

    <span data-ttu-id="fdb21-578">地區及語言選項</span><span class="sxs-lookup"><span data-stu-id="fdb21-578">Regional and Language Options</span></span>

    <span data-ttu-id="fdb21-579">**正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-579">**Correct:**</span></span>

    <span data-ttu-id="fdb21-580">協助工具選項</span><span class="sxs-lookup"><span data-stu-id="fdb21-580">Accessibility</span></span>

    <span data-ttu-id="fdb21-581">數據機</span><span class="sxs-lookup"><span data-stu-id="fdb21-581">Modem</span></span>

    <span data-ttu-id="fdb21-582">電源</span><span class="sxs-lookup"><span data-stu-id="fdb21-582">Power</span></span>

    <span data-ttu-id="fdb21-583">地區格式和語言</span><span class="sxs-lookup"><span data-stu-id="fdb21-583">Regional Formats and Languages</span></span>

    <span data-ttu-id="fdb21-584">在正確的範例中，會移除不必要的字組。</span><span class="sxs-lookup"><span data-stu-id="fdb21-584">In the correct examples, unnecessary words are removed.</span></span>

-   <span data-ttu-id="fdb21-585">**如果 [控制台] 專案設定了相關的功能，只會列出識別該專案所需的功能，並列出最可能先辨識或使用的功能。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-585">**If the control panel item configures related features, list only those features required to identify the item, and list the features the most likely to be recognized or used first.**</span></span>

    <span data-ttu-id="fdb21-586">**不正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-586">**Incorrect:**</span></span>

    <span data-ttu-id="fdb21-587">資料夾選項</span><span class="sxs-lookup"><span data-stu-id="fdb21-587">Folder Options</span></span>

    <span data-ttu-id="fdb21-588">電話和數據機選項</span><span class="sxs-lookup"><span data-stu-id="fdb21-588">Phone and Modem Options</span></span>

    <span data-ttu-id="fdb21-589">**正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-589">**Correct:**</span></span>

    <span data-ttu-id="fdb21-590">檔案和資料夾</span><span class="sxs-lookup"><span data-stu-id="fdb21-590">Files and Folders</span></span>

    <span data-ttu-id="fdb21-591">數據機</span><span class="sxs-lookup"><span data-stu-id="fdb21-591">Modem</span></span>

    <span data-ttu-id="fdb21-592">在正確的範例中，主要的 [控制台] 專案功能會獲得強調。</span><span class="sxs-lookup"><span data-stu-id="fdb21-592">In the correct examples, the primary control panel item features are given emphasis.</span></span>

-   <span data-ttu-id="fdb21-593">使用 [標題樣式的大小寫](glossary.md)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-593">Use [title-style capitalization](glossary.md).</span></span>

### <a name="page-titles"></a><span data-ttu-id="fdb21-594">頁面標題</span><span class="sxs-lookup"><span data-stu-id="fdb21-594">Page titles</span></span>

<span data-ttu-id="fdb21-595">**注意：** 如同所有 Explorer 視窗，[控制台] 頁面標題會顯示在 [網址](glossary.md)列上，但不會顯示在標題列上。</span><span class="sxs-lookup"><span data-stu-id="fdb21-595">**Note:** As with all Explorer windows, control panel page titles are displayed on the [address bar](glossary.md), but not the title bar.</span></span>

-   <span data-ttu-id="fdb21-596">**若為中樞頁面，請使用 [控制台] 專案名稱。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-596">**For hub pages, use the control panel item name.**</span></span>
-   <span data-ttu-id="fdb21-597">**針對輪輻頁面，請使用頁面的簡短摘要。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-597">**For spoke pages, use a concise summary of the page's purpose.**</span></span> <span data-ttu-id="fdb21-598">如果它很簡潔，請使用頁面的主要指示;否則，請使用主要指令的精確重述。</span><span class="sxs-lookup"><span data-stu-id="fdb21-598">Use the page's main instruction if it is concise; otherwise use a concise restatement of the main instruction.</span></span>

    ![<span data-ttu-id="fdb21-599">[電源選項] 對話方塊的螢幕擷取畫面</span><span class="sxs-lookup"><span data-stu-id="fdb21-599">screen shot of power options dialog box</span></span> ](images/winenv-ctrl-panels-image6.png)

    <span data-ttu-id="fdb21-600">在此範例中，電源選項用於頁面標題，而不是主要指令。</span><span class="sxs-lookup"><span data-stu-id="fdb21-600">In this example, Power Options is used for the page title instead of the main instruction.</span></span>

-   <span data-ttu-id="fdb21-601">使用標題樣式的大小寫。</span><span class="sxs-lookup"><span data-stu-id="fdb21-601">Use title-style capitalization.</span></span>

### <a name="task-link-text"></a><span data-ttu-id="fdb21-602">工作連結文字</span><span class="sxs-lookup"><span data-stu-id="fdb21-602">Task link text</span></span>

<span data-ttu-id="fdb21-603">下列指導方針適用于工作頁面的連結，例如類別頁面工作連結及另請參閱連結。</span><span class="sxs-lookup"><span data-stu-id="fdb21-603">The following guidelines apply to links to task pages, such as Category page task links and See also links.</span></span>

-   <span data-ttu-id="fdb21-604">**選擇清楚傳達和區分工作的精確工作名稱。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-604">**Choose concise task names that clearly communicate and differentiate the task.**</span></span> <span data-ttu-id="fdb21-605">使用者不應該需要找出工作真正的意義，或與其他工作有何不同。</span><span class="sxs-lookup"><span data-stu-id="fdb21-605">Users shouldn't have to figure out what the task really means or how it differs from other tasks.</span></span>

    <span data-ttu-id="fdb21-606">**不正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-606">**Incorrect:**</span></span>

    <span data-ttu-id="fdb21-607">調整顯示設定</span><span class="sxs-lookup"><span data-stu-id="fdb21-607">Adjust display settings</span></span>

    <span data-ttu-id="fdb21-608">**正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-608">**Correct:**</span></span>

    <span data-ttu-id="fdb21-609">螢幕解析度</span><span class="sxs-lookup"><span data-stu-id="fdb21-609">Screen resolution</span></span>

    <span data-ttu-id="fdb21-610">在正確的範例中，文字表達的精確度更高。</span><span class="sxs-lookup"><span data-stu-id="fdb21-610">In the correct example, the wording conveys more precision.</span></span>

-   <span data-ttu-id="fdb21-611">**在工作連結與它們所指向的頁面之間保留類似的語言。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-611">**Retain similar language between task links and the pages they point to.**</span></span> <span data-ttu-id="fdb21-612">連結所顯示的頁面應該不會讓使用者感到驚訝。</span><span class="sxs-lookup"><span data-stu-id="fdb21-612">Users shouldn't be surprised by the page that is displayed by a link.</span></span>
-   <span data-ttu-id="fdb21-613">**針對工作頁面，請將主要指示、認可按鈕和工作連結設計為一組相關的文字。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-613">**For task pages, design the main instruction, commit buttons, and task links as a related set of text.**</span></span>

    <span data-ttu-id="fdb21-614">**範例：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-614">**Examples:**</span></span>

    

    |                              |                                                       |
    |------------------------------|-------------------------------------------------------|
    | <span data-ttu-id="fdb21-615">工作連結：</span><span class="sxs-lookup"><span data-stu-id="fdb21-615">Task link:</span></span><br/>        | <span data-ttu-id="fdb21-616">連接到無線網路</span><span class="sxs-lookup"><span data-stu-id="fdb21-616">Connect to a wireless network</span></span><br/>              |
    | <span data-ttu-id="fdb21-617">主要指示：</span><span class="sxs-lookup"><span data-stu-id="fdb21-617">Main instruction:</span></span><br/> | <span data-ttu-id="fdb21-618">選擇要連接的網路</span><span class="sxs-lookup"><span data-stu-id="fdb21-618">Choose a network to connect to</span></span><br/>             |
    | <span data-ttu-id="fdb21-619">認可按鈕：</span><span class="sxs-lookup"><span data-stu-id="fdb21-619">Commit button:</span></span><br/>    | <span data-ttu-id="fdb21-620">連線</span><span class="sxs-lookup"><span data-stu-id="fdb21-620">Connect</span></span><br/>                                    |
    | <span data-ttu-id="fdb21-621">工作連結：</span><span class="sxs-lookup"><span data-stu-id="fdb21-621">Task link:</span></span><br/>        | <span data-ttu-id="fdb21-622">設定家長監護控制項</span><span class="sxs-lookup"><span data-stu-id="fdb21-622">Set up parental controls</span></span><br/>                   |
    | <span data-ttu-id="fdb21-623">主要指示：</span><span class="sxs-lookup"><span data-stu-id="fdb21-623">Main instruction:</span></span><br/> | <span data-ttu-id="fdb21-624">選擇使用者並設定家長監護控制項</span><span class="sxs-lookup"><span data-stu-id="fdb21-624">Choose a user and set up parental controls</span></span><br/> |
    | <span data-ttu-id="fdb21-625">認可按鈕：</span><span class="sxs-lookup"><span data-stu-id="fdb21-625">Commit button:</span></span><br/>    | <span data-ttu-id="fdb21-626">套用家長監護</span><span class="sxs-lookup"><span data-stu-id="fdb21-626">Apply parental control</span></span><br/>                     |
    | <span data-ttu-id="fdb21-627">工作連結：</span><span class="sxs-lookup"><span data-stu-id="fdb21-627">Task link:</span></span><br/>        | <span data-ttu-id="fdb21-628">解決同步衝突</span><span class="sxs-lookup"><span data-stu-id="fdb21-628">Resolve your sync conflicts</span></span><br/>                |
    | <span data-ttu-id="fdb21-629">主要指示：</span><span class="sxs-lookup"><span data-stu-id="fdb21-629">Main instruction:</span></span><br/> | <span data-ttu-id="fdb21-630">您要如何解決這種衝突？</span><span class="sxs-lookup"><span data-stu-id="fdb21-630">How do you want to resolve this conflict?</span></span><br/>  |
    | <span data-ttu-id="fdb21-631">認可按鈕：</span><span class="sxs-lookup"><span data-stu-id="fdb21-631">Commit button:</span></span><br/>    | <span data-ttu-id="fdb21-632">解決</span><span class="sxs-lookup"><span data-stu-id="fdb21-632">Resolve</span></span><br/>                                    |

    

     

    <span data-ttu-id="fdb21-633">這些範例顯示工作連結文字、主要指令和認可按鈕文字的關聯性。</span><span class="sxs-lookup"><span data-stu-id="fdb21-633">These examples show the relationship of the task link text, main instruction, and commit button text.</span></span>

-   <span data-ttu-id="fdb21-634">雖然工作通常是以動詞開頭，但 **如果它是泛型、設定相關的動詞，而不能協助傳達工作，請考慮省略類別頁面上的動詞命令。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-634">While tasks often start with verbs, **consider omitting the verb on Category pages if it is a generic, configuration-related verb that doesn't help communicate the task.**</span></span>

    <span data-ttu-id="fdb21-635">**特定且有用的動詞：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-635">**Specific, helpful verbs:**</span></span>

    <span data-ttu-id="fdb21-636">加</span><span class="sxs-lookup"><span data-stu-id="fdb21-636">Add</span></span>

    <span data-ttu-id="fdb21-637">勾選</span><span class="sxs-lookup"><span data-stu-id="fdb21-637">Check</span></span>

    <span data-ttu-id="fdb21-638">連線</span><span class="sxs-lookup"><span data-stu-id="fdb21-638">Connect</span></span>

    <span data-ttu-id="fdb21-639">複製</span><span class="sxs-lookup"><span data-stu-id="fdb21-639">Copy</span></span>

    <span data-ttu-id="fdb21-640">建立</span><span class="sxs-lookup"><span data-stu-id="fdb21-640">Create</span></span>

    <span data-ttu-id="fdb21-641">刪除</span><span class="sxs-lookup"><span data-stu-id="fdb21-641">Delete</span></span>

    <span data-ttu-id="fdb21-642">中斷連線</span><span class="sxs-lookup"><span data-stu-id="fdb21-642">Disconnect</span></span>

    <span data-ttu-id="fdb21-643">安裝</span><span class="sxs-lookup"><span data-stu-id="fdb21-643">Install</span></span>

    <span data-ttu-id="fdb21-644">移除</span><span class="sxs-lookup"><span data-stu-id="fdb21-644">Remove</span></span>

    <span data-ttu-id="fdb21-645">設定</span><span class="sxs-lookup"><span data-stu-id="fdb21-645">Set up</span></span>

    <span data-ttu-id="fdb21-646">開始</span><span class="sxs-lookup"><span data-stu-id="fdb21-646">Start</span></span>

    <span data-ttu-id="fdb21-647">Stop</span><span class="sxs-lookup"><span data-stu-id="fdb21-647">Stop</span></span>

    <span data-ttu-id="fdb21-648">疑難排解</span><span class="sxs-lookup"><span data-stu-id="fdb21-648">Troubleshoot</span></span>

    <span data-ttu-id="fdb21-649">**泛型、無用動詞：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-649">**Generic, unhelpful verbs:**</span></span>

    <span data-ttu-id="fdb21-650">Adjust</span><span class="sxs-lookup"><span data-stu-id="fdb21-650">Adjust</span></span>

    <span data-ttu-id="fdb21-651">變更</span><span class="sxs-lookup"><span data-stu-id="fdb21-651">Change</span></span>

    <span data-ttu-id="fdb21-652">Choose</span><span class="sxs-lookup"><span data-stu-id="fdb21-652">Choose</span></span>

    <span data-ttu-id="fdb21-653">設定</span><span class="sxs-lookup"><span data-stu-id="fdb21-653">Configure</span></span>

    <span data-ttu-id="fdb21-654">編輯</span><span class="sxs-lookup"><span data-stu-id="fdb21-654">Edit</span></span>

    <span data-ttu-id="fdb21-655">管理</span><span class="sxs-lookup"><span data-stu-id="fdb21-655">Manage</span></span>

    <span data-ttu-id="fdb21-656">開啟</span><span class="sxs-lookup"><span data-stu-id="fdb21-656">Open</span></span>

    <span data-ttu-id="fdb21-657">挑選</span><span class="sxs-lookup"><span data-stu-id="fdb21-657">Pick</span></span>

    <span data-ttu-id="fdb21-658">設定</span><span class="sxs-lookup"><span data-stu-id="fdb21-658">Set</span></span>

    <span data-ttu-id="fdb21-659">選取</span><span class="sxs-lookup"><span data-stu-id="fdb21-659">Select</span></span>

    <span data-ttu-id="fdb21-660">顯示</span><span class="sxs-lookup"><span data-stu-id="fdb21-660">Show</span></span>

    <span data-ttu-id="fdb21-661">檢視</span><span class="sxs-lookup"><span data-stu-id="fdb21-661">View</span></span>

-   <span data-ttu-id="fdb21-662">**如果工作設定數個相關的功能，只會列出代表該集合的功能。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-662">**If the task configures several related features, list only the features that are representative of the set.**</span></span> <span data-ttu-id="fdb21-663">省略可以立即推斷的詳細資料。</span><span class="sxs-lookup"><span data-stu-id="fdb21-663">Omit details that can be readily inferred.</span></span>

    <span data-ttu-id="fdb21-664">**不正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-664">**Incorrect:**</span></span>

    <span data-ttu-id="fdb21-665">喇叭音量、靜音、音量圖示</span><span class="sxs-lookup"><span data-stu-id="fdb21-665">Speaker volume, mute, volume icon</span></span>

    <span data-ttu-id="fdb21-666">喇叭、麥克風、MIDI 和音效方案</span><span class="sxs-lookup"><span data-stu-id="fdb21-666">Speakers, microphones, MIDI, and sound schemes</span></span>

    <span data-ttu-id="fdb21-667">**正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-667">**Correct:**</span></span>

    <span data-ttu-id="fdb21-668">喇叭音量</span><span class="sxs-lookup"><span data-stu-id="fdb21-668">Speaker volume</span></span>

    <span data-ttu-id="fdb21-669">喇叭和麥克風</span><span class="sxs-lookup"><span data-stu-id="fdb21-669">Speakers and microphones</span></span>

    <span data-ttu-id="fdb21-670">在正確的範例中，工作連結中只會提供代表性的功能。</span><span class="sxs-lookup"><span data-stu-id="fdb21-670">In the correct examples, only the representative features are given in the task link.</span></span>

-   <span data-ttu-id="fdb21-671">**只有當目標使用者這麼做時，才應該將工作以技術為依據。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-671">**You should phrase tasks in terms of technology only if target users would do so as well.**</span></span>

    <span data-ttu-id="fdb21-672">**不正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-672">**Incorrect:**</span></span>

    <span data-ttu-id="fdb21-673">Connectoids</span><span class="sxs-lookup"><span data-stu-id="fdb21-673">Connectoids</span></span>

    <span data-ttu-id="fdb21-674">圖元深度</span><span class="sxs-lookup"><span data-stu-id="fdb21-674">Pixel depth</span></span>

    <span data-ttu-id="fdb21-675">DPI</span><span class="sxs-lookup"><span data-stu-id="fdb21-675">dpi</span></span>

    <span data-ttu-id="fdb21-676">**正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-676">**Correct:**</span></span>

    <span data-ttu-id="fdb21-677">印表機</span><span class="sxs-lookup"><span data-stu-id="fdb21-677">Printers</span></span>

    <span data-ttu-id="fdb21-678">掃描器</span><span class="sxs-lookup"><span data-stu-id="fdb21-678">Scanners</span></span>

    <span data-ttu-id="fdb21-679">滑鼠</span><span class="sxs-lookup"><span data-stu-id="fdb21-679">Mouse</span></span>

    <span data-ttu-id="fdb21-680">正確的範例是以技術為基礎的目標使用者可能會使用的詞彙，而不正確的範例則不正確。</span><span class="sxs-lookup"><span data-stu-id="fdb21-680">The correct examples are technology-based terms that target users are likely to use, whereas the incorrect examples are not.</span></span>

-   <span data-ttu-id="fdb21-681">只有在系統可以支援一個以上時，才使用複數名詞。</span><span class="sxs-lookup"><span data-stu-id="fdb21-681">Use plural nouns only if the system can support more than one.</span></span>
-   <span data-ttu-id="fdb21-682">使用 [句子樣式的大小寫](glossary.md)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-682">Use [sentence-style capitalization](glossary.md).</span></span>
-   <span data-ttu-id="fdb21-683">請勿使用結束標點符號。</span><span class="sxs-lookup"><span data-stu-id="fdb21-683">Don't use ending punctuation.</span></span>

### <a name="main-instructions"></a><span data-ttu-id="fdb21-684">主要指示</span><span class="sxs-lookup"><span data-stu-id="fdb21-684">Main instructions</span></span>

-   <span data-ttu-id="fdb21-685">**在 [中樞] 頁面上，使用主要指示，透過 [控制台] 專案來說明使用者的目標。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-685">**For the hub page, use the main instruction to explain the user's objective with the control panel item.**</span></span> <span data-ttu-id="fdb21-686">主要指示應可協助使用者判斷他們是否已選取正確的控制台專案。</span><span class="sxs-lookup"><span data-stu-id="fdb21-686">The main instruction should help users determine if they have selected the right control panel item.</span></span> <span data-ttu-id="fdb21-687">請記住，使用者可能已在錯誤中選取您的 [控制台] 專案，而且實際上正在尋找屬於其他 [控制台] 專案的設定。</span><span class="sxs-lookup"><span data-stu-id="fdb21-687">Keep in mind that users might have selected your control panel item in error and are actually looking for settings that are part of another control panel item.</span></span>

    <span data-ttu-id="fdb21-688">**範例：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-688">**Examples:**</span></span>

    <span data-ttu-id="fdb21-689">保持電腦的安全和最新狀態</span><span class="sxs-lookup"><span data-stu-id="fdb21-689">Keep your computer secure and up-to-date</span></span>

    <span data-ttu-id="fdb21-690">優化您的電腦，以便更輕鬆地查看、聆聽及控制</span><span class="sxs-lookup"><span data-stu-id="fdb21-690">Optimize your computer so it's easier to see, hear, and control</span></span>

-   <span data-ttu-id="fdb21-691">**針對輪輻頁面，請使用主要指示來說明頁面上要執行的動作。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-691">**For spoke pages, use the main instruction to explain what to do on the page.**</span></span> <span data-ttu-id="fdb21-692">指令應該是特定的語句、命令式方向或問題。</span><span class="sxs-lookup"><span data-stu-id="fdb21-692">The instruction should be a specific statement, imperative direction, or question.</span></span> <span data-ttu-id="fdb21-693">良好的指示會以頁面傳達使用者的目標，而不是單純專注于操作它的機制。</span><span class="sxs-lookup"><span data-stu-id="fdb21-693">Good instructions communicate the user's objective with the page rather than focusing purely on the mechanics of manipulating it.</span></span>

    <span data-ttu-id="fdb21-694">**不正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-694">**Incorrect:**</span></span>

    <span data-ttu-id="fdb21-695">挑選通知工作</span><span class="sxs-lookup"><span data-stu-id="fdb21-695">Pick a notification task</span></span>

    <span data-ttu-id="fdb21-696">**正確：**</span><span class="sxs-lookup"><span data-stu-id="fdb21-696">**Correct:**</span></span>

    <span data-ttu-id="fdb21-697">指出如何處理連入資訊</span><span class="sxs-lookup"><span data-stu-id="fdb21-697">Indicate how to handle incoming information</span></span>

    <span data-ttu-id="fdb21-698">正確的版本會更妥善地傳達頁面所達到的目標。</span><span class="sxs-lookup"><span data-stu-id="fdb21-698">The correct version better communicates the goal achieved by the page.</span></span>

-   <span data-ttu-id="fdb21-699">**盡可能使用特定動詞。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-699">**Use specific verbs whenever possible.**</span></span> <span data-ttu-id="fdb21-700">特定動詞對使用者來說更有意義，而不是泛型。</span><span class="sxs-lookup"><span data-stu-id="fdb21-700">Specific verbs are more meaningful to users than generic ones.</span></span>
-   <span data-ttu-id="fdb21-701">使用 [句子樣式的大小寫](glossary.md)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-701">Use [sentence-style capitalization](glossary.md).</span></span>
-   <span data-ttu-id="fdb21-702">**如果指令是語句，則不包含最後句號。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-702">**Don't include final periods if the instruction is a statement.**</span></span> <span data-ttu-id="fdb21-703">如果指令是問題，請包含最後一個問號。</span><span class="sxs-lookup"><span data-stu-id="fdb21-703">If the instruction is a question, include a final question mark.</span></span>

### <a name="supplemental-instructions"></a><span data-ttu-id="fdb21-704">補充指示</span><span class="sxs-lookup"><span data-stu-id="fdb21-704">Supplemental instructions</span></span>

-   <span data-ttu-id="fdb21-705">**針對 [中樞] 頁面，請使用選擇性的補充指示，進一步說明 [控制台] 專案的用途。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-705">**For the hub page, use the optional supplemental instruction to further explain the purpose of the control panel item.**</span></span>
-   <span data-ttu-id="fdb21-706">**若為輪輻頁面，請使用選擇性補充指示來呈現有助於瞭解或使用頁面的其他資訊。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-706">**For spoke pages, use the optional supplemental instruction to present additional information helpful to understanding or using the page.**</span></span> <span data-ttu-id="fdb21-707">您可以提供更詳細的資訊並定義術語。</span><span class="sxs-lookup"><span data-stu-id="fdb21-707">You can provide more detailed information and define terminology.</span></span>
-   <span data-ttu-id="fdb21-708">使用完整句子和 [句子樣式的大小寫](glossary.md)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-708">Use complete sentences and [sentence-style capitalization](glossary.md).</span></span>

### <a name="page-text"></a><span data-ttu-id="fdb21-709">頁面文字</span><span class="sxs-lookup"><span data-stu-id="fdb21-709">Page text</span></span>

-   <span data-ttu-id="fdb21-710">**請勿重新聲明內容區域中的主要指示。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-710">**Don't restate the main instruction in the content area.**</span></span>
-   <span data-ttu-id="fdb21-711">**使用 "page" 一詞來參考頁面本身。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-711">**Use the word "page" to refer to the page itself.**</span></span>
-   <span data-ttu-id="fdb21-712">**使用第二個人 (您的) ，告訴使用者要** 在主要指示和內容區域中執行的動作。</span><span class="sxs-lookup"><span data-stu-id="fdb21-712">**Use the second person (you, your) to tell users what to do** in the main instruction and content area.</span></span> <span data-ttu-id="fdb21-713">通常會隱含第二個人。</span><span class="sxs-lookup"><span data-stu-id="fdb21-713">Often the second person is implied.</span></span>

    <span data-ttu-id="fdb21-714">**範例︰**</span><span class="sxs-lookup"><span data-stu-id="fdb21-714">**Example:**</span></span>

    <span data-ttu-id="fdb21-715">選擇您要列印的圖片。</span><span class="sxs-lookup"><span data-stu-id="fdb21-715">Choose the pictures you want to print.</span></span>

-   <span data-ttu-id="fdb21-716">**使用第一個人 (我，我的) 讓使用者知道要** 在內容區域中執行的動作，以回應主要指示。</span><span class="sxs-lookup"><span data-stu-id="fdb21-716">**Use the first person (I, me, my) to let users tell the control panel item what to do** in the content area that responds to the main instruction.</span></span>

    <span data-ttu-id="fdb21-717">**範例︰**</span><span class="sxs-lookup"><span data-stu-id="fdb21-717">**Example:**</span></span>

    <span data-ttu-id="fdb21-718">列印相機上的相片。</span><span class="sxs-lookup"><span data-stu-id="fdb21-718">Print the photos on my camera.</span></span>

### <a name="task-links"></a><span data-ttu-id="fdb21-719">工作連結</span><span class="sxs-lookup"><span data-stu-id="fdb21-719">Task links</span></span>

-   <span data-ttu-id="fdb21-720">**選擇清楚的連結文字，以明確地傳達和區分工作連結的用途。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-720">**Choose concise link text that clearly communicates and differentiates what the task link does.**</span></span> <span data-ttu-id="fdb21-721">它應該是一目了然的，且對應至主要指令。</span><span class="sxs-lookup"><span data-stu-id="fdb21-721">It should be self-explanatory and correspond to the main instruction.</span></span> <span data-ttu-id="fdb21-722">使用者不應該需要找出連結真正的意義，或與其他連結有何不同。</span><span class="sxs-lookup"><span data-stu-id="fdb21-722">Users shouldn't have to figure out what the link really means or how it differs from other links.</span></span>
-   <span data-ttu-id="fdb21-723">**一律使用動詞啟動工作連結。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-723">**Always start task links with a verb.**</span></span>
-   <span data-ttu-id="fdb21-724">使用 [句子樣式的大小寫](glossary.md)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-724">Use [sentence-style capitalization](glossary.md).</span></span>
-   <span data-ttu-id="fdb21-725">請勿使用結束標點符號。</span><span class="sxs-lookup"><span data-stu-id="fdb21-725">Don't use ending punctuation.</span></span>
-   <span data-ttu-id="fdb21-726">**如果工作連結需要進一步說明，請使用完整的句子和結束標點符號，在個別的文字控制項中提供說明** 。</span><span class="sxs-lookup"><span data-stu-id="fdb21-726">**If the task link requires further explanation, provide the explanation in a separate text control** using complete sentences and ending punctuation.</span></span> <span data-ttu-id="fdb21-727">不過，只有在需要時才新增這類說明，因為另一個工作連結需要一個。</span><span class="sxs-lookup"><span data-stu-id="fdb21-727">However, add such explanations only when needed don't add explanations to all task links because another task link needs one.</span></span>

<span data-ttu-id="fdb21-728">如需詳細資訊和範例，請參閱 [連結](ctrl-command-links.md)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-728">For more information and examples, see [Links](ctrl-command-links.md).</span></span>

### <a name="commit-buttons"></a><span data-ttu-id="fdb21-729">認可按鈕</span><span class="sxs-lookup"><span data-stu-id="fdb21-729">Commit buttons</span></span>

-   <span data-ttu-id="fdb21-730">**使用對自身有意義的特定認可按鈕標籤，並符合主要指令。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-730">**Use specific commit button labels that make sense on their own and match the main instruction.**</span></span> <span data-ttu-id="fdb21-731">在理想的情況下，使用者不需要讀取任何其他人來瞭解標籤。</span><span class="sxs-lookup"><span data-stu-id="fdb21-731">Ideally users shouldn't have to read anything else to understand the label.</span></span> <span data-ttu-id="fdb21-732">使用者比靜態文字更可能讀取命令按鈕標籤。</span><span class="sxs-lookup"><span data-stu-id="fdb21-732">Users are far more likely to read command button labels than static text.</span></span>
-   <span data-ttu-id="fdb21-733">**一律使用動詞來啟動認可按鈕標籤。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-733">**Always start commit button labels with a verb.**</span></span>
-   <span data-ttu-id="fdb21-734">**請勿使用 [關閉]、[完成] 或 [完成]。**</span><span class="sxs-lookup"><span data-stu-id="fdb21-734">**Don't use Close, Done, or Finish.**</span></span> <span data-ttu-id="fdb21-735">這些按鈕標籤更適合其他類型的視窗。</span><span class="sxs-lookup"><span data-stu-id="fdb21-735">These button labels are better suited for other types of windows.</span></span>
-   <span data-ttu-id="fdb21-736">使用 [句子樣式的大小寫](glossary.md)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-736">Use [sentence-style capitalization](glossary.md).</span></span>
-   <span data-ttu-id="fdb21-737">請勿使用結束標點符號。</span><span class="sxs-lookup"><span data-stu-id="fdb21-737">Don't use ending punctuation.</span></span>
-   <span data-ttu-id="fdb21-738">指派唯一的 [存取金鑰](glossary.md)。</span><span class="sxs-lookup"><span data-stu-id="fdb21-738">Assign a unique [access key](glossary.md).</span></span>
    -   <span data-ttu-id="fdb21-739">**例外狀況：** 請勿將存取金鑰指派給 [取消] 按鈕，因為 Esc 是其存取金鑰。</span><span class="sxs-lookup"><span data-stu-id="fdb21-739">**Exception:** Don't assign access keys to Cancel buttons, because Esc is its access key.</span></span> <span data-ttu-id="fdb21-740">這麼做可讓您更輕鬆地指派其他存取金鑰。</span><span class="sxs-lookup"><span data-stu-id="fdb21-740">Doing so makes the other access keys easier to assign.</span></span>

## <a name="documentation"></a><span data-ttu-id="fdb21-741">文件</span><span class="sxs-lookup"><span data-stu-id="fdb21-741">Documentation</span></span>

<span data-ttu-id="fdb21-742">參考 [控制台] 首頁或類別頁面時：</span><span class="sxs-lookup"><span data-stu-id="fdb21-742">When referring to the control panel home page or category pages:</span></span>

-   <span data-ttu-id="fdb21-743">在使用者檔中，請參閱主控台，使用標題樣式的大小寫，並省略前面的明確文章。</span><span class="sxs-lookup"><span data-stu-id="fdb21-743">In user documentation, refer to Control Panel, using title-style capitalization and omitting a preceding definite article the.</span></span>

    <span data-ttu-id="fdb21-744">**範例︰**</span><span class="sxs-lookup"><span data-stu-id="fdb21-744">**Example:**</span></span>

    <span data-ttu-id="fdb21-745">在主控台中，開啟 [ **安全性中心**]。</span><span class="sxs-lookup"><span data-stu-id="fdb21-745">In Control Panel, open **Security Center**.</span></span>

-   <span data-ttu-id="fdb21-746">在程式設計和其他技術檔中，請參閱 [控制台] 首頁和 [控制台類別] 頁面，而不將任何單字變成大寫。</span><span class="sxs-lookup"><span data-stu-id="fdb21-746">In programming and other technical documentation, refer to control panel home page and control panel category page, without capitalizing any of the words.</span></span> <span data-ttu-id="fdb21-747">上述的明確文章是選擇性的。</span><span class="sxs-lookup"><span data-stu-id="fdb21-747">A preceding definite article is optional.</span></span>

<span data-ttu-id="fdb21-748">針對 [控制台] 專案：</span><span class="sxs-lookup"><span data-stu-id="fdb21-748">For control panel items:</span></span>

-   <span data-ttu-id="fdb21-749">參考個別的控制台專案時，請 \[ 在使用者檔中使用「主控台中的控制台專案名稱」 \] 或一般「主控台專案」。</span><span class="sxs-lookup"><span data-stu-id="fdb21-749">When referring to an individual control panel item, use "\[control panel item name\] in Control Panel" or generally "Control Panel item" in user documentation.</span></span> <span data-ttu-id="fdb21-750">請勿使用小程式、程式或控制台來參考個別的控制台專案。</span><span class="sxs-lookup"><span data-stu-id="fdb21-750">Don't use applet, program, or control panel to refer to individual control panel items.</span></span>
-   <span data-ttu-id="fdb21-751">參考控制台專案的 [中樞] 頁面時，請使用 [主要 \[ 控制台專案名稱 \] ] 頁面。</span><span class="sxs-lookup"><span data-stu-id="fdb21-751">When referring to a control panel item's hub page, use "main \[control panel item name\] page."</span></span>
-   <span data-ttu-id="fdb21-752">可能的話，請使用粗體文字來格式化 [控制台] 名稱。</span><span class="sxs-lookup"><span data-stu-id="fdb21-752">When possible, format the control panel name using bold text.</span></span> <span data-ttu-id="fdb21-753">否則，請只在必要時才將名稱放在引號中，以避免混淆。</span><span class="sxs-lookup"><span data-stu-id="fdb21-753">Otherwise, put the name in quotation marks only if required to prevent confusion.</span></span>

<span data-ttu-id="fdb21-754">範例：</span><span class="sxs-lookup"><span data-stu-id="fdb21-754">Examples:</span></span>

-   <span data-ttu-id="fdb21-755">在主控台中，開啟 [ **家長** 監護]。</span><span class="sxs-lookup"><span data-stu-id="fdb21-755">In Control Panel, open **Parental Controls**.</span></span>
-   <span data-ttu-id="fdb21-756">回到主要的 [ **家長** 監護] 頁面。</span><span class="sxs-lookup"><span data-stu-id="fdb21-756">Return to the main **Parental Controls** page.</span></span>
