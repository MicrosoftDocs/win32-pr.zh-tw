...
3.  建立 **Connect**、 **Check** 和 **Disconnect** 回呼函數，並適當地設定 **lpfnConnect**、 **lpfnCheck** 和 **lpfnDisconnect** 資料成員。
    > [!Note]  
...
5.  [一般] 對話方塊接著會搜尋要求的卡片。 如果找到相符的卡片名稱或 [*ATR 字串*](../secgloss/a-gly.md) ，就會依序呼叫 **Connect**、 **Check** 和 **Disconnect** 回呼函數。 如果卡片通過 **檢查** 常式 (亦即， **檢查** 回呼會傳回 **TRUE**) ，則會在顯示中將此卡片反白顯示給使用者。
    > [!Note]  
