...
ms.locfileid: "119141531"
---
# <a name="acquirecredentialshandle-ntlm-function"></a>AcquireCredentialsHandle (NTLM) 函數

**AcquireCredentialsHandle (NTLM)** 函數會取得 [*安全性主體*](../secgloss/s-gly.md)預先存在的認證控制碼。 [**InitializeSecurityCoNtext (ntlm)**](initializesecuritycontext--ntlm.md)和 [**AcceptSecurityCoNtext (ntlm)**](acceptsecuritycontext--ntlm.md)函式需要此控制碼。 這些可以是預先存在的 *認證*，其是透過系統登入所建立，而不是此處所述，或呼叫者可以提供替代的認證。
...
以 null 結束的字串指標，指定將使用這些認證之 [*安全性封裝*](../secgloss/s-gly.md) 的名稱。 這是在 [**EnumerateSecurityPackages**](/windows/win32/api/sspi/ns-sspi-secpkginfoa)函數所傳回之 [**SecPkgInfo**](/windows/win32/api/sspi/ns-sspi-secpkginfoa)結構的 **name** 成員中傳回的 [*安全性封裝*](../secgloss/s-gly.md)名稱。 建立內容之後，您可以呼叫 [**QueryCoNtextAttributes (NTLM)**](querycontextattributes--ntlm.md) ，並將 *ULATTRIBUTE* 設定為 SECPKG \_ ATTR \_ 封裝 \_ 資訊，以傳回使用中 [*安全性封裝*](../secgloss/s-gly.md) 的相關資訊。
...
| <span id="SECPKG_CRED_INBOUND"></span><span id="secpkg_cred_inbound"></span><dl> <dt>**SECPKG \_ 認證 \_ 輸入**</dt> </dl>    | 驗證傳入的伺服器認證。 當呼叫 [**InitializeSecurityCoNtext (ntlm)**](initializesecuritycontext--ntlm.md) 或 [**AcceptSecurityCoNtext (ntlm)**](acceptsecuritycontext--ntlm.md) 時，可能會使用驗證授權單位來驗證輸入認證。 如果無法使用這類授權單位，此函式將會失敗，並傳回 SEC \_ E \_ NO \_ 驗證 \_ 授權單位。 驗證是套件特定的。<br/> |
...
使用 NTLM 封裝時，使用者名稱、密碼和網域的最大字元長度分別為256、256和15。
...
**AcquireCredentialsHandle (NTLM)** 函數會傳回主體的控制碼，例如使用者或用戶端，以供特定的 [*限制委派*](../secgloss/s-gly.md)使用。 這可能是預先存在的認證的控制碼，或函式可以建立新的認證集，並將其傳回。 這個控制碼可用於對 [**AcceptSecurityCoNtext (ntlm)**](acceptsecuritycontext--ntlm.md) 和 [**InitializeSecurityCoNtext (ntlm)**](initializesecuritycontext--ntlm.md) 函式的後續呼叫中。

一般而言， **AcquireCredentialsHandle (NTLM)** 不允許處理常式取得其他登入相同電腦之使用者的認證控制碼。 不過，具有 SE TCB 名稱許可權的呼叫端可以 \_ \_ 選擇指定任何現有登入會話權杖的 [*登入識別碼*](../secgloss/l-gly.md) (LUID) ，以取得該會話認證的控制碼 [](../secgloss/s-gly.md) 。 一般來說，這是由必須代表已登入使用者的核心模式模組所使用。
