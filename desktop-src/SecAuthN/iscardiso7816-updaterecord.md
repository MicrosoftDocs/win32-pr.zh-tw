...
---
# <a name="iscardiso7816updaterecord-method"></a>ISCardISO7816：： UpdateRecord 方法

\[**UpdateRecord** 方法可用於 [需求] 區段中指定的作業系統。 它無法在 Windows Server 2003 （含 Service Pack 1） (SP1) 和更新版本、Windows Vista、Windows Server 2008 和後續版本的作業系統中使用。 [智慧卡模組](/previous-versions/windows/desktop/secsmart/smart-card-modules)提供類似的功能。\]

**UpdateRecord** 方法會建立 [*應用程式協定資料單位*](../secgloss/a-gly.md) (APDU) 命令，此命令會使用 APDU 命令中指定的位來更新特定記錄。
...
如果建立的命令套用至線性固定或迴圈結構化的基本檔案，則如果記錄長度與現有記錄的長度不同，則會中止。

如果命令套用至線性變數結構化的基本檔案，則當記錄長度與現有記錄的長度不同時，可能會執行此命令。

命令的 "previous" 選項 (P2 = xxxxx011) ，套用至迴圈檔案，其行為與 [**AppendRecord**](iscardiso7816-appendrecord.md)所建立的命令相同。

無法讀取沒有記錄結構的基本檔案。 如果套用至不含記錄結構的基本檔案，則會中止已建立的命令。
