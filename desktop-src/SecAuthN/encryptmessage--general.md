...
**EncryptMessage (一般)** 函數會加密訊息以提供 [*隱私權*](../secgloss/p-gly.md)。 **EncryptMessage (一般)** 可讓應用程式在所選機制支援的 [*密碼編譯演算法*](../secgloss/c-gly.md) 之間進行選擇。 **EncryptMessage (一般)** 函數會使用內容控制碼所參考的 [*安全性內容*](../secgloss/s-gly.md)。 某些封裝沒有要加密或解密的訊息，而是提供可檢查的完整性 [*雜湊*](../secgloss/h-gly.md) 。

使用 (SSP) 的摘要式 [*安全性支援提供者*](../secgloss/s-gly.md) 時，此函式只會以 SASL 機制的形式提供。
...
> [!Note]  
> 您可以從單一 [*安全性支援提供者介面*](../secgloss/s-gly.md)的兩個不同執行緒同時呼叫 **EncryptMessage (一般)** 和 [**DECRYPTMESSAGE (一般)**](decryptmessage--general.md) (SSPI) 內容（如果一個執行緒正在加密，另一個執行緒正在解密）。 如果有一個以上的執行緒加密，或有多個執行緒正在解密，則每個執行緒都應該取得唯一的內容。

如需有關使用此函式搭配特定 SSP 的詳細資訊，請參閱下列主題。

| 主題                                                            | 描述                                               |
|------------------------------------------------------------------|-----------------------------------------------------------|
| [**EncryptMessage (摘要)**](encryptmessage--digest.md)       | 使用摘要來加密訊息，以提供隱私權。    |
| [**EncryptMessage (Kerberos)**](encryptmessage--kerberos.md)   | 使用 Kerberos 加密訊息以提供隱私權。  |
| [**EncryptMessage (Negotiate)**](encryptmessage--negotiate.md) | 使用 Negotiate 加密訊息以提供隱私權。 |
| [**EncryptMessage (NTLM)**](encryptmessage--ntlm.md)           | 使用 NTLM 加密訊息以提供隱私權。      |
| [**EncryptMessage (Schannel)**](encryptmessage--schannel.md)   | 使用 Schannel 加密訊息以提供隱私權。  |
...
包含訊息的 [**之 secbuffer**](/windows/win32/api/sspi/ns-sspi-secbuffer) 結構長度不能大於 **cbMaximumMessage**，這是從 [**QUERYCONTEXTATTRIBUTES (一般)**](querycontextattributes--general.md) (SECPKG \_ ATTR \_ 資料流程大小) 函式取得 \_ 。

使用摘要 SSP 時，必須有類型為之 SECBUFFER \_ 填補或 SEC 緩衝區資料的第二個緩衝區， \_ \_ 才能保存簽章資訊。 [](../secgloss/d-gly.md#_security_digital_signature_gly) 若要取得輸出緩衝區的大小，請呼叫 [**QueryCoNtextAttributes (一般)**](querycontextattributes--general.md) 函數，並指定 SECPKG \_ \_ 的 ATTR 大小。 函數會傳回 [**SecPkgCoNtext \_ 大小**](/windows/win32/api/sspi/ns-sspi-secpkgcontext_sizes) 結構。 輸出緩衝區的大小是 **cbMaxSignature** 和 **cbBlockSize** 成員中值的總和。
...
**EncryptMessage (一般)** 函式會根據訊息和 [*安全性內容*](../secgloss/s-gly.md)中的 [*工作階段金鑰*](../secgloss/s-gly.md)來加密訊息。
...
當您使用摘要 SSP 時，請藉由呼叫 [**QueryCoNtextAttributes (一般)**](querycontextattributes--general.md) 函數並指定 SECPKG ATTR 大小，來取得輸出緩衝區的大小 \_ \_ 。 函數會傳回 [**SecPkgCoNtext \_ 大小**](/windows/win32/api/sspi/ns-sspi-secpkgcontext_sizes) 結構。 輸出緩衝區的大小是 **cbMaxSignature** 和 **cbBlockSize** 成員中值的總和。
...
當您使用安全通道 SSP 時，請藉由呼叫 [**QueryCoNtextAttributes (一般)**](querycontextattributes--general.md) 函數並指定 SECPKG \_ ATTR \_ 資料流程大小屬性，判斷每個緩衝區的大小上限 \_ 。 此函式會 [**傳回 SecPkgCoNtext \_ StreamSizes**](/windows/win32/api/sspi/ns-sspi-secpkgcontext_streamsizes) 結構，其成員包含標頭 (**cbHeader** 成員) 、message (**CbMaximumMessage** member) 和結尾 (**cbTrailer** 成員) 緩衝區的大小上限。
...
- [**DecryptMessage (一般)**](decryptmessage--general.md)
- [**InitializeSecurityCoNtext (一般)**](initializesecuritycontext--general.md)
- [**QueryCoNtextAttributes (一般)**](querycontextattributes--general.md)
