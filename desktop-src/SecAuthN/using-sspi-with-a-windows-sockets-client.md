---
description: 此範例程式適用于搭配 Windows 通訊端伺服器使用 SSPI 的伺服器程式。
ms.assetid: 7ec15770-d3bd-4488-abe8-058115fec071
title: 使用 SSPI 搭配 Windows 通訊端用戶端
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104513816"
---
# <a name="using-sspi-with-a-windows-sockets-client"></a><span data-ttu-id="96308-103">使用 SSPI 搭配 Windows 通訊端用戶端</span><span class="sxs-lookup"><span data-stu-id="96308-103">Using SSPI with a Windows Sockets Client</span></span>

<span data-ttu-id="96308-104">此範例程式適用于搭配 [Windows 通訊端伺服器使用 SSPI](using-sspi-with-a-windows-sockets-server.md)的伺服器程式。</span><span class="sxs-lookup"><span data-stu-id="96308-104">This sample program works with the server program [Using SSPI with a Windows Sockets Server](using-sspi-with-a-windows-sockets-server.md).</span></span> <span data-ttu-id="96308-105">用戶端和伺服器範例程式是設計來一起使用。</span><span class="sxs-lookup"><span data-stu-id="96308-105">The client and server sample programs are designed to work together.</span></span> <span data-ttu-id="96308-106">這兩個程式都使用標頭檔 SspiExample，可在 [SSPI 用戶端和伺服器範例的標頭檔](header-file-for-sspi-client-and-server.md)中找到。</span><span class="sxs-lookup"><span data-stu-id="96308-106">Both programs use the header file SspiExample.h, which can be found in [Header File for SSPI Client and Server Samples](header-file-for-sspi-client-and-server.md).</span></span> <span data-ttu-id="96308-107">這項套裝程式括呼叫 Secur32 .lib 和 Ws2 \_ 32 .lib 中的函式，這些函數必須包含在程式庫中。</span><span class="sxs-lookup"><span data-stu-id="96308-107">This program includes calls to functions in Secur32.lib and Ws2\_32.lib, which must be included among the link libraries.</span></span>

<span data-ttu-id="96308-108">此程式示範下列各項：</span><span class="sxs-lookup"><span data-stu-id="96308-108">This program demonstrates the following:</span></span>

-   <span data-ttu-id="96308-109">建立與伺服器的 Windows 通訊端連接。</span><span class="sxs-lookup"><span data-stu-id="96308-109">Establishing a Windows Sockets connection with a server.</span></span>
-   <span data-ttu-id="96308-110">使用 Negotiate SSP 初始化已驗證的 SSPI 會話。</span><span class="sxs-lookup"><span data-stu-id="96308-110">Initializing an authenticated SSPI session with the Negotiate SSP.</span></span>
-   <span data-ttu-id="96308-111">連接伺服器並建立安全的通訊會話。</span><span class="sxs-lookup"><span data-stu-id="96308-111">Connecting with a server and establishing a secure communication session.</span></span>
-   <span data-ttu-id="96308-112">從伺服器接收和解密安全會話中的訊息。</span><span class="sxs-lookup"><span data-stu-id="96308-112">Receiving and decrypt a message from the server within the secure session.</span></span>

<span data-ttu-id="96308-113">此範例程式使用有限的錯誤處理。</span><span class="sxs-lookup"><span data-stu-id="96308-113">This sample program uses limited error handling.</span></span>
