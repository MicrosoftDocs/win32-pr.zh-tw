...
---
# <a name="using-sspi-with-a-windows-sockets-server"></a><span data-ttu-id="c671f-104">搭配 Windows 通訊端伺服器使用 SSPI</span><span class="sxs-lookup"><span data-stu-id="c671f-104">Using SSPI with a Windows Sockets Server</span></span>

<span data-ttu-id="c671f-105">此範例程式適用于搭配 [Windows 通訊端用戶端使用 SSPI](using-sspi-with-a-windows-sockets-client.md)的用戶端程式。</span><span class="sxs-lookup"><span data-stu-id="c671f-105">This sample program works with the client program [Using SSPI with a Windows Sockets Client](using-sspi-with-a-windows-sockets-client.md).</span></span> <span data-ttu-id="c671f-106">這兩個範例程式都會使用標頭檔 SspiExample，此檔案可在 [SSPI 用戶端和伺服器範例的標頭檔](header-file-for-sspi-client-and-server.md)中找到。</span><span class="sxs-lookup"><span data-stu-id="c671f-106">Both sample programs use the header file SspiExample.h which can be found in [Header File for SSPI Client and Server Samples](header-file-for-sspi-client-and-server.md).</span></span>

<span data-ttu-id="c671f-107">這項套裝程式括呼叫 Secur32 .lib 和 Ws2 \_ 32 .lib 中的函式，這些函數必須包含在程式庫中。</span><span class="sxs-lookup"><span data-stu-id="c671f-107">This program includes calls to functions in Secur32.lib and Ws2\_32.lib, which must be included among the link libraries.</span></span>

<span data-ttu-id="c671f-108">此程式示範下列各項：</span><span class="sxs-lookup"><span data-stu-id="c671f-108">This program demonstrates the following:</span></span>

-   <span data-ttu-id="c671f-109">建立與用戶端的 Windows 通訊端連線。</span><span class="sxs-lookup"><span data-stu-id="c671f-109">Establishing a Windows Sockets connection with a client.</span></span>
-   <span data-ttu-id="c671f-110">初始化會話。</span><span class="sxs-lookup"><span data-stu-id="c671f-110">Initializing a session.</span></span>
-   <span data-ttu-id="c671f-111">連接用戶端並建立安全的通訊會話。</span><span class="sxs-lookup"><span data-stu-id="c671f-111">Connecting with a client and establishing a secure communication session.</span></span> <span data-ttu-id="c671f-112">這裡的伺服器提供可透過 NTLM 或 Kerberos 回應用戶端的 Negotiate SSP。</span><span class="sxs-lookup"><span data-stu-id="c671f-112">The server here offers the Negotiate SSP which can respond to the client with either NTLM or Kerberos.</span></span>
-   <span data-ttu-id="c671f-113">使用用戶端模擬並還原至本身。</span><span class="sxs-lookup"><span data-stu-id="c671f-113">Uses client impersonation and reverting to self.</span></span>
-   <span data-ttu-id="c671f-114">將加密的訊息傳送給用戶端。</span><span class="sxs-lookup"><span data-stu-id="c671f-114">Sends the client an encrypted message.</span></span>

<span data-ttu-id="c671f-115">此範例程式使用有限的錯誤處理。</span><span class="sxs-lookup"><span data-stu-id="c671f-115">This sample program uses limited error handling.</span></span>
