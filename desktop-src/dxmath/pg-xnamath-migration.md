...
將 `xnamath.h` 標頭取代為 `DirectXMath.h` ，並 `DirectXPackedVector.h` 為 *GPU 封裝* 類型新增。

中的 DirectX SDK 衝突範例中的周框類型 `xnacollision.h` 現在是中 DirectXMath 程式庫的一部分 `DirectXCollision.h` 。 這些已修改為使用 c + + 類別，而不是 C 樣式的 API。
...
DirectXMath 程式庫會使用 c + + 命名空間來組織類型。 只有全域命名空間才會使用未使用的運算。 DirectXMath 類型在 **directx** 或 **Directx：:P ackedvector** 命名空間中是常見的。
...
如果是標頭，則不會被視為加入 using 語句的最佳作法。 請改為新增完整命名空間。
