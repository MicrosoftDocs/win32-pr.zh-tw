...
[**IADs：:P**](/windows/desktop/api/Iads/nf-iads-iads-put) ui 方法會依名稱將 Active Directory 物件的屬性值儲存至屬性快取中。 使用 [**IADs：:P utex**](/windows/desktop/api/Iads/nf-iads-iads-putex) 將多重值屬性儲存至屬性快取，或從物件中移除屬性。 在呼叫 [**IADs：： SetInfo**](/windows/desktop/api/Iads/nf-iads-iads-setinfo) 之前，這些值不會保存到基礎目錄服務。
...
下列程式碼範例示範如何搭配單一值使用 [**IADs：:P 的內容**](/windows/desktop/api/Iads/nf-iads-iads-put) ：
...
下列程式碼範例示範如何使用 [**IADs：**](/windows/desktop/api/Iads/nf-iads-iads-put) 具有多個值的:P 長時間：
...
下列程式碼範例示範如何使用 [**IADs：**](/windows/desktop/api/Iads/nf-iads-iads-put) 具有多個值和單一值的:P 長時間：
