...
---
# <a name="example-code-for-reading-attributes"></a>用於讀取屬性的範例程式碼

下列程式碼範例會藉由搜尋使用者，然後使用 [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) 來列舉其屬性，以列舉目前網域中指定之使用者的屬性。 請注意，系統會處理時間和日期值（如大型整數），以及如何處理 **objectSID** 和 **objectGUID** 的多位元組字元串。
...
下列 Visual Basic 程式碼範例顯示如何取得使用者物件的屬性。 若要使用此程式碼範例，請在您的 Visual Basic 專案中建立 Active DS 類型程式庫和 Microsoft ActiveX Data Objects 程式庫的參考。
