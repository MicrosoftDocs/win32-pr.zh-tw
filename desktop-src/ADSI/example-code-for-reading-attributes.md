---
title: 用於讀取屬性的範例程式碼
description: 下列程式碼範例會藉由搜尋使用者，然後使用 IADsPropertyList 來列舉其屬性，以列舉目前網域中指定之使用者的屬性。
...
ms.locfileid: "104507032"
---
# <a name="example-code-for-reading-attributes"></a><span data-ttu-id="b7c12-104">用於讀取屬性的範例程式碼</span><span class="sxs-lookup"><span data-stu-id="b7c12-104">Example Code for Reading Attributes</span></span>

<span data-ttu-id="b7c12-105">下列程式碼範例會藉由搜尋使用者，然後使用 [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) 來列舉其屬性，以列舉目前網域中指定之使用者的屬性。</span><span class="sxs-lookup"><span data-stu-id="b7c12-105">The following code example enumerates the properties of the specified user in the current domain, by searching for the user and then using [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist) to enumerate its properties.</span></span> <span data-ttu-id="b7c12-106">請注意，系統會處理時間和日期值（如大型整數），以及如何處理 **objectSID** 和 **objectGUID** 的多位元組字元串。</span><span class="sxs-lookup"><span data-stu-id="b7c12-106">Be aware that time and date values, as large integers, are handled and how octet strings for **objectSID** and **objectGUID** are handled.</span></span>
...
<span data-ttu-id="b7c12-107">下列 Visual Basic 程式碼範例顯示如何取得使用者物件的屬性。</span><span class="sxs-lookup"><span data-stu-id="b7c12-107">The following Visual Basic code example shows how to get the properties of a user object.</span></span> <span data-ttu-id="b7c12-108">若要使用此程式碼範例，請在您的 Visual Basic 專案中建立 Active DS 類型程式庫和 Microsoft ActiveX Data Objects 程式庫的參考。</span><span class="sxs-lookup"><span data-stu-id="b7c12-108">To use this code example, create a reference to the Active DS Type Library and the Microsoft ActiveX Data Objects Library in your Visual Basic project.</span></span>
