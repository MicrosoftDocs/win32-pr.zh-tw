...
當您 \[ [](/windows/desktop/Midl/out-idl) \] 在 Microsoft RPC 中使用 out only 參考指標時，產生的伺服器 stub 只會配置可從參考指標存取的第一個指標層級。 較深層層級的指標不會由存根配置，但必須由伺服器應用層配置。 例如，假設介面指定 \[  \] 參考指標的 out 陣列：
...
在此範例中，伺服器 stub 會為10個指標配置記憶體，並將每個指標的值設定為 null。 伺服器應用程式必須為指標所參考的10個 [**短**](/windows/desktop/Midl/short) 整數配置記憶體，然後設定10個指標以指向整數。

當 \[ [](/windows/desktop/Midl/out-idl) \] 僅限輸出的資料結構包含嵌套參考指標時，伺服器存根只會配置參考指標可存取的第一個指標。 例如：
