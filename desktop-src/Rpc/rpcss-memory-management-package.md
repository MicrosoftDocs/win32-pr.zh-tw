...
ms.locfileid: "120018178"
---
# <a name="rpcss-memory-management-package"></a>RpcSs 記憶體管理套件

在代表應用程式佈建記憶體時，存根和執行時間所使用的預設配置器/釋放器組是 **midl \_ 使用者 \_** / **\_ \_ 免費配置 midl 使用者**。 不過，您可以使用 ACF 屬性 **\[ 啟用 \_ 配置 \]** 來選擇 RpcSs 封裝，而不是預設值。 此 RpcSs 封裝包含以首碼 **RpcSs** 或 **RPCSM** 開頭的 RPC 函數。 不建議 Windows 的應用程式使用 RpcSs 套件。
