...
ms.locfileid: "119010646"
---
# <a name="using-rpc-with-winsock-proxy"></a>使用 RPC 搭配 Winsock Proxy

Microsoft Internet Access Server 的發行版本包含 Winsock Proxy，此為增強版的 Windows 通訊端 API 版本1.1。 Winsock Proxy 可讓在私人網路用戶端上執行的 Windows 通訊端應用程式，其行為就像是直接連接到遠端網際網路伺服器應用程式一樣。 Microsoft Proxy 伺服器作為此連線的主機。 這表示所有應用層級的通訊都是透過單一受保護的電腦（執行 Microsoft Proxy 伺服器的閘道電腦）來傳遞。
