...
---
# <a name="creating-a-binding-handle"></a><span data-ttu-id="ccd74-104">建立系結控制碼</span><span class="sxs-lookup"><span data-stu-id="ccd74-104">Creating a Binding Handle</span></span>

<span data-ttu-id="ccd74-105">分散式應用程式的用戶端程式需要建立系結控制碼，告知 RPC 執行時間應聯繫的伺服器，以及應如何聯繫伺服器。</span><span class="sxs-lookup"><span data-stu-id="ccd74-105">The client program of a distributed application needs to create a binding handle that tells the RPC run time which server should be contacted, and how the server should be contacted.</span></span>

<span data-ttu-id="ccd74-106">下列程式碼片段示範建立系結控制碼的常見方法：</span><span class="sxs-lookup"><span data-stu-id="ccd74-106">The following code fragment demonstrates a common approach to creating a binding handle:</span></span>
