...
第二個設定適用于不包含 NetWare 檔案伺服器或路由器的環境 (例如，兩部電腦的網路： Windows 伺服器和 MS-DOS 用戶端) 。 名稱解析是在第一次呼叫系結控制碼時完成的，會比第一次設定時稍微慢一點。 此外，第二個設定會導致透過網路產生更多流量。
...
> [!Note]  
> 使用 **ncacn \_ spx** 或 **ncadg \_ ipx** 傳輸的16位 Windows 用戶端應用程式 Nwipxspx.dll 需要安裝檔案，才能在 WOW 子系統下執行。 請聯絡 Novell 以取得此檔案。
