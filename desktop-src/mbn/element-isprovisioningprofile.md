...
指定此設定檔僅用於布建。否則，此設定檔並不適用，也無法用來啟用 (PDP) 內容的封包資料通訊協定。 此元素是 v4 的新專案。

如果 **IsProvisioningProfile** 為 True， [**IsDefault**](element-isdefault.md) 必須為 false，而且 [**ConnectionMode**](element-connectionmode.md) 必須是手動。
