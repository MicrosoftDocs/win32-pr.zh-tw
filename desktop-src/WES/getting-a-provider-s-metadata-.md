...
---
# <a name="getting-a-providers-metadata"></a><span data-ttu-id="89a8e-103">取得提供者的中繼資料</span><span class="sxs-lookup"><span data-stu-id="89a8e-103">Getting a Provider's Metadata</span></span>

<span data-ttu-id="89a8e-104">提供者會使用檢測資訊清單來識別本身、定義它所寫入的事件，以及其他元件，例如通道、工作和關鍵字。</span><span class="sxs-lookup"><span data-stu-id="89a8e-104">A provider uses an instrumentation manifest to identify itself, define the events that it writes, and other components such as channels, tasks, and keywords.</span></span> <span data-ttu-id="89a8e-105">若要在執行時間存取此資訊，請呼叫 [**EvtOpenPublisherMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtopenpublishermetadata) 函式來取得發行者中繼資料的控制碼。</span><span class="sxs-lookup"><span data-stu-id="89a8e-105">To access this information at run time, call the [**EvtOpenPublisherMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtopenpublishermetadata) function to get a handle to the publisher's metadata.</span></span>

<span data-ttu-id="89a8e-106">中繼資料會分割成發行者中繼資料和事件中繼資料。</span><span class="sxs-lookup"><span data-stu-id="89a8e-106">The metadata is divided between the publisher metadata and the event metadata.</span></span> <span data-ttu-id="89a8e-107">發行者中繼資料包含可識別發行者的中繼資料，以及它所定義的任何中繼資料，例如通道、層級、工作、opcode 和關鍵字。</span><span class="sxs-lookup"><span data-stu-id="89a8e-107">The publisher metadata contains the metadata that identifies the publisher and any metadata that it defines such as channels, levels, tasks, opcodes, and keywords.</span></span> <span data-ttu-id="89a8e-108">For a list of the provider metadata that you can retrieve, see the [**EVT\_PUBLISHER\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_publisher_metadata_property_id) enumeration.</span><span class="sxs-lookup"><span data-stu-id="89a8e-108">For a list of the provider metadata that you can retrieve, see the [**EVT\_PUBLISHER\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_publisher_metadata_property_id) enumeration.</span></span> <span data-ttu-id="89a8e-109">若要取得中繼資料，請呼叫 [**EvtGetPublisherMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetpublishermetadataproperty) 函數。</span><span class="sxs-lookup"><span data-stu-id="89a8e-109">To get the metadata, call the [**EvtGetPublisherMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetpublishermetadataproperty) function.</span></span> <span data-ttu-id="89a8e-110">此函式會傳回屬性的屬性值，這些屬性會識別提供者和通道、層級、工作、opcode 和關鍵字元件的控制碼。</span><span class="sxs-lookup"><span data-stu-id="89a8e-110">This function returns the property values for the properties that identify the provider and handles to the channels, levels, tasks, opcodes, and keywords components.</span></span> <span data-ttu-id="89a8e-111">若要存取這些元件的中繼資料，請呼叫 [**EvtGetObjectArrayProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetobjectarrayproperty) 函數。</span><span class="sxs-lookup"><span data-stu-id="89a8e-111">To access the metadata for these components, call the [**EvtGetObjectArrayProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetobjectarrayproperty) function.</span></span>

<span data-ttu-id="89a8e-112">若要取得事件中繼資料，請呼叫 [**EvtOpenEventMetadataEnum**](/windows/desktop/api/WinEvt/nf-winevt-evtopeneventmetadataenum) 函數，以取得提供者所定義的事件清單。</span><span class="sxs-lookup"><span data-stu-id="89a8e-112">To get the event metadata, call the [**EvtOpenEventMetadataEnum**](/windows/desktop/api/WinEvt/nf-winevt-evtopeneventmetadataenum) function to get a list of the events that the provider defines.</span></span> <span data-ttu-id="89a8e-113">然後，在迴圈中呼叫 [**EvtNextEventMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtnexteventmetadata) 函式來列舉事件。</span><span class="sxs-lookup"><span data-stu-id="89a8e-113">Then call the [**EvtNextEventMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtnexteventmetadata) function in a loop to enumerate the events.</span></span> <span data-ttu-id="89a8e-114">For a list of metadata that you can get for an event, see the [**EVT\_EVENT\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_event_metadata_property_id) enumeration.</span><span class="sxs-lookup"><span data-stu-id="89a8e-114">For a list of metadata that you can get for an event, see the [**EVT\_EVENT\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_event_metadata_property_id) enumeration.</span></span> <span data-ttu-id="89a8e-115">若要取得中繼資料，請呼叫 [**EvtGetEventMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgeteventmetadataproperty) 函數。</span><span class="sxs-lookup"><span data-stu-id="89a8e-115">To get the metadata, call the [**EvtGetEventMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgeteventmetadataproperty) function.</span></span> <span data-ttu-id="89a8e-116">這些屬性會針對事件定義的每個元素傳回整數值。</span><span class="sxs-lookup"><span data-stu-id="89a8e-116">These properties return an integer value for each element of the event definition.</span></span> <span data-ttu-id="89a8e-117">針對事件識別碼和版本，顯示整數值是沒問題的，但針對其他元素（例如通道和層級），顯示整數值對使用者可能沒有任何意義。</span><span class="sxs-lookup"><span data-stu-id="89a8e-117">For event ID and version, displaying the integer value is fine but for the other elements, such as channel and level, displaying the integer value may not have any meaning to the user.</span></span> <span data-ttu-id="89a8e-118">這些元素的整數值是 value 屬性的值。</span><span class="sxs-lookup"><span data-stu-id="89a8e-118">The integer value for these elements is the value of the value attribute.</span></span> <span data-ttu-id="89a8e-119">您可以使用整數值對應至提供者的中繼資料，並取得專案的訊息字串或名稱值，並改為將它顯示給使用者。</span><span class="sxs-lookup"><span data-stu-id="89a8e-119">You can use the integer value to map into the provider's metadata and get the message string or name value for the element and display it to the user instead.</span></span>

<span data-ttu-id="89a8e-120">如需示範如何列舉電腦上已註冊之提供者的範例，請參閱 [列舉已註冊的提供者](#enumerating-registered-providers)。</span><span class="sxs-lookup"><span data-stu-id="89a8e-120">For an example that shows how to enumerate the registered providers on the computer, see [Enumerating registered providers](#enumerating-registered-providers).</span></span>

<span data-ttu-id="89a8e-121">如需示範如何取得已註冊提供者之中繼資料的範例，請參閱 [取得提供者的中繼資料](#getting-a-providers-metadata)。</span><span class="sxs-lookup"><span data-stu-id="89a8e-121">For an example that shows how to get the metadata for a registered provider, see [Getting the metadata for a provider](#getting-a-providers-metadata).</span></span>

## <a name="enumerating-registered-providers"></a><span data-ttu-id="89a8e-122">列舉已註冊的提供者</span><span class="sxs-lookup"><span data-stu-id="89a8e-122">Enumerating registered providers</span></span>

<span data-ttu-id="89a8e-123">下列範例示範如何列舉電腦上註冊的提供者。</span><span class="sxs-lookup"><span data-stu-id="89a8e-123">The following example shows how to enumerate the providers that are registered on the computer.</span></span>
...
## <a name="getting-the-metadata-for-a-provider"></a><span data-ttu-id="89a8e-124">取得提供者的中繼資料</span><span class="sxs-lookup"><span data-stu-id="89a8e-124">Getting the metadata for a provider</span></span>

<span data-ttu-id="89a8e-125">下列範例示範如何取得提供者的中繼資料，以及它所定義的事件。</span><span class="sxs-lookup"><span data-stu-id="89a8e-125">The following example shows how to get the metadata for the provider and the events that it defines.</span></span>
