...
提供者會使用檢測資訊清單來識別本身、定義它所寫入的事件，以及其他元件，例如通道、工作和關鍵字。 若要在執行時間存取此資訊，請呼叫 [**EvtOpenPublisherMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtopenpublishermetadata) 函式來取得發行者中繼資料的控制碼。

中繼資料會分割成發行者中繼資料和事件中繼資料。 發行者中繼資料包含可識別發行者的中繼資料，以及它所定義的任何中繼資料，例如通道、層級、工作、opcode 和關鍵字。 For a list of the provider metadata that you can retrieve, see the [**EVT\_PUBLISHER\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_publisher_metadata_property_id) enumeration. 若要取得中繼資料，請呼叫 [**EvtGetPublisherMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetpublishermetadataproperty) 函數。 此函式會傳回屬性的屬性值，這些屬性會識別提供者和通道、層級、工作、opcode 和關鍵字元件的控制碼。 若要存取這些元件的中繼資料，請呼叫 [**EvtGetObjectArrayProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetobjectarrayproperty) 函數。

若要取得事件中繼資料，請呼叫 [**EvtOpenEventMetadataEnum**](/windows/desktop/api/WinEvt/nf-winevt-evtopeneventmetadataenum) 函數，以取得提供者所定義的事件清單。 然後，在迴圈中呼叫 [**EvtNextEventMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtnexteventmetadata) 函式來列舉事件。 For a list of metadata that you can get for an event, see the [**EVT\_EVENT\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_event_metadata_property_id) enumeration. 若要取得中繼資料，請呼叫 [**EvtGetEventMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgeteventmetadataproperty) 函數。 這些屬性會針對事件定義的每個元素傳回整數值。 針對事件識別碼和版本，顯示整數值是沒問題的，但針對其他元素（例如通道和層級），顯示整數值對使用者可能沒有任何意義。 這些元素的整數值是 value 屬性的值。 您可以使用整數值對應至提供者的中繼資料，並取得專案的訊息字串或名稱值，並改為將它顯示給使用者。

如需示範如何列舉電腦上已註冊之提供者的範例，請參閱 [列舉已註冊的提供者](#enumerating-registered-providers)。

如需示範如何取得已註冊提供者之中繼資料的範例，請參閱 [取得提供者的中繼資料](#getting-a-providers-metadata)。

## <a name="enumerating-registered-providers"></a>列舉已註冊的提供者
...
## <a name="getting-the-metadata-for-a-provider"></a>取得提供者的中繼資料

下列範例示範如何取得提供者的中繼資料，以及它所定義的事件。
