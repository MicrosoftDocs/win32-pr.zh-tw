...
提供者必須定義它們撰寫的所有事件。 若要定義事件，請使用 **event** 元素。

**值** 屬性是事件識別碼，且必須是您定義之事件的唯一識別碼。 您是否設定其他屬性，取決於將取用事件的人員和來源。 如果系統管理員將使用 Windows 事件檢視器之類的工具來取用您的事件，則您必須設定 **通道** 屬性。 如果通道類型為 Admin，則您也必須指定 **level** 屬性，並將它設定為 Winmeta.xml (Win： Critical 到 Win： Verbose) 中定義的其中一個層級。

如果事件包含事件特定的資料，您必須將 **範本** 屬性設定為定義事件特定資料之範本的識別碼。 **層級**、**關鍵字**、**工作和** **opcode** 屬性是用來群組或值區事件。 雖然這些屬性是選擇性的，但您應該考慮指定層級、工作、opcode 和關鍵字，讓取用者只能輕鬆存取感興趣的事件。 ETW 追蹤會話也可以使用 **level** 和 **關鍵字** 屬性來限制寫入事件追蹤記錄檔的事件。 **關鍵字** 屬性包含以空格分隔的資訊清單中所定義的關鍵字名稱清單。 如果指定了多個關鍵字，它們的遮罩值會 OR'ed 在一起，以建立事件將使用的關鍵字值。

您應該設定 **符號** 屬性來指定編譯器所產生的符號常數，以識別事件的事件描述項-在寫入事件時，您會使用事件描述項。 如果您未指定符號，編譯器會為您產生名稱。

**Message** 屬性包含與事件一起顯示的當地語系化訊息字串。 若要在字串中包含事件特定的資料，請在郵件內文中加入插入字串。 例如，若要在範本中包含第三個數據項，請包含 %3。
