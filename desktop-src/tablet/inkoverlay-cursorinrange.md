...
產生 [**CursorInRange**](inkcollector-cursorinrange.md)事件的 [**IInkCursor**](/windows/desktop/api/msinkaut/nn-msinkaut-iinkcursor)物件。
...
這是否為第一次使用此筆墨收集器時，與產生 [**CursorInRange**](inkcollector-cursorinrange.md)事件的 [**IInkCursor**](/windows/desktop/api/msinkaut/nn-msinkaut-iinkcursor)物件有關。
...
產生 [**CursorInRange**](inkcollector-cursorinrange.md) 事件的資料指標之按鈕的狀態。
...
即使在 [選取] 或 [清除] 模式中，也不只是在 [筆墨] 模式中，也會引發 [**CursorInRange**](inkcollector-cursorinrange.md) 事件。 這需要您監視編輯模式 (您負責設定) 並在解讀事件之前留意模式。 這項需求的優點是透過更清楚地瞭解平臺事件，更能自由地在平臺上進行創新。
