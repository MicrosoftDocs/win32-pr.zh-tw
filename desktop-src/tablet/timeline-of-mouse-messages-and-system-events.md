...
執行指定的動作時， \_ 應用程式幾乎會立即傳送和接收系統事件 (前面加上 ISG) 。 在執行動作時，會傳送以 WM) 為首碼 (的滑鼠訊息 \_ ，並在由 Microsoft Windows 訊息處理服務處理事件所需的時間之後，由應用程式接收。 此外， **CursorDown** 和 **CursorUp** 是從畫筆硬體接收的畫筆事件。 當 tablet 畫筆觸及螢幕，以及從螢幕中移除時，就會傳送它們。
...
-   使用 tablet 畫筆的點一下，通常比按兩下滑鼠的方式更不精確。 這是利用 tablet 畫筆來執行兩點的固有本質。 由於使用者必須提起 tablet 畫筆以執行點一下，所以點擊之間的時間通常會大於按兩下時的對應時間。 此外，這兩個 tablet 畫筆的點擊率很可能會在螢幕座標上發生，而不是在兩次按下滑鼠的情況下執行。 為了因應這種情況，Windows XP Tablet PC Edition 具有設定，可讓您在兩個點之間的時間點距離與滑鼠按兩下分開的點點。 這些設定可以在主控台中的 Tablet 和畫筆設定中調整。
