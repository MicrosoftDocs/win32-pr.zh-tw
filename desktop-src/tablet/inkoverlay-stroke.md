...
產生 [**筆劃**](inkcollector-stroke.md)事件的 [**IInkCursor**](/windows/desktop/api/msinkaut/nn-msinkaut-iinkcursor)物件。
...
指定是否應該取消事件。 若 **為 TRUE**，則表示筆劃的集合已取消。
...
在 [選取] 或 [清除] 模式中（而不只是在插入筆墨時），就會引發 [**筆劃**](inkcollector-stroke.md) 事件。 這需要您監視您負責設定) 的編輯模式 (，並在解讀事件之前感知模式。 這項需求的優點是透過更清楚地瞭解平臺事件，更能自由地在平臺上進行創新。

> [!Note]  
> 當使用者完成繪製筆劃時，不會在將筆劃加入至 [InkStrokes](/previous-versions/windows/desktop/legacy/ms703293(v=vs.85))集合時引發 [**筆劃**](inkcollector-stroke.md)事件。 當使用者第一次開始繪製筆劃時，會立即將其新增至 InkStrokes 集合;不過，在筆劃完成之前，不會引發 **筆劃** 事件。 因此，筆劃可以存在於 **筆劃** 事件處理常式未看到的 InkStrokes 集合中。
