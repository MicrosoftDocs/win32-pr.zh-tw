...
-   [Windows Forms 執行緒考慮](#windows-forms-threading-considerations)
...
## <a name="windows-forms-threading-considerations"></a>Windows Forms 執行緒考慮

[InkPicture](/previous-versions/aa514604(v=msdn.10))和[InkEdit](/previous-versions/ms552265(v=vs.100))控制項會擴充 Windows Forms 控制項。 Windows Forms 控制項使用單一執行緒的單元 (STA) 模型，因為 Windows Forms 是以原本就是單一執行緒的原生 Win32 視窗為基礎。 在 managed 程式碼中，筆墨控制項應該在與表單主執行緒相同的執行緒中建立。
