...
\[Windows Vista 不支援 **RAS \_ PPP \_ IPCP \_ 結果** 結構。\]

**RAS \_ ppp \_ IPCP \_ 結果** 結構是用來報告埠的 PPP 網際網路通訊協定 (IP) 投影作業的結果。
...
表示 IP 投射作業的結果。 沒有錯誤的值 \_ 表示成功，在這種情況下， **wszAddress** 成員是有效的。 如果投射作業不成功，則 **dwError** 是 winerror.h .H 或 Raserror 的錯誤碼。
...
以 null 終止的 Unicode 字串，指定指派給遠端用戶端的 IP 位址。 這個字串的格式 *為 ***。**_b_*_._**  c. *_d._
