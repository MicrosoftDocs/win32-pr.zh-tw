---
title: 路由及遠端存取登錄版面配置
description: 下列語法顯示路由器服務的登錄版面配置範例。
...
系統中安裝的每個路由器管理員都有一個在路由器金鑰下建立的登錄機碼。 DLLPath 變數會指定對應至路由器管理員的 DLL 位置，而 ProtocolID 變數會指定路由器管理員的通訊協定系列識別碼。
