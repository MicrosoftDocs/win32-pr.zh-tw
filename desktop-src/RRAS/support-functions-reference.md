...
路由器管理員會提供下列功能給路由通訊協定。 當路由器管理員呼叫 [**StartProtocol**](/windows/desktop/api/Routprot/nc-routprot-pstart_protocol)函式 (由路由通訊協定) 執行時，路由器管理員會將包含這些函式指標的支援函式結構傳遞給路由通訊協定： [**\_**](/windows/desktop/api/Routprot/ns-routprot-support_functions_50)
