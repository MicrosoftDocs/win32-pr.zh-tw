---
title: 多播群組管理員參考
description: 下列檔說明使用多播群組管理員時要使用的函數、回呼、結構和列舉類型
ms.assetid: bd32d8e9-e2f0-4406-8e9c-979dc6e85221
keywords:
- 路由及遠端存取服務 RRAS，多播群組管理員，參考
- 多播群組管理員 RRAS
- 多播群組管理員 RRAS，參考
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 38e33d33cc2fd5a5f5cece8b78c28693ba4427e823e2d2fb405d5ce726700b11
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/11/2021
ms.locfileid: "120029418"
---
# <a name="multicast-group-manager-reference"></a>多播群組管理員參考

下列檔說明使用多播群組管理員時要使用的函式、回呼、結構和列舉類型：

-   [多播群組管理員函數](multicast-group-manager-functions.md)
-   [多播群組管理員回呼](multicast-group-manager-callbacks.md)
-   [多播群組管理員結構](multicast-group-manager-structures.md)
-   [多播群組管理員列舉](multicast-group-manager-enumerations.md)

 

 




